(() => {
  ...
  function basePlan(state, scopeKey = scope(state)) {
    ...
    return {
      schemaVersion: 1,
      caseMetadata: { caseId: text(c.id), subject: text(c.subject || state.documentData?.documentSubject), complainant: text(c.complainant), allegation: text(p.allegations || c.subject || state.documentData?.documentSubject), complaintChannel: text(c.channel), receivedAt: text(c.received || intake.receivedFirstAt || p.startedAt), sourceTypes: { fromNacc: Boolean(intake.m62?.flag || text(c.decision).includes("62")), misconduct: text(c.decision).includes("18/4") } },
      eventContext: { occurredAtPlace: text(p.place) },
      limitationDates: { preliminaryDeadlineAt: text(p.deadlineAt), twoYearDeadlineAt: dayAfter(c.received || intake.receivedFirstAt || p.startedAt, 730), limitationRows: [] },
  ...
  function normalizeCasePlanA5(sourceState) {
    ...
    const canonicalPlan = active(state, currentIds.form)?.payload || basePlan(state);
  ...
  function validateCasePlanA5(plan, options = {}) {
    ...
    if (intent === "SUBMIT" || intent === "HEAD_APPROVAL") {
      ...
      for (const [field, value] of [["caseMetadata.caseId", metadata.caseId], ["caseMetadata.receivedAt", metadata.receivedAt], ["caseMetadata.complainant", metadata.complainant], ["caseMetadata.allegation", metadata.allegation], ["eventContext.occurredAtPlace", context.occurredAtPlace], ["limitationDates.preliminaryDeadlineAt", dates.preliminaryDeadlineAt]]) if (!text(value)) errors.push({ field, message: "ข้อมูลแผนงานคดีไม่ครบถ้วน" });
      ...
      if ((model.witnesses || []).some(row => !text(row?.name)) || (model.requestedDocuments || []).some(row => !text(row?.name)) || (model.otherOperations || []).some(row => !text(row?.description))) errors.push({ field: "optionalRows", message: "รายการเพิ่มเติมที่ระบุต้องมีรายละเอียด" });
  ...
  function appendWorklogEntryA5(sourceState, command) {
    ...
    if (missingEntry || !Array.isArray(entry.relatedDocumentVersionIds)) return { ok: false, code: "INVALID_PAYLOAD", state: source, errors: [{ field: missingEntry || "relatedDocumentVersionIds", message: "ข้อมูลบันทึกการปฏิบัติงานไม่ครบถ้วน" }], focusTarget: missingEntry || "relatedDocumentVersionIds" };
    ...
    const result = domain().saveA5DocumentDraft(normalized.state, { caseId: normalized.state.caseData.id, documentId: logRecord.documentId, revisionNo: logRecord.revisionNo, expectedVersion: input.expectedVersion, actorId: text(input.actorId), at, idempotencyKey: text(input.idempotencyKey), payload });
  ...
  function renderCasePlanEditorA5(plan, worklog, options = {}) {
    const model = object(plan), log = object(worklog), disabled = options.editable === false ? " disabled" : "";
    ...
    const field = (labelText, path, value, type = "text") => `<div class="ws-field"><label>${labelText}</label><input type="${type}" data-a5-plan-bind="${path}" value="${escapeHtml(value)}"${disabled}></div>`;
    const list = (labelText, path, items) => `<div class="ws-field"><label>${labelText}</label><textarea data-a5-plan-list="${path}"${disabled}>${escapeHtml((items || []).map(item => item.name || item.description || item.action || "").join("\n"))}</textarea></div>`;
    ...
    const accusedBlocks = Array.from({ length: accusedCount }, (_, accusedIndex) => {
      ...
      const issueBlocks = ISSUE_KEYS.map(([issue], issueIndex) => {
        ...
        return `<div class="a5-plan-issue-block"><div class="a5-plan-issue-title">${escapeHtml(issue)}</div><div class="ws-field"><label>ข้อเท็จจริง</label><input data-a5-plan-bind="accusedRows.${accusedIndex}.fourIssues.${issueIndex}.details" value="${escapeHtml(details)}"${disabled}></div><div class="ws-field"><label>หลักฐานที่ต้องใช้</label>${checkboxHtml ? `<div class="a5-plan-cb-group">${checkboxHtml}</div>` : ""}<input data-a5-plan-bind="accusedRows.${accusedIndex}.requiredEvidenceActions.${issueIndex}.requiredEvidence" value="${escapeHtml(requiredEvidence)}" placeholder="หลักฐานอื่น (ระบุ)"${disabled}></div><div class="ws-field"><label>สิ่งที่ต้องดำเนินการ</label><input data-a5-plan-bind="accusedRows.${accusedIndex}.requiredEvidenceActions.${issueIndex}.action" value="${escapeHtml(action)}"${disabled}></div></div>`;
      }).join("");
    ...
    const rowField = (labelText, fieldKey, value, type = "text") => `<div class="ws-field"><label>${labelText}</label><input type="${type}" data-a5-plan-row-field="${fieldKey}" value="${escapeHtml(value)}"${disabled}></div>`;
    ...
    const signButtons = options.editable === false ? "" : `<button type="button" class="ws-button secondary" data-a5-plan-action="sign-owner">ลงนามเจ้าของสำนวน</button>${signatures.assistant?.officerId ? '<button type="button" class="ws-button secondary" data-a5-plan-action="sign-assistant">ลงนามผู้ช่วยเจ้าของสำนวน</button>' : ''}`;
    ...
    return `<section class="ws-section a5-plan-editor">${part1}${part2}${part3}${part4}${part5}</section><section class="ws-section a5-worklog-editor"><h3>บันทึกการปฏิบัติงาน</h3><ol class="ws-history a5-worklog-timeline">${(log.entries || []).map(entry => `<li><strong>${escapeHtml(label(entry.activityType))}</strong><span>${escapeHtml(entry.description)}</span><time>${escapeHtml(entry.occurredAt || entry.createdAt)}</time></li>`).join("") || "<li>ยังไม่มีบันทึกการปฏิบัติงาน</li>"}</ol>${options.editable === false ? "" : "<div class=\"ws-actions\"><button type=\"button\" class=\"ws-button secondary\" data-a5-plan-action=\"worklog-add\">เพิ่มบันทึกการปฏิบัติงาน</button><button type=\"button\" class=\"ws-button primary\" data-a5-plan-action=\"worklog-submit\">ส่งบันทึกฉบับปัจจุบัน</button></div>"}</section>`;
  }

  const api = Object.freeze({ normalizeCasePlanA5, validateCasePlanA5, saveCasePlanA5, submitCasePlanA5, submitWorklogA5, appendWorklogEntryA5, renderCasePlanEditorA5, FORM_ID, WORKLOG_ID, FORM_644_ID, WORKLOG_644_ID });
