(() => {
  ...
  function saveComplaintToWorkspace(caseData) {
    ...
    if (existing?.workflow && (!["admin", "admin-registry"].includes(existing.workflow.stage) || existing.workflow.owner !== "admin" || existing.workflow.complete)) {
      return existing;
    }
