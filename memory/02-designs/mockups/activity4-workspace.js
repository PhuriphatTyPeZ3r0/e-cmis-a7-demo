(() => {
  ...
  const isCentralRegistryChannel = (channel,region) => region==='ส่วนกลาง'&&(channelKey(channel)==='จดหมาย'||isM62Channel(channel));
  ...
  function readStore(){
    try{
      ...
      const originalStore=JSON.parse(workspaceRaw||'{}');
  ...
  function transitionCase(state,action,payload,role,name){
    const result=window.ECMISActivity4DocumentRules?.transitionCase(state,action,payload,{role,name,at:new Date().toISOString(),region:role.startsWith('regional-')?activeRegion:''});
  ...
  function officialDocumentIds(state){
    ...
    const incomingM62=isM62Channel(state.caseData.channel);
  ...
  function officialFieldIds(state){
    const d=state.documentData,incomingM62=isM62Channel(state.caseData.channel),groups=[];
  ...
  function officialFieldValue(state,id){
    ...
    const exact={
      ...
      naccRecipientName:c.naccRecipientName||(isM62Channel(c.channel)&&/ป\.ป\.ช\./.test(c.complainant||'')?c.complainant:''),
      naccProvince:c.naccProvince||(isM62Channel(c.channel)?c.province:'')
    };
    return exact[id]||'';
  }
  ...
  function officerSendReviewHtml(previewPages,reviewTarget){
    const documentCount=new Set(previewPages.map(page=>page.name)).size;
    const pageList=previewPages.map((page,index)=>`<li><button type="button" class="officer-review-document" data-officer-preview-index="${index}" ${index?'disabled':''}><span class="officer-review-document-state" aria-hidden="true">${index+1}</span><span><strong>${escapeHtml(page.name)}</strong><small>${page.total>1?`หน้าที่ ${page.index} จาก ${page.total}`:'1 หน้า'}</small></span></button></li>`).join('');
  ...
  function renderWalkin(){
    ...
    const intakeChannels=regionalIntake?REGIONAL_CHANNELS:['Walk-In','สายด่วน 1206','เบอร์โทร ปปท. เขต 1-9','Email','สปน. 1111','Traffy Fondue','ศปท. ป.ป.ท.','Feedback','การข่าวภายใน ป.ป.ท.','ศบช. / กนป.','เครือข่ายภาคประชาชน','ข้อสั่งการ','เหตุสงสัย','หนังสือจากหน่วยงานอื่น','สื่อสารองค์กร / Facebook','จดหมาย','ม.62 (ป.ป.ช.)'];
  ...
  function renderStaff(){
    const root=$('#staffApp');if(!root)return;const params=new URLSearchParams(location.search);let selectedRole=params.get('role')||sessionStorage.getItem('ecmis-a4-role')||'admin';let activeRole=selectedRole;let activeRegion=REGIONAL_REGIONS.includes(params.get('region'))?params.get('region'):(sessionStorage.getItem('ecmis-a4-regional-region')||'เขต 1');if(!REGIONAL_REGIONS.includes(activeRegion))activeRegion='เขต 1';let selectedId=params.get('case')||null;let adminQueue=['unassigned','assigned'].includes(params.get('queue'))?params.get('queue'):'unassigned';let filters={};
    ...
    $('#wsRole').value=selectedRole;updateRoleControls();$('#wsRole').onchange=e=>{selectedRole=e.target.value;activeRole=selectedRole;sessionStorage.setItem('ecmis-a4-role',selectedRole);updateRoleControls();delete $('#caseListView').dataset.anonymousReady;if(selectedId&&!canViewState(getState(selectedId)))selectedId=null;syncUrl();if(selectedId)renderDetail();else{$('#caseDetailView').classList.add('ws-hidden');$('#caseListView').classList.remove('ws-hidden')}renderList()};
    ...
    function outgoingRegisterPanel(state){
      ...
      const cards=slots.map(slot=>{
        ...
        if(slot.mode==='blocked-unconfigured')return `<article class="outgoing-register-card blocked" data-outgoing-slot="${escapeHtml(slot.slotId)}"><header><div><span class="outgoing-register-eyebrow">ทะเบียนเขตยังไม่ได้กำหนด</span><strong>${escapeHtml(title)}</strong></div><b class="outgoing-register-status attention">รอบันทึกเลขทางการ</b></header><p class="outgoing-register-note">ห้ามใช้ prefix ส่วนกลางแทนเลขเขต บันทึกได้เฉพาะเลขทางการที่เขตออกจริง</p><div class="outgoing-manual-fields"><input data-outgoing-manual-number="${escapeHtml(slot.slotId)}" placeholder="เลขหนังสือที่เขตออกจริง"><input data-outgoing-manual-note="${escapeHtml(slot.slotId)}" placeholder="ที่มา/หมายเหตุทะเบียนเขต"><button type="button" class="ws-button secondary" data-action="record-manual-outgoing" data-slot-id="${escapeHtml(slot.slotId)}">บันทึกเลขทางการของเขต</button></div></article>`;
      ...
      return `<section class="ws-section outgoing-register-section" data-outgoing-register-section><header class="outgoing-register-section-head"><div><h3>เลขหนังสือขาออก</h3><p>เลือกทะเบียน แล้วให้ระบบ Running ตามทะเบียนและปี พ.ศ. เอกสารแต่ละฉบับแยกเลขกัน</p></div></header><div class="outgoing-register-list">${cards}</div></section>`;
    }
...
opinionPaper.addEventListener('blur',event=>{
    ...
    async function handleAction(action,state){
      ...
      if(action==='officer-send'){const canSubmit=selectedRole==='regional-officer'?w.owner==='regional-officer'&&w.stage==='regional-officer'&&canRegionalAccess(state.caseData):w.owner==='officer'&&w.stage==='officer';if(!canSubmit)return notify('warning','ไม่สามารถส่งพิจารณา','เรื่องไม่ได้อยู่ในขั้นเจ้าหน้าที่ก่อนเสนอผู้มีอำนาจ');
        ...
        const reviewResult=window.Swal?await Swal.fire({title:`ตรวจสอบชุดเอกสารก่อนส่ง ${reviewTarget}`,html:officerSendReviewHtml(previewPages,reviewTarget),customClass:{popup:'ecmis-document-preview-dialog',htmlContainer:'ecmis-document-preview-content',confirmButton:'ecmis-preview-confirm',cancelButton:'ecmis-preview-cancel'},showCancelButton:true,confirmButtonText:`ยืนยันส่งต่อ ${reviewTarget}`,cancelButtonText:'กลับไปแก้ไข',confirmButtonColor:'#082b50',cancelButtonColor:'#687789',allowOutsideClick:false,didOpen:()=>{
          let current=0;
          let zoom=Number.parseFloat(getComputedStyle($('.officer-document-preview')).getPropertyValue('--officer-preview-zoom'))||.72;
    ...
    syncUrl();renderList();if(selectedId&&canViewState(getState(selectedId)))renderDetail();else{selectedId=null;syncUrl()}
  }
