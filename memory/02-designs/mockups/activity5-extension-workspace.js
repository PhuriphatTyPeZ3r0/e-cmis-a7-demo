(function initializeActivity5ExtensionWorkspace(root) {
  ...
  function hasOpenableSystemPreview(item) {
    if (!item || item.availability !== "AVAILABLE" || item.source !== "SYSTEM"
      || !["CASE_PLAN", "WORK_LOG"].includes(item.documentType)) return false;
  ...
  function recoverPreviewRepository(persistedRepository, freshRepository) {
    ...
    return (persistedRepository || []).map(sourceItem => {
      ...
      const requiresSystemPreview = item.availability === "AVAILABLE"
        ...
        && (item.previewDocument === "plan" || Boolean(item.previewReportType) || Boolean(item.previewSourceField) || Boolean(item.previewSnapshot));
  ...
  function renderRequestStep(model) {
    ...
    return `<section class="a5-extension-step" data-extension-step="1">
      ...
      <div class="ws-field${model.ui?.fieldErrors?.requestedDays ? " has-error" : ""}"><label for="a5Extension-requestedDays">จำนวนวันที่ขอ</label><input id="a5Extension-requestedDays" name="requestedDays" data-a5-extension-field="requestedDays" type="number" min="1" inputmode="numeric" value="${escapeHtml(payload.requestedDays ?? "")}"${model.ui?.fieldErrors?.requestedDays ? ' aria-invalid="true" aria-describedby="a5Extension-requestedDays-error"' : ""}><small>จำนวนวันที่อนุมัติและผู้พิจารณาอยู่ระหว่างยืนยันกติกากระบวนงาน</small>${model.ui?.fieldErrors?.requestedDays ? `<p class="a5-extension-field-error" id="a5Extension-requestedDays-error">${escapeHtml(model.ui.fieldErrors.requestedDays)}</p>` : ""}</div>
    </section>`;
  }
  ...
  function renderRequirementChecklist(model, compact = false) {
    ...
    return `<section class="a5-extension-checklist${compact ? " compact" : ""}" aria-label="รายการเอกสารบังคับ">
      <header><h3>รายการเอกสารบังคับ ${model.requestState?.formId === "FORM_2" ? "3" : "4"} รายการ</h3><span class="ws-status${readiness.ok && readiness.result.complete ? " success" : ""}">${readiness.ok && readiness.result.complete ? "พร้อม" : "ยังไม่ครบ"}</span></header>
      <ol>${requirements.map(requirement => `<li data-requirement-state="${requirement.complete ? "ready" : "missing"}"><span aria-hidden="true"></span><div><strong>${escapeHtml(REQUIREMENT_LABELS[requirement.requirementCode] || requirement.requirementCode)}</strong><small>${escapeHtml(readiness.ok ? readinessMessage(requirement) : "ตรวจรายการไม่ได้ กรุณาตรวจข้อมูลเอกสาร")}</small></div></li>`).join("")}</ol>
    </section>`;
  }
  ...
  function renderDocumentsStep(model) {
    ...
    return `<section class="a5-extension-step" data-extension-step="2">
      <header><p class="ws-kicker">ขั้นที่ 2</p><h3>เอกสารประกอบ</h3><p>เลือกเวอร์ชันที่ใช้จริงและผูกกับรายการบังคับของคำขอนี้</p></header>
      ...
      <div class="ws-field"><label for="a5ExtensionRequirement">นำเอกสารที่เลือกไปใช้กับ</label><select id="a5ExtensionRequirement">${(rule?.requiredDocumentCodes || []).map(code => `<option value="${escapeHtml(code)}"${code === activeRequirement ? " selected" : ""}>${escapeHtml(REQUIREMENT_LABELS[code] || code)}</option>`).join("")}</select></div>
      ...
      <button type="button" class="ws-button secondary" data-a5-extension-action="assign-requirement">ใช้เอกสารที่เลือกกับรายการนี้</button>
