(function initializeActivity5Report644(root) {
  ...
  const ACTIONS = Object.freeze([
    ...
    "report-644-sign", "report-644-create-package", "report-644-send-a7", "report-644-record-receipt", "report-644-record-result"
  ]);
  ...
  function reportPackage(state, record, lifecycle, input, person) {
    ...
    return {
      ...
      evidence: payload.evidence.map(item => copy(exactVersion(state, item.documentVersionId))), renderedPayload: renderReport644PaperA5(payload), submittedBy: person.id, submittedAt: input.at
    };
  }
  ...
  function executeReport644Action(sourceState, actor, action, command) {
    ...
    if (action === "form-5-sign") {
      if (person.role !== "director") return fail(sourceState, "FORBIDDEN_ACTOR", "actor", "ผอ.สำนักงาน ป.ป.ท. เขต/ประธานอนุกรรมการไต่สวนเท่านั้นที่ลงนามหนังสือแจ้งข้อกล่าวหาได้");
      const accusedRowId = text(input.accusedRowId);
    ...
    if (pending && action !== "report-644-record-receipt") return pending;

    if (action === "report-644-review-record-opinion") {
      if (person.role !== "director") return fail(sourceState, "FORBIDDEN_ACTOR", "actor", "ผอ.สำนักงาน ป.ป.ท. เขต/ผู้บังคับบัญชาสายตรวจเท่านั้นที่บันทึกความเห็นได้");
      if (lifecycle.status !== "REPORT_644_REVIEW_PENDING" || !text(input.opinionText) || !text(input.opinionId)) return fail(sourceState, "INVALID_TRANSITION", "opinionText", "รายงานไม่อยู่ระหว่างตรวจหรือความเห็นไม่ครบ");
      const opinion = { opinionId: input.opinionId, reportDocumentId: FORM_7_ID, reportRevisionNo: submission.report.revisionNo, sequence: lifecycle.reviewOpinions.length + 1, reviewerId: person.id, reviewerName: person.name, reviewerRole: person.role, decision: "ENDORSE", opinionText: input.opinionText, affectedFields: [], affectedDocumentVersionIds: [], recordedAt: input.at, signature: null };
      lifecycle.reviewOpinions.push(opinion);
      if (input.finalLevel === true) lifecycle.status = "REPORT_644_BOARD_READY";
      bump(state);
      return finish(context, state, "REPORT_644_OPINION_RECORDED", { opinion: copy(opinion) });
    }

    if (action === "report-644-review-return") {
      if (person.role !== "director") return fail(sourceState, "FORBIDDEN_ACTOR", "actor", "ผอ.สำนักงาน ป.ป.ท. เขต/ผู้บังคับบัญชาสายตรวจเท่านั้นที่ส่งกลับแก้ไขได้");
      ...
      lifecycle.reviewOpinions.push({ opinionId: input.opinionId || `return-${input.idempotencyKey}`, reportDocumentId: FORM_7_ID, reportRevisionNo: submission.report.revisionNo, sequence: lifecycle.reviewOpinions.length + 1, reviewerId: person.id, reviewerName: person.name, reviewerRole: person.role, decision: "RETURN", opinionText: input.reason, affectedFields, affectedDocumentVersionIds, recordedAt: input.at, signature: null });
    ...
    if (action === "report-644-sign") {
      if (person.role !== "director") return fail(sourceState, "FORBIDDEN_ACTOR", "actor", "ผอ.สำนักงาน ป.ป.ท. เขต/ประธานอนุกรรมการไต่สวนเท่านั้นที่ลงนามรายงานได้");
      if (lifecycle.status !== "REPORT_644_BOARD_READY" || !text(input.methodLabel)) return fail(sourceState, "SIGNATURE_REQUIRED", "methodLabel", "รายงานยังไม่พร้อมลงนาม");
      const signature = { reportRevisionNo: submission.report.revisionNo, submissionPackageId: submission.packageId, signedBy: person.id, signerName: person.name, signerRole: person.role, signedAt: input.at, methodLabel: input.methodLabel };
      lifecycle.signatures.push(signature); bump(state);
      return finish(context, state, "REPORT_644_SIGNED", { signature: copy(signature) });
    }
    ...
    if (action === "report-644-create-package") {
      ...
      if (!signature) return fail(sourceState, "SIGNATURE_REQUIRED", "signature", "ต้องลงนามก่อนจัดชุดเสนอคณะกรรมการ");
      const packageRecord = { packageId: input.boardPackageId || `board-${submission.packageId}`, submissionPackageId: submission.packageId, signedReportRef: { documentId: FORM_7_ID, revisionNo: submission.report.revisionNo, signature }, reviewOpinionIds: lifecycle.reviewOpinions.map(item => item.opinionId), attachmentVersionIds: copy(input.attachmentVersionIds || []), createdAt: input.at };
  ...
  function renderReport644SectionEditor(payload, key, disabled, options = {}) {
    ...
    if (key === "allegations") return rows(payload.allegations, (row, index) => card(`${hidden(`allegations.${index}.rowId`, row.rowId)}${hidden(`allegations.${index}.order`, row.order)}${control("รายละเอียดข้อกล่าวหา", `allegations.${index}.summary`, row.summary, disabled)}${control("วันเวลาและสถานที่เกิดเหตุ", `allegations.${index}.eventDescription`, row.eventDescription, disabled)}`, index));
    if (key === "evidence") return rows(payload.evidence, (row, index) => card(`${hidden(`evidence.${index}.rowId`, row.rowId)}${hidden(`evidence.${index}.order`, row.order)}${hidden(`evidence.${index}.documentVersionId`, row.documentVersionId)}${selectField("หมวดพยานหลักฐาน", `evidence.${index}.category`, row.category, disabled, EVIDENCE_CATEGORY_LABELS_A5)}${control("ชื่อพยานหลักฐาน", `evidence.${index}.title`, row.title, disabled, "input")}${control("ข้อเท็จจริงที่สนับสนุน", `evidence.${index}.factSupported`, row.factSupported, disabled)}${control("การเก็บรักษา", `evidence.${index}.custodyNote`, row.custodyNote, disabled)}`, index));
    if (key === "otherMeasures") return rows(payload.otherMeasures, (row, index) => card(`${hidden(`otherMeasures.${index}.rowId`, row.rowId)}${hidden(`otherMeasures.${index}.order`, row.order)}${selectField("หมวด", `otherMeasures.${index}.kind`, row.kind, disabled, OTHER_MEASURE_LABELS_A5)}${control("การดำเนินการ", `otherMeasures.${index}.detail`, row.detail, disabled)}${control("ผลการดำเนินการ", `otherMeasures.${index}.result`, row.result, disabled)}`, index));
    ...
    if (key === "limitation") {
      ...
      return rows(payload.limitation, (row, index) => card(`${hidden(`limitation.${index}.rowId`, row.rowId)}${hidden(`limitation.${index}.order`, row.order)}${selectField("ข้อกล่าวหาที่เกี่ยวข้อง", `limitation.${index}.allegationRowId`, row.allegationRowId, disabled, allegationOptions)}${control("วันเริ่มนับ", `limitation.${index}.startAt`, row.startAt, disabled, "input")}${control("วันขาดอายุความ", `limitation.${index}.expiresAt`, row.expiresAt, disabled, "input")}${control("ที่มาของการคำนวณ", `limitation.${index}.source`, row.source, disabled)}${control("หมายเหตุ", `limitation.${index}.note`, row.note, disabled)}`, index));
    }
    if (key === "legalBasis") return rows(payload.legalBasis, (row, index) => card(`${hidden(`legalBasis.${index}.rowId`, row.rowId)}${hidden(`legalBasis.${index}.order`, row.order)}${control("ชื่อกฎหมาย", `legalBasis.${index}.lawName`, row.lawName, disabled, "input")}${control("มาตรา", `legalBasis.${index}.section`, row.section, disabled, "input")}${control("เหตุผลที่นำมาใช้", `legalBasis.${index}.applicationReason`, row.applicationReason, disabled)}`, index));
    if (key === "chargeNotice") {
  ...
  function renderForm5PaperA5(payload) {
    ...
    return `<article class="a5-report-paper a5-form5-paper"><header class="a5-f5-head">
      <p class="a5-f5-top"><span class="a5-f5-letterno"><strong>ที่ ${escapeHtml(show(payload.noticeMeta?.letterNo))}</strong></span><span class="a5-f5-address">สำนักงาน ป.ป.ท.<br>อาคารซอฟต์แวร์ปาร์ค ถนนแจ้งวัฒนะ<br>อำเภอปากเกร็ด จังหวัดนนทบุรี ๑๑๑๒๐</span></p>
  ...
  const api = Object.freeze({
    FORM_5_ID, FORM_6_ID, FORM_7_ID, REPORT_SECTION_KEYS, ACTIONS, form5DocId, form6DocId, normalizeReport644A5, validateForm5A5, validateForm6A5, validateReport644A5, executeReport644Action,
