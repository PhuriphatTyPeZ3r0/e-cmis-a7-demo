...
const aligned = ECMIS.g1Triggers(ECMIS.getCase("0012/2565"));
assert.equal(aligned.diverged, false, "0012/2565 ความเห็นตรงกันทุกชั้น");
assert.equal(aligned.required, false, "ความเห็นตรงกัน + ไม่ซับซ้อน ต้องไม่เข้า G1");

assert.equal(
  ECMIS.g1Triggers({ ...ECMIS.getCase("0012/2565"), complex: true }).required, true,
...
assert.equal(ECMIS.effectiveSlaLimit(ECMIS.getCase("0012/2565")), 15,
  "รายงาน 644 ที่รอลงนาม ต้องใช้เพดาน 15 วัน");

/* เพดานของชั้นเลขาธิการฯ ต้องไม่ไปทับ SLA ของขั้นตอนอื่น */
const screening = ECMIS.getCase("1119/2565");
...
const r644 = ECMIS.getCase("0012/2565");
...
assert.equal(ECMIS.m28Pending().some((c) => c.id === "1525/2558"), false,
...
assert.equal(ECMIS.canAct(ECMIS.getCase("1609/2568"), "secgen"), false,
  "สำนวนที่ยังร่างอยู่ในกอง/เขต เลขาธิการฯ ต้องยังสั่งการไม่ได้");
assert.equal(ECMIS.canAct(ECMIS.getCase("0719/2568"), "secgen"), true,
