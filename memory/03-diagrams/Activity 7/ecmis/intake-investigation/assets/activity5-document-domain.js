(function initializeActivity5DocumentDomain(root) {
  ...
  function createA5DocumentRevisionInternal(sourceState, sourceCommand) {
    const context = validateCommon(sourceState, sourceCommand, "createA5DocumentRevision");
    if (context.failure || context.replay) return context.failure || context.replay;
    const { state, command, store, commandFingerprint } = context;
    if (!isPlainObject(command.payload)) return failure("INVALID_PAYLOAD", state, "payload", "ข้อมูลเอกสารไม่ถูกต้อง");
    const active = activeRecord(store, command.documentId);
    if (!active) return failure("DOCUMENT_NOT_FOUND", state, "documentId", "ไม่พบเอกสาร");
    if (active.status !== "SUBMITTED" || !active.submittedSnapshot) return failure("INVALID_TRANSITION", state, "revisionNo", "ฉบับก่อนหน้ายังไม่ได้ส่ง");
    const record = {
    ...
    store.records.push(record);
    store.version += 1;
    recordReceipt(store, command, "createA5DocumentRevision", commandFingerprint, "DOCUMENT_REVISION_CREATED", record);
    return envelope(true, "DOCUMENT_REVISION_CREATED", state);
  }
  ...
  function createA5DocumentRevision(sourceState, sourceCommand) {
    return preserveSourceOnFailure(sourceState, () => createA5DocumentRevisionInternal(sourceState, sourceCommand));
  }
  ...
  const api = Object.freeze({ normalizeA5DocumentStore, createA5DocumentDraft, createA5DocumentRevision, saveA5DocumentDraft, submitA5DocumentRevision, returnA5DocumentRevision, fingerprintA5SubmittedSnapshot });
