(function initializeActivity5ExtensionAuthority(root) {
  ...
  function roleForTier(tier) {
    ...
    if (tier === rules?.AUTHORITY_TIERS?.SUPERVISING_EXECUTIVE) return "executive";
    return "secretary";
  }
  ...
  function createMockAuthorityRegistry({ unitKey, effectiveFrom }) {
    ...
    const assignments = [
      rules.AUTHORITY_TIERS.GROUP_DIRECTOR,
      rules.AUTHORITY_TIERS.UNIT_DIRECTOR,
      rules.AUTHORITY_TIERS.SUPERVISING_EXECUTIVE,
      rules.AUTHORITY_TIERS.SECRETARY_GENERAL_PERSONAL
    ].map(tier => ({
  ...
  function resolveReviewerContract(source = {}) {
    ...
    const authorityTiers = Array.isArray(round.result.authorityTiers) ? round.result.authorityTiers : [round.result.authorityTier];
    let resolvedTier = authorityTiers.find(tier => registry && effectiveAssignments(registry, unitKey, tier, effectiveDate).some(item => item.status === "ACTIVE")) || round.result.authorityTier;
    let matches = registry ? effectiveAssignments(registry, unitKey, resolvedTier, effectiveDate) : [];
    const hasExplicitForKey = Array.isArray(registry?.assignments) && authorityTiers.some(tier => registry.assignments.some(item => normalizedUnitKey(item?.unitKey) === unitKey
      && item?.authorityTier === tier));
    if (!matches.length && !hasExplicitForKey && source.allowMockRoleSlot === true) {
      ...
      resolvedTier = round.result.authorityTier;
      matches = effectiveAssignments(registry, unitKey, resolvedTier, effectiveDate);
    }
    ...
    if (assignment.status !== "ACTIVE" || !text(assignment.actorId) || !text(assignment.assignmentId)
      || assignment.actorRole !== roleForTier(resolvedTier)) {
      return pending("AUTHORITY_ASSIGNMENT_MISSING");
    }
    if (assignment.actingForTier && assignment.actingForTier !== resolvedTier) {
      return pending("ACTING_ASSIGNMENT_INCOMPLETE");
    }
    if (resolvedTier === "SECRETARY_GENERAL_PERSONAL"
