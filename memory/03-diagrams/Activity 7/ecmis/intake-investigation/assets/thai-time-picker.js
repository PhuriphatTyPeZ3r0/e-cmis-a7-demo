...
(function (global) {
  ...
  function wire(id) {
    ...
    function onDocClick(evt) { if (!evt.target.closest(`[data-tdp-time-id="${id}"]`) && !evt.target.closest(`#${id}Popover`)) closePicker() }
    function positionPopover() {
      const rect = trigger.getBoundingClientRect();
      const margin = 8;
      const popW = popover.offsetWidth || 290;
    ...
    function openPicker() {
      ...
      requestAnimationFrame(() => { hourWheelApi.setValue(h); minuteWheelApi.setValue(m); positionPopover() });
    }
