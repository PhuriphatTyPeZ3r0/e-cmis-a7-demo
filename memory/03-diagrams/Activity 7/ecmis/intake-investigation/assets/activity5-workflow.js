(function initializeActivity5Workflow(root) {
  ...
  const ADMIN_ACTIONS = Object.freeze({
    ...
    "panel-change-record-signed-order": Object.freeze({ id: "panel-change-record-signed-order", label: "บันทึกคำสั่งปรับองค์คณะฉบับลงนาม", role: "clerk", primary: false }),
  ...
  const DOWNSTREAM_STATUSES = Object.freeze([
    "REPORT_213_DRAFT", "REPORT_213_REVIEW_PENDING", "REPORT_213_RETURNED", "REPORT_213_FINAL_SIGNATURE_PENDING", "REPORT_213_SIGNED_AWAITING_ROUTE",
    "REPORT_213_URGENT_LETTER_PENDING", "REPORT_213_SUPPORT_COMMITTEE_PENDING", "REPORT_213_SUPPORT_SECRETARY_CONFIRM_PENDING",
    "REPORT_213_GBK_PENDING", "REPORT_213_SCREENING_PENDING", "REPORT_213_CHAIR_AGENDA_PENDING", "REPORT_213_BOARD_READY",
    "REPORT_213_SENT_TO_A7", "REPORT_213_WAIT_RESULT", "REPORT_213_RESULT_RECEIVED",
    "NACC_REFERRAL_DRAFT", "NACC_REFERRAL_GROUP_REVIEW", "NACC_REFERRAL_DIRECTOR_REVIEW", "NACC_REFERRAL_RETURNED", "NACC_REFERRAL_GBK_PENDING", "NACC_REFERRAL_BOARD_PENDING", "NACC_REFERRAL_RESULT_RECEIVED",
    "REPORT_644_DRAFT", "REPORT_644_REVIEW_PENDING", "REPORT_644_RETURNED", "REPORT_644_ROUTE_PENDING", "REPORT_644_URGENT_LETTER_PENDING", "REPORT_644_SUPPORT_COMMITTEE_PENDING", "REPORT_644_SUPPORT_SECRETARY_CONFIRM_PENDING", "REPORT_644_BOARD_READY", "REPORT_644_SENT_TO_A7", "REPORT_644_WAIT_RESULT", "REPORT_644_RESULT_RECEIVED",
  ...
  const INTERVENTION_ACTIONS = Object.freeze({
    "search-warrant-mock-request": Object.freeze({ id: "search-warrant-mock-request", label: "ขอหมายค้น (จำลอง)", role: "investigator" }),
    "search-warrant-create": Object.freeze({ id: "search-warrant-create", label: "จัดทำคำร้องขอหมายค้น", role: "investigator" }),
    "search-warrant-send-a9": Object.freeze({ id: "search-warrant-send-a9", label: "รอยืนยันปลายทางคำร้องหมายค้น", role: "investigator" }),
    "search-warrant-record-a9-receipt": Object.freeze({ id: "search-warrant-record-a9-receipt", label: "บันทึกกิจกรรมที่ 9 รับเรื่อง", role: "investigator" }),
    "search-warrant-record-court-result": Object.freeze({ id: "search-warrant-record-court-result", label: "บันทึกผลศาลเกี่ยวกับหมายค้น", role: "investigator" }),
    "search-warrant-record-search": Object.freeze({ id: "search-warrant-record-search", label: "บันทึกการค้นและหลักฐานที่พบ", role: "investigator" })
  });
  const DOWNSTREAM_ACTIONS = Object.freeze({
    "nacc-referral-start": { id: "nacc-referral-start", label: "แยกเส้นทางจัดทำรายงานสืบสวนส่ง ป.ป.ช.", role: "investigator", from: ["REPORT_213_DRAFT", "REPORT_213_RETURNED"] },
    "nacc-referral-submit": { id: "nacc-referral-submit", label: "เสนอรายงานสืบสวนพร้อมแนบร่าง 213", role: "investigator", from: ["NACC_REFERRAL_DRAFT", "NACC_REFERRAL_RETURNED"] },
    "nacc-referral-group-approve": { id: "nacc-referral-group-approve", label: "ผอ.กลุ่มเห็นชอบรายงานสืบสวน", role: "group-director", from: ["NACC_REFERRAL_GROUP_REVIEW"] },
    "nacc-referral-group-return": { id: "nacc-referral-group-return", label: "ผอ.กลุ่มส่งรายงานสืบสวนกลับแก้ไข", role: "group-director", from: ["NACC_REFERRAL_GROUP_REVIEW"], primary: false },
    "nacc-referral-group-skip": { id: "nacc-referral-group-skip", label: "ข้ามชั้น ผอ.กลุ่ม เพราะไม่อยู่ในสาย", role: "director", from: ["NACC_REFERRAL_GROUP_REVIEW"] },
    "nacc-referral-director-approve": { id: "nacc-referral-director-approve", label: "ผอ.หน่วยงานเห็นชอบส่ง กบค.", role: "director", from: ["NACC_REFERRAL_DIRECTOR_REVIEW"] },
    "nacc-referral-director-return": { id: "nacc-referral-director-return", label: "ผอ.หน่วยงานส่งรายงานสืบสวนกลับแก้ไข", role: "director", from: ["NACC_REFERRAL_DIRECTOR_REVIEW"], primary: false },
    "nacc-referral-gbk-receive": { id: "nacc-referral-gbk-receive", label: "กบค.รับเรื่องและเสนอเข้าบอร์ด", role: "clerk", from: ["NACC_REFERRAL_GBK_PENDING"] },
    "nacc-referral-board-record-result": { id: "nacc-referral-board-record-result", label: "บันทึกมติบอร์ด", role: "committee", from: ["NACC_REFERRAL_BOARD_PENDING"] },
    ...
    "report-213-record-result": { id: "report-213-record-result", label: "บันทึกผลพิจารณา 213", role: "clerk", from: ["REPORT_213_WAIT_RESULT"] },
    ...
    "report-644-record-result": { id: "report-644-record-result", label: "บันทึกรับผลตรวจเสนอรายงาน 644", role: "clerk", from: ["REPORT_644_WAIT_RESULT"] },
    ...
    "prepare-644-order-sign": { id: "prepare-644-order-sign", label: "ลงนามคำสั่งแต่งตั้ง ม.24 ว.1", role: "secretary", from: ["OUTCOME_TASKS_IN_PROGRESS"] },
    "prepare-644-order-record": { id: "prepare-644-order-record", label: "บันทึกคำสั่งแต่งตั้งที่ลงนามแล้ว", role: "clerk", from: ["OUTCOME_TASKS_IN_PROGRESS"] },
    "prepare-644-notify": { id: "prepare-644-notify", label: "แจ้งมติและคำสั่งแก่ผู้รับผิดชอบ 644", role: "clerk", from: ["OUTCOME_TASKS_IN_PROGRESS"] },
    "prepare-644-acknowledge": { id: "prepare-644-acknowledge", label: "รับทราบมติและคำสั่งแต่งตั้ง", role: "investigator", from: ["OUTCOME_TASKS_IN_PROGRESS"] },
    "prepare-644-handover-send": { id: "prepare-644-handover-send", label: "ส่งมอบสำนวนให้ผู้รับผิดชอบชั้น 644", role: "investigator", from: ["OUTCOME_TASKS_IN_PROGRESS"] },
    "prepare-644-handover-accept": { id: "prepare-644-handover-accept", label: "ลงนามรับมอบสำนวนชั้น 644", role: "investigator", from: ["OUTCOME_TASKS_IN_PROGRESS"] },
    ...
    "closure-evaluate": { id: "closure-evaluate", label: "ตรวจความพร้อมปิดสำนวน", role: "clerk", from: ["REPORT_213_RESULT_RECEIVED", "REPORT_644_DRAFT", "REPORT_644_REVIEW_PENDING", "REPORT_644_RETURNED", "REPORT_644_ROUTE_PENDING", "REPORT_644_URGENT_LETTER_PENDING", "REPORT_644_SUPPORT_COMMITTEE_PENDING", "REPORT_644_SUPPORT_SECRETARY_CONFIRM_PENDING", "REPORT_644_BOARD_READY", "REPORT_644_SENT_TO_A7", "REPORT_644_WAIT_RESULT", "REPORT_644_RESULT_RECEIVED", "OUTCOME_TASKS_PENDING", "OUTCOME_TASKS_IN_PROGRESS", "PROSECUTOR_RESULT_RECEIVED"] },
  ...
  function roleModelA5() {
    return root.ECMISActivity5RoleModel
      || (typeof require === "function" ? require("./activity5-role-model.js") : null);
  }

  function permissionRoleA5(role) {
    return roleModelA5()?.getA5PermissionRole?.(role) || String(role || "");
  }

  function roleActionContextA5(sourceState) {
    const source = asObject(sourceState);
    const stage = String(asObject(source.workflow).stage || "");
    const orderType = String(asObject(asObject(source.inquiry).committee213).orderType || "");
    return { reportType: ["a5-inquiry", "a5-inquiry-review", "a7-644", "a5-outcome", "a5-prosecutor", "closed"].includes(stage) ? "644" : "213", orderType };
  }

  function roleAllowsA5(role, actionId, sourceState) {
    return roleModelA5()?.canA5RolePerform?.(role, actionId, roleActionContextA5(sourceState)) !== false;
  }

  function filterRoleActionsA5(role, sourceState, actions) {
    return actions.filter(action => roleAllowsA5(role, action.id, sourceState));
  }
  ...
  function normalizeA5State(sourceState) {
    ...
    state.staffDirectory = Array.isArray(state.staffDirectory) ? state.staffDirectory.map(entry => ({
      officerId: String(entry?.officerId || "").trim(),
      officerName: String(entry?.officerName || entry?.name || "").trim(),
      positionName: String(entry?.positionName || entry?.position || "").trim(),
      unitName: String(entry?.unitName || entry?.unit || "").trim(),
      status: String(entry?.status || "ACTIVE").trim().toUpperCase()
    })).filter(entry => entry.officerId && entry.officerName) : [];
    ...
    state.appointmentOrders = asObject(state.appointmentOrders);
    state.appointmentOrders.primary = asObject(state.appointmentOrders.primary);
    state.appointmentOrders.supplements = Array.isArray(state.appointmentOrders.supplements) ? state.appointmentOrders.supplements : [];
    ...
    state.caseAdministration.caseValueAmount = Number(state.caseAdministration.caseValueAmount) || 0;
    ...
    state.naccReferral = asObject(state.naccReferral);
    state.searchWarrantRequests = Array.isArray(state.searchWarrantRequests) ? state.searchWarrantRequests : [];
    state.a5EvidenceRepository = Array.isArray(state.a5EvidenceRepository) ? state.a5EvidenceRepository : [];
    ...
    const hasReport213Draft = (Array.isArray(state.a5DocumentStore?.records) ? state.a5DocumentStore.records : [])
      .some(record => record?.documentId === "FORM_4_REPORT_213" && record?.status === "DRAFT");
    ...
    else if (state.workflow.stage === "a5-prelim" && (hasReport213Draft || String(asObject(asObject(state.inquiry).prelim).report || "").trim()) && [PROCESS_STATES.PLAN_APPROVED, PROCESS_STATES.AMENDMENT_APPROVED, PROCESS_STATES.LEGACY_ACTIVE].includes(state.workflow.a5Status)) state.workflow.downstreamStatus = "REPORT_213_DRAFT";
  ...
  function downstreamActionsFor(state, role) {
    ...
    if (status.startsWith("REPORT_213_")) {
      const report213 = root.ECMISActivity5Report213 || (typeof require === "function" ? require("./activity5-report-213.js") : null);
      const canonical = report213?.getReport213ActionModelA5?.(state, role) || [];
      if (Array.isArray(state.a5Report213Lifecycle?.submissions) && state.a5Report213Lifecycle.submissions.length) return canonical.map(action => Object.freeze({ ...action }));
    }
    const task = state.downstreamTasks.find(item => item.status !== "COMPLETED" && item.type !== "REPORT_NACC_SECTION_65");
    if (task?.type === "PREPARE_644_START" && status === "OUTCOME_TASKS_IN_PROGRESS") {
      if (task.status === "IN_PROGRESS" && task.orderType === "24v1" && role === "secretary") return [Object.freeze({ ...DOWNSTREAM_ACTIONS["prepare-644-order-sign"], enabled: true })];
      if (task.status === "IN_PROGRESS" && task.orderType === "24v3" && role === "clerk") return [Object.freeze({ ...DOWNSTREAM_ACTIONS["prepare-644-order-record"], enabled: true })];
      if (task.status === "AWAITING_NOTIFICATION" && role === "clerk") return [Object.freeze({ ...DOWNSTREAM_ACTIONS["prepare-644-notify"], enabled: true })];
      if (task.status === "AWAITING_ACKNOWLEDGEMENT" && role === "investigator") return [Object.freeze({ ...DOWNSTREAM_ACTIONS["prepare-644-acknowledge"], enabled: true })];
      if (task.status === "AWAITING_HANDOVER_SEND" && role === "investigator") return [Object.freeze({ ...DOWNSTREAM_ACTIONS["prepare-644-handover-send"], enabled: true })];
      if (task.status === "AWAITING_HANDOVER_ACCEPT" && role === "investigator") return [Object.freeze({ ...DOWNSTREAM_ACTIONS["prepare-644-handover-accept"], enabled: true })];
      return [];
    }
    if (role === "clerk" && ["REPORT_213_RESULT_RECEIVED", "REPORT_644_RESULT_RECEIVED", "NACC_REFERRAL_RESULT_RECEIVED", "OUTCOME_TASKS_PENDING", "OUTCOME_TASKS_IN_PROGRESS"].includes(status)) {
  ...
  function getA5AvailableActionsCore(sourceState, role) {
    ...
    // ศรร (officer) ต้องรับมอบใน ก5 ได้ด้วย — อนุโลมให้ investigator/officer/regional-officer รับแทนได้
    const acceptorRoles = new Set(["investigator", "officer", "regional-officer"]);
    if (acceptorRoles.has(role) && state.assignment.primaryOfficerId && state.assignment.acceptedAssignmentVersion !== state.assignment.assignmentVersion) {
    ...
    const naccReportAction = (state.caseData?.decision === "62" || asObject(asObject(state.inquiry).intake).m62?.flag === true) && role === NACC_REPORT_ACTION.role
      ? Object.freeze({ ...NACC_REPORT_ACTION, enabled: true })
      : null;
    const downstreamActions = downstreamActionsFor(state, role);
    const canonicalReport213Active = String(state.workflow.downstreamStatus || "").startsWith("REPORT_213_")
      && Array.isArray(state.a5Report213Lifecycle?.submissions) && state.a5Report213Lifecycle.submissions.length > 0;
    if (canonicalReport213Active) return naccReportAction ? [...downstreamActions, naccReportAction] : downstreamActions;
    if (downstreamActions.length) {
      const amendmentAction = role === "investigator" && [PROCESS_STATES.PLAN_APPROVED, PROCESS_STATES.AMENDMENT_APPROVED].includes(state.workflow.a5Status)
        ...
        : null;
      return [...downstreamActions, ...(amendmentAction ? [amendmentAction] : []), ...(naccReportAction ? [naccReportAction] : [])];
    }
    ...
    if (naccReportAction) actions.push(naccReportAction);
    return actions;
  }

  function getA5InterventionActionsCore(sourceState, role) {
    const state = normalizeA5State(sourceState);
    if (state.migrationBlocked || state.caseAdministration.lockedByMerge || state.workflow.a5Status === PROCESS_STATES.COMPLETED || role !== "investigator") return [];
    if (state.assignment.primaryOfficerId && state.assignment.acceptedAssignmentVersion !== state.assignment.assignmentVersion) return [];
    const current = state.searchWarrantRequests.at(-1);
    if (current?.mock === true) return [];
    const actionId = !current || ["DENIED", "COMPLETED"].includes(current.status)
      ? state.workflow.stage === "a5-prelim" ? "search-warrant-create" : state.workflow.stage === "a5-inquiry" ? "search-warrant-mock-request" : ""
      : ({ DRAFT: "search-warrant-send-a9", SENT_TO_A9: "search-warrant-record-a9-receipt", RECEIVED_BY_A9: "search-warrant-record-court-result", ISSUED: "search-warrant-record-search" })[current.status] || "";
    return actionId ? [Object.freeze({ ...INTERVENTION_ACTIONS[actionId], primary: false, enabled: true })] : [];
  }

  function getA5AdminActionsCore(sourceState, role) {
    const state = normalizeA5State(sourceState);
    const actions = [];
    ...
    if (!state.caseAdministration.lockedByMerge && state.workflow.a5Status !== PROCESS_STATES.COMPLETED) {
      ...
      const panelStatus = String(state.panelChangeRequests.at(-1)?.status || "");
      if (role === "investigator" && !["SUBMITTED"].includes(panelStatus)) actions.push(Object.freeze({ ...ADMIN_ACTIONS[panelStatus === "DRAFT" ? "panel-change-submit" : "panel-change-draft"], enabled: true }));
      if (role === "clerk" && panelStatus === "SUBMITTED") actions.push(Object.freeze({ ...ADMIN_ACTIONS["panel-change-record-signed-order"], enabled: true }));
    }
    return actions;
  }

  function getA5AvailableActions(sourceState, role) {
    return filterRoleActionsA5(role, sourceState, getA5AvailableActionsCore(sourceState, permissionRoleA5(role)));
  }

  function getA5InterventionActions(sourceState, role) {
    return filterRoleActionsA5(role, sourceState, getA5InterventionActionsCore(sourceState, permissionRoleA5(role)));
  }

  function getA5AdminActions(sourceState, role) {
    return filterRoleActionsA5(role, sourceState, getA5AdminActionsCore(sourceState, permissionRoleA5(role)));
  }
  ...
  function executeA5Action(sourceState, role, actionId, sourcePayload) {
    if (!roleAllowsA5(role, actionId, sourceState)) return failure("ACTOR_MISMATCH", sourceState);
    ...
    if (phase0Guard) return phase0Guard.withAction(actionId, () => executeA5ActionCore(sourceState, permissionRole, actionId, sourcePayload));
    return executeA5ActionCore(sourceState, permissionRole, actionId, sourcePayload);
  }
  function executeA5ActionCore(sourceState, role, actionId, sourcePayload) {
    ...
    const incomingHandoverTask = ["prepare-644-handover-accept", "prepare-644-acknowledge"].includes(actionId) ? state.downstreamTasks.find(item => item.id === String(payload.taskId || "")) : null;
    const isIncomingHandoverAction = Boolean(incomingHandoverTask && actorOfficerId === String(incomingHandoverTask.pendingOrder?.investigator644Id || ""));
    const isAnyInvestigatorAction = actionId === "officer-accept";
    const acceptorRolesForExec = new Set(["investigator", "officer", "regional-officer"]);
    if (acceptorRolesForExec.has(role)) {
      if (!actorOfficerId) return failure("MISSING_REQUIRED_FIELD", sourceState, ["actorOfficerId"]);
      if (isAnyInvestigatorAction) {
        // อนุโลม: ศรร/officer หรือ นักสืบคนใดก็รับมอบแทนได้
      } else if (isIncomingHandoverAction) {
        // ผู้รับผิดชอบชั้น 644 ลงนามรับก่อนระบบเปิดงาน 644 และก่อนเปลี่ยน assignment หลัก
      } else if (isHandoffAction ? actorOfficerId !== String(state.pendingReassignment.fromOfficerId || "") : isCaseInvestigatorAction ? !caseInvestigatorIds.has(actorOfficerId) : (!assignedOfficer || actorOfficerId !== assignedOfficer)) return failure("ACTOR_MISMATCH", sourceState);
      if (!isCaseInvestigatorAction && !isHandoffAction && !isIncomingHandoverAction && actionId !== "officer-accept" && state.assignment.acceptedAssignmentVersion !== state.assignment.assignmentVersion) {
    ...
    if (INTERVENTION_ACTIONS[actionId]) return executeInterventionAction(state, role, actionId, payload, sourceState);
    ...
    const acceptorRoles = new Set(["investigator", "officer", "regional-officer"]);
    const isOfficerAccept = actionId === "officer-accept" && acceptorRoles.has(role);
    if (!isOfficerAccept && action.role !== role) return failure("ACTOR_MISMATCH", sourceState);
  ...
  function executeNaccReportAction(state, role, payload) {
    ...
    const required = ["letterNo", "reportDate", "summary", "reportDocumentVersionId", "naccReceiptEvidenceVersionId", "naccReceivedAt"];
    const missing = required.filter(field => !String(payload[field] || "").trim());
    if (missing.length) return failure("MISSING_REQUIRED_FIELD", state, missing);
    const at = String(payload.at || new Date().toISOString());
    const cycle = { sequence: state.naccReportCycles.length + 1, letterNo: String(payload.letterNo), reportDate: isoDate(payload.reportDate), summary: String(payload.summary), reportDocumentVersionId: String(payload.reportDocumentVersionId), naccReceiptEvidenceVersionId: String(payload.naccReceiptEvidenceVersionId), naccReceivedAt: isoDate(payload.naccReceivedAt), addedBy: actorName, addedAt: at };
    state.naccReportCycles.push(cycle);
    const section65Task = state.downstreamTasks.find(task => task.type === "REPORT_NACC_SECTION_65" && task.status !== "COMPLETED");
    if (section65Task) {
      section65Task.status = "COMPLETED";
      section65Task.completedAt = at;
      section65Task.completedBy = actorName;
      section65Task.letterNo = cycle.letterNo;
      section65Task.reportDate = cycle.reportDate;
      section65Task.reportDocumentVersionId = cycle.reportDocumentVersionId;
      section65Task.naccReceiptEvidenceVersionId = cycle.naccReceiptEvidenceVersionId;
      section65Task.naccReceivedAt = cycle.naccReceivedAt;
    }
  ...
  function executeInterventionAction(state, role, actionId, payload, sourceState) {
    const action = INTERVENTION_ACTIONS[actionId];
    if (!action || action.role !== role) return failure("ACTOR_MISMATCH", sourceState);
    const actorName = String(payload.actorName || "").trim();
    if (!actorName) return failure("MISSING_REQUIRED_FIELD", sourceState, ["actorName"]);
    const available = getA5InterventionActions(state, role).some(item => item.id === actionId);
    if (!available) return failure("INVALID_TRANSITION", sourceState);
    const at = String(payload.at || new Date().toISOString());
    if (actionId === "search-warrant-mock-request") {
      state.searchWarrantRequests.push({
        requestId: `SEARCH-MOCK-${state.caseData?.id || "CASE"}-${state.searchWarrantRequests.length + 1}`,
        documentType: "SEARCH_WARRANT_MOCK",
        status: "ISSUED",
        mock: true,
        createdStage: "a5-inquiry",
        inquiryDeadlineAtCreation: String(asObject(asObject(state.inquiry).inquiry644).deadlineAt || ""),
        createdBy: actorName,
        createdAt: at,
        mockResult: { result: "ISSUED", recordedAt: at },
        history: [{ actionId, by: actorName, at }]
      });
    } else if (actionId === "search-warrant-create") {
      const required = ["reason", "location", "targetEvidence", "supportingEvidence"];
      const missing = required.filter(field => !String(payload[field] || "").trim());
      if (missing.length) return failure("MISSING_REQUIRED_FIELD", sourceState, missing);
      state.searchWarrantRequests.push({
        requestId: `SEARCH-${state.caseData?.id || "CASE"}-${state.searchWarrantRequests.length + 1}`,
        documentType: "SEARCH_WARRANT_REQUEST",
        status: "DRAFT",
        reason: String(payload.reason),
        location: String(payload.location),
        targetEvidence: String(payload.targetEvidence),
        supportingEvidence: String(payload.supportingEvidence),
        preliminaryDeadlineAtCreation: String(asObject(asObject(state.inquiry).prelim).deadlineAt || ""),
        createdBy: actorName,
        createdAt: at,
        history: [{ actionId, by: actorName, at }]
      });
    } else if (actionId === "search-warrant-send-a9") {
      const ruleApi = root.ECMISActivity5Rules || (typeof require === "function" ? require("./activity5-rules.js") : null);
      return Object.freeze({ ok: false, code: "PENDING_CONFIRMATION", state: clone(sourceState), errors: Object.freeze([]), rule: ruleApi?.getA5Rule("search-warrant-activity-route") || null });
    } else if (actionId === "search-warrant-record-a9-receipt") {
      const required = ["receivedAt", "receivedBy", "receiptEvidence"];
      const missing = required.filter(field => !String(payload[field] || "").trim());
      if (missing.length) return failure("MISSING_REQUIRED_FIELD", sourceState, missing);
      const request = state.searchWarrantRequests.at(-1);
      const exchange = [...state.externalExchanges].reverse().find(item => item.type === "SEARCH_WARRANT_REQUEST" && item.requestId === request.requestId && item.status === "SENT");
      if (!exchange) return failure("MISSING_EXTERNAL_SEND", sourceState);
      request.status = "RECEIVED_BY_A9";
      request.activity9Receipt = { receivedAt: String(payload.receivedAt), receivedBy: String(payload.receivedBy), receiptEvidence: String(payload.receiptEvidence), recordedBy: actorName, recordedAt: at };
      request.history.push({ actionId, by: actorName, at });
      Object.assign(exchange, { status: "RECEIVED", receivedAt: String(payload.receivedAt), receivedBy: String(payload.receivedBy), receiptEvidence: String(payload.receiptEvidence) });
    } else if (actionId === "search-warrant-record-court-result") {
      const required = ["result", "court", "decidedAt", "orderReference", "resultEvidence"];
      const missing = required.filter(field => !String(payload[field] || "").trim());
      if (missing.length) return failure("MISSING_REQUIRED_FIELD", sourceState, missing);
      if (!["ISSUED", "DENIED"].includes(payload.result)) return failure("INVALID_COURT_RESULT", sourceState, ["result"]);
      const request = state.searchWarrantRequests.at(-1);
      request.status = String(payload.result);
      request.courtResult = { result: String(payload.result), court: String(payload.court), decidedAt: String(payload.decidedAt), orderReference: String(payload.orderReference), resultEvidence: String(payload.resultEvidence), recordedBy: actorName, recordedAt: at };
      request.history.push({ actionId, by: actorName, at });
      state.externalExchanges.push({ type: "SEARCH_WARRANT_COURT_RESULT", requestId: request.requestId, direction: "IN", status: "RECEIVED", receivedAt: String(payload.decidedAt), reference: String(payload.orderReference), evidence: String(payload.resultEvidence), by: actorName });
    } else if (actionId === "search-warrant-record-search") {
      const required = ["searchedAt", "officers", "resultSummary", "evidenceFound", "custodyHolder"];
      const missing = required.filter(field => !String(payload[field] || "").trim());
      if (missing.length) return failure("MISSING_REQUIRED_FIELD", sourceState, missing);
      const request = state.searchWarrantRequests.at(-1);
      const versionId = `${request.requestId}-FOUND-EVIDENCE-${Date.parse(at) || state.a5EvidenceRepository.length + 1}`;
      request.status = "COMPLETED";
      request.searchExecution = { searchedAt: String(payload.searchedAt), officers: String(payload.officers), resultSummary: String(payload.resultSummary), evidenceFound: String(payload.evidenceFound), custodyHolder: String(payload.custodyHolder), evidenceVersionId: versionId, recordedBy: actorName, recordedAt: at };
      request.history.push({ actionId, by: actorName, at });
      state.a5EvidenceRepository.push({ versionId, documentType: "SEARCH_FOUND_EVIDENCE", name: String(payload.evidenceFound), availability: "AVAILABLE", custodyHolder: String(payload.custodyHolder), source: "SEARCH_WARRANT_EXECUTION", sourceRequestId: request.requestId, createdAt: at, binaryPersisted: false });
    }
    state.decisionHistory.push(eventFor(action, actorName, at));
    return Object.freeze({ ok: true, code: "TRANSITIONED", state, errors: Object.freeze([]) });
  }
  ...
  function executeDownstreamActionCore(state, role, actionId, payload, sourceState = state) {
    ...
    if (!actorName) return failure("MISSING_REQUIRED_FIELD", state, ["actorName"]);
    const at = String(payload.at || new Date().toISOString());
    if (actionId.startsWith("nacc-referral-")) return executeNaccReferralAction(state, action, actionId, payload, actorName, at, sourceState);
  ...
  function executeNaccReferralAction(state, action, actionId, payload, actorName, at, sourceState) {
    if (actionId === "nacc-referral-start") {
      const records = Array.isArray(state.a5DocumentStore?.records) ? state.a5DocumentStore.records : [];
      const report213 = records
        .filter(record => record?.documentId === "FORM_4_REPORT_213")
        .sort((left, right) => Number(right.revisionNo || 0) - Number(left.revisionNo || 0))[0];
      if (!report213 || report213.status !== "DRAFT") return failure("UNFINISHED_REPORT_213_REQUIRED", sourceState, ["FORM_4_REPORT_213"]);
      state.naccReferral = {
        status: "DRAFT",
        startedBy: actorName,
        startedAt: at,
        ...
        report213Attachment: {
          documentId: report213.documentId,
          revisionNo: Number(report213.revisionNo || 0),
          sourceStatus: report213.status,
          payloadSnapshot: clone(report213.payload),
          attachedAt: at
        },
        reviewHistory: []
      };
      state.workflow.downstreamStatus = "NACC_REFERRAL_DRAFT";
    } else if (actionId === "nacc-referral-submit") {
      const required = ["summary", "finding", "reason"];
      const missing = required.filter(field => !String(payload[field] || "").trim());
      if (missing.length) return failure("MISSING_REQUIRED_FIELD", sourceState, missing);
      if (payload.finding !== "NACC_JURISDICTION") return failure("INVALID_FINDING", sourceState, ["finding"]);
      if (!state.naccReferral.report213Attachment?.payloadSnapshot) return failure("UNFINISHED_REPORT_213_REQUIRED", sourceState, ["FORM_4_REPORT_213"]);
      state.naccReferral.status = "GROUP_REVIEW";
      state.naccReferral.investigationReport = {
      ...
      state.workflow.downstreamStatus = "NACC_REFERRAL_GROUP_REVIEW";
    } else if (actionId === "nacc-referral-group-approve") {
      if (!String(payload.opinion || "").trim()) return failure("MISSING_REQUIRED_FIELD", sourceState, ["opinion"]);
      state.naccReferral.status = "DIRECTOR_REVIEW";
      state.naccReferral.reviewHistory.push({ level: "GROUP_DIRECTOR", decision: "APPROVED", opinion: String(payload.opinion), by: actorName, at });
      state.workflow.downstreamStatus = "NACC_REFERRAL_DIRECTOR_REVIEW";
    } else if (actionId === "nacc-referral-group-return") {
      if (!String(payload.reason || "").trim()) return failure("MISSING_REQUIRED_FIELD", sourceState, ["reason"]);
      state.naccReferral.status = "RETURNED";
      state.naccReferral.investigationReport.status = "RETURNED";
      state.naccReferral.reviewHistory.push({ level: "GROUP_DIRECTOR", decision: "RETURNED", reason: String(payload.reason), by: actorName, at });
      state.workflow.downstreamStatus = "NACC_REFERRAL_RETURNED";
    } else if (actionId === "nacc-referral-group-skip") {
      if (!String(payload.reason || "").trim()) return failure("MISSING_REQUIRED_FIELD", sourceState, ["reason"]);
      state.naccReferral.status = "DIRECTOR_REVIEW";
      state.naccReferral.reviewHistory.push({ level: "GROUP_DIRECTOR", decision: "SKIPPED", reason: String(payload.reason), by: actorName, at });
      state.workflow.downstreamStatus = "NACC_REFERRAL_DIRECTOR_REVIEW";
    } else if (actionId === "nacc-referral-director-approve") {
      if (!String(payload.opinion || "").trim()) return failure("MISSING_REQUIRED_FIELD", sourceState, ["opinion"]);
      state.naccReferral.status = "GBK_PENDING";
      state.naccReferral.reviewHistory.push({ level: "UNIT_DIRECTOR", decision: "APPROVED", opinion: String(payload.opinion), by: actorName, at });
      state.workflow.downstreamStatus = "NACC_REFERRAL_GBK_PENDING";
    } else if (actionId === "nacc-referral-director-return") {
      if (!String(payload.reason || "").trim()) return failure("MISSING_REQUIRED_FIELD", sourceState, ["reason"]);
      state.naccReferral.status = "RETURNED";
      state.naccReferral.investigationReport.status = "RETURNED";
      state.naccReferral.reviewHistory.push({ level: "UNIT_DIRECTOR", decision: "RETURNED", reason: String(payload.reason), by: actorName, at });
      state.workflow.downstreamStatus = "NACC_REFERRAL_RETURNED";
    } else if (actionId === "nacc-referral-gbk-receive") {
      const required = ["receivedAt", "packageReference"];
      const missing = required.filter(field => !String(payload[field] || "").trim());
      if (missing.length) return failure("MISSING_REQUIRED_FIELD", sourceState, missing);
      state.naccReferral.status = "BOARD_PENDING";
      state.naccReferral.gbkReceipt = { receivedAt: String(payload.receivedAt), packageReference: String(payload.packageReference), receivedBy: actorName, recordedAt: at };
      state.workflow.downstreamStatus = "NACC_REFERRAL_BOARD_PENDING";
    } else if (actionId === "nacc-referral-board-record-result") {
      const required = ["result", "mtiNo", "decidedAt"];
      const missing = required.filter(field => !String(payload[field] || "").trim());
      if (missing.length) return failure("MISSING_REQUIRED_FIELD", sourceState, missing);
      if (!["SEND_NACC", "END_NO_WRONGDOING"].includes(payload.result)) return failure("INVALID_BOARD_RESULT", sourceState, ["result"]);
      state.naccReferral.status = "RESULT_RECEIVED";
      state.naccReferral.boardResult = { result: String(payload.result), mtiNo: String(payload.mtiNo), decidedAt: String(payload.decidedAt), recordedBy: actorName, recordedAt: at };
      const taskType = payload.result === "SEND_NACC" ? "SEND_NACC" : "NOTIFY_DECISION";
      state.downstreamTasks.push({ id: `NACC-REFERRAL-${Date.parse(at) || 0}`, type: taskType, status: "PENDING", createdAt: at });
      state.workflow.stage = "a5-outcome";
      state.workflow.owner = "clerk";
      state.workflow.downstreamStatus = "NACC_REFERRAL_RESULT_RECEIVED";
      state.publicStatusEvents.push({ code: "A5_NACC_REFERRAL_RESULT_RECEIVED", queuedAt: at, status: "PENDING_WORDING" });
    } else return failure("INVALID_TRANSITION", sourceState);
    state.decisionHistory.push(eventFor(action, actorName, at));
    return Object.freeze({ ok: true, code: actionId === "nacc-referral-start" ? "NACC_REFERRAL_STARTED" : "TRANSITIONED", state, errors: Object.freeze([]) });
  }

  function completePrepare644Start(state, task, payload, actorName, at) {
    const required = ["orderNo", "orderSignedAt", "orderDocumentVersionId", "orderSignatureMethod", "investigator644Id", "investigator644Name"];
    const missing = required.filter(field => !String(payload[field] || "").trim());
    const authority = asObject(payload.orderSignerAuthority);
    const orderType = String(task.orderType || "");
    if (orderType === "24v3" && authority.authorityType !== "BOARD_CHAIR") missing.push("orderSignerAuthority.boardChair");
    if (orderType === "24v3" && (authority.status !== "CONFIRMED" || !String(authority.referenceNo || "").trim())) missing.push("orderSignerAuthority");
    if (missing.length) return failure("MISSING_REQUIRED_FIELD", state, [...new Set(missing)]);
    if (!/^\d{4}-\d{2}-\d{2}$/.test(String(payload.orderSignedAt))) return failure("INVALID_DATE", state, ["orderSignedAt"]);

    const investigator213Id = String(state.assignment?.primaryOfficerId || "").trim();
    const investigator644Id = String(payload.investigator644Id).trim();
    const needsHandover = Boolean(investigator213Id && investigator644Id !== investigator213Id);
    if (needsHandover) {
      const handoverFields = ["handoverLetterNo", "handoverSentAt", "handoverAcceptedAt", "handoverAcceptedBy", "handoverEvidenceVersionId"];
      const missingHandover = handoverFields.filter(field => !String(payload[field] || "").trim());
      if (missingHandover.length) return failure("HANDOVER_REQUIRED", state, missingHandover);
      if (!/^\d{4}-\d{2}-\d{2}$/.test(String(payload.handoverSentAt)) || !/^\d{4}-\d{2}-\d{2}$/.test(String(payload.handoverAcceptedAt))) return failure("INVALID_DATE", state, ["handoverSentAt", "handoverAcceptedAt"]);
      if (String(payload.handoverAcceptedAt) < String(payload.handoverSentAt)) return failure("INVALID_CHRONOLOGY", state, ["handoverAcceptedAt"]);
    }

    const lifecycleResults = Array.isArray(state.a5Report213Lifecycle?.results) ? state.a5Report213Lifecycle.results : [];
    const result = [...lifecycleResults].reverse().find(item => item.resolutionReference === task.resolutionReference) || lifecycleResults.at(-1);
    const startedAt = orderType === "24v3" ? isoDate(result?.decidedAt) : isoDate(payload.orderSignedAt);
    if (!startedAt) return failure("MISSING_REQUIRED_FIELD", state, [orderType === "24v3" ? "boardDecidedAt" : "orderSignedAt"]);
    if (isoDate(result?.decidedAt) && isoDate(payload.orderSignedAt) < isoDate(result.decidedAt)) return failure("INVALID_CHRONOLOGY", state, ["orderSignedAt"]);
    const deadlineAt = addCalendarDays(startedAt, 270);
    if (!deadlineAt) return failure("INVALID_DATE", state, [orderType === "24v3" ? "boardDecidedAt" : "orderSignedAt"]);

    state.inquiry = asObject(state.inquiry);
    state.inquiry.committee213 = asObject(state.inquiry.committee213);
    state.inquiry.inquiry644 = asObject(state.inquiry.inquiry644);
    state.inquiry.committee213.orderType = orderType;
    state.inquiry.committee213.orderNo = String(payload.orderNo);
    state.inquiry.committee213.orderDate = isoDate(payload.orderSignedAt);
    state.inquiry.committee213.orderDocumentVersionId = String(payload.orderDocumentVersionId);
    state.inquiry.committee213.orderSignature = { methodLabel: String(payload.orderSignatureMethod), authority: clone(authority), signedAt: isoDate(payload.orderSignedAt) };
    state.inquiry.committee213.investigator644 = String(payload.investigator644Name);
    state.inquiry.committee213.handoverDoc = needsHandover ? {
      required: true,
      letterNo: String(payload.handoverLetterNo),
      sentAt: isoDate(payload.handoverSentAt),
      acceptedAt: isoDate(payload.handoverAcceptedAt),
      acceptedBy: String(payload.handoverAcceptedBy),
      evidenceVersionId: String(payload.handoverEvidenceVersionId)
    } : { required: false };
    state.inquiry.inquiry644.investigatorId = investigator644Id;
    state.inquiry.inquiry644.investigator = String(payload.investigator644Name);
    state.inquiry.inquiry644.startedAt = startedAt;
    state.inquiry.inquiry644.deadlineAt = deadlineAt;
    task.status = "COMPLETED";
    task.completedAt = at;
    task.completedBy = actorName;
    task.orderDocumentVersionId = String(payload.orderDocumentVersionId);
    state.workflow.stage = "a5-inquiry";
    state.workflow.owner = "investigator";
    state.workflow.downstreamStatus = "REPORT_644_DRAFT";
    state.requiredMilestones = Array.isArray(state.requiredMilestones) ? state.requiredMilestones : [];
    if (!state.requiredMilestones.includes("REPORT_644_RESULT_RECEIVED")) state.requiredMilestones.push("REPORT_644_RESULT_RECEIVED");
    return null;
  }

  function executeDownstreamOperation(state, action, role, actionId, payload, actorName, at) {
    const requiredByAction = {
      "outcome-task-start": ["taskId"], "outcome-task-complete": ["taskId"],
      "prepare-644-order-sign": ["taskId"], "prepare-644-order-record": ["taskId"],
      "prepare-644-notify": ["taskId", "notifiedAt", "notificationChannel", "notificationDocumentVersionId"],
      "prepare-644-acknowledge": ["taskId"],
      "prepare-644-handover-send": ["taskId", "handoverLetterNo", "handoverSentAt", "handoverEvidenceVersionId"],
      "prepare-644-handover-accept": ["taskId", "handoverAcceptedAt"],
      "outcome-task-send": ["taskId", "letterNo", "sentAt", "destination", "dispatchDocumentVersionId", "deliveryMethod"], "outcome-task-record-receipt": ["taskId", "receivedAt", "evidence"],
    ...
    if (["outcome-task-start", "outcome-task-complete", "outcome-task-send", "outcome-task-record-receipt", "prepare-644-order-sign", "prepare-644-order-record", "prepare-644-notify", "prepare-644-acknowledge", "prepare-644-handover-send", "prepare-644-handover-accept"].includes(actionId)) {
      ...
      if (actionId === "outcome-task-start") {
      ...
      } else if (["prepare-644-order-sign", "prepare-644-order-record"].includes(actionId)) {
        if (task.status !== "IN_PROGRESS" || task.type !== "PREPARE_644_START") return failure("INVALID_TRANSITION", state);
        if (actionId === "prepare-644-order-sign" && task.orderType !== "24v1") return failure("ACTOR_MISMATCH", state);
        if (actionId === "prepare-644-order-record" && task.orderType !== "24v3") return failure("ACTOR_MISMATCH", state);
        const directoryEntry = state.staffDirectory.find(entry => entry.officerId === String(payload.investigator644Id || "") && entry.status === "ACTIVE");
        if (!directoryEntry) return failure("STAFF_DIRECTORY_ENTRY_REQUIRED", state, ["investigator644Id"]);
        if (actionId === "prepare-644-order-record" && payload.appointmentOfficerConfirmed !== true) return failure("APPOINTMENT_OFFICER_CONFIRMATION_REQUIRED", state, ["appointmentOfficerConfirmed"]);
        const orderPayload = { ...payload, investigator644Name: directoryEntry.officerName, appointmentOfficerSnapshot: clone(directoryEntry) };
        const probeState = clone(state);
        const probeTask = probeState.downstreamTasks.find(item => item.id === task.id);
        const validation = completePrepare644Start(probeState, probeTask, orderPayload, actorName, at);
        if (validation && validation.code !== "HANDOVER_REQUIRED") return validation;
        task.pendingOrder = clone(orderPayload);
        state.appointmentOrders.primary = { orderNo: String(orderPayload.orderNo), orderSignedAt: String(orderPayload.orderSignedAt), orderDocumentVersionId: String(orderPayload.orderDocumentVersionId), orderType: String(task.orderType), authorityRef: clone(orderPayload.orderSignerAuthority), investigator644Id: directoryEntry.officerId, investigator644Name: directoryEntry.officerName, recordedBy: actorName, recordedAt: at };
        task.status = "AWAITING_NOTIFICATION";
        task.orderRecordedAt = at;
        task.orderRecordedBy = actorName;
        state.workflow.downstreamStatus = "OUTCOME_TASKS_IN_PROGRESS";
      } else if (actionId === "prepare-644-notify") {
        if (task.status !== "AWAITING_NOTIFICATION" || task.type !== "PREPARE_644_START") return failure("INVALID_TRANSITION", state);
        if (!/^\d{4}-\d{2}-\d{2}$/.test(String(payload.notifiedAt))) return failure("INVALID_DATE", state, ["notifiedAt"]);
        task.notification = { notifiedAt: String(payload.notifiedAt), channel: String(payload.notificationChannel), documentVersionId: String(payload.notificationDocumentVersionId), notifiedBy: actorName, recordedAt: at, recipientOfficerId: String(task.pendingOrder?.investigator644Id || "") };
        task.status = "AWAITING_ACKNOWLEDGEMENT";
      } else if (actionId === "prepare-644-acknowledge") {
        if (task.status !== "AWAITING_ACKNOWLEDGEMENT" || task.type !== "PREPARE_644_START" || payload.acknowledged !== true || String(payload.actorOfficerId || "") !== String(task.pendingOrder?.investigator644Id || "")) return failure("ACTOR_MISMATCH", state);
        task.notification.acknowledgedAt = at;
        task.notification.acknowledgedBy = actorName;
        task.notification.acknowledgedByOfficerId = String(payload.actorOfficerId);
        if (String(task.pendingOrder.investigator644Id) === String(state.assignment?.primaryOfficerId || "")) {
          const completed = completePrepare644Start(state, task, task.pendingOrder, actorName, at);
          if (completed) return completed;
          delete task.pendingOrder;
        } else {
          task.status = "AWAITING_HANDOVER_SEND";
        }
      } else if (actionId === "prepare-644-handover-send") {
        if (task.status !== "AWAITING_HANDOVER_SEND" || task.type !== "PREPARE_644_START" || String(payload.actorOfficerId || "") !== String(state.assignment?.primaryOfficerId || "")) return failure("ACTOR_MISMATCH", state);
        if (!/^\d{4}-\d{2}-\d{2}$/.test(String(payload.handoverSentAt))) return failure("INVALID_DATE", state, ["handoverSentAt"]);
        task.handover = { letterNo: String(payload.handoverLetterNo), sentAt: String(payload.handoverSentAt), evidenceVersionId: String(payload.handoverEvidenceVersionId), sentBy: actorName, sentAtRecorded: at };
        task.status = "AWAITING_HANDOVER_ACCEPT";
      } else if (actionId === "prepare-644-handover-accept") {
        if (task.status !== "AWAITING_HANDOVER_ACCEPT" || task.type !== "PREPARE_644_START" || String(payload.actorOfficerId || "") !== String(task.pendingOrder?.investigator644Id || "")) return failure("ACTOR_MISMATCH", state);
        const completed = completePrepare644Start(state, task, {
          ...task.pendingOrder,
          handoverLetterNo: task.handover.letterNo,
          handoverSentAt: task.handover.sentAt,
          handoverAcceptedAt: payload.handoverAcceptedAt,
          handoverAcceptedBy: actorName,
          handoverEvidenceVersionId: task.handover.evidenceVersionId
        }, actorName, at);
        if (completed) return completed;
        delete task.pendingOrder;
        delete task.handover;
        ...
        if (task.type === "PREPARE_644_START") return failure("ACTOR_MISMATCH", state);
        else {
          if (!String(payload.evidence || "").trim()) return failure("MISSING_REQUIRED_FIELD", state, ["evidence"]);
        ...
        state.externalExchanges.push({ type: task.type, taskId: task.id, direction: "OUT", status: "SENT", destination: String(payload.destination), letterNo: String(payload.letterNo), dispatchDocumentVersionId: String(payload.dispatchDocumentVersionId), deliveryMethod: String(payload.deliveryMethod), sentAt: String(payload.sentAt), by: actorName });
  ...
  function executeAdminActionCore(state, role, actionId, payload) {
    ...
    const requiredByAction = {
      ...
      "panel-change-draft": ["reason", "reportDocumentVersionId", "proposedMembers"],
      "panel-change-record-signed-order": ["orderNo", "orderSignedAt", "orderDocumentVersionId", "parentOrderReference"],
    ...
    if (actionId === "panel-change-draft") {
      const memberIds = [...new Set(payload.proposedMembers.map(String).map(value => value.trim()).filter(Boolean))];
      const proposedPanel = memberIds.map(officerId => state.staffDirectory.find(entry => entry.officerId === officerId && entry.status === "ACTIVE")).filter(Boolean);
      if (proposedPanel.length !== memberIds.length) return failure("STAFF_DIRECTORY_ENTRY_REQUIRED", state, ["proposedMembers"]);
      const currentPanel = Array.isArray(asObject(asObject(state.inquiry).committee213).panel) ? state.inquiry.committee213.panel.map(item => clone(item)) : [];
      state.panelChangeRequests.push({ requestId: `panel-change-${Date.parse(at) || state.panelChangeRequests.length + 1}`, status: "DRAFT", reason: String(payload.reason), reportDocumentVersionId: String(payload.reportDocumentVersionId), currentPanel, proposedMembers: proposedPanel.map(entry => ({ ...clone(entry), panelRole: "MEMBER" })), draftedBy: actorName, draftedAt: at });
    }
    ...
    if (actionId === "panel-change-record-signed-order") {
      const request = state.panelChangeRequests.at(-1);
      const authority = asObject(payload.authorityRef);
      if (!request || request.status !== "SUBMITTED") return failure("INVALID_TRANSITION", state);
      if (!String(state.appointmentOrders.primary?.orderNo || "").trim() || String(payload.parentOrderReference) !== String(state.appointmentOrders.primary.orderNo)) return failure("PARENT_ORDER_MISMATCH", state, ["parentOrderReference"]);
      if (authority.status !== "CONFIRMED" || !String(authority.referenceNo || "").trim()) return failure("PENDING_CONFIRMATION", state, ["authorityRef"]);
      if (!/^\d{4}-\d{2}-\d{2}$/.test(String(payload.orderSignedAt))) return failure("INVALID_DATE", state, ["orderSignedAt"]);
      const signedOrder = { orderType: "PANEL_CHANGE_SUPPLEMENT", orderNo: String(payload.orderNo), orderSignedAt: String(payload.orderSignedAt), orderDocumentVersionId: String(payload.orderDocumentVersionId), parentOrderReference: String(payload.parentOrderReference), authorityRef: clone(authority), recordedBy: actorName, recordedAt: at, requestId: request.requestId };
      state.appointmentOrders.supplements.push(signedOrder);
      request.status = "RECORDED";
      request.signedOrder = clone(signedOrder);
      request.recordedBy = actorName;
      request.recordedAt = at;
      state.inquiry = asObject(state.inquiry);
      state.inquiry.committee213 = asObject(state.inquiry.committee213);
      state.inquiry.committee213.panel = request.proposedMembers.map(item => clone(item));
    }
    if (actionId === "case-size-set") {
      ...
      if (size === "XL" && !String(payload.reason || "").trim()) return failure("MISSING_REQUIRED_FIELD", state, ["reason"]);
      const hasFacts = Object.prototype.hasOwnProperty.call(payload, "caseSizeFacts");
      if (hasFacts) {
        const invalidFacts = invalidCaseSizeFactFieldsA5(payload.caseSizeFacts);
        if (invalidFacts.length) return failure("MISSING_REQUIRED_FIELD", state, invalidFacts);
      }
      const scored = hasFacts ? assessCaseSizeA5(payload.caseSizeFacts) : computeCaseSizeA5(asObject(payload.caseSizeComponents));
      if (hasFacts) state.caseAdministration.caseSizeFacts = scored.facts;
      else delete state.caseAdministration.caseSizeFacts;
      ...
      state.caseAdministration.caseValueAmount = hasFacts ? scored.facts.caseValueAmount : Number(payload.caseValueAmount) || 0;
      if (size === "XL") {
        const reason = String(payload.reason || "").trim();
        ...
        state.caseAdministration.caseSize = ["S", "M", "L"].includes(size) && hasFacts ? scored.suggestion : size;
  ...
  function invalidCaseSizeFactFieldsA5(rawFacts) {
    const facts = asObject(rawFacts);
    const scoreFields = ["positionScore", "impactScore", "evidenceComplexityScore"];
    const countFields = ["accusedCount", "allegationCount", "witnessCount"];
    const amountFields = ["caseValueAmount"];
    const requiredFields = [...scoreFields, ...countFields, ...amountFields];
    const isNumericInput = value => (typeof value === "number" || typeof value === "string" && value.trim() !== "") && Number.isFinite(Number(value));
    return requiredFields.filter(field => {
      if (!Object.prototype.hasOwnProperty.call(facts, field) || !isNumericInput(facts[field])) return true;
      const value = Number(facts[field]);
      if (scoreFields.includes(field)) return !Number.isInteger(value) || value < 1 || value > 4;
      if (countFields.includes(field)) return !Number.isInteger(value) || value < 0;
      return value < 0;
    });
  }
  function assessCaseSizeA5(rawFacts) {
    const source = asObject(rawFacts);
    const clampScore = value => Math.max(1, Math.min(4, Math.round(Number(value)) || 1));
    const normalizeCount = value => Math.max(0, Math.round(Number(value)) || 0);
    const normalizeAmount = value => Math.max(0, Number(value) || 0);
    const facts = {
      positionScore: clampScore(source.positionScore),
      accusedCount: normalizeCount(source.accusedCount),
      allegationCount: normalizeCount(source.allegationCount),
      caseValueAmount: normalizeAmount(source.caseValueAmount),
      impactScore: clampScore(source.impactScore),
      witnessCount: normalizeCount(source.witnessCount),
      evidenceComplexityScore: clampScore(source.evidenceComplexityScore)
    };
    const accusedScore = facts.accusedCount >= 7 ? 4 : facts.accusedCount >= 5 ? 3 : facts.accusedCount >= 3 ? 2 : 1;
    const allegationScore = facts.allegationCount >= 9 ? 4 : facts.allegationCount >= 7 ? 3 : facts.allegationCount >= 5 ? 2 : 1;
    const amountScore = facts.caseValueAmount > 1000000 ? 4 : facts.caseValueAmount > 500000 ? 3 : facts.caseValueAmount > 200000 ? 2 : 1;
    const witnessScore = facts.witnessCount >= 61 ? 4 : facts.witnessCount >= 31 ? 3 : facts.witnessCount >= 11 ? 2 : 1;
    const assessed = computeCaseSizeA5({
      position: facts.positionScore,
      personsOrAllegations: Math.max(accusedScore, allegationScore),
      budgetOrDamage: Math.max(amountScore, facts.impactScore),
      evidenceDifficulty: Math.max(witnessScore, facts.evidenceComplexityScore)
    });
    return { facts, ...assessed };
  }
  ...
  function getA5DeadlineAlert(sourceState, todayValue) {
    ...
    const approvedExtensions = Array.isArray(report.extensionHistory)
      ? report.extensionHistory.filter(item => item?.status === "APPROVED" && Number.isInteger(Number(item.approvedDays || item.requestedDays)))
      : [];
    const latestExtension = approvedExtensions.at(-1);
    const latestApprovedDays = Number(latestExtension?.approvedDays || latestExtension?.requestedDays || 0);
    const derivedExtensionStart = latestApprovedDays > 0 && isoDate(report.deadlineAt)
      ? new Date(Date.parse(`${report.deadlineAt}T00:00:00Z`) - latestApprovedDays * 86400000).toISOString().slice(0, 10)
      : "";
    const startedAt = isoDate(latestExtension?.previousDeadline || derivedExtensionStart || report.startedAt);
    ...
    if ([15, 30, 45].includes(elapsedDays)) return Object.freeze({ level: "warning", elapsedDays, remainingDays, reminderDay: elapsedDays, label: `แจ้งเตือนวันที่ ${elapsedDays} · เหลือ ${remainingDays} วัน` });
    if (remainingDays <= 15) return Object.freeze({ level: "warning", elapsedDays, remainingDays, label: `เหลือ ${remainingDays} วัน` });
