(() => {
  ...
  const REPORT_213_REVIEW_STEPS = Object.freeze({
  ...
  const REPORT_213_ROUTES = Object.freeze({ NORMAL: "NORMAL", SUPPORT_COMMITTEE: "SUPPORT_COMMITTEE", URGENT: "URGENT" });
  const REVIEW_STEP_DEFAULTS = Object.freeze({
    LINE_SUPERVISOR: Object.freeze({ roleCode: "group-director", roleLabel: "ผู้บังคับบัญชาชั้นต้น", optional: true }),
    UNIT_DIRECTOR: Object.freeze({ roleCode: "director", roleLabel: "ผอ.หน่วยงาน", optional: false }),
    SUPERVISING_EXECUTIVE: Object.freeze({ roleCode: "executive", roleLabel: "ผู้ช่วย/รองเลขาธิการตามสายกำกับ", optional: false }),
    SECRETARY_GENERAL: Object.freeze({ roleCode: "secretary", roleLabel: "เลขาธิการ ป.ป.ท.", optional: false })
  });
  ...
  function normalizeReport213A5(sourceState) {
    ...
    const lifecycle = object(state.a5Report213Lifecycle);
    if (lifecycle.status === "REPORT_213_SCREENING_PENDING" && lifecycle.route === REPORT_213_ROUTES.NORMAL) {
      const submissions = Array.isArray(lifecycle.submissions) ? lifecycle.submissions : [];
      const submission = submissions.at(-1);
      const boardPackages = Array.isArray(lifecycle.boardPackages) ? lifecycle.boardPackages : [];
      if (submission && !boardPackages.some(item => item.submissionPackageId === submission.packageId)) {
        const signatures = Array.isArray(lifecycle.signatures) ? lifecycle.signatures : [];
        const opinions = Array.isArray(lifecycle.reviewOpinions) ? lifecycle.reviewOpinions : [];
        const signature = [...signatures].reverse().find(item => item.submissionPackageId === submission.packageId && ["SECRETARY_GENERAL", "SECRETARY_SUPPORT_CONFIRMATION"].includes(item.signatureType));
        boardPackages.push({ packageId: `board-${submission.packageId}`, submissionPackageId: submission.packageId, signedReportRef: { documentId: FORM_ID, revisionNo: submission.report?.revisionNo, signature: signature ? copy(signature) : null }, boardCoverRef: { documentId: "MOCK_GBK_BOARD_HANDOFF", revisionNo: 1 }, reviewOpinionIds: opinions.filter(item => item.reportRevisionNo === submission.report?.revisionNo).map(item => item.opinionId), attachmentVersionIds: [], createdAt: "" });
      }
      lifecycle.boardPackages = boardPackages;
      lifecycle.status = "REPORT_213_WAIT_RESULT";
      state.a5Report213Lifecycle = lifecycle;
      state.workflow = { ...object(state.workflow), stage: "a7-213", owner: "case-clerk", downstreamStatus: lifecycle.status };
    }
  ...
  function validateReport213A5(input, context = {}) {
    ...
    dateFields.forEach(path => { if (!isDate(getPath(payload, path))) err(errors, path, "กรุณาเลือกวันที่จากปฏิทิน", "INVALID_DATE"); });
    ...
    (payload.allegations || []).forEach((row, index) => ["eventDateFrom", "eventDateTo"].forEach(key => { if (!isDate(row[key])) err(errors, `allegations.${index}.${key}`, "กรุณาเลือกวันที่เกิดเหตุจากปฏิทิน", "INVALID_DATE"); }));
    (payload.factFindings?.chronologyRows || []).forEach((row, index) => { if (!isDate(row.occurredAt)) err(errors, `factFindings.chronologyRows.${index}.occurredAt`, "กรุณาเลือกวันที่ข้อเท็จจริงจากปฏิทิน", "INVALID_DATE"); });
    (payload.limitation?.offenceRows || []).forEach((row, index) => ["startAt", "expiresAt"].forEach(key => { if (!isDate(row[key])) err(errors, `limitation.offenceRows.${index}.${key}`, "กรุณาเลือกวันที่อายุความจากปฏิทิน", "INVALID_DATE"); }));
  ...
  function buildReport213SubmissionA5(sourceState, command) {
    ...
    const payload = object(record.payload);
    const links = object(object(payload.documentMeta).sourceLinks);
    const factFindings = object(payload.factFindings);
    const worklogEntryIds = Array.isArray(factFindings.worklogEntryIds) ? copy(factFindings.worklogEntryIds) : [];
    const evidenceVersionIds = Array.isArray(payload.evidence) ? payload.evidence.map(row => text(row?.documentVersionId)).filter(Boolean) : [];
    return {
      ok: true,
      ...
      submissionCandidate: {
        ...
        plan: copy(object(links.plan)),
        worklog: { ...copy(object(links.worklog)), includedEntryIds: worklogEntryIds },
        evidenceVersionIds
      }
    };
  }
  ...
  const REPORT_PLACEHOLDERS = Object.freeze({
    "documentMeta.preparedAt": "YYYY-MM-DD",
    "receipt.sourceReference": "เช่น เลขรับเรื่องหรือเลขอ้างอิงจากระบบต้นทาง",
    "receipt.sourceProvince": "เช่น นนทบุรี",
    "receipt.sourceResolutionNo": "เช่น มติที่ 12/2569 หรือหนังสือที่ ปป 001/2569",
    "receipt.sourceResolutionAt": "YYYY-MM-DD",
    "receipt.divisionReceivedAt": "YYYY-MM-DD",
    "receipt.assignedAt": "YYYY-MM-DD",
    "factFindings.summary": "สรุปข้อเท็จจริงที่ตรวจพบจากพยานหลักฐานโดยเรียงลำดับเหตุการณ์",
    "damage.description": "อธิบายลักษณะความเสียหายและผู้ได้รับผลกระทบ",
    "damage.amount": "เช่น 250000",
    "damage.currency": "เช่น THB",
    "damage.injuredParties": "ระบุบุคคล หน่วยงาน หรือรัฐที่ได้รับความเสียหาย",
    "damage.calculationNote": "อธิบายที่มาและวิธีคำนวณมูลค่าความเสียหาย",
    "legalBasis.authorityAnalysis": "วิเคราะห์ว่าเรื่องอยู่ในอำนาจหน้าที่ของหน่วยงานอย่างไร",
    "limitation.overallStatus": "สรุปว่าแต่ละข้อกล่าวหายังอยู่ในอายุความหรือไม่",
    "limitation.analysis": "อธิบายวันเริ่มนับ ระยะเวลา และวันครบอายุความ",
    "witnessProtection.summary": "สรุปเหตุความเสี่ยงและผลการพิจารณาคุ้มครองพยาน",
    "analysis.overallAnalysis": "วิเคราะห์ข้อเท็จจริง พยานหลักฐาน และข้อกฎหมายโดยรวม",
    "opinion.summaryNote": "สรุปสาระสำคัญจากข้อเท็จจริงและการวิเคราะห์",
    "opinion.finding": "ระบุความเห็นว่าควรรับไว้ไต่สวนหรือดำเนินการอย่างไร",
    "opinion.rationale": "อธิบายเหตุผลและพยานหลักฐานที่รองรับความเห็น",
    "proposal.reason": "ระบุเหตุผลประกอบข้อเสนอที่เลือก"
  });
  const FIELD_PLACEHOLDERS = Object.freeze({
    name: "ระบุชื่อและนามสกุลหรือชื่อหน่วยงาน", address: "ระบุที่อยู่ที่ติดต่อได้", contact: "เช่น โทรศัพท์ อีเมล หรือช่องทางติดต่อ", capacity: "เช่น ผู้ร้อง ผู้เสียหาย หรือผู้แทน",
    position: "ระบุตำแหน่งขณะเกิดเหตุ", agency: "ระบุหน่วยงานหรือสังกัด", statusAtEvent: "ระบุสถานะราชการหรือการดำรงตำแหน่งขณะเกิดเหตุ",
    summary: "สรุปพฤติการณ์หรือข้อกล่าวหาโดยย่อ", eventDateFrom: "YYYY-MM-DD", eventDateTo: "YYYY-MM-DD", place: "ระบุสถานที่เกิดเหตุ",
    occurredAt: "YYYY-MM-DD", fact: "ระบุข้อเท็จจริงที่เกิดขึ้นในลำดับเหตุการณ์", background: "อธิบายความเป็นมาของเรื่อง", method: "อธิบายวิธีการหรือพฤติการณ์ที่ตรวจพบ",
    participants: "ระบุบุคคลหรือหน่วยงานที่เกี่ยวข้อง", location: "ระบุสถานที่ที่เกี่ยวข้อง", period: "ระบุช่วงเวลาที่เกิดเหตุ",
    category: "เช่น เอกสาร บุคคล วัตถุ หรือข้อมูลอิเล็กทรอนิกส์", title: "ระบุชื่อพยานหลักฐาน", factSupported: "ระบุว่าหลักฐานนี้สนับสนุนข้อเท็จจริงใด", custodyNote: "ระบุแหล่งที่มา ผู้ครอบครอง และการเก็บรักษา",
    lawName: "ระบุชื่อกฎหมาย ระเบียบ หรือข้อบังคับ", section: "เช่น มาตรา 157", applicationReason: "อธิบายเหตุผลที่ข้อกฎหมายนี้เกี่ยวข้องกับข้อเท็จจริง",
    startAt: "YYYY-MM-DD", expiresAt: "YYYY-MM-DD", source: "ระบุฐานกฎหมายหรือเอกสารที่ใช้คำนวณ", note: "ระบุหมายเหตุเพิ่มเติม",
    nameOrCode: "ระบุชื่อหรือรหัสพยาน", measure: "ระบุมาตรการคุ้มครองที่ขอ", reason: "ระบุเหตุผลประกอบ", factAnalysis: "วิเคราะห์ข้อเท็จจริงของข้อกล่าวหานี้", conclusion: "สรุปผลการวิเคราะห์ข้อกล่าวหา"
  });
  const placeholderFor = (path, field, label, type = "text") => REPORT_PLACEHOLDERS[path] || FIELD_PLACEHOLDERS[field] || (type === "date" ? "เลือกวันที่" : `ระบุ${label}`);
  const dateHint = type => type === "date" ? '<small>เลือกวันที่จากปฏิทิน</small>' : "";
  const inputControl = (path, value, label, type = "text") => `<label class="a5-report-field"><span>${escapeHtml(label)}</span><input type="${type}" value="${htmlValue(value)}" placeholder="${escapeHtml(placeholderFor(path, path.split(".").at(-1), label, type))}" data-a5-report-bind="${escapeHtml(path)}">${dateHint(type)}</label>`;
  const textareaControl = (path, value, label) => `<label class="a5-report-field"><span>${escapeHtml(label)}</span><textarea data-a5-report-bind="${escapeHtml(path)}" placeholder="${escapeHtml(placeholderFor(path, path.split(".").at(-1), label))}">${htmlValue(value)}</textarea></label>`;
  ...
  function rowFieldControl(path, field, value, payload, options) {
    ...
    const placeholder = placeholderFor(path, field, label, type);
    const control = multiline
      ? `<textarea data-a5-report-row-field="${field}" placeholder="${escapeHtml(placeholder)}">${htmlValue(value)}</textarea>`
      : `<input type="${type}" value="${htmlValue(value)}" placeholder="${escapeHtml(placeholder)}" data-a5-report-row-field="${field}">${dateHint(type)}`;
    return `<label class="a5-report-field"><span>${escapeHtml(label)}</span>${control}</label>`;
  }
  ...
  function renderReport213PaperA5(payload, pageOnly) {
    ...
    const reviewByRole = (roleLabel, stepType = '') => stepType
      ? (p.reviewOpinions || []).find(row => row.stepType === stepType)
      : (p.reviewOpinions || []).find(row => String(row.reviewerRole || '').includes(roleLabel));
    const signatureBlock = (officerName, roleLabel) => FORM4_SIG_TRIPLE(officerName || '', roleLabel || '');
    const opinionBlock = (no, heading, note, roleKey, slotText, stepType = '') => {
      const review = reviewByRole(roleKey, stepType);
    ...
    const page5 = `
      ...
      ${opinionBlock('๑๕', 'ความเห็นผู้บังคับบัญชาชั้นต้น. (หัวหน้าพนักงาน ป.ป.ท.) (เรื่องที่', '(ให้เสนอความเห็นพร้อมเหตุผล เช่น เห็นควรรับ / ไม่รับไว้พิจารณา เนื่องจาก........... )', 'ชั้นต้น', meta.subject, REPORT_213_REVIEW_STEPS.LINE_SUPERVISOR)}
      ${opinionBlock('๑๖', 'ความเห็นผู้อำนวยการสำนัก (หัวหน้าพนักงาน ป.ป.ท.) (เรื่องที่', '(ให้เสนอความเห็นพร้อมเหตุผล เช่น เห็นควรรับ / ไม่รับไว้พิจารณา เนื่องจาก........... )', 'ผู้อำนวยการ', meta.subject, REPORT_213_REVIEW_STEPS.UNIT_DIRECTOR)}
      ${opinionBlock('๑๗', 'ความเห็นผู้ช่วย/รองเลขาธิการฯ ตามสายกำกับ (เรื่องที่', '', 'รองเลขาธิการ', meta.subject, REPORT_213_REVIEW_STEPS.SUPERVISING_EXECUTIVE)}
    ...
    const secretaryReview = reviewByRole('เลขาธิการ', REPORT_213_REVIEW_STEPS.SECRETARY_GENERAL);
    const page6 = `${form4Section('๑๘', 'ความเห็นเลขาธิการฯ', `${s18Rest}<p class="a5-f4-item">${showVal(secretaryReview?.opinionText || '')}</p>${signatureBlock(secretaryReview?.reviewerName, secretaryReview?.reviewerRole || 'เลขาธิการฯ')}`)}
      <section class="a5-f4-section a5-f4-boardcover">
        ...
        ${signatureBlock(p.boardCover?.signerName, p.boardCover?.signerRole)}
      </section>`;
  ...
  const REPORT_213_ACTIONS = Object.freeze([
    ...
    "report-213-route-select", "report-213-urgent-letter-attach", "report-213-support-record-opinion",
    "report-213-secretary-confirm-support", "report-213-gbk-receive", "report-213-screening-record-opinion",
    "report-213-chair-sign-agenda", "report-213-create-board-cover", "report-213-dispatch-board-package",
    "report-213-record-receipt", "report-213-record-result"
  ]);
  const REPORT_213_DECISIONS = Object.freeze(["ACCEPT", "NOT_ACCEPT", "ADDITIONAL"]);
  const REPORT_213_RESOLUTION_TASKS = Object.freeze({
    ACCEPT_24V1: "PREPARE_644_START",
    ACCEPT_24V3: "PREPARE_644_START",
    END_NO_EVIDENCE: "NOTIFY_DECISION",
    SEND_NACC: "SEND_NACC",
    SEND_POLICE: "SEND_POLICE",
    SEND_DISCIPLINE_AGENCY: "SEND_DISCIPLINE_AGENCY",
    ADDITIONAL_213: "ADDITIONAL_213",
    OTHER: "OTHER_RESOLUTION_ACTION"
  });
  ...
  function lifecycleFor213(state) {
    ...
    return {
      ...
      reviewChain: Array.isArray(current.reviewChain) ? copy(current.reviewChain) : [],
      signatures: Array.isArray(current.signatures) ? copy(current.signatures) : [],
      signedVersions: Array.isArray(current.signedVersions) ? copy(current.signedVersions) : [],
      route: text(current.route),
      routeHistory: Array.isArray(current.routeHistory) ? copy(current.routeHistory) : [],
      supplementalDocuments: Array.isArray(current.supplementalDocuments) ? copy(current.supplementalDocuments) : [],
      supportCommitteeOpinions: Array.isArray(current.supportCommitteeOpinions) ? copy(current.supportCommitteeOpinions) : [],
      screeningOpinions: Array.isArray(current.screeningOpinions) ? copy(current.screeningOpinions) : [],
      chairDecisions: Array.isArray(current.chairDecisions) ? copy(current.chairDecisions) : [],
      auditHistory: Array.isArray(current.auditHistory) ? copy(current.auditHistory) : [],
  ...
  function normalizeReviewChain(state, command) {
    const configured = Array.isArray(command.reviewChain) ? command.reviewChain : object(state.a5Report213ReviewConfiguration).steps;
    const supplied = Array.isArray(configured) ? configured : [];
    const byType = new Map(supplied.map(item => [text(item.stepType), object(item)]));
    const order = [REPORT_213_REVIEW_STEPS.LINE_SUPERVISOR, REPORT_213_REVIEW_STEPS.UNIT_DIRECTOR, REPORT_213_REVIEW_STEPS.SUPERVISING_EXECUTIVE, REPORT_213_REVIEW_STEPS.SECRETARY_GENERAL];
    const includeLine = byType.has(REPORT_213_REVIEW_STEPS.LINE_SUPERVISOR);
    const steps = order.filter(stepType => stepType !== REPORT_213_REVIEW_STEPS.LINE_SUPERVISOR || includeLine).map((stepType, index) => {
      const source = byType.get(stepType) || {};
      const defaults = REVIEW_STEP_DEFAULTS[stepType];
      return {
        stepId: text(source.stepId) || `review-${index + 1}-${stepType.toLowerCase()}`,
        sequence: index + 1,
        stepType,
        roleCode: text(source.roleCode) || defaults.roleCode,
        roleLabel: text(source.roleLabel) || defaults.roleLabel,
        actorId: text(source.actorId),
        actorName: text(source.actorName),
        authorityRef: object(source.authorityRef),
  ...
  function actorCanPerformStep(step, person, authority) {
    if (!step || (step.actorId && step.actorId !== text(person.id))) return false;
    if (step.roleCode === text(person.role)) return true;
    return step.stepType === REPORT_213_REVIEW_STEPS.SECRETARY_GENERAL
      && text(person.role) === "executive"
      && text(authority.actingForRoleCode) === "secretary";
  }

  function advanceReviewChain(lifecycle, step, at, opinionId) {
  ...
  function syncLifecycleProjection(state, lifecycle, submission) {
    const record = exactRecord(state, FORM_ID, submission?.report?.revisionNo);
    if (!record?.payload) return;
    record.payload.reviewOpinions = copy(lifecycle.reviewOpinions.filter(item => item.reportRevisionNo === submission.report.revisionNo));
    const investigatorSignature = lifecycle.signatures.find(item => item.signatureType === "INVESTIGATOR" && item.submissionPackageId === submission.packageId);
    if (investigatorSignature) record.payload.investigatorSignatures = [{
      slotId: "responsible", order: 1, officerName: investigatorSignature.signerName,
      positionName: investigatorSignature.signerPosition || "พนักงาน ป.ป.ท. ผู้รับผิดชอบ",
      roleLabel: "พนักงาน ป.ป.ท. ผู้รับผิดชอบ", signatureStatusLabel: "ลงนามแล้ว", signedAt: investigatorSignature.signedAt
    }];
    const chair = lifecycle.chairDecisions.find(item => item.submissionPackageId === submission.packageId);
    if (chair) record.payload.boardCover = {
      displayReference: chair.agendaReference, statusLabel: "ประธานลงนามบรรจุวาระแล้ว",
      signerName: chair.signerName, signerRole: chair.signerRole, signedAt: chair.signedAt
    };
  }
  ...
  function confirmedAuthority(command, actor, state, field = "authorityRef") {
    ...
    if (authority.status !== "CONFIRMED" || !text(authority.referenceNo) || (text(authority.roleCode) && text(authority.roleCode) !== text(actor.role))) {
  ...
  function normalizeResolutionItems(input) {
    const supplied = Array.isArray(input.resolutionItems) ? input.resolutionItems : [];
    return supplied.map((item, index) => ({
      itemId: text(object(item).itemId) || `resolution-item-${index + 1}`,
      code: text(object(item).code),
      instructionText: text(object(item).instructionText),
      destination: text(object(item).destination),
      orderedDueAt: text(object(item).orderedDueAt)
    }));
  }

  function validateResolution(decisionCode, items) {
    if (!REPORT_213_DECISIONS.includes(decisionCode)) return { field: "decisionCode", message: "ต้องระบุผลหลักของมติ" };
    if (!items.length) return { field: "resolutionItems", message: "ต้องมีข้อสั่งการตามมติอย่างน้อยหนึ่งรายการ" };
    const unknown = items.find(item => !REPORT_213_RESOLUTION_TASKS[item.code]);
    if (unknown) return { field: "resolutionItems", message: "พบข้อสั่งการที่ระบบไม่รองรับ" };
    const missingText = items.find(item => !item.instructionText);
    if (missingText) return { field: "resolutionItems", message: "ทุกข้อสั่งการต้องมีข้อความตามมติ" };
    const acceptItems = items.filter(item => ["ACCEPT_24V1", "ACCEPT_24V3"].includes(item.code));
    if (decisionCode === "ACCEPT" && acceptItems.length !== 1) return { field: "resolutionItems", message: "มติรับไว้ไต่สวนต้องระบุ ม.24 ว.1 หรือ ม.24 ว.3 เพียงแบบเดียว" };
    if (decisionCode === "ACCEPT" && items.some(item => !["ACCEPT_24V1", "ACCEPT_24V3"].includes(item.code))) return { field: "resolutionItems", message: "ข้อสั่งการไม่ตรงกับผลรับไว้ไต่สวน" };
    if (decisionCode === "NOT_ACCEPT" && items.some(item => ["ACCEPT_24V1", "ACCEPT_24V3", "ADDITIONAL_213"].includes(item.code))) return { field: "resolutionItems", message: "ข้อสั่งการไม่ตรงกับผลไม่รับไว้ไต่สวน" };
    if (decisionCode === "ADDITIONAL" && (items.length !== 1 || items[0].code !== "ADDITIONAL_213")) return { field: "resolutionItems", message: "มติไต่สวนเพิ่มเติมต้องระบุข้อสั่งการไต่สวนเพิ่มเติม" };
    return null;
  }

  function createResolutionTasks(state, result, at) {
    state.downstreamTasks = Array.isArray(state.downstreamTasks) ? state.downstreamTasks : [];
    result.resolutionItems.forEach((item, index) => {
      state.downstreamTasks.push({
        id: `213-${Date.parse(at) || 0}-${index + 1}`,
        type: REPORT_213_RESOLUTION_TASKS[item.code],
        status: item.code === "ADDITIONAL_213" ? "IN_PROGRESS" : "PENDING",
        createdAt: at,
        resolutionReference: result.resolutionReference,
        resolutionItem: copy(item),
        ...(item.code === "ACCEPT_24V1" ? { orderType: "24v1" } : {}),
        ...(item.code === "ACCEPT_24V3" ? { orderType: "24v3" } : {})
      });
    });
    const section62 = text(state.caseData?.decision) === "62" || object(object(state.inquiry).intake).m62?.flag === true;
    if (section62) {
      state.downstreamTasks.push({
        id: `213-${Date.parse(at) || 0}-section-65`,
        type: "REPORT_NACC_SECTION_65",
        status: "PENDING",
        createdAt: at,
        resolutionReference: result.resolutionReference
      });
  ...
  function executeReport213Action(sourceState, actor, action, command) {
    ...
    if (action === "report-213-submit") {
      ...
      if (!text(object(input.signature).methodLabel)) return lifecycleFailure(sourceState, "SIGNATURE_REQUIRED", "signature", "ผู้รับผิดชอบต้องลงนามรายงานก่อนเสนอ");
      ...
      const additionalTask = (Array.isArray(state.downstreamTasks) ? state.downstreamTasks : []).find(item => item.type === "ADDITIONAL_213" && item.status === "IN_PROGRESS");
      if (additionalTask) {
        additionalTask.status = "COMPLETED";
        additionalTask.completedAt = input.at;
        additionalTask.completedBy = person.id;
        additionalTask.resubmissionPackageId = packageRecord.packageId;
      }
      lifecycle.reviewChain = normalizeReviewChain(state, input);
      const investigatorSignature = {
        signatureType: "INVESTIGATOR", reportDocumentId: FORM_ID, reportRevisionNo: packageRecord.report.revisionNo,
        submissionPackageId: packageRecord.packageId, signedBy: person.id, signerName: person.name,
        signerRole: person.role, signerPosition: text(input.signature.positionName), signedAt: input.at,
        methodLabel: text(input.signature.methodLabel), authorityRef: copy(object(input.authorityRef)),
        signedSnapshot: copy(packageRecord.report.snapshot)
      };
      lifecycle.signatures.push(investigatorSignature);
      lifecycle.signedVersions.push({ submissionPackageId: packageRecord.packageId, revisionNo: packageRecord.report.revisionNo, signatureType: "INVESTIGATOR", signedAt: input.at, signedBy: person.id, snapshot: copy(packageRecord.report.snapshot) });
      lifecycle.auditHistory.push({ action: action, fromStatus: "REPORT_213_DRAFT", toStatus: lifecycle.status, by: person.id, role: person.role, at: input.at, documentRevisionNo: packageRecord.report.revisionNo });
      syncLifecycleProjection(state, lifecycle, packageRecord);
    ...
    if (["report-213-review-record-opinion", "report-213-review-return", "report-213-route-select", "report-213-urgent-letter-attach", "report-213-support-record-opinion", "report-213-gbk-receive", "report-213-screening-record-opinion", "report-213-create-board-cover", "report-213-dispatch-board-package", "report-213-record-result"].includes(action)) {
    ...
    if (action === "report-213-review-record-opinion") {
      ...
      if (!text(object(input.signature).methodLabel)) return lifecycleFailure(sourceState, "SIGNATURE_REQUIRED", "signature", "ผู้ให้ความเห็นต้องลงนามดิจิทัล");
      const step = currentReviewStep(lifecycle);
      if (!actorCanPerformStep(step, person, object(input.authorityRef))) return lifecycleFailure(sourceState, "FORBIDDEN_ACTOR", "actor", "ผู้ใช้นี้ไม่ใช่ผู้ตรวจในลำดับปัจจุบัน");
      if (step.stepType === REPORT_213_REVIEW_STEPS.SECRETARY_GENERAL) return lifecycleFailure(sourceState, "INVALID_TRANSITION", "action", "ขั้นเลขาธิการต้องใช้การให้ความเห็นและลงนามขั้นสุดท้าย");
      const opinion = { opinionId: text(input.opinionId), reportDocumentId: FORM_ID, reportRevisionNo: submission.report.revisionNo, sequence: lifecycle.reviewOpinions.length + 1, stepId: step.stepId, stepType: step.stepType, reviewerId: person.id, reviewerName: person.name, reviewerRole: text(input.positionName) || step.roleLabel, reviewerRoleCode: person.role, authorityRef: copy(object(input.authorityRef)), decision: "ENDORSE", opinionText: text(input.opinionText), affectedFields: [], affectedDocumentVersionIds: [], recordedAt: input.at, signature: { signedBy: person.id, signedAt: input.at, methodLabel: text(input.signature.methodLabel) } };
      lifecycle.reviewOpinions.push(opinion);
      const next = advanceReviewChain(lifecycle, step, input.at, opinion.opinionId);
      if (next?.stepType === REPORT_213_REVIEW_STEPS.SECRETARY_GENERAL) lifecycle.status = "REPORT_213_FINAL_SIGNATURE_PENDING";
      lifecycle.auditHistory.push({ action, fromStatus: "REPORT_213_REVIEW_PENDING", toStatus: lifecycle.status, stepId: step.stepId, by: person.id, role: person.role, at: input.at, documentRevisionNo: submission.report.revisionNo });
      syncLifecycleProjection(state, lifecycle, submission);
      bumpStore(state);
    ...
    if (action === "report-213-review-return") {
      ...
      lifecycle.reviewOpinions.push({ opinionId: text(input.opinionId) || `return-${input.idempotencyKey}`, reportDocumentId: FORM_ID, reportRevisionNo: submission.report.revisionNo, sequence: lifecycle.reviewOpinions.length + 1, stepId: step.stepId, stepType: step.stepType, reviewerId: person.id, reviewerName: person.name, reviewerRole: text(input.positionName) || step.roleLabel, reviewerRoleCode: person.role, authorityRef: copy(object(input.authorityRef)), decision: "RETURN", opinionText: text(input.reason), affectedFields, affectedDocumentVersionIds, recordedAt: input.at, signature: null });
      lifecycle.status = "REPORT_213_RETURNED";
      lifecycle.auditHistory.push({ action, fromStatus: "REPORT_213_REVIEW_PENDING", toStatus: lifecycle.status, stepId: step.stepId, reason: text(input.reason), by: person.id, role: person.role, at: input.at, documentRevisionNo: submission.report.revisionNo });
    ...
    if (action === "report-213-sign") {
      if (lifecycle.status !== "REPORT_213_FINAL_SIGNATURE_PENDING" || !text(input.methodLabel) || !text(input.opinionText)) return lifecycleFailure(sourceState, "SIGNATURE_REQUIRED", "methodLabel", "เลขาธิการต้องให้ความเห็นและลงนามขั้นสุดท้าย");
      const step = currentReviewStep(lifecycle);
      const signerRole = text(input.positionName) || (text(person.role) === "executive" ? "ผู้ปฏิบัติหน้าที่แทนเลขาธิการ ป.ป.ท." : step.roleLabel);
      const opinion = { opinionId: text(input.opinionId) || `final-${input.idempotencyKey}`, reportDocumentId: FORM_ID, reportRevisionNo: submission.report.revisionNo, sequence: lifecycle.reviewOpinions.length + 1, stepId: step.stepId, stepType: step.stepType, reviewerId: person.id, reviewerName: person.name, reviewerRole: signerRole, reviewerRoleCode: person.role, authorityRef: copy(object(input.authorityRef)), decision: "ENDORSE", opinionText: text(input.opinionText), affectedFields: [], affectedDocumentVersionIds: [], recordedAt: input.at, signature: { signedBy: person.id, signedAt: input.at, methodLabel: text(input.methodLabel) } };
      lifecycle.reviewOpinions.push(opinion);
      advanceReviewChain(lifecycle, step, input.at, opinion.opinionId);
      const signature = { signatureType: "SECRETARY_GENERAL", reportDocumentId: FORM_ID, reportRevisionNo: submission.report.revisionNo, submissionPackageId: submission.packageId, signedBy: person.id, signerName: person.name, signerRole, signedAt: input.at, methodLabel: text(input.methodLabel), authorityRef: copy(object(input.authorityRef)), signedSnapshot: copy(submission.report.snapshot) };
      lifecycle.signatures.push(signature);
      lifecycle.signedVersions.push({ submissionPackageId: submission.packageId, revisionNo: submission.report.revisionNo, signatureType: "SECRETARY_GENERAL", signedAt: input.at, signedBy: person.id, snapshot: copy(submission.report.snapshot) });
      lifecycle.status = "REPORT_213_SIGNED_AWAITING_ROUTE";
      // นับแต้ม 213: กฎ REC6 — เลขาธิการลงนาม = +1 แต้มให้ผู้รับผิดชอบสำนวน
      try {
        const meritApi = root.ECMISActivity5MeritPoints || (typeof require === "function" ? require("./activity5-merit-points.js") : null);
        if (meritApi) {
          const responsibleId = text(object(object(activeRecord?.payload).documentMeta).responsibleOfficer?.officerId) || text(person.id);
          const meritResult = meritApi.accrue(state, { officerId: responsibleId, officerName: text(object(object(activeRecord?.payload).documentMeta).responsibleOfficer?.name), reportType: "213", caseId: state.caseData?.id, signedAt: input.at, signedBy: person.id });
          if (meritResult.ok) lifecycle.auditHistory.push({ action: "merit-accrue-213", fromStatus: lifecycle.status, toStatus: "MERIT_ACCRUED", by: person.id, role: person.role, at: input.at, documentVersionId: opinion.documentVersionId });
        }
      } catch (meritError) { /* merit accrual must never block the sign flow */ }
      lifecycle.auditHistory.push({ action, fromStatus: "REPORT_213_FINAL_SIGNATURE_PENDING", toStatus: lifecycle.status, stepId: step.stepId, by: person.id, role: person.role, at: input.at, documentRevisionNo: submission.report.revisionNo });
      syncLifecycleProjection(state, lifecycle, submission);
      bumpStore(state);
      return finishLifecycle(context, state, "REPORT_213_SIGNED", { signature: copy(signature) });
    }

    if (action === "report-213-route-select") {
      if (lifecycle.status !== "REPORT_213_SIGNED_AWAITING_ROUTE") return lifecycleFailure(sourceState, "INVALID_TRANSITION", "status", "รายงานยังไม่พร้อมเลือกเส้นทางเสนอ");
      if (person.role !== "secretary" && !(person.role === "executive" && text(object(input.authorityRef).actingForRoleCode) === "secretary")) return lifecycleFailure(sourceState, "FORBIDDEN_ACTOR", "actor", "เลขาธิการหรือผู้ปฏิบัติหน้าที่แทนเป็นผู้เลือกเส้นทาง");
      const route = text(input.route);
      if (!Object.values(REPORT_213_ROUTES).includes(route)) return lifecycleFailure(sourceState, "MISSING_REQUIRED_FIELD", "route", "ต้องเลือกเส้นทางเสนอรายงาน");
      if (!text(input.reason)) return lifecycleFailure(sourceState, "MISSING_REQUIRED_FIELD", "reason", "ต้องระบุเหตุผลเลือกเส้นทางเสนอรายงาน");
      if (route === REPORT_213_ROUTES.SUPPORT_COMMITTEE && !["SUPPORT_1", "SUPPORT_2"].includes(text(input.committeeId))) return lifecycleFailure(sourceState, "MISSING_REQUIRED_FIELD", "committeeId", "ต้องระบุคณะอนุกรรมการสนับสนุนคณะที่ 1 หรือ 2");
      lifecycle.route = route;
      lifecycle.status = route === REPORT_213_ROUTES.NORMAL ? "REPORT_213_GBK_PENDING" : route === REPORT_213_ROUTES.SUPPORT_COMMITTEE ? "REPORT_213_SUPPORT_COMMITTEE_PENDING" : "REPORT_213_URGENT_LETTER_PENDING";
      const routeEvent = { route, committeeId: text(input.committeeId), reason: text(input.reason), selectedBy: person.id, selectedAt: input.at };
      lifecycle.routeHistory.push(routeEvent);
      lifecycle.auditHistory.push({ action, fromStatus: "REPORT_213_SIGNED_AWAITING_ROUTE", toStatus: lifecycle.status, route, by: person.id, role: person.role, at: input.at, documentRevisionNo: submission.report.revisionNo });
      bumpStore(state);
      return finishLifecycle(context, state, "REPORT_213_ROUTE_SELECTED", { route: copy(routeEvent) });
    }

    if (action === "report-213-urgent-letter-attach") {
      if (lifecycle.status !== "REPORT_213_URGENT_LETTER_PENDING" || lifecycle.route !== REPORT_213_ROUTES.URGENT) return lifecycleFailure(sourceState, "INVALID_TRANSITION", "status", "สำนวนไม่ได้อยู่ในขั้นจัดทำหนังสือขอบรรจุวาระด่วน");
      if (!text(input.documentVersionId) || !text(input.referenceNo) || !text(input.reason)) return lifecycleFailure(sourceState, "MISSING_REQUIRED_FIELD", "documentVersionId", "ต้องแนบหนังสือขอบรรจุวาระด่วนและระบุเหตุผลอันสมควร");
      ...
      lifecycle.status = "REPORT_213_CHAIR_AGENDA_PENDING";
      lifecycle.auditHistory.push({ action, fromStatus: "REPORT_213_URGENT_LETTER_PENDING", toStatus: lifecycle.status, by: person.id, role: person.role, at: input.at, documentVersionId: document.documentVersionId });
      bumpStore(state);
      return finishLifecycle(context, state, "REPORT_213_URGENT_LETTER_ATTACHED", { document: copy(document) });
    }

    if (action === "report-213-support-record-opinion") {
      if (lifecycle.status !== "REPORT_213_SUPPORT_COMMITTEE_PENDING" || lifecycle.route !== REPORT_213_ROUTES.SUPPORT_COMMITTEE) return lifecycleFailure(sourceState, "INVALID_TRANSITION", "status", "สำนวนไม่ได้อยู่ระหว่างคณะอนุกรรมการสนับสนุนพิจารณา");
      const selectedRoute = lifecycle.routeHistory.at(-1);
      const authority = object(input.authorityRef);
      if (person.role !== "committee" || text(authority.committeeId) !== text(selectedRoute.committeeId)) return lifecycleFailure(sourceState, "FORBIDDEN_ACTOR", "actor", "ต้องเป็นสมาชิกคณะอนุกรรมการสนับสนุนที่ได้รับมอบหมาย");
      if (!text(input.opinionText) || !text(input.documentVersionId) || !text(object(input.signature).methodLabel)) return lifecycleFailure(sourceState, "MISSING_REQUIRED_FIELD", "opinionText", "ต้องมีความเห็น เอกสารประกอบ และลายเซ็น");
      const opinion = { committeeId: selectedRoute.committeeId, opinionText: text(input.opinionText), documentVersionId: text(input.documentVersionId), recordedBy: person.id, recorderName: person.name, recordedAt: input.at, authorityRef: copy(authority), signature: { signedBy: person.id, signedAt: input.at, methodLabel: text(input.signature.methodLabel) } };
      ...
      lifecycle.status = "REPORT_213_SUPPORT_SECRETARY_CONFIRM_PENDING";
      lifecycle.auditHistory.push({ action, fromStatus: "REPORT_213_SUPPORT_COMMITTEE_PENDING", toStatus: lifecycle.status, by: person.id, role: person.role, at: input.at, documentVersionId: opinion.documentVersionId });
      bumpStore(state);
      return finishLifecycle(context, state, "REPORT_213_SUPPORT_OPINION_RECORDED", { opinion: copy(opinion) });
    }

    if (action === "report-213-secretary-confirm-support") {
      if (lifecycle.status !== "REPORT_213_SUPPORT_SECRETARY_CONFIRM_PENDING") return lifecycleFailure(sourceState, "INVALID_TRANSITION", "status", "ยังไม่มีความเห็นคณะอนุกรรมการให้เลขาธิการยืนยัน");
      if (!text(input.opinionText) || !text(input.methodLabel)) return lifecycleFailure(sourceState, "SIGNATURE_REQUIRED", "methodLabel", "ต้องให้ความเห็นและระบุวิธีลงนามยืนยันชุดเสนอ");
      const signature = { signatureType: "SECRETARY_SUPPORT_CONFIRMATION", submissionPackageId: submission.packageId, reportDocumentId: FORM_ID, reportRevisionNo: submission.report.revisionNo, signedBy: person.id, signerName: person.name, signerRole: text(input.positionName) || "เลขาธิการ ป.ป.ท.", opinionText: text(input.opinionText), signedAt: input.at, methodLabel: text(input.methodLabel), authorityRef: copy(object(input.authorityRef)), signedSnapshot: copy(submission.report.snapshot) };
      lifecycle.signatures.push(signature);
      lifecycle.signedVersions.push({ submissionPackageId: submission.packageId, revisionNo: submission.report.revisionNo, signatureType: signature.signatureType, signedAt: input.at, signedBy: person.id, snapshot: copy(submission.report.snapshot) });
      lifecycle.status = "REPORT_213_CHAIR_AGENDA_PENDING";
      lifecycle.auditHistory.push({ action, fromStatus: "REPORT_213_SUPPORT_SECRETARY_CONFIRM_PENDING", toStatus: lifecycle.status, by: person.id, role: person.role, at: input.at, documentRevisionNo: submission.report.revisionNo });
      bumpStore(state);
      return finishLifecycle(context, state, "REPORT_213_SUPPORT_CONFIRMED", { signature: copy(signature) });
    }

    if (action === "report-213-gbk-receive") {
      if (lifecycle.status !== "REPORT_213_GBK_PENDING" || lifecycle.route !== REPORT_213_ROUTES.NORMAL) return lifecycleFailure(sourceState, "INVALID_TRANSITION", "status", "สำนวนไม่ได้อยู่ระหว่าง กบค. รับเรื่อง");
      if (person.role !== "clerk" || !text(input.receivedAt) || !text(input.receiptReference)) return lifecycleFailure(sourceState, "MISSING_REQUIRED_FIELD", "receivedAt", "ต้องบันทึกวันรับและเลขอ้างอิง กบค.");
      lifecycle.supplementalDocuments.push({ documentType: "GBK_RECEIPT", referenceNo: text(input.receiptReference), receivedBy: person.id, receivedAt: text(input.receivedAt) });
      const signature = [...lifecycle.signatures].reverse().find(item => item.submissionPackageId === submission.packageId && ["SECRETARY_GENERAL", "SECRETARY_SUPPORT_CONFIRMATION"].includes(item.signatureType));
      if (!lifecycle.boardPackages.some(item => item.submissionPackageId === submission.packageId)) {
        lifecycle.boardPackages.push({
          packageId: `board-${submission.packageId}`,
          submissionPackageId: submission.packageId,
          signedReportRef: { documentId: FORM_ID, revisionNo: submission.report.revisionNo, signature: copy(signature) },
          boardCoverRef: { documentId: "MOCK_GBK_BOARD_HANDOFF", revisionNo: 1 },
      ...
      lifecycle.status = "REPORT_213_WAIT_RESULT";
      state.workflow = { ...object(state.workflow), stage: "a7-213", owner: "case-clerk" };
      lifecycle.auditHistory.push({ action, fromStatus: "REPORT_213_GBK_PENDING", toStatus: lifecycle.status, by: person.id, role: person.role, at: input.at, referenceNo: text(input.receiptReference) });
      bumpStore(state);
      return finishLifecycle(context, state, "REPORT_213_SENT_TO_BOARD");
    }

    if (action === "report-213-screening-record-opinion") {
      if (lifecycle.status !== "REPORT_213_SCREENING_PENDING") return lifecycleFailure(sourceState, "INVALID_TRANSITION", "status", "สำนวนไม่ได้อยู่ระหว่างคณะอนุกรรมการกลั่นกรอง");
      const committeeId = text(input.committeeId);
      if (person.role !== "committee" || !/^SCREENING_[1-8]$/.test(committeeId) || text(object(input.authorityRef).committeeId) !== committeeId) return lifecycleFailure(sourceState, "FORBIDDEN_ACTOR", "actor", "ต้องเป็นคณะอนุกรรมการกลั่นกรองคณะที่ 1–8 ตามสาย");
      if (!text(input.opinionText) || !text(input.documentVersionId) || !text(object(input.signature).methodLabel)) return lifecycleFailure(sourceState, "MISSING_REQUIRED_FIELD", "opinionText", "ต้องมีความเห็น เอกสาร และลายเซ็นคณะอนุกรรมการกลั่นกรอง");
      const opinion = { committeeId, opinionText: text(input.opinionText), documentVersionId: text(input.documentVersionId), recordedBy: person.id, recorderName: person.name, recordedAt: input.at, authorityRef: copy(object(input.authorityRef)), signature: { signedBy: person.id, signedAt: input.at, methodLabel: text(input.signature.methodLabel) } };
      lifecycle.screeningOpinions.push(opinion);
      lifecycle.supplementalDocuments.push({ documentType: "SCREENING_COMMITTEE_OPINION", documentVersionId: opinion.documentVersionId, committeeId, attachedBy: person.id, attachedAt: input.at });
      lifecycle.status = "REPORT_213_CHAIR_AGENDA_PENDING";
      lifecycle.auditHistory.push({ action, fromStatus: "REPORT_213_SCREENING_PENDING", toStatus: lifecycle.status, by: person.id, role: person.role, at: input.at, documentVersionId: opinion.documentVersionId });
      bumpStore(state);
      return finishLifecycle(context, state, "REPORT_213_SCREENING_OPINION_RECORDED", { opinion: copy(opinion) });
    }

    if (action === "report-213-chair-sign-agenda") {
      if (lifecycle.status !== "REPORT_213_CHAIR_AGENDA_PENDING") return lifecycleFailure(sourceState, "INVALID_TRANSITION", "status", "ชุดเสนอรายงานยังไม่พร้อมให้ประธานลงนามบรรจุวาระ");
      const authority = object(input.authorityRef);
      if (!text(input.agendaReference) || !text(input.methodLabel)) return lifecycleFailure(sourceState, "SIGNATURE_REQUIRED", "methodLabel", "ต้องระบุเลขอ้างอิงวาระและลายเซ็นประธาน");
      const decision = { submissionPackageId: submission.packageId, route: lifecycle.route, agendaReference: text(input.agendaReference), signerId: person.id, signerName: person.name, signerRole: text(input.positionName) || "ประธานกรรมการ ป.ป.ท.", signedAt: input.at, methodLabel: text(input.methodLabel), authorityRef: copy(authority) };
      lifecycle.chairDecisions.push(decision);
      lifecycle.status = "REPORT_213_BOARD_READY";
      lifecycle.auditHistory.push({ action, fromStatus: "REPORT_213_CHAIR_AGENDA_PENDING", toStatus: lifecycle.status, by: person.id, role: person.role, at: input.at, agendaReference: decision.agendaReference });
      syncLifecycleProjection(state, lifecycle, submission);
      bumpStore(state);
      return finishLifecycle(context, state, "REPORT_213_CHAIR_AGENDA_SIGNED", { decision: copy(decision) });
    }

    if (action === "report-213-create-board-cover") {
      if (lifecycle.status !== "REPORT_213_BOARD_READY" || !lifecycle.chairDecisions.some(item => item.submissionPackageId === submission.packageId)) return lifecycleFailure(sourceState, "INVALID_TRANSITION", "status", "ประธานยังไม่ได้ลงนามบรรจุวาระ");
      const signature = [...lifecycle.signatures].reverse().find(item => item.submissionPackageId === submission.packageId && ["SECRETARY_GENERAL", "SECRETARY_SUPPORT_CONFIRMATION"].includes(item.signatureType));
    ...
    if (lifecycle.results.some(item => item.packageId === boardPackage.packageId)) return lifecycleFailure(sourceState, "RESULT_ALREADY_RECORDED", "result", "บันทึกผลพิจารณาของชุดเอกสารนี้แล้ว");
    if (text(input.manualSource) !== "MANUAL_COPY_FROM_ACTIVITY_7" || input.certifiedTrueCopy !== true) {
      return lifecycleFailure(sourceState, "MANUAL_SOURCE_CONFIRMATION_REQUIRED", "certifiedTrueCopy", "ธุรการคดีต้องยืนยันว่าบันทึกข้อมูลตรงตามมติฉบับลงนามจากกิจกรรมที่ 7");
    }
    const decisionCode = text(input.decisionCode || input.resultCode);
    const resolutionReference = text(input.resolutionReference || input.resultReference);
    const resolutionItems = normalizeResolutionItems(input);
    for (const field of ["decidedAt", "resolutionDocumentVersionId", "resolutionText"]) {
      if (!text(input[field])) return lifecycleFailure(sourceState, "MISSING_REQUIRED_FIELD", field, "ข้อมูลหรือเอกสารมติไม่ครบถ้วน");
    }
    if (!isDate(input.decidedAt)) return lifecycleFailure(sourceState, "INVALID_DATE", "decidedAt", "วันที่มีมติไม่ถูกต้อง");
    if (!resolutionReference) return lifecycleFailure(sourceState, "MISSING_REQUIRED_FIELD", "resolutionReference", "ต้องระบุเลขที่มติ");
    const resolutionError = validateResolution(decisionCode, resolutionItems);
    if (resolutionError) return lifecycleFailure(sourceState, "INVALID_BOARD_RESULT", resolutionError.field, resolutionError.message);
    const result = {
      packageId: boardPackage.packageId,
      decisionCode,
      resultCode: decisionCode,
      resultLabel: text(input.resultLabel),
      resolutionReference,
      resultReference: resolutionReference,
      resolutionDocumentVersionId: text(input.resolutionDocumentVersionId),
      resolutionText: text(input.resolutionText),
      resolutionItems,
      decidedAt: input.decidedAt,
      recordedBy: person.id,
      recordedAt: input.at,
      sourceType: "MANUAL_COPY_FROM_ACTIVITY_7",
      certifiedTrueCopy: true,
      sourceSnapshot: {
        resolutionReference,
        resolutionDocumentVersionId: text(input.resolutionDocumentVersionId),
        resolutionText: text(input.resolutionText),
        resolutionItems: copy(resolutionItems),
        decidedAt: input.decidedAt
      },
      authorityRef: copy(object(input.authorityRef))
    };
    lifecycle.results.push(result);
    createResolutionTasks(state, result, input.at);
    if (decisionCode === "ADDITIONAL") {
      const returned = domain().returnA5DocumentRevision(state, {
        caseId: input.caseId,
        documentId: FORM_ID,
        revisionNo: submission.report.revisionNo,
        expectedVersion: Number(state.a5DocumentStore?.version || 0),
        actorId: person.id,
        at: input.at,
        idempotencyKey: `${input.idempotencyKey}:additional`,
        reason: result.resolutionText,
        affectedFields: ["factFindings", "analysis", "opinion"],
        affectedDocumentVersionIds: []
      });
      if (!returned.ok) return { ...returned, state: copy(sourceState) };
      state = returned.state;
      lifecycle.status = "REPORT_213_RETURNED";
      state.workflow = { ...object(state.workflow), stage: "a5-prelim", owner: "investigator" };
    } else {
      lifecycle.status = "REPORT_213_RESULT_RECEIVED";
      state.workflow = { ...object(state.workflow), stage: "a5-outcome", owner: "clerk" };
      bumpStore(state);
    }
    lifecycle.auditHistory.push({ action, fromStatus: "REPORT_213_WAIT_RESULT", toStatus: lifecycle.status, by: person.id, role: person.role, at: input.at, resolutionReference });
    const finished = finishLifecycle(context, state, "REPORT_213_RESULT_RECORDED", { result: copy(result) });
    if (decisionCode !== "ADDITIONAL") finished.state.workflow.downstreamStatus = "OUTCOME_TASKS_PENDING";
    return finished;
  }

  function getReport213ActionModelA5(sourceState, role = "") {
    ...
    const labels = {
      ...
      "report-213-sign": "ให้ความเห็นและลงนามขั้นสุดท้าย", "report-213-route-select": "เลือกเส้นทางเสนอรายงาน",
      "report-213-urgent-letter-attach": "แนบหนังสือขอบรรจุวาระด่วน", "report-213-support-record-opinion": "บันทึกความเห็นคณะอนุกรรมการสนับสนุนฯ",
      "report-213-secretary-confirm-support": "ยืนยันความเห็นคณะสนับสนุนและลงนาม", "report-213-gbk-receive": "ส่งมติบอร์ด",
      "report-213-screening-record-opinion": "บันทึกความเห็นคณะอนุกรรมการกลั่นกรอง", "report-213-chair-sign-agenda": "ประธานลงนามบรรจุวาระ",
      "report-213-create-board-cover": "จัดทำชุดเสนอคณะกรรมการ", "report-213-dispatch-board-package": "ส่งชุดเอกสารให้กิจกรรมที่ 7",
      "report-213-record-receipt": "บันทึกการรับชุดเอกสาร", "report-213-record-result": "บันทึกผลพิจารณา"
    };
    const current = currentReviewStep(lifecycle);
    const byStatus = {
      REPORT_213_DRAFT: [["report-213-submit", "investigator"]], REPORT_213_RETURNED: [["report-213-submit", "investigator"]],
      REPORT_213_REVIEW_PENDING: [["report-213-review-record-opinion", current?.roleCode], ["report-213-review-return", current?.roleCode]],
      REPORT_213_FINAL_SIGNATURE_PENDING: [["report-213-sign", "secretary"], ["report-213-sign", "executive"]],
      REPORT_213_SIGNED_AWAITING_ROUTE: [["report-213-route-select", "secretary"], ["report-213-route-select", "executive"]],
      REPORT_213_URGENT_LETTER_PENDING: [["report-213-urgent-letter-attach", "secretary"], ["report-213-urgent-letter-attach", "clerk"]],
      REPORT_213_SUPPORT_COMMITTEE_PENDING: [["report-213-support-record-opinion", "committee"]],
      REPORT_213_SUPPORT_SECRETARY_CONFIRM_PENDING: [["report-213-secretary-confirm-support", "secretary"], ["report-213-secretary-confirm-support", "executive"]],
      REPORT_213_GBK_PENDING: [["report-213-gbk-receive", "clerk"]],
      REPORT_213_SCREENING_PENDING: [["report-213-screening-record-opinion", "committee"]],
      REPORT_213_CHAIR_AGENDA_PENDING: [["report-213-chair-sign-agenda", "committee"]],
      REPORT_213_BOARD_READY: lifecycle.boardPackages.some(item => item.submissionPackageId === lifecycle.submissions.at(-1)?.packageId)
        ? [["report-213-dispatch-board-package", "clerk"]] : [["report-213-create-board-cover", "clerk"]],
      REPORT_213_SENT_TO_A7: [["report-213-record-receipt", "clerk"]], REPORT_213_WAIT_RESULT: [["report-213-record-result", "clerk"]]
    };
    return (byStatus[lifecycle.status] || []).filter(([, requiredRole]) => requiredRole === role).map(([id]) => ({ id, label: labels[id], primary: !["report-213-review-return"].includes(id), enabled: true, reason: "" }));
  }

  const api = Object.freeze({ FORM_ID, SECTION_KEYS, SECTION_TITLES, REPORT_213_GROUPS, BRANCHES, REPORT_213_ACTIONS, REPORT_213_REVIEW_STEPS, REPORT_213_ROUTES, normalizeReport213A5, validateReport213A5, saveReport213DraftA5, buildReport213SubmissionA5, executeReport213Action, getReport213ActionModelA5, mutateReport213RowsA5, captureReport213EditorA5, renderReport213EditorA5, renderReport213PaperA5, renderReport213PaperA5Legacy });
