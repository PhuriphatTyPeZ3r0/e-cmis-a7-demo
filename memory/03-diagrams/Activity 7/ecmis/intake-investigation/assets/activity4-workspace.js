(() => {
  ...
  const STORE_SCHEMA_VERSION_KEY = 'ecmis-a4-workspace-schema-version';
  const STORE_MIGRATION_BACKUP_PREFIX = 'ecmis-a4-workspace-migration-backup-v3';
  ...
  const GOV_AGENCIES = ['กระทรวงมหาดไทย','กระทรวงศึกษาธิการ','กระทรวงสาธารณสุข','กระทรวงการคลัง','กระทรวงคมนาคม','กระทรวงเกษตรและสหกรณ์','กระทรวงยุติธรรม','กระทรวงแรงงาน','กระทรวงทรัพยากรธรรมชาติและสิ่งแวดล้อม','กระทรวงพาณิชย์','กระทรวงอุตสาหกรรม','กระทรวงการพัฒนาสังคมและความมั่นคงของมนุษย์','กระทรวงดิจิทัลเพื่อเศรษฐกิจและสังคม','กระทรวงพลังงาน','กระทรวงการต่างประเทศ','กระทรวงการท่องเที่ยวและกีฬา','กระทรวงวัฒนธรรม','กระทรวงกลาโหม','กระทรวงการอุดมศึกษา วิทยาศาสตร์ วิจัยและนวัตกรรม','สำนักนายกรัฐมนตรี','สำนักงานตำรวจแห่งชาติ','กรมการปกครอง','กรมที่ดิน','กรมโยธาธิการและผังเมือง','กรมส่งเสริมการปกครองท้องถิ่น','กรมสรรพากร','กรมศุลกากร','กรมบัญชีกลาง','สำนักงานอัยการสูงสุด','กรุงเทพมหานคร (กทม.)','องค์การบริหารส่วนจังหวัด (อบจ.)','องค์การบริหารส่วนตำบล (อบต.)','เทศบาลนคร/เทศบาลเมือง/เทศบาลตำบล','หน่วยงานรัฐวิสาหกิจ','สถานศึกษา/มหาวิทยาลัยของรัฐ','โรงพยาบาลของรัฐ','อื่นๆ (โปรดระบุในรายละเอียด)'];
  function buildAgencyOptions(placeholder){const items=placeholder?[{value:'',label:placeholder}]:[];return items.concat(GOV_AGENCIES.map(name=>({value:name,label:name}))).map(item=>`<option value=\"${item.value}\">${item.label}</option>`).join('')}
  const ROLE_LABELS = { admin:'ธุรการ ศรร.', 'central-registry-clerk':'ธุรการสารบรรณกลาง ชั้น 14', 'case-admin-clerk':'ธุรการ กบค.', officer:'เจ้าหน้าที่รับเรื่อง', 'regional-clerk':'ธุรการประจำเขต', 'regional-officer':'เจ้าหน้าที่รับเรื่องประจำเขต', 'regional-director':'ผอ.สำนักงาน ป.ป.ท. เขต', center:'ผู้อำนวยการศูนย์รับเรื่องร้องเรียน', division:'ผู้อำนวยการกองบริหารคดี', acting:'ผู้รักษาราชการแทนตามคำสั่ง', anonymous:'กล่องบัตรสนเท่ห์', activity7:'กิจกรรมที่ 7' };
  ...
  const REGIONAL_PREFIX_BY_REGION = {'เขต 1':'ปป 0014','เขต 2':'ปป 0015','เขต 3':'ปป 0016','เขต 4':'ปป 0017','เขต 5':'ปป 0018','เขต 6':'ปป 0019','เขต 7':'ปป 0020','เขต 8':'ปป 0021','เขต 9':'ปป 0022','ส่วนกลาง':'ปป 0004.2'};
  const REGIONAL_REGIONS = REGIONS.filter(region=>region!=='ส่วนกลาง');
  const REGIONAL_CHANNELS = ['Walk-In','จดหมาย','ม.62 (ป.ป.ช.)','เบอร์โทร ปปท. เขต 1-9'];
  const NACC_API_CHANNEL = 'ป.ป.ช. (API)';
  const isOfficerRole = role => ['officer','regional-officer'].includes(role);
  const isRegionalRole = role => ['regional-clerk','regional-officer','regional-director'].includes(role);
  const channelKey = channel => String(channel||'').toLowerCase().replace(/[-\s]/g,'');
  const isM62Channel = channel => channelKey(channel).includes('ม.62')||channelKey(channel).includes('ม62');
  const isNaccApiChannel = channel => channelKey(channel)===channelKey(NACC_API_CHANNEL);
  const isNaccApiCase = record => window.ECMISActivity4DocumentRules?.isNaccApiCase(record)===true;
  const isRegionalChannel = channel => ['walkin','จดหมาย',channelKey('เบอร์โทร ปปท. เขต 1-9')].includes(channelKey(channel))||isM62Channel(channel);
  const isCentralRegistryChannel = (channel,region) => region==='ส่วนกลาง'&&(channelKey(channel)==='จดหมาย'||isM62Channel(channel)||isNaccApiChannel(channel));
  const isRegionalPhoneChannel = channel => channelKey(channel)===channelKey('เบอร์โทร ปปท. เขต 1-9');
  const issuesTrackingImmediately = channel => ['สายด่วน 1206','Website','ทางรัฐ'].some(item=>channelKey(channel)===channelKey(item))||isRegionalPhoneChannel(channel);
  const CENTRAL_KBK_CHANNELS = ['Walk-In','Email','สปน.1111','Traffy','ศปท.','Feedback','การข่าว','ศบช./กนป.','ศนธ./กนป.','เครือข่ายภาคประชาชน','ข้อสั่งการ','เหตุสงสัย','หนังสือจากหน่วยงานอื่น','สื่อสารองค์กร/Facebook'];
  function registryChannelKey(channel){return channelKey(channel).replace(/[./]/g,'')}
  function requiresCaseAdminRegistry(channel,region){
    if(region!=='ส่วนกลาง')return false;
    const key=registryChannelKey(channel);
    return CENTRAL_KBK_CHANNELS.some(item=>{const approved=registryChannelKey(item);return key===approved||key.startsWith(approved)||approved.startsWith(key)});
  }
  function initialRoute(channel,region){
    if(isCentralRegistryChannel(channel,region))return ['central-registry-clerk','case-admin-clerk','admin','officer'];
    if(REGIONAL_REGIONS.includes(region)&&(channelKey(channel)==='จดหมาย'||isM62Channel(channel)))return ['regional-clerk','regional-officer'];
    if(channelKey(channel)===channelKey('สายด่วน 1206'))return ['officer','admin','officer'];
    if(REGIONAL_REGIONS.includes(region)&&isRegionalPhoneChannel(channel))return ['regional-officer','regional-clerk','regional-officer'];
    if(requiresCaseAdminRegistry(channel,region))return ['case-admin-clerk','admin','officer'];
    if(REGIONAL_REGIONS.includes(region)&&isRegionalChannel(channel))return ['regional-clerk','regional-officer'];
    return ['admin','officer'];
  }
  function postScreeningRoute(channel,region){
    return REGIONAL_REGIONS.includes(region)&&isRegionalChannel(channel)
      ?['regional-director','regional-officer']
      :['center','division','officer'];
  }
  function routeAfterNewScreening(channel,region){
    if(channelKey(channel)===channelKey('สายด่วน 1206'))return {owner:'admin',stage:'admin-registry',status:'รอธุรการ ศรร. ออกเลขรับ ศรร. และมอบหมาย'};
    if(REGIONAL_REGIONS.includes(region)&&isRegionalPhoneChannel(channel))return {owner:'regional-clerk',stage:'regional-registry',status:'รอธุรการประจำเขตออกเลขทะเบียนรับ'};
    return null;
  }
  function intakeWorkflow(item){
    const registry=item.registry||{};
    if(isCentralRegistryChannel(item.channel,item.region)&&!registry.office)return {owner:'central-registry-clerk',stage:'central-registry',status:'รอธุรการสารบรรณกลาง ชั้น 14 บันทึกเลขรับ',complete:false};
    if(isCentralRegistryChannel(item.channel,item.region)&&!registry.kbk)return {owner:'case-admin-clerk',stage:'case-admin-registry',status:'รอธุรการ กบค. ออกเลขรับ กบค.',complete:false};
    if(channelKey(item.channel)===channelKey('สายด่วน 1206')&&!registry.srr)return {owner:'officer',stage:'pre-registry-screening',status:'รอเจ้าหน้าที่รับเรื่องตรวจเรื่องซ้ำก่อนส่งลงรับ ศรร.',complete:false};
    if(REGIONAL_REGIONS.includes(item.region)&&isRegionalPhoneChannel(item.channel)&&!registry.region)return {owner:'regional-officer',stage:'pre-registry-screening',status:'รอเจ้าหน้าที่รับเรื่องประจำเขตตรวจเรื่องซ้ำก่อนส่งลงรับเขต',complete:false};
    if(isRegionalChannel(item.channel)&&REGIONAL_REGIONS.includes(item.region)&&!registry.region)return {owner:'regional-clerk',stage:'regional-registry',status:'รอธุรการประจำเขตออกเลขทะเบียนรับ',complete:false};
    if(isRegionalChannel(item.channel)&&REGIONAL_REGIONS.includes(item.region))return {owner:'regional-officer',stage:'regional-officer',status:'รอเจ้าหน้าที่รับเรื่องประจำเขตกลั่นกรองและพิจารณา',complete:false};
    if(requiresCaseAdminRegistry(item.channel,item.region)&&!registry.kbk)return {owner:'case-admin-clerk',stage:'case-admin-registry',status:'รอธุรการ กบค. ออกเลขรับ กบค.',complete:false};
    return {owner:'admin',stage:'admin-registry',status:'รอธุรการ ศรร. ออกเลขรับ ศรร. และมอบหมาย',complete:false};
  }
  const NACC_TRANSFER_REASONS = window.ECMISActivity4DocumentRules?.FORM3_NACC_REASONS||[];
  const naccReasonLabel = id => NACC_TRANSFER_REASONS.find(reason=>reason.id===id)?.label||id;
  ...
  const CASES = [
    {id:'seed-admin-registry-letter-002',allocationStatus:'pending',assignedOfficer:'',anonymousDetected:false,confidentialityRequested:false,complainant:'นายธนกฤต วัฒนกิจ',citizenId:'1-1001-00678-90-1',id4:'8901',phone:'0810008901',email:'thanakrit.demo@example.com',address:'88 ถนนราชดำเนิน แขวงบวรนิเวศ เขตพระนคร กรุงเทพมหานคร 10200',subject:'ขอให้ตรวจสอบการจัดซื้ออุปกรณ์สำนักงาน',agency:'สำนักงานบริหารงานส่วนกลางตัวอย่าง',channel:'จดหมาย',province:'กรุงเทพมหานคร',region:'ส่วนกลาง',registry:{office:'0142/2569',kbk:'0087/2569'},received:'10 สิงหาคม 2569 14:20 น.',receivedAt:'2026-08-10T14:20:00+07:00',type:'การจัดซื้อจัดจ้าง',place:'สำนักงานบริหารงานส่วนกลางตัวอย่าง กรุงเทพมหานคร',detail:'ผู้ร้องขอให้ตรวจสอบกระบวนการจัดซื้ออุปกรณ์สำนักงานและการกำหนดคุณลักษณะเฉพาะ',damage:'อาจเกิดความเสียหายต่องบประมาณของทางราชการ',request:'ขอให้ตรวจสอบข้อเท็จจริงและแจ้งผลตามช่องทางที่ระบุ',attachments:[{name:'หนังสือร้องเรียนตัวอย่าง.pdf',type:'PDF',pages:3,size:'620 KB',desc:'หนังสือร้องเรียนและเอกสารประกอบตัวอย่าง'}]},
    {id:'seed-admin-registry-m62-003',allocationStatus:'pending',assignedOfficer:'',anonymousDetected:false,confidentialityRequested:false,complainant:'สำนักงาน ป.ป.ช. ประจำจังหวัดนนทบุรี',citizenId:'',id4:'',phone:'',email:'',address:'จังหวัดนนทบุรี',subject:'ส่งเรื่องกล่าวหาให้ดำเนินการตามมาตรา 62',agency:'หน่วยงานราชการส่วนท้องถิ่นตัวอย่าง',channel:'ม.62 (ป.ป.ช.)',province:'นนทบุรี',region:'ส่วนกลาง',registry:{office:'0143/2569',kbk:'0088/2569'},received:'10 สิงหาคม 2569 13:45 น.',receivedAt:'2026-08-10T13:45:00+07:00',type:'เรื่องส่งจากสำนักงาน ป.ป.ช. ตามมาตรา 62',place:'จังหวัดนนทบุรี',detail:'สำนักงาน ป.ป.ช. ส่งเรื่องกล่าวหาพร้อมเอกสารประกอบเพื่อให้สำนักงาน ป.ป.ท. ตรวจสอบอำนาจและดำเนินการตามขั้นตอน',damage:'อยู่ระหว่างการตรวจสอบข้อเท็จจริง',request:'ตรวจรับเอกสาร ออกเลขรับ ศรร. และมอบหมายเจ้าหน้าที่ตรวจสอบ',attachments:[{name:'หนังสือนำส่งและสำนวนตัวอย่าง.pdf',type:'PDF',pages:12,size:'2.4 MB',desc:'หนังสือนำส่งและเอกสารสำนวนตัวอย่างจากสำนักงาน ป.ป.ช.'}]},
    {id:'seed-central-registry-letter-001',allocationStatus:'pending',assignedOfficer:'',anonymousDetected:false,confidentialityRequested:false,complainant:'นางสาวกมลชนก สุขใจ',citizenId:'1-1001-00234-56-7',id4:'4567',phone:'0892345678',email:'kamonchanok.s@example.com',address:'99/12 ถนนพหลโยธิน แขวงพญาไท เขตพญาไท กรุงเทพมหานคร 10400',subject:'ร้องเรียนการจัดซื้อครุภัณฑ์โดยไม่เปิดเผยราคากลาง',agency:'สำนักงานบริหารโครงการส่วนกลาง',channel:'จดหมาย',province:'กรุงเทพมหานคร',region:'ส่วนกลาง',registry:{},received:'4 สิงหาคม 2569 10:30 น.',type:'การจัดซื้อจัดจ้าง',place:'สำนักงานบริหารโครงการส่วนกลาง กรุงเทพมหานคร',detail:'ผู้ร้องขอให้ตรวจสอบการจัดซื้อครุภัณฑ์สำนักงานซึ่งไม่พบการเปิดเผยราคากลางและรายละเอียดการคัดเลือกผู้ขายตามขั้นตอน',damage:'อาจเกิดความเสียหายต่องบประมาณของทางราชการ',request:'ขอให้ตรวจสอบข้อเท็จจริงและดำเนินการตามอำนาจหน้าที่',attachments:[{name:'หนังสือร้องเรียนและเอกสารประกอบ.pdf',type:'PDF',pages:4,size:'1.1 MB',desc:'หนังสือร้องเรียนพร้อมสำเนาเอกสารการจัดซื้อ'}]},
    {id:'ECMIS-2569-000184',trackingYear:'690001',trackingCode:'5424',assignedOfficer:'คุณสุพจน์',anonymousDetected:false,confidentialityRequested:false,complainant:'นายสมชาย ใจดี',citizenId:'1-1001-00123-45-6',id4:'1234',phone:'0812341234',email:'somchai.jaidee@gmail.com',address:'123/45 ถนนสุขุมวิท แขวงคลองเตย เขตคลองเตย กรุงเทพมหานคร 10110',citizenId:'1-1001-00123-45-6',subject:'ร้องเรียนการจัดซื้อวัสดุสำนักงานไม่เป็นไปตามระเบียบ',agency:'สำนักงานจัดการทรัพย์สินภาครัฐ',channel:'Website',province:'กรุงเทพมหานคร',region:'ส่วนกลาง',registry:{srr:'0102/2569'},received:'3 สิงหาคม 2569 09:10 น.',type:'การจัดซื้อจัดจ้าง',place:'สำนักงานจัดการทรัพย์สินภาครัฐ กรุงเทพมหานคร',detail:'ผู้ถูกร้องมีพฤติการณ์จัดซื้อวัสดุสำนักงานโดยไม่ปฏิบัติตามระเบียบและอาจเอื้อประโยชน์แก่ผู้เสนอราคารายหนึ่ง',damage:'เกิดความเสียหายต่องบประมาณของหน่วยงาน',request:'ขอให้ตรวจสอบกระบวนการจัดซื้อและผู้เกี่ยวข้อง',attachments:[{name:'ใบเสนอราคา.pdf',type:'PDF',pages:2,size:'245 KB',desc:'ใบเสนอราคาจากผู้เสนอราคา 1 ราย'},{name:'สำเนาประกาศจัดซื้อ.pdf',type:'PDF',pages:3,size:'890 KB',desc:'ประกาศจัดซื้อวัสดุสำนักงาน ประจำปีงบประมาณ 2569'},{name:'ภาพถ่ายเอกสาร.jpg',type:'ภาพถ่าย',pages:1,size:'1.2 MB',desc:'ภาพถ่ายเอกสารการจัดซื้อและตู้ปิดประกาศ'}]},
    ...
    {id:'ECMIS-2569-000180',trackingYear:'690005',trackingCode:'3198',assignedOfficer:'คุณสุธาทิพย์',anonymousDetected:true,confidentialityRequested:false,complainant:'ไม่ระบุข้อมูลผู้ร้อง',id4:'',subject:'แจ้งเบาะแสการเรียกรับผลประโยชน์ในการอนุมัติใบอนุญาต',agency:'หน่วยงานท้องถิ่นตัวอย่าง',channel:'Website',province:'นนทบุรี',region:'ส่วนกลาง',registry:{srr:'0107/2569'},received:'2 สิงหาคม 2569 11:20 น.',type:'เรียกรับผลประโยชน์',place:'จังหวัดนนทบุรี',detail:'ผู้ร้องแจ้งพฤติการณ์โดยไม่ระบุข้อมูลส่วนตัว',damage:'ประชาชนอาจได้รับความเสียหาย',request:'ขอให้ตรวจสอบข้อเท็จจริง',attachments:[{name:'หลักฐานประกอบ.pdf',type:'PDF',pages:1,size:'320 KB',desc:'หลักฐานประกอบการแจ้งเบาะแส'}]}
  ];
  const A4_DEMO_START_UNITS=['ส่วนกลาง','เขต 1','เขต 2','เขต 3','เขต 4','เขต 5','เขต 6','เขต 7','เขต 8','เขต 9'];
  const A4_DEMO_START_TYPES=['การจัดซื้อจัดจ้าง','ใช้อำนาจโดยมิชอบ','เรียกรับผลประโยชน์','เบิกจ่ายงบประมาณ','ละเว้นการปฏิบัติหน้าที่'];
  function buildA4DemoStartCases(){
    const cases=[];
    A4_DEMO_START_UNITS.forEach((region,regionIndex)=>{
      for(let slot=0;slot<3;slot+=1){
        const sequence=(regionIndex*3)+slot+1;
        const centralChannels=['Website','ทางรัฐ','จดหมาย'];
        const regionalChannels=['Walk-In','จดหมาย','เบอร์โทร ป.ป.ท. เขต 1-9'];
        const channel=(region==='ส่วนกลาง'?centralChannels:regionalChannels)[slot];
        const trackingYear=`69${String(300+sequence).padStart(4,'0')}`;
        const trackingCode=String(6000+sequence);
        const type=A4_DEMO_START_TYPES[(sequence-1)%A4_DEMO_START_TYPES.length];
        cases.push({
          id:`demo-start-a4-${String(sequence).padStart(3,'0')}`,
          allocationStatus:'pending',
          trackingYear,
          trackingCode,
          assignedOfficer:'',
          anonymousDetected:slot===2&&regionIndex%3===0,
          confidentialityRequested:slot===1,
          complainant:slot===2&&regionIndex%3===0?'ไม่ระบุข้อมูลผู้ร้อง':`ผู้ร้องสำหรับนำเสนอ ${sequence}`,
          citizenId:slot===2&&regionIndex%3===0?'':`1-1001-${String(10000+sequence).padStart(5,'0')}-00-${sequence%10}`,
          id4:slot===2&&regionIndex%3===0?'':String(7000+sequence).slice(-4),
          phone:`08${String(10000000+sequence).slice(-8)}`,
          email:`demo.start.${sequence}@example.com`,
          address:`ที่อยู่ตัวอย่างสำหรับการนำเสนอ ${sequence} ${region}`,
          subject:`Seed จุดเริ่มต้นระบบรับเรื่อง — ${type} (${region}) ชุดที่ ${slot+1}`,
          agency:`หน่วยงานถูกร้องตัวอย่าง ${sequence}`,
          channel,
          province:region==='ส่วนกลาง'?'กรุงเทพมหานคร':`จังหวัดตัวอย่าง ${region}`,
          region,
          registry:{},
          received:`21 สิงหาคม 2569 ${String(9+slot).padStart(2,'0')}:${String(regionIndex*3).padStart(2,'0')} น.`,
          receivedAt:`2026-08-21T${String(9+slot).padStart(2,'0')}:${String(regionIndex*3).padStart(2,'0')}:00+07:00`,
          type,
          place:`สถานที่เกิดเหตุตัวอย่าง ${sequence} ${region}`,
          detail:`ข้อเท็จจริงตั้งต้นสำหรับทดลองรับเรื่องและดำเนินกระบวนงาน รายการที่ ${sequence}`,
          damage:`ความเสียหายตัวอย่างสำหรับการนำเสนอ รายการที่ ${sequence}`,
          request:'ขอให้ตรวจสอบข้อเท็จจริงและดำเนินการตามอำนาจหน้าที่',
          attachments:[{name:`เอกสารตั้งต้น-${sequence}.pdf`,type:'PDF',pages:2+(slot%2),size:`${400+sequence} KB`,desc:'เอกสารจำลองสำหรับเริ่มกระบวนงานรับเรื่อง'}]
        });
      }
    });
    return cases;
  }
  CASES.push(...(window.ECMISActivity4DocumentRules?.NACC_API_SEEDS||[]));
  const activeSeeds=window.ECMISActivity4SeedPolicy?.selectActiveSeeds?.(CASES);
  if(Array.isArray(activeSeeds))CASES.splice(0,CASES.length,...activeSeeds);
  globalThis.ECMISActivity4SeedRegistry = Object.freeze({
    ids: Object.freeze(CASES.map(item => item.id)),
    createStates: () => Object.fromEntries(CASES.map(item => [item.id, initialState(item)]))
  });
  ...
  function registryPlan(channel,region){
    return window.ECMISActivity4DocumentRules?.registryPlan(channel,region) ?? (()=>{
  ...
  function registryReceiptNumber(c){const r=c.registry||{};return r.srr||r.kbk||r.region||r.office||''}
  function caseReferenceForRole(c,role){
    if(!c)return 'รอออกเลขรับเรื่อง';
    const registry=c.registry||{};
    if(role==='central-registry-clerk')return registry.office||'รอออกเลขรับเรื่อง';
    if(role==='case-admin-clerk')return registry.kbk||registry.office||'รอออกเลขรับเรื่อง';
    if(role==='admin')return registry.srr||c.trackingYear||'รอออกเลขรับเรื่อง';
    if(isRegionalRole(role))return registry.region||c.trackingYear||'รอออกเลขรับเรื่อง';
    return registry.srr||c.trackingYear||'รอออกเลขรับเรื่อง';
  }
  function caseDisplayId(c,role='admin'){return caseReferenceForRole(c,role)}
  function roleCaseSearchText(state,role){
    const c=state?.caseData||state||{};
    const tracking=['central-registry-clerk','case-admin-clerk'].includes(role)?[]:[c.trackingYear];
    return [caseReferenceForRole(c,role),...tracking,c.complainant,c.subject,c.agency,c.channel,c.province,c.region,state?.workflow?.status,state?.documentData?.documentSubject].filter(Boolean).join(' ').toLowerCase();
  }
  function hasCompletedAdminAssignment(state){
    const hasSrrReceipt=Boolean(String(state?.caseData?.registry?.srr||'').trim());
    const hasAssignedOfficer=Boolean(String(state?.documentData?.assignedOfficer||'').trim());
    const hasAssignmentHistory=(state?.assignmentHistory||[]).some(entry=>['assign','reassign'].includes(entry.action)||/\[(มอบหมาย|มอบหมายใหม่)\]|มอบหมายให้/.test(entry.text||''));
    return hasSrrReceipt&&hasAssignedOfficer&&hasAssignmentHistory;
  }
  function roleCanSeeState(state,{role='admin',activeRegion='',adminQueue='unassigned'}={}){
    const c=state?.caseData||{},w=state?.workflow||{},d=state?.documentData||{};
    const regional=isRegionalRole(role);
    if(regional&&(c.region!==activeRegion||!isRegionalChannel(c.channel)))return false;
    if(role==='admin'){
      const assigned=hasCompletedAdminAssignment(state);
      if(adminQueue==='assigned')return assigned&&c.region==='ส่วนกลาง';
      return w.owner==='admin'&&['admin','admin-registry','admin-assign'].includes(w.stage)&&!assigned;
    }
    const policies={
      'central-registry-clerk':['central-registry-clerk','central-registry'],
      'case-admin-clerk':['case-admin-clerk','case-admin-registry'],
      officer:['officer',['pre-registry-screening','officer','duplicate-closed','officer-finalize-screening','officer-dispatch','notification-confirmed']],
      'regional-clerk':['regional-clerk','regional-registry'],
      'regional-officer':['regional-officer',['pre-registry-screening','regional-officer','duplicate-closed','officer-finalize-screening','officer-dispatch','notification-confirmed']],
      'regional-director':['regional-director','regional-director'],
      center:['center','center'],division:['division',['division','division-order']],acting:['division',['division','division-order']],
      anonymous:['anonymous','anonymous-box'],activity7:['activity7','activity7']
    };
    const policy=policies[role];
    if(!policy)return false;
    const stages=Array.isArray(policy[1])?policy[1]:[policy[1]];
    return w.owner===policy[0]&&stages.includes(w.stage);
  }
  function dashboardBucket(state){
    if(state?.workflow?.complete)return 'done';
    return ['central-registry','case-admin-registry','admin','admin-registry','admin-assign','regional-registry','pre-registry-screening'].includes(state?.workflow?.stage)?'pending':'review';
  }
  function normalizeCitizenId(value=''){return String(value).replace(/\D/g,'')}
  function formatCitizenId(value=''){
    const digits=normalizeCitizenId(value).slice(0,13);
    let formatted='';
    for(let index=0;index<digits.length;index++){
      if(index===1||index===5||index===10||index===12)formatted+='-';
      formatted+=digits[index];
    }
    return formatted;
  }
  function bindCitizenIdInput(input){
    if(!input)return;
    const format=()=>{input.value=formatCitizenId(input.value)};
    format();
  ...
  function positionPopoverNear(trigger,popover,fallbackW,fallbackH){
    const rect=trigger.getBoundingClientRect();
    const margin=8;
    const popW=popover.offsetWidth||fallbackW||290;
    const popH=popover.offsetHeight||fallbackH||400;
    let left=rect.left;
    if(left+popW>window.innerWidth-margin)left=Math.max(margin,window.innerWidth-popW-margin);
    let top=rect.bottom+8;
    if(top+popH>window.innerHeight-margin)top=Math.max(margin,rect.top-popH-8);
  ...
  function isIncorrectWebsiteTestCase(caseData,trackingField){
    return String(caseData?.[trackingField]||'')==='690004'&&caseData?.channel==='Website'&&caseData?.subject==='ร้องเรียนทดสอบ สังกัดกระทรวงศึกษาธิการ';
  }
  function readStore(){
    try{
      let workspaceRaw=localStorage.getItem(STORAGE_KEY);
      const demoRaw=localStorage.getItem('ecmis-demo-cases');
      const versionRaw=localStorage.getItem(STORE_SCHEMA_VERSION_KEY);
      let originalStore=JSON.parse(workspaceRaw||'{}');
      if(!originalStore||Array.isArray(originalStore)||typeof originalStore!=='object')return {};
      const seedCompaction=window.ECMISActivity4SeedPolicy?.compactWorkspaceStore(originalStore);
      if(seedCompaction?.removed){
        const compactRaw=JSON.stringify(seedCompaction.store);
        try{localStorage.setItem(STORAGE_KEY,compactRaw);workspaceRaw=compactRaw;originalStore=seedCompaction.store}catch{return originalStore}
      }
      let store=JSON.parse(JSON.stringify(originalStore));
      let workspaceChanged=false;
      const reasonMigration=window.ECMISActivity4DocumentRules?.migrateForm3Reasons(store);
      if(reasonMigration?.changed){store=reasonMigration.store;workspaceChanged=true}
      Object.keys(store).forEach(id=>{
        const caseData=store[id]?.caseData||store[id];
        if(isIncorrectWebsiteTestCase(caseData,'trackingYear')){delete store[id];workspaceChanged=true;return}
        if(Number(versionRaw)<2&&['Website','ทางรัฐ'].includes(caseData?.channel)){
          const registry=caseData.registry||{};
          if(registry.office!==undefined||registry.kbk!==undefined||registry.region!==undefined){
            caseData.registry={...registry};
            delete caseData.registry.office;delete caseData.registry.kbk;delete caseData.registry.region;
            workspaceChanged=true;
          }
          const workflow=store[id]?.workflow;
          if(workflow?.owner==='case-admin-clerk'&&workflow.stage==='case-admin-registry'&&!workflow.complete){
            Object.assign(workflow,{owner:'admin',stage:'admin-registry',status:'รอธุรการ ศรร. ออกเลขรับ ศรร. และมอบหมาย'});
            workspaceChanged=true;
          }
        }
        const record=store[id];
        if(record?.workflow&&record?.documentData){
          const assignedOfficer=String(record.documentData.assignedOfficer||record.caseData?.assignedOfficer||'').trim();
          if(!record.workflow.intakeOwner&&assignedOfficer){
            const role=record.workflow.owner==='regional-officer'||record.workflow.stage==='regional-officer'?'regional-officer':'officer';
            record.workflow.intakeOwner={role,officer:assignedOfficer};
            workspaceChanged=true;
          }
          if(record.documentData.duplicatePackagePreparedAt===undefined){record.documentData.duplicatePackagePreparedAt='';workspaceChanged=true}
          if(record.documentData.duplicatePackagePreparedBy===undefined){record.documentData.duplicatePackagePreparedBy='';workspaceChanged=true}
        }
      });
      let demo=[];
      let demoIsValid=true;
      try{
        const parsedDemo=JSON.parse(demoRaw||'[]');
        demo=Array.isArray(parsedDemo)?parsedDemo:[];
      }catch{demoIsValid=false}
      const cleanedDemo=demo.filter(record=>!isIncorrectWebsiteTestCase(record,'trackingNumber'));
      const demoChanged=demoIsValid&&cleanedDemo.length!==demo.length;
      const versionChanged=Number(versionRaw)<STORE_SCHEMA_VERSION;
      if(workspaceChanged||demoChanged||versionChanged){
        try{
          const backupKey=`${STORE_MIGRATION_BACKUP_PREFIX}-${Date.now()}`;
          localStorage.setItem(backupKey,JSON.stringify({workspace:originalStore,demoCases:demoIsValid?demo:null,schemaVersion:versionRaw}));
          if(workspaceChanged)localStorage.setItem(STORAGE_KEY,JSON.stringify(store));
          if(demoChanged)localStorage.setItem('ecmis-demo-cases',JSON.stringify(cleanedDemo));
          if(versionChanged)localStorage.setItem(STORE_SCHEMA_VERSION_KEY,String(STORE_SCHEMA_VERSION));
        }catch(error){
          try{if(workspaceRaw===null)localStorage.removeItem(STORAGE_KEY);else localStorage.setItem(STORAGE_KEY,workspaceRaw)}catch{}
          try{if(demoRaw===null)localStorage.removeItem('ecmis-demo-cases');else localStorage.setItem('ecmis-demo-cases',demoRaw)}catch{}
          try{if(versionRaw===null)localStorage.removeItem(STORE_SCHEMA_VERSION_KEY);else localStorage.setItem(STORE_SCHEMA_VERSION_KEY,versionRaw)}catch{}
  ...
  const DOCUMENT_SOURCE_VERSION='official-forms-1-01-to-1-14-v3';
  const ALL_DOCUMENTS_ID='all-documents';
  const REVIEW_RECORD_ID=window.ECMISActivity4DocumentRules?.REVIEW_RECORD_ID||'ecmis-review-record';
  const LEGACY_REVIEW_RECORD_ID=window.ECMISActivity4DocumentRules?.LEGACY_REVIEW_RECORD_ID||'pcms-review-record';
  const REVIEW_RECORD_NAME='แบบบันทึกการพิจารณาและกลั่นกรองเรื่องร้องเรียน/เบาะแส';
  const OFFICIAL_DOCUMENTS={
    'back-cover':{name:'ใบปกเอกสารลับ',pages:1,register:'none',numbered:false,counted:false},
    ...
    '1-03':{name:'แบบบันทึกการตรวจสอบเรื่องร้องเรียน',pages:2,register:'pending'},
    ...
    '1-07':{name:'แบบหนังสือแจ้งผู้ร้องเรียน กรณีสำนักงาน ป.ป.ช. ส่งเรื่องให้สำนักงาน ป.ป.ท. ดำเนินการตามมาตรา 62',pages:1,register:'case-admin'},
    ...
    ,'58/2-03':{name:'ตัวอย่างหนังสือบันทึกย่อแจ้งผู้ร้องเรียนทราบ ศรร.',pages:2,register:'pending'}
    ,'58/2-04':{name:'ตัวอย่างหนังสือบันทึกย่อแจ้งผู้ร้องเรียนทราบ ปปท. เขต',pages:2,register:'pending'}
    ,'58/2-05':{name:'ตัวอย่างหนังสือส่งเอกสารเรื่องร้องเรียนไป ปปท. เขต',pages:3,register:'pending'}
  };
  function initialState(item){
    const state={caseData:{...item,registry:{...(item.registry||{})}},documentData:{decision:'18/1ก',naccTransferKind:'',reliefOperationKind:'',centralSendsDossierToRegion:null,reasons:[],legacyReasonLabels:[],needsReasonReview:false,naccOtherChecked:false,naccOtherReason:'',anonymous:Boolean(item.anonymousDetected??item.anonymous),documentSubject:item.subject||'',assignedOfficer:item.assignedOfficer||'',officerOpinion:'',proposedRegion:item.region||'',reviewRoute:'center',centerDecision:'',centerOpinion:'',documentPack:{caseType:'18/1ก',sourceVersion:DOCUMENT_SOURCE_VERSION,pages:{complainant:10,'1-03':2},steps:{}},officerSignature:{signed:false,signerName:'',signerRole:'เจ้าหน้าที่รับเรื่อง',signedAt:'',signatureId:''},centerSignature:{signed:false,signerName:'',signerRole:'ผู้อำนวยการศูนย์รับเรื่องร้องเรียน',signedAt:'',signatureId:''},divisionOpinion:'',divisionSignature:{signed:false,signerName:'',signerRole:'ผู้อำนวยการกองบริหารคดี',signedAt:'',signatureId:''},outgoingRegisters:{srr:{number:'',date:''},caseAdmin:{number:'',date:''}},internalLetterNo:'',internalLetterDate:'',caseNumber:'',caseNumberIssuedAt:'',caseNumberIssuedBy:'',publicStatus:'',approvedAt:'',approvedBy:'',actingOfficer:'',actingOrder:'',assignedOfficer:'',backupOfficer:'',previousOfficer:'',previousBackupOfficer:'',adminNote:'',screening:{decision:'',decidedAt:'',decidedBy:'',linkedCaseId:'',linkedTracking:'',note:'',additionalInfo:'',reportedToSupervisor:null},absenceReasonType:'',absenceNote:'',notAcceptReason:'',notAcceptOtherChecked:false,notAcceptOtherReason:'',dispatchLetterNo:'',dispatchLetterDate:'',dispatchSendMethod:'EMS',dispatchEms:'',dispatchSentDate:'',dispatchProofName:'',dispatchDestinationUnit:item.region||'',dispatchNote:'',dispatchConfirmedAt:'',naccIncomingLetterNo:'',naccIncomingLetterDate:'',naccBlackTrackingNo:'',naccRecipientName:'',naccProvince:'',naccBoardNo:'',naccBoardDate:'',naccBoardAgenda:'',naccBoardAttachmentPages:'',naccDossierAttachmentPages:'',naccReportYear:'',accusedName:'',accusedPositionAffiliation:'',complaintLetterDate:'',recipientName:'',contactOfficeName:'',contactOfficeAddress:'',contactOfficePhone:'',contactOfficeFax:'',ownerPosition:'',naccLetterNo:'',naccLetterDate:'',naccSendMethod:'EMS',naccEms:'',naccSentDate:'',naccBoardNote:'',naccProofName:'',naccNotified:true},workflow:intakeWorkflow(item),assignmentHistory:[],decisionHistory:[],anonymousHistory:[],documentVersions:[]};
    return isNaccApiCase(state)?window.ECMISActivity4DocumentRules.normalizeNaccApiIntake(state):state;
  }
  ...
  function isCaseNumberEligible(documentData){return (documentData?.screening?.officerOpinion||documentData?.screening?.decision)==='new'&&['18/1ก','18/1ข','18/4'].includes(documentData?.decision)}
  ...
  function signedDocumentSnapshot(state,signerType,signature){
    const documentIds=window.ECMISActivity4DocumentRules?.signedSnapshotDocumentIds?.(state)||[REVIEW_RECORD_ID];
    const role=signerType==='officer'?'officer':signerType==='center'?'center':'division';
    return {
      caseId:state.caseData.id,
      decision:state.documentData.decision,
      signerType,
      signature:JSON.parse(JSON.stringify(signature)),
      capturedAt:signature.signedAt,
      documents:documentIds.map(documentId=>({documentId,html:documentPaper(state,documentId,role)}))
    };
  }
  function recordSignedDocumentVersion(state,signerType,signature){
    const snapshotState=JSON.parse(JSON.stringify(state));
    snapshotState.documentData[`${signerType}Signature`]=signature;
    const result=window.ECMISActivity4DocumentRules?.recordDocumentSignature(snapshotState,{
      signerType,
      signature,
      renderVersion:DOCUMENT_SOURCE_VERSION,
      signedSnapshot:signedDocumentSnapshot(snapshotState,signerType,signature)
    });
    if(!result?.valid)return false;
    Object.assign(state.documentData,result.record.documentData);
    state.documentVersions=result.record.documentVersions;
    return true;
  }
  function createCorrectionVersion(state,actorType,actorName,reason){
    const normalized=window.ECMISActivity4DocumentRules?.normalizeLegacyDocumentEvidence(state);
    const correctionState=normalized?.record||state;
    const result=window.ECMISActivity4DocumentRules?.returnDocumentForCorrection(correctionState,{
      actorType,
      actorName,
      reason,
  ...
  function transitionCase(state,action,payload,role,name){
    let regionVal='';
    try{
      if(role.startsWith('regional-')){
        const params=new URLSearchParams(location.search);
        regionVal=params.get('region')||sessionStorage.getItem('ecmis-a4-regional-region')||localStorage.getItem('ecmis-a4-regional-region')||'';
        if(!regionVal) regionVal=state?.caseData?.region||'';
      }
    }catch(e){ regionVal=state?.caseData?.region||''; }
    const result=window.ECMISActivity4DocumentRules?.transitionCase(state,action,payload,{role,name,at:new Date().toISOString(),region:regionVal});
    if(!result?.ok)return result||{ok:false,code:'TRANSITION_RULES_UNAVAILABLE',errors:['TRANSITION_RULES_UNAVAILABLE'],effects:[]};
    Object.assign(state.caseData,result.record.caseData);
    Object.assign(state.documentData,result.record.documentData);
    Object.assign(state.workflow,result.record.workflow);
    state.assignmentHistory=result.record.assignmentHistory;
    state.decisionHistory=result.record.decisionHistory;
    state.documentVersions=result.record.documentVersions;
    return result;
  }
  function transitionBlocked(result,title='ไม่สามารถเปลี่ยนสถานะ'){
    const code=result?.code||'UNKNOWN_TRANSITION';
    notify('warning',title,`สถานะหรือสิทธิ์ปัจจุบันไม่อนุญาตให้ดำเนินการ (${code})`);
  }
  function signedDocumentLocked(state){
    return window.ECMISActivity4DocumentRules?.isSignedDocumentLocked(state)===true;
  }
  function lockSignedPaper(state,paper){
    if(!paper||!signedDocumentLocked(state))return;
    paper.querySelectorAll('[contenteditable]').forEach(element=>element.setAttribute('contenteditable','false'));
    paper.classList.remove('doc-edit-mode');
  }
  function lockSignedDocumentSources(state){
    const locked=section=>window.ECMISActivity4DocumentRules?.documentSourceSectionLocked(state,section)===true;
    if(locked('officer')){
      const selector='.working-copy-section input,.working-copy-section textarea,.working-copy-section select,.official-document-fields-section input,.official-document-fields-section textarea,.official-document-fields-section select,input[name="decision"],input[name="naccTransferKind"],input[name="reliefOperationKind"],input[name="centralSendsDossierToRegion"],input[name="naccReason"],input[name="route"],#anonymous,#documentSubject,#officerOpinion,#proposedRegion,#notAcceptReason,#naccOtherChecked,#naccOtherReason,#absenceReasonType,#absenceNote,#actingOfficer,#actingOrder,[data-pack-page]';
      $$(selector).forEach(element=>element.disabled=true);
    }
    if(locked('center')){$$('input[name="centerDecision"],#centerOpinion').forEach(element=>element.disabled=true)}
    if(locked('division')){$$('#divisionOpinion,#actingOrder').forEach(element=>element.disabled=true)}
  }
    ...
    return Swal.fire({
      ...
      html:`<div class="signature-confirm-summary">
        ...
        <label class="signature-attestation" for="ecmisSignatureAttestation">
          <input id="ecmisSignatureAttestation" type="checkbox" aria-describedby="ecmisSignatureAttestationHint">
        ...
        <p class="signature-attestation-hint" id="ecmisSignatureAttestationHint">ต้องติ๊กคำรับรองก่อนยืนยันลงนาม</p>
      ...
      didOpen:()=>{
        const checkbox=document.getElementById('ecmisSignatureAttestation');
        const hint=document.getElementById('ecmisSignatureAttestationHint');
        const sync=()=>{hint.textContent=checkbox.checked?'พร้อมลงนามและบันทึกหลักฐาน':'ต้องติ๊กคำรับรองก่อนยืนยันลงนาม';hint.classList.toggle('ready',checkbox.checked)};
        checkbox.addEventListener('input',sync);checkbox.addEventListener('change',sync);sync();checkbox.focus();
      },
      preConfirm:()=>{
  ...
  function signatureCard(state,type,roleContext){
    ...
    const screeningPending=isOfficer&&!['new','suspected_duplicate'].includes(String(d.screening?.officerOpinion||d.screening?.decision||'').trim());
    const officerOpinionMissing=isOfficer&&!String(d.officerOpinion||'').trim();
    const hint=isOfficer&&screeningPending?'ต้องยืนยันกลั่นกรองเรื่องซ้ำก่อนลงนาม':isOfficer&&officerOpinionMissing?'ต้องกรอกความเห็นเจ้าหน้าที่ก่อนลงนาม':'แบบบันทึกการพิจารณาและกลั่นกรองเรื่องร้องเรียน/เบาะแส';
    const proof=signature?.signed
      ...
      :`<div class="signature-pending"><span>ผู้มีหน้าที่ลงนาม</span><strong>${escapeHtml(role)}</strong><small>${escapeHtml(hint)}</small></div><button type="button" class="ws-button primary" data-action="${action}" ${screeningPending||officerOpinionMissing?'aria-describedby="signatureHint"':''}>ลงนามเอกสาร</button>`;
    return `<section class="signature-workflow"><header><div><span>การลงนามเอกสารอิเล็กทรอนิกส์</span><h3>${escapeHtml(role)}</h3></div><b class="${signature?.signed?'signed':''}">${signature?.signed?'ลงนามแล้ว':screeningPending?'รอกลั่นกรอง':officerOpinionMissing?'รอความเห็น':'รอการลงนาม'}</b></header><div class="signature-workflow-body">${proof}</div></section>`;
  }
  function packCaseTypeOf(state){
    const d=state.documentData;
    if(d.decision==='18/1ข')return '18/1ข';
    if(d.decision==='18/4')return '18/4';
    if(d.decision==='62')return 'legacy-62';
  ...
  function officialDocumentIds(state){
    const resolved=window.ECMISActivity4DocumentRules?.resolveDocumentRequirements(state);
    if(resolved?.supported){
      return resolved.officialDocumentIds.filter(id=>![REVIEW_RECORD_ID,LEGACY_REVIEW_RECORD_ID].includes(id));
    }
    ...
    if(['18/1ก','18/1ข','18/4','legacy-62'].includes(caseType))ids.push('1-03');
    const incomingM62=isM62Channel(state.caseData.channel)||isNaccApiCase(state);
    if(caseType==='18/1ข'&&incomingM62)ids.push('1-05',...(anonymous?[]:['1-07']),'1-08');
    if(caseType==='18/4'){
      ids.push('1-04');
      if(!anonymous)ids.push('1-06');
    }
    if(caseType==='legacy-62'){
      ids.push('1-05','1-08');
      if(!anonymous)ids.push('1-07');
    }
    if(caseType==='send-nacc'){
      if(state.documentData.naccTransferKind==='return')ids.push('1-09');
      else{
  ...
  function canonicalDocumentId(id){return ({pakfa:'1-01',n103:'1-03',f104:'1-04',f105:'1-05',f106:'1-06',f107:'1-07'})[id]||id}
  function outgoingAt(state,id){
    const documentId=canonicalDocumentId(id),registers=window.ECMISActivity4OutgoingRegisters;
    if(registers){
      const slotId=documentId==='1-12'?'1-11:main':`${documentId}:main`;
      const issuance=registers.issuanceForSlot(state,slotId,registers.readLedger(localStorage));
      if(issuance&&['issued','manual-authoritative'].includes(issuance.status))return issuance.number;
    }
    return (state.documentData.outgoingNumbers||{})[documentId]||'';
  }
  function officialText(value){return escapeHtml(window.ECMISActivity4DocumentRules?.toThaiDigits(value)??String(value??''))}
  function officialPaper(html){const toThai=window.ECMISActivity4DocumentRules?.toThaiDigits||String;return String(html||'').split(/(<[^>]*>)/).map(part=>part.startsWith('<')?part:toThai(part)).join('')}
  function officialBindings(state,id){return window.ECMISActivity4DocumentRules?.renderOfficialDocumentBindings(state,id)||{}}
  function documentRegisterLabel(doc,state,documentId){
    const api=window.ECMISActivity4OutgoingRegisters;
    if(api&&state&&documentId){
      const slot=api.resolveOutgoingSlots(state).find(item=>item.documentId===documentId&&item.purpose==='main');
      if(slot){
        const ledger=api.readLedger(localStorage),issuance=slot.mode==='reference'?api.issuanceForSlot(state,slot.referenceSlotId,ledger):api.issuanceForSlot(state,slot.slotId,ledger);
        if(issuance&&['issued','manual-authoritative'].includes(issuance.status))return `${issuance.registerId?api.REGISTER_DEFINITIONS[issuance.registerId]?.label||'ทะเบียนที่เลือก':'ทะเบียนเขต'} · ${issuance.number}`;
        if(issuance?.status==='legacy-unverified')return `เลขเดิม ${issuance.number} · ต้องตรวจสอบทะเบียนก่อนจัดส่ง`;
        if(slot.mode==='reference')return 'อ้างอิงเลขหนังสือหลักจากแบบ 1-11';
        if(slot.mode==='fixed')return `${api.REGISTER_DEFINITIONS[slot.fixedRegisterId]?.label||slot.fixedRegisterId} · รอออกเลข`;
        if(slot.mode==='select')return 'รอเลือกทะเบียนและออกเลข';
        if(slot.mode==='blocked-unconfigured')return 'ทะเบียนเขตยังไม่ได้กำหนด · รอบันทึกเลขทางการของเขต';
      }
    }
    if(doc.register==='case')return 'ใช้เลขสำนวนซึ่งออกหลัง ผู้อำนวยการกองบริหารคดี ลงนาม';
  ...
  function documentPackPanel(state){
    ...
    const stepItem=(no,id)=>{const doc=OFFICIAL_DOCUMENTS[id]||{name:'เอกสารทางราชการ',register:'none'};const excluded=doc.counted===false||id==='1-01';return `<li class="pack-step"><b class="pack-step-no">${no}</b><div class="pack-step-body"><strong>${id==='back-cover'?'':'ปปท. '+id+' '}${escapeHtml(doc.name)}</strong><small>${escapeHtml(documentRegisterLabel(doc,state,id))}</small></div><button type="button" class="pack-open" data-doc-open="${id}">เปิดเอกสาร</button><span class="pack-badge ${excluded?'muted':''}">${excluded?'ไม่นับหน้า':`${doc.pages} หน้า`}</span></li>`};
  ...
  function caseIssuanceCard(state){
    ...
    return `<section class="case-issuance"><span>การออกเลขสำนวน ปี ${buddhistYear}</span><strong>ยังไม่ออกเลขสำนวน</strong><p>ระบบจะออกเลขหลัง ผู้อำนวยการกองบริหารคดี ลงนามสำเร็จ</p></section>`;
  }
  ...
  function caseAgeCard(state){
    ...
    const c=state.caseData||{},registry=c.registry||{};
    const fallback=Date.parse(c.receivedAt||'')||parseThaiDate(c.received)?.getTime()||Date.parse(d.caseNumberIssuedAt);
    const stamps=[
      registry.office&&{at:Date.parse(registry.officeAt||'')||fallback,label:'เลขรับสารบรรณกลาง ชั้น 14'},
      registry.kbk&&{at:Date.parse(registry.kbkAt||'')||fallback,label:'เลขรับ กบค.'},
      registry.srr&&{at:Date.parse(registry.srrAt||'')||fallback,label:'เลขรับ ศรร.'},
      registry.region&&{at:Date.parse(registry.regionAt||'')||fallback,label:`เลขรับ ${c.region||'สำนักงานเขต'}`}
    ].filter(item=>item&&Number.isFinite(item.at)).sort((a,b)=>a.at-b.at);
    const firstStamp=stamps[0];
    if(!firstStamp)return '';
    const startedAt=firstStamp.at;
    const ageDays=Math.max(0,Math.floor((Date.now()-startedAt)/86400000));
    const startedText=new Date(startedAt).toLocaleDateString('th-TH',{day:'numeric',month:'long',year:'numeric'});
    ...
    if(Number.isFinite(startedAt)){
      const deadline=startedAt+60*86400000;
    ...
    return `<section class="case-age"><div class="age-row"><span><strong>อายุเรื่อง</strong> ${ageDays} วัน</span><small>เริ่มนับ ${startedText} (${escapeHtml(firstStamp.label)} ซึ่งเป็นเลขรับแรกของเรื่อง)</small></div><div class="age-row"><span><strong>อายุความ</strong> ${remain.text}</span>${badge}</div>${r213}</section>`;
  }

  function getState(id){
    const store=readStore();
    const raw=store[id];
    if(raw&&typeof raw==='object'&&raw.caseData&&raw.documentData){const seed=CASES.find(c=>c.id===id);if(seed){for(const key in seed)if(raw.caseData[key]===undefined)raw.caseData[key]=seed[key];const wc=raw.documentData.workingCopy;if(wc)for(const [key] of WORKING_COPY_FIELDS)if(wc[key]===undefined&&seed[key]!==undefined)wc[key]=String(seed[key])}}
    let state=(raw&&typeof raw==='object'&&raw.caseData&&raw.documentData)?raw:initialState(CASES.find(c=>c.id===id)||CASES[0]);
    if(isNaccApiCase(state))state=window.ECMISActivity4DocumentRules.normalizeNaccApiIntake(state);
    if(!state.documentData.assignedOfficer&&state.workflow?.status!=='รอมอบหมายใหม่')state.documentData.assignedOfficer=state.caseData?.assignedOfficer||'';
    ...
    // migration: เคสเขตที่เคยบันทึก intakeOwner เป็น officer (ก่อนแก้ 20260821) ให้แก้เป็น regional-officer
    try{
      const isRegCase = String(state.caseData?.region||'').includes('เขต') && isRegionalChannel(state.caseData?.channel);
      if(isRegCase){
        let migrated=false;
        if(state.workflow?.intakeOwner?.role==='officer'){
          state.workflow.intakeOwner.role='regional-officer';
          migrated=true;
        }
        if(state.workflow?.owner==='officer' && ['officer','officer-finalize-screening','officer-dispatch','notification-confirmed'].includes(state.workflow.stage)){
          state.workflow.owner='regional-officer';
          if(state.workflow.stage==='officer') state.workflow.stage='regional-officer';
          migrated=true;
        }
        if(state.workflow?.owner==='regional-officer' && state.workflow.stage==='officer'){
          state.workflow.stage='regional-officer';
          migrated=true;
        }
        if(migrated){
          const store2=readStore();
          // write back lazily - will be saved on next saveState, but also persist now
          store2[id]=state;
          try{ writeStore(store2); }catch(e){}
        }
      }
    }catch(e){}
    state.decisionHistory=state.decisionHistory||[];
    state.documentVersions=state.documentVersions||[];
    const normalizedEvidence=window.ECMISActivity4DocumentRules?.normalizeLegacyDocumentEvidence(state);
    if(normalizedEvidence?.changed){state.documentVersions=normalizedEvidence.record.documentVersions;Object.assign(state.documentData,normalizedEvidence.record.documentData)}
    ...
    ['naccIncomingLetterNo','naccIncomingLetterDate','naccBlackTrackingNo','naccRecipientName','naccProvince','naccBoardNo','naccBoardDate','naccBoardAgenda','naccBoardAttachmentPages','naccDossierAttachmentPages','naccReportYear','accusedName','accusedPositionAffiliation','complaintLetterDate','recipientName','contactOfficeName','contactOfficeAddress','contactOfficePhone','contactOfficeFax','ownerPosition'].forEach(field=>{if(state.documentData[field]===undefined)state.documentData[field]=''});
    ...
    state.documentData.screening={decision:'',officerOpinion:'',decidedAt:'',decidedBy:'',linkedCaseId:'',linkedTracking:'',note:'',additionalInfo:'',reportedToSupervisor:null,...(state.documentData.screening||{})};
    if(!state.documentData.screening.officerOpinion&&state.documentData.screening.decision==='new')state.documentData.screening.officerOpinion='new';
    if(!state.documentData.screening.officerOpinion&&state.documentData.screening.decision==='duplicate'&&state.workflow.stage!=='duplicate-closed')state.documentData.screening.officerOpinion='suspected_duplicate';
    state.documentData.naccTransferKind=['forward','return'].includes(state.documentData.naccTransferKind)?state.documentData.naccTransferKind:'';
    state.documentData.reliefOperationKind=['central','regional'].includes(state.documentData.reliefOperationKind)?state.documentData.reliefOperationKind:'';
    state.documentData.centralSendsDossierToRegion=typeof state.documentData.centralSendsDossierToRegion==='boolean'?state.documentData.centralSendsDossierToRegion:null;
  ...
  async function showCaseNumberIssuedSuccess(state,{regional=false}={}){
    const caseNumber=String(state?.documentData?.caseNumber||'').trim();
    if(!caseNumber)return false;
    const authority=regional?'ผู้อำนวยการสำนักงาน ป.ป.ท. เขต':'ผู้อำนวยการกองบริหารคดี (ผอ.กบค.)';
    const nextOfficer=regional?'เจ้าหน้าที่รับเรื่องประจำเขตเดิม':'เจ้าหน้าที่รับเรื่องเดิม';
    const timing=regional?`ออกหลัง ${authority} อนุมัติแล้ว`:`ออกหลัง ${authority} ลงนามแล้ว`;
    const nextStep=regional?`เมื่อปิดหน้าต่าง ระบบจะส่งงานต่อไปยัง${nextOfficer}`:'ตรวจสอบเลขสำนวนก่อนดำเนินการอนุมัติและส่งงานต่อ';
    if(!window.Swal){alert(`ออกเลขสำนวนเรียบร้อยแล้ว\nเลขสำนวน ${caseNumber}\n${timing}\n${nextStep}`);return true}
    await Swal.fire({
      icon:'success',
      title:'ออกเลขสำนวนเรียบร้อยแล้ว',
      html:`<div class="case-number-success"><span>เลขสำนวน</span><strong>${escapeHtml(caseNumber)}</strong><p>${escapeHtml(timing)}</p><small>${escapeHtml(nextStep)}</small></div>`,
      confirmButtonText:regional?'ปิดและดำเนินการต่อ':'ปิด',
      confirmButtonColor:'#082b50',
      allowOutsideClick:false,
  ...
  function attachmentsOf(source){
    const value=source?.attachments;
    if(Array.isArray(value))return value;
    if(typeof value==='string')return value.trim()&&value!=='[object Object]'?[value]:[];
    if(value&&typeof value==='object'){
      if(value.name)return [value];
      return Object.values(value).filter(item=>typeof item==='string'||(item&&typeof item==='object'&&item.name));
    }
    return [];
  }
  ...
  function caseView(state){const w=state.documentData.workingCopy;if(!w)return state.caseData;const merged={};for(const key in state.caseData)merged[key]=state.caseData[key];for(const [key] of WORKING_COPY_FIELDS)if(w[key]!==undefined&&w[key]!=='')merged[key]=w[key];return merged}
  ...
  function workingCopySection(state){const c=state.caseData,w=ensureWorkingCopy(state);const cap=key=>key[0].toUpperCase()+key.slice(1);const num=registryReceiptNumber(c)||'....................';const field=(key,label,full,textarea)=>{const citizen=key==='citizenId',value=citizen?formatCitizenId(w[key]):w[key];return `<div class="ws-field${full?' ws-field-full':''}"><label for="wc${cap(key)}">${label}</label>${textarea?`<textarea id="wc${cap(key)}">${escapeHtml(value)}</textarea>`:`<input id="wc${cap(key)}" value="${escapeHtml(value)}"${citizen?' inputmode="numeric" maxlength="17" autocomplete="off"':''}>`}</div>`};return `<div class="ws-section working-copy-section"><div class="wc-section-head"><h3>สำเนาเรื่องร้องเรียน (ฉบับเจ้าหน้าที่)</h3><span class="wc-copy-chip">สำเนา</span></div><div class="wc-tabs" role="tablist" aria-label="เลือกฉบับเอกสาร"><button type="button" class="wc-tab" data-wc-tab="original" aria-selected="false">ฉบับผู้ร้อง</button><button type="button" class="wc-tab active" data-wc-tab="copy" aria-selected="true">ฉบับเจ้าหน้าที่</button></div><div class="wc-panel ws-hidden" data-wc-panel="original"><div class="wc-orig-note">ต้นฉบับของผู้ร้อง เลขรับ ${escapeHtml(num)} — ระบบไม่แก้ไขข้อมูลนี้ เจ้าหน้าที่แก้ไขได้เฉพาะในฉบับเจ้าหน้าที่</div>${workingCopyOriginalView(c)}</div><div class="wc-panel" data-wc-panel="copy"><div class="wc-editor-banner"><strong>แก้ไขในสำเนาเท่านั้น</strong> — ต้นฉบับของผู้ร้อง (เลขรับ ${escapeHtml(num)}) อยู่ในแท็บ "ฉบับผู้ร้อง" และไม่ถูกเขียนทับ</div><div class="ws-grid-2">${field('complainant','ผู้ร้องเรียน')}${field('citizenId','เลขบัตรประชาชน')}${field('phone','โทรศัพท์')}${field('email','อีเมล')}${field('address','ที่อยู่',true)}${field('agency','ผู้ถูกร้อง/หน่วยงาน')}${field('subject','เรื่องร้องเรียน')}${field('type','ประเภทเรื่อง')}${field('detail','รายละเอียดพฤติการณ์',true,true)}${field('place','สถานที่เกิดเหตุ')}${field('province','จังหวัด')}${field('region','เขตพื้นที่')}${field('damage','ความเสียหายหรือความเดือดร้อน',true)}${field('request','ความประสงค์ของผู้ร้อง',true)}</div></div></div>`}
  const OFFICIAL_FIELD_DEFINITIONS={
    naccIncomingLetterNo:['เลขหนังสือขาเข้าจากสำนักงาน ป.ป.ช.','text'],naccIncomingLetterDate:['วันที่หนังสือขาเข้า','text'],naccBlackTrackingNo:['เลขดำติดตาม','text'],naccRecipientName:['ผู้รับ/สำนักงาน ป.ป.ช. ประจำจังหวัด','text'],naccProvince:['จังหวัดของสำนักงาน ป.ป.ช.','text'],
    naccBoardNo:['ครั้งที่ประชุม/เลขมติ','text'],naccBoardDate:['วันที่มีมติ','text'],naccBoardAgenda:['วาระที่','text'],naccBoardAttachmentPages:['จำนวนแผ่นสำเนามติ','number'],naccDossierAttachmentPages:['จำนวนแผ่นสำนวนต้นฉบับ','number'],naccReportYear:['ปี พ.ศ. ของบัญชีส่งเรื่อง','text'],
    accusedName:['ชื่อผู้ถูกกล่าวหา/หน่วยงาน','text'],accusedPositionAffiliation:['ตำแหน่ง/สังกัด','text'],complaintLetterDate:['วันที่หนังสือร้องเรียน','text'],recipientName:['ชื่อผู้รับหนังสือแจ้งผล','text'],contactOfficeName:['ชื่อหน่วยงานติดต่อ','text'],contactOfficeAddress:['ที่อยู่หน่วยงานติดต่อ','text'],contactOfficePhone:['โทรศัพท์หน่วยงาน','text'],contactOfficeFax:['โทรสารหน่วยงาน','text'],ownerPosition:['ตำแหน่งผู้รับผิดชอบ','text']
  };
  const OFFICIAL_FIELD_GROUPS={
    accused:['accusedName','accusedPositionAffiliation'],
    complainantNotice:['complaintLetterDate','recipientName'],
    contact:['contactOfficeName','contactOfficeAddress','contactOfficePhone','contactOfficeFax'],
    naccIncoming:['naccIncomingLetterNo','naccIncomingLetterDate','naccBlackTrackingNo','naccRecipientName','naccProvince'],
    naccBoardAttachments:['naccBoardNo','naccBoardDate','naccBoardAgenda','naccBoardAttachmentPages','naccDossierAttachmentPages'],
    naccReport:['naccReportYear','naccDossierAttachmentPages','ownerPosition']
  };
  function officialFieldIds(state){
    const d=state.documentData,incomingM62=isM62Channel(state.caseData.channel)||isNaccApiCase(state),groups=[];
    if(d.decision==='not-accept')return [];
    if(d.decision==='18/1ก'||(d.decision==='18/1ข'&&!incomingM62))groups.push('accused');
    if(d.decision==='18/4')groups.push('accused','complainantNotice','contact');
    if(d.decision==='18/1ข'&&incomingM62)groups.push('naccIncoming','complainantNotice','contact');
    if(d.decision==='send-nacc'&&d.naccTransferKind==='return')groups.push('naccIncoming','naccBoardAttachments');
    if(d.decision==='send-nacc'&&d.naccTransferKind==='forward')groups.push('naccReport','accused','complainantNotice','contact');
    if(d.decision==='58/2')groups.push('complainantNotice','contact');
    return groups.flatMap(group=>OFFICIAL_FIELD_GROUPS[group]).filter((id,index,ids)=>ids.indexOf(id)===index);
  }
  function officialFieldValue(state,id){
    const d=state.documentData,c=caseView(state),stored=String(d[id]??'').trim();
    if(stored)return stored;
    const centralOffice='ศูนย์รับเรื่องร้องเรียน สำนักงาน ป.ป.ท.';
    const contactOffice=c.contactOfficeName||((c.region&&c.region!=='ส่วนกลาง')?`สำนักงาน ป.ป.ท. ${c.region}`:centralOffice);
    const exact={
      accusedName:c.accusedName||c.agency,
      accusedPositionAffiliation:c.accusedPositionAffiliation,
      complaintLetterDate:c.complaintLetterDate||c.received,
      recipientName:d.anonymous?'':c.complainant,
      contactOfficeName:contactOffice,
      contactOfficeAddress:c.contactOfficeAddress,
      contactOfficePhone:c.contactOfficePhone,
      contactOfficeFax:c.contactOfficeFax,
      ownerPosition:c.ownerPosition||c.assignedOfficerPosition,
      naccIncomingLetterNo:c.naccIncomingLetterNo,
      naccIncomingLetterDate:c.naccIncomingLetterDate,
      naccBlackTrackingNo:c.naccBlackTrackingNo,
      naccRecipientName:c.naccRecipientName||((isM62Channel(c.channel)||isNaccApiCase(state))&&/ป\.ป\.ช\./.test(c.complainant||'')?c.complainant:''),
      naccProvince:c.naccProvince||((isM62Channel(c.channel)||isNaccApiCase(state))?c.province:'')
    };
    return exact[id]||'';
  }
  function officialDocumentFieldsSection(state){
    const ids=officialFieldIds(state);
    if(!ids.length)return '';
    const field=id=>{const [label,type]=OFFICIAL_FIELD_DEFINITIONS[id];return `<div class="ws-field"><label for="${id}">${label}</label><input id="${id}" type="${type}" value="${escapeHtml(officialFieldValue(state,id))}"></div>`};
    return `<div class="ws-section official-document-fields-section"><h3>ข้อมูลที่ระบบเตรียมให้สำหรับแบบพิมพ์ทางราชการ</h3><div class="ws-grid-2">${ids.map(field).join('')}</div><small>ระบบใส่ค่าเริ่มต้นจากเรื่องร้องเรียนและหน่วยงานผู้รับผิดชอบแล้ว เจ้าหน้าที่แก้เฉพาะข้อมูลที่ไม่ถูกต้องหรือยังไม่ครบก่อนลงนาม</small></div>`;
  }
  ...
  function form3Paper(state,role){
    ...
    const centralIntake=state.workflow?.intakeOwner?.role==='officer'||(!state.workflow?.intakeOwner?.role&&c.region==='ส่วนกลาง');
    const reasonChecked = id => d.reasons.includes(id);
    const writingArea = (checked,value,lineCount) => checked&&value ? `<p class="form3-writing-text compact">${escapeHtml(value)}</p>` : `<div class="form3-writing-lines compact">${'<div></div>'.repeat(lineCount)}</div>`;
    ...
    const mainPage=`<article class="a4-paper template-form3" data-auto-paginate>
      ...
      <div class="form3-decision-grid">
        <section class="form3-decision-column">
          ...
          ${NACC_TRANSFER_REASONS.map(reason=>`<p>${checkMark(selected('send-nacc')&&reasonChecked(reason.id))} ${reason.label}</p>`).join('')}
          <p>${checkMark(selected('send-nacc')&&d.naccOtherChecked)} อื่น ๆ ${selected('send-nacc')&&d.naccOtherChecked&&d.naccOtherReason?escapeHtml(d.naccOtherReason):'.....................................................................'}</p>
          <h3>${checkMark(selected('not-accept'))} ไม่รับไว้ดำเนินการ</h3>
          ${writingArea(selected('not-accept'),d.notAcceptReason,2)}
        </section>
        <section class="form3-decision-column form3-accept-column">
          ${[['18/1ก','รับไว้ตรวจสอบ กรณี มาตรา 18/1(ก)'],['18/1ข','รับไว้ตรวจสอบ กรณี มาตรา 18/1(ข)'],['18/4','รับไว้ตรวจสอบ กรณี มาตรา 18/4'],['58/2','รับไว้ตรวจสอบ กรณี มาตรา 58/2']].map(([key,label])=>`<h3>${checkMark(selected(key))} ${label}</h3><p>${checkMark(selected(key)&&centralIntake)} มอบหมาย สำนัก/กอง (สำหรับ ศรร. กบค.)</p><p class="form3-line">${selected(key)&&centralIntake?escapeHtml(d.proposedRegion||'กองบริหารคดี'):''}</p><p>${checkMark(selected(key)&&!centralIntake)} มอบหมาย ผู้รับผิดชอบ (สำหรับ สำนัก/กอง)</p><p class="form3-line">${selected(key)&&!centralIntake?escapeHtml(d.proposedRegion):''}</p>`).join('')}
        </section>
      </div>
  ...
  function receiptPaper(c,qr='',opts={}){
    ...
    return `<div class="document-page-set" id="activePaper">
      <article class="a4-paper template-receipt">
        <header class="receipt-header"><img src="${PACC_LOGO}" alt="ตราสำนักงาน ป.ป.ท."><div class="receipt-code-box"><strong>58/2-02</strong><span>แบบแจ้งเลขเรื่องร้องเรียน/เลขติดตาม E-CMIS</span></div></header>
        ...
        ${isDuplicate?`<p class="receipt-duplicate-note"><strong>เรื่องซ้ำ — เชื่อมโยงเรื่องเดิม</strong> เรื่องนี้ได้รับเลขติดตามเมื่อบันทึกรับแล้ว และไม่มีการออกเลขเพิ่มในขั้นกลั่นกรอง โดยรวมดำเนินการกับเรื่องเดิม (เลขรับเรื่อง ${escapeHtml(opts.duplicateOf||'')}) กรุณาใช้เลขรับบริการและ PIN ของเรื่องเดิมข้างต้นเพื่อติดตามสถานะเรื่องร้องเรียน</p>`:''}
  ...
  function duplicateReceiptPaper(newCase,originalCase,qr){
    return receiptPaper(newCase,qr,{duplicateOf:caseDisplayId(originalCase),originalTracking:originalCase?.trackingYear||'',originalPin:originalCase?.trackingCode||''});
  }
  async function uploadedOfficerPreviewPages(state){
    const api=window.ECMISUniversalDocumentUpload;
    if(!api?.listActiveDocuments)return [];
    let records=[];
    try{records=await api.listActiveDocuments('a4',state.caseData.id)}catch(error){return []}
    return Promise.all(records.map(async record=>{
      const kind=api.previewKind(record);
      const meta=`ไฟล์ที่อัปโหลด · ${record.type||'ไม่ระบุชนิด'} · ${api.formatSize(record.size||0)}`;
      let objectUrl='';
      let content='';
      if(kind==='text'){
        let text='';
        try{text=await record.blob.text()}catch(error){text='[เปิดอ่านข้อความไม่สำเร็จ กรุณาดาวน์โหลดไฟล์เพื่อตรวจสอบ]'}
        content=`<pre>${escapeHtml(text.slice(0,200000))}</pre>`;
      }else if(kind==='unsupported'){
        content='<div class="officer-upload-unavailable"><strong>ไฟล์ชนิดนี้ Preview ใน Browser ไม่ได้</strong><span>ตรวจชื่อไฟล์และขนาด แล้วดาวน์โหลดไปเปิดด้วยโปรแกรมที่รองรับ</span></div>';
      }else{
        objectUrl=URL.createObjectURL(record.blob);
        if(kind==='pdf')content=`<iframe src="${escapeHtml(objectUrl)}" title="ตัวอย่างเอกสาร ${escapeHtml(record.name)}"></iframe>`;
        else if(kind==='image')content=`<img src="${escapeHtml(objectUrl)}" alt="${escapeHtml(record.name)}">`;
        else content=`<${kind} src="${escapeHtml(objectUrl)}" controls preload="metadata"></${kind}>`;
      }
      return {name:record.name,documentKey:`upload:${record.id}`,index:1,total:1,meta,objectUrl,html:`<article class="a4-paper officer-upload-preview"><header><span>ไฟล์ที่อัปโหลด</span><strong>${escapeHtml(record.name)}</strong><small>${escapeHtml(meta)}</small></header><div>${content}</div></article>`};
    }));
  }
  async function officerPreviewPages(state){
    ...
    const docs=[['back-cover',OFFICIAL_DOCUMENTS['back-cover'].name],...cover,...remaining,...(usesReviewRecord(state)?[[REVIEW_RECORD_ID,REVIEW_RECORD_NAME]]:[]),['complainant-original','ข้อมูลเรื่องร้องเรียน (ต้นฉบับ)'],['working-copy','สำเนาเรื่องร้องเรียน (ฉบับเจ้าหน้าที่)']];
    const pages=[];
    docs.forEach(([id,name])=>{
      ...
      if(papers.length){
        papers.forEach((paper,index)=>{
          ...
          pages.push({name,documentKey:id,index:index+1,total:papers.length,html:copy.outerHTML});
        });
        return;
      }
      pages.push({name,documentKey:id,index:1,total:1,html:`<article class="a4-paper document-placeholder officer-preview-placeholder">${source.innerHTML}</article>`});
    });
    return pages.concat(await uploadedOfficerPreviewPages(state));
  }
  function officerSendReviewHtml(previewPages,reviewTarget){
    const documentCount=new Set(previewPages.map(page=>page.documentKey||page.name)).size;
    const pageList=previewPages.map((page,index)=>`<li><button type="button" class="officer-review-document" data-officer-preview-index="${index}" ${index?'disabled':''}><span class="officer-review-document-state" aria-hidden="true">${index+1}</span><span><strong>${escapeHtml(page.name)}</strong><small>${escapeHtml(page.meta||(page.total>1?`หน้าที่ ${page.index} จาก ${page.total}`:'1 หน้า'))}</small></span></button></li>`).join('');
    return `<div class="officer-document-preview">
      <header class="officer-review-command"><div id="officerReviewTitleSlot"></div><div class="officer-review-progress"><span>ความคืบหน้าการตรวจ</span><strong id="officerPreviewCounter">หน้า 1 จาก ${previewPages.length}</strong></div></header>
      <div class="officer-review-layout">
        <aside class="officer-review-sidebar" aria-label="รายการเอกสาร"><div class="officer-review-panel-head"><span>ชุดเอกสาร</span><strong>${documentCount} รายการ · ${previewPages.length} หน้า</strong></div><ol class="officer-review-documents">${pageList}</ol></aside>
        <section class="officer-review-canvas" aria-label="ตัวอย่างเอกสาร"><div class="officer-preview-toolbar"><div><span>เอกสารปัจจุบัน</span><strong id="officerPreviewTitle">—</strong></div><div class="officer-preview-tools"><button type="button" id="officerPreviewPrev" aria-label="เปิดหน้าก่อนหน้า">ก่อนหน้า</button><button type="button" id="officerPreviewNext" aria-label="เปิดหน้าถัดไป">ถัดไป</button><span class="officer-preview-zoom" role="group" aria-label="ปรับขนาดตัวอย่างเอกสาร"><button type="button" id="officerPreviewZoomOut" aria-label="ย่อเอกสาร">−</button><output id="officerPreviewZoomValue" aria-live="polite">72%</output><button type="button" id="officerPreviewZoomIn" aria-label="ขยายเอกสาร">+</button></span></div></div><div class="officer-preview-viewport" id="officerPreviewViewport" tabindex="0"></div></section>
        <aside class="officer-review-summary" aria-label="สรุปการตรวจสอบ"><div><p class="ws-kicker">สรุปก่อนส่ง</p><h3>พร้อมส่งพิจารณา</h3><dl><div><dt>ผู้รับถัดไป</dt><dd>${escapeHtml(reviewTarget)}</dd></div><div><dt>ชุดเอกสาร</dt><dd>${documentCount} รายการ · ${previewPages.length} หน้า</dd></div></dl></div><ul class="officer-review-checklist"><li class="complete"><span aria-hidden="true">✓</span><div><strong>ข้อมูลสำหรับส่งครบถ้วน</strong><small>ผ่านเงื่อนไขตรวจสอบก่อนเปิดชุดเอกสาร</small></div></li><li class="complete"><span aria-hidden="true">✓</span><div><strong>เจ้าหน้าที่ลงนามแล้ว</strong><small>ลายมือชื่อถูกบันทึกในชุดเอกสาร</small></div></li><li id="officerReviewSequenceCheck"><span aria-hidden="true">•</span><div><strong>ตรวจเอกสารตามลำดับ</strong><small id="officerReviewSequenceText">ตรวจแล้ว 0 จาก ${previewPages.length} หน้า</small></div></li></ul><p class="officer-review-guidance" id="officerReviewGuidance">ปุ่มส่งต่อจะพร้อมใช้งานเมื่อตรวจถึงหน้าสุดท้าย</p><div id="officerReviewActions"></div></aside>
      </div>
    </div>`;
  }
  function usesReviewRecord(state){return packCaseTypeOf(state)!=='62'}
  function firstDocumentId(state,role){if(usesReviewRecord(state))return REVIEW_RECORD_ID;return isOfficerRole(role)?ALL_DOCUMENTS_ID:'working-copy'}
  function docTabs(state,role){
    ...
    const docs=[['back-cover',OFFICIAL_DOCUMENTS['back-cover'].name],...cover,...remaining,...(usesReviewRecord(state)?[[REVIEW_RECORD_ID,REVIEW_RECORD_NAME]]:[]),...(state.documentData?.screening?.decision==='duplicate'?[['duplicate-receipt','ใบแจ้งเลขติดตาม (เลขเดิม)']]:[]),...(isOfficerRole(role)?[[ALL_DOCUMENTS_ID,'ดูชุดเอกสารทั้งหมด']]:[]),['complainant-original','ข้อมูลเรื่องร้องเรียน (ต้นฉบับ)'],['working-copy','สำเนาเรื่องร้องเรียน (ฉบับเจ้าหน้าที่)']];
  ...
  function officialDocumentPlaceholder(state,id){
    ...
    return `<article class="a4-paper document-placeholder official-document-placeholder" id="activePaper"><span>ปปท. ${escapeHtml(id)}</span><h2>${escapeHtml(doc?.name||'เอกสารทางราชการ')}</h2><p>${escapeHtml(documentRegisterLabel(doc||{register:'pending'},state,id))}</p><small>ยังไม่มีเนื้อหาแบบพิมพ์ที่ยืนยันจากไฟล์ต้นฉบับ จึงไม่สร้างข้อความราชการแทน</small></article>`;
  }
  function corpusDocumentPaper(state,id){
    const c=caseView(state),d=state.documentData,blank='........................................',binding=officialBindings(state,id);
    const signature=`<div class="official-signature"><p>(${blank})</p><p>เลขาธิการคณะกรรมการป้องกันและปราบปรามการทุจริตในภาครัฐ</p><p>หรือผู้ที่ได้รับมอบหมาย</p></div>`;
    if(id==='1-09')return `<article class="a4-paper template-official-letter" id="activePaper"><div class="official-letter-head"><p><strong>ที่</strong> ${officialText(outgoingAt(state,id)||'ปป ๐๐..../.........')}</p><p>สำนักงาน ป.ป.ท.<br>อาคารซอฟต์แวร์ปาร์ค ถนนแจ้งวัฒนะ<br>อำเภอปากเกร็ด จังหวัดนนทบุรี ๑๑๑๒๐</p></div><p class="official-date">${officialText(d.naccLetterDate||'(วัน เดือน ปี)')}</p><div class="official-subject"><strong>เรื่อง</strong><span>ขอส่งเรื่องกล่าวหาร้องเรียนคืนคณะกรรมการ ป.ป.ช.</span><strong>เรียน</strong><span>${escapeHtml(d.naccRecipientName||'เลขาธิการคณะกรรมการ ป.ป.ช.')}</span><strong>อ้างถึง</strong><span>หนังสือสำนักงาน ป.ป.ช. ที่ ${officialText(d.naccIncomingLetterNo||blank)} ลงวันที่ ${officialText(d.naccIncomingLetterDate||blank)}</span><strong>สิ่งที่ส่งมาด้วย</strong><span>๑. สำเนามติคณะกรรมการ ป.ป.ท. ครั้งที่ ${officialText(d.naccBoardNo||blank)} เมื่อวันที่ ${officialText(d.naccBoardDate||blank)} จำนวน ${officialText(d.naccBoardAttachmentPages||blank)} แผ่น<br>๒. ต้นฉบับสำนวนตามหนังสือที่อ้างถึง เลขดำติดตามที่ ${officialText(d.naccBlackTrackingNo||blank)} จำนวน ${officialText(d.naccDossierAttachmentPages||blank)} แผ่น</span></div><div class="official-body"><p>ตามหนังสือที่อ้างถึง สำนักงาน ป.ป.ช. ได้ส่งเรื่องกล่าวหาร้องเรียนให้คณะกรรมการ ป.ป.ท. ดำเนินการแทน กรณีกล่าวหา ${escapeHtml(d.documentSubject||c.subject)} ความละเอียดแจ้งแล้ว นั้น</p><p>คณะกรรมการ ป.ป.ท. ได้พิจารณาและมีมติในการประชุม ครั้งที่ ${officialText(d.naccBoardNo||blank)} วันที่ ${officialText(d.naccBoardDate||blank)} วาระที่ ${officialText(d.naccBoardAgenda||blank)} เห็นชอบให้ส่งคืนเรื่องกล่าวหาร้องเรียนดังกล่าวให้คณะกรรมการ ป.ป.ช.</p><p>จึงเรียนมาเพื่อโปรดพิจารณา</p><p class="official-thanks">ขอแสดงความนับถือ</p></div>${signature}<p class="official-form-code">ปปท. ๑-๐๙</p></article>`;
    if(id==='1-12')return `<article class="a4-paper a4-landscape template-memo template-report-112" id="activePaper">${binding.reportHeadingHtml}<p>หนังสือสำนักงาน ป.ป.ท. ลับ ที่ ${officialText(d.naccLetterNo||blank)} ลงวันที่ ${officialText(d.naccLetterDate||blank)}</p><table class="official-table"><thead><tr><th>ลำดับที่</th><th>เลขรับที่</th><th>วันที่รับเรื่อง</th><th>ชื่อผู้ร้องเรียน</th><th>ชื่อผู้ถูกร้องเรียน</th><th>ตำแหน่ง/สังกัด</th><th>พฤติการณ์</th><th>จำนวนเอกสาร (แผ่น)</th><th>หมายเหตุ</th></tr></thead><tbody><tr><td>๑</td><td>${officialText(registryReceiptNumber(c)||blank)}</td><td>${officialText(c.received||blank)}</td><td>${escapeHtml(c.complainant||blank)}</td><td>${escapeHtml(d.accusedName||blank)}</td><td>${escapeHtml(d.accusedPositionAffiliation||blank)}</td><td>สำนักงาน ป.ป.ท. ได้รับหนังสือร้องเรียนกรณี ${escapeHtml(d.documentSubject||c.subject)}</td><td>${binding.attachmentPagesHtml}</td><td></td></tr></tbody></table><p>รวมทั้งสิ้นจำนวน ๑ เรื่อง</p><p>ผู้รับผิดชอบ ${escapeHtml(d.assignedOfficer||blank)} ตำแหน่ง ${escapeHtml(d.ownerPosition||blank)}</p><p class="official-form-code">ปปท. ๑-๑๒</p></article>`;
    if(id==='1-08')return `<article class="a4-paper template-official-letter" id="activePaper"><div class="official-letter-head"><p><strong>ที่</strong> ${escapeHtml(outgoingAt(state,id)||'ปป ๐๐..../.........')}</p><p>สำนักงาน ป.ป.ท.<br>อาคารซอฟต์แวร์ปาร์ค ถนนแจ้งวัฒนะ<br>อำเภอปากเกร็ด จังหวัดนนทบุรี ๑๑๑๒๐</p></div><p class="official-date">(วัน เดือน ปี)</p><div class="official-subject"><strong>เรื่อง</strong><span>แจ้งการรับเรื่องร้องเรียนไว้ดำเนินการตามอำนาจหน้าที่</span><strong>เรียน</strong><span>${binding.recipientHtml}</span><strong>อ้างถึง</strong>${binding.referenceHtml}</div><div class="official-body"><p>ตามหนังสือที่อ้างถึง สำนักงานคณะกรรมการป้องกันและปราบปรามการทุจริตแห่งชาติ (สำนักงาน ป.ป.ช.) / สำนักงาน ป.ป.ช. ประจำจังหวัด ${binding.provinceHtml} ได้ส่งเรื่องกล่าวหาร้องเรียนให้สำนักงานคณะกรรมการป้องกันและปราบปรามการทุจริตในภาครัฐ (สำนักงาน ป.ป.ท.) ดำเนินการแทนตามมาตรา ๖๒ แห่งพระราชบัญญัติประกอบรัฐธรรมนูญว่าด้วยการป้องกันและปราบปรามการทุจริต พ.ศ. ๒๕๖๑ กรณี ${escapeHtml(d.documentSubject||c.subject)} ความละเอียดแจ้งแล้ว นั้น</p><p>บัดนี้ สำนักงาน ป.ป.ท. ได้รับเรื่องดังกล่าวไว้ดำเนินการ ตามเลขรับศูนย์รับเรื่องร้องเรียนที่ ${escapeHtml(registryReceiptNumber(c)||blank)} ลงวันที่ ${escapeHtml(c.received||blank)} เรื่องที่ ${escapeHtml(d.caseNumber||blank)} และได้ส่งเรื่องกล่าวหาร้องเรียนดังกล่าวให้ ${escapeHtml(d.proposedRegion||'(กอง/สำนัก)')} พิจารณาดำเนินการตามอำนาจหน้าที่แล้ว</p><p>กรณีที่ไม่ได้รับรหัส ให้ระบุข้อความนี้ไว้ด้วย “ทั้งนี้ สำนักงาน ป.ป.ช. มิได้ส่งข้อมูลชื่อผู้ใช้งาน และรหัสผ่าน สำหรับรายงานผลการติดตามเรื่องกล่าวหาตามมาตรา ๖๕ แห่งพระราชบัญญัติประกอบรัฐธรรมนูญว่าด้วยการป้องกันและปราบปรามการทุจริต พ.ศ. ๒๕๖๑ มาพร้อมสำนวนคดีดังกล่าวแต่อย่างใด”</p><p>จึงเรียนมาเพื่อโปรดทราบ</p><p class="official-thanks">ขอแสดงความนับถือ</p></div>${signature}${binding.contactHtml}<div class="official-contact"><p>(${escapeHtml(d.assignedOfficer||blank)} เจ้าของสำนวน)</p></div><p class="official-form-code">ปปท. 1-08</p></article>`;
    if(id==='1-10')return `<article class="a4-paper template-memo" id="activePaper"><div class="memo-heading"><img class="memo-garuda" src="${GARUDA_IMAGE}" alt="ตราครุฑ"><h2>บันทึกข้อความ</h2></div><div class="memo-metadata"><p><strong>ส่วนราชการ</strong> สำนัก/กอง ${blank} <strong>โทร.</strong> ...............</p><p><strong>ที่</strong> ${escapeHtml(outgoingAt(state,id)||'ปป ๐๐.............')} <strong>วันที่</strong> ${blank}</p></div><div class="official-subject"><strong>เรื่อง</strong><span>ขออนุมัติส่งเรื่องกล่าวหาร้องเรียนให้สำนักงาน ป.ป.ช.</span><strong>เรียน</strong><span>เลขาธิการคณะกรรมการ ป.ป.ท.</span></div><div class="official-body"><p><strong>๑. เรื่องเดิม</strong><br>พระราชบัญญัติมาตรการของฝ่ายบริหารในการป้องกันและปราบปรามการทุจริต พ.ศ. ๒๕๕๑ และที่แก้ไขเพิ่มเติม มีผลบังคับใช้เมื่อวันที่ ๑๙ กุมภาพันธ์ ๒๕๖๘ กำหนดให้กรณีที่ได้รับเรื่องร้องเรียนเป็นการกล่าวหาเจ้าหน้าที่ของรัฐที่มีพฤติการณ์ทุจริตต่อหน้าที่รวมอยู่ด้วย รวมทั้งที่อยู่ในหน้าที่และอำนาจของคณะกรรมการ ป.ป.ช. ให้ส่งเรื่องให้คณะกรรมการดำเนินการตามอำนาจหน้าที่ตามมาตรา ๑๘/๔ (๑)</p><p><strong>๒. ข้อเท็จจริง</strong><br>กอง/สำนัก ได้รับหนังสือร้องเรียนจากประชาชนผ่านช่องทางไปรษณีย์และช่องทางอื่น ๆ ภายหลังพระราชบัญญัติประกอบรัฐธรรมนูญว่าด้วยการป้องกันและปราบปรามการทุจริต พ.ศ. ๒๕๖๑ มีผลใช้บังคับ รวมทั้งพระราชบัญญัติมาตรการของฝ่ายบริหารในการป้องกันและปราบปรามการทุจริต พ.ศ. ๒๕๕๑ และที่แก้ไขเพิ่มเติม รวมจำนวน ๑ เรื่อง โดยเรื่องร้องเรียนดังกล่าวเป็นการกล่าวหาเจ้าหน้าที่ของรัฐที่มีพฤติการณ์ทุจริตต่อหน้าที่รวมอยู่ด้วยและอยู่ในอำนาจของคณะกรรมการ ป.ป.ช.</p><p><strong>๓. ข้อพิจารณา/ข้อเสนอ</strong><br>เพื่อให้เป็นไปตามมาตรา ๕๙ แห่งพระราชบัญญัติประกอบรัฐธรรมนูญว่าด้วยการป้องกันและปราบปรามการทุจริต พ.ศ. ๒๕๖๑ และพระราชบัญญัติมาตรการของฝ่ายบริหารในการป้องกันและปราบปรามการทุจริต พ.ศ. ๒๕๕๑ และที่แก้ไขเพิ่มเติม จึงเห็นควรส่งเรื่องร้องเรียนดังกล่าวให้สำนักงาน ป.ป.ช. ดำเนินการตามอำนาจหน้าที่ต่อไป จำนวน ๑ เรื่อง</p><p>จึงเรียนมาเพื่อโปรดพิจารณา</p></div><div class="official-signature"><p>(${blank})</p><p>ตำแหน่ง ${blank}</p></div><p class="official-form-code">ปปท. 1-10</p></article>`;
    if(id==='1-13')return `<article class="a4-paper template-memo" id="activePaper"><div class="memo-heading"><img class="memo-garuda" src="${GARUDA_IMAGE}" alt="ตราครุฑ"><h2>บันทึกข้อความ</h2></div><div class="memo-metadata"><p><strong>ส่วนราชการ</strong> สำนัก/กอง ${blank} <strong>โทร.</strong> ...............</p><p><strong>ที่</strong> ${escapeHtml(outgoingAt(state,id)||'ปป ๐๐.............')} <strong>วันที่</strong> ${blank}</p></div><div class="official-subject"><strong>เรื่อง</strong><span>รายงานการรับเรื่องร้องเรียน และแจ้งให้ผู้ร้องเรียนทราบผลการดำเนินการ</span><strong>เรียน</strong><span>ผอ. สำนัก/กอง</span></div><div class="official-body"><p>เรื่องที่ ${escapeHtml(registryReceiptNumber(c)||blank)}<br>รหัสติดตามเรื่อง ${escapeHtml(c.trackingYear||blank)}<br>สำนักงาน ป.ป.ท. รับเรื่อง เมื่อวันที่ ${escapeHtml(c.received||blank)}<br>ศรร. รับเรื่องเมื่อวันที่ ${escapeHtml(c.received||blank)}<br>ชื่อผู้ร้องเรียน ${escapeHtml(c.complainant||blank)}<br>สำนัก/กอง ที่มีเขตอำนาจ ${escapeHtml(d.proposedRegion||blank)}</p><p>ตรวจสอบเรื่องร้องเรียนแล้วเห็นควรแจ้งให้ผู้ร้องเรียนทราบการดำเนินการของสำนักงาน ป.ป.ท. ต่อไป</p><p>จึงเรียนมาเพื่อโปรดพิจารณา</p></div><div class="official-signature"><p>(${blank})</p><p>ตำแหน่ง ${blank}</p><p>ผู้บังคับบัญชาระดับต้น</p></div><p class="official-form-code">ปปท. 1-13</p></article>`;
    if(id==='1-14')return `<article class="a4-paper template-official-letter" id="activePaper"><div class="official-letter-head"><p><strong>ที่</strong> ${escapeHtml(outgoingAt(state,id)||'ปป ๐๐...../.....')}</p><p>สำนักงาน ป.ป.ท.<br>อาคารซอฟต์แวร์ปาร์ค ถนนแจ้งวัฒนะ<br>อำเภอปากเกร็ด จังหวัดนนทบุรี ๑๑๑๒๐</p></div><p class="official-date">(วัน เดือน ปี)</p><div class="official-subject"><strong>เรื่อง</strong><span>การแจ้งผลการดำเนินการ</span><strong>เรียน</strong><span>${binding.recipientHtml}</span><strong>อ้างถึง</strong>${binding.complaintReferenceHtml}</div><div class="official-body"><p>ตามหนังสือที่อ้างถึง ท่านส่งเรื่องร้องเรียนมายังสำนักงานคณะกรรมการป้องกันและปราบปรามการทุจริตในภาครัฐ (สำนักงาน ป.ป.ท.) กรณีเจ้าหน้าที่ของรัฐ สังกัด ${escapeHtml(c.agency||blank)} กรณี ${escapeHtml(d.documentSubject||c.subject)} โดยรัฐธรรมนูญแห่งราชอาณาจักรไทย พุทธศักราช ๒๕๖๐ มาตรา ๒๓๔ (๒) ได้กำหนดให้คณะกรรมการ ป.ป.ช. มีหน้าที่และอำนาจในการไต่สวนและวินิจฉัย ประกอบกับพระราชบัญญัติประกอบรัฐธรรมนูญว่าด้วยการป้องกันและปราบปรามการทุจริต พ.ศ. ๒๕๖๑ มาตรา ๕๙ และพระราชบัญญัติมาตรการของฝ่ายบริหารในการป้องกันและปราบปรามการทุจริต พ.ศ. ๒๕๕๑ และที่แก้ไขเพิ่มเติม ตามมาตรา ๑๘/๑ (ข) (๑) มาตรา ๑๘/๓ และมาตรา ๑๘/๔ (๑) ให้ส่งเรื่องร้องเรียนที่อยู่ในอำนาจหน้าที่ของคณะกรรมการ ป.ป.ช. ให้คณะกรรมการ ป.ป.ช. ดำเนินการตามอำนาจหน้าที่ต่อไป นั้น</p><p>ในการนี้ สำนักงาน ป.ป.ท. ได้ดำเนินการส่งเรื่องร้องเรียนของท่านไปยังสำนักงาน ป.ป.ช. ตามหนังสือสำนักงาน ป.ป.ท. ที่ ${escapeHtml(d.naccLetterNo||blank)} ลงวันที่ ${escapeHtml(d.naccLetterDate||blank)} เพื่อพิจารณาดำเนินการตามอำนาจหน้าที่ต่อไปแล้ว ทั้งนี้ ท่านสามารถติดตามเรื่องร้องเรียนของท่านได้ที่สำนักงาน ป.ป.ช. เลขที่ 361 ถนนนนทบุรี ตำบลท่าทราย อำเภอเมืองนนทบุรี จังหวัดนนทบุรี 11000 โทรศัพท์ 0 2528 4800 โดยแจ้งเลขที่หนังสือของสำนักงาน ป.ป.ท. ตามที่ให้ไว้ดังกล่าวข้างต้น</p><p>จึงเรียนมาเพื่อทราบ</p><p class="official-thanks">ขอแสดงความนับถือ</p></div>${signature}${binding.contactHtml}<p class="official-form-code">ปปท. 1-14</p></article>`;
    if(id==='58/2-03'||id==='58/2-04'){
      const regional=id==='58/2-04',unit=regional?`ปปท. เขต ${escapeHtml(d.proposedRegion||'.......')}`:'ศูนย์รับเรื่องร้องเรียน กบค.',outgoing=regional?'ปป ๐๐..../':(outgoingAt(state,id)||'ปป 0004.2/');
      return `<div class="document-page-set" id="activePaper"><article class="a4-paper template-memo"><div class="memo-heading"><img class="memo-garuda" src="${GARUDA_IMAGE}" alt="ตราครุฑ"><h2>บันทึกข้อความ</h2></div><div class="memo-metadata"><p><strong>ส่วนราชการ</strong> ${unit} <strong>โทร.</strong> ${regional?'................':'1907'}</p><p><strong>ที่</strong> ${escapeHtml(outgoing)} <strong>วันที่</strong> ${blank}</p></div><div class="official-subject"><strong>เรื่อง</strong><span>การแจ้งให้ผู้ร้องเรียนทราบผลการดำเนินการ</span><strong>เรียน</strong><span>ผอ. กบค.</span></div><div class="official-body"><p>เรื่องร้องเรียนที่ ${escapeHtml(registryReceiptNumber(c)||blank)}<br>รหัสติดตามเรื่อง ${escapeHtml(c.trackingYear||blank)}<br>เมื่อวันที่ ${escapeHtml(c.received||blank)}<br>ชื่อผู้ร้องเรียน ${escapeHtml(c.complainant||blank)}</p><p>ตรวจสอบเรื่องร้องเรียนแล้วเห็นควรแจ้งให้ผู้ร้องเรียนทราบการดำเนินการของสำนักงาน ป.ป.ท. ต่อไป</p><p>จึงเรียนมาเพื่อโปรดพิจารณา</p></div><div class="official-signature"><p>(${blank})</p><p>นสส./นยธ</p><p>(${blank})</p><p>นยธ. ชก. รกน.${regional?'':' ผอ. ศรร.'}</p></div><p class="official-form-code">${id}</p></article><article class="a4-paper template-official-letter"><div class="official-letter-head"><p><strong>ที่</strong> ${regional?'ปป ๐๐../':'ปป ๐๐๐๔/'}</p><p>${regional?`สำนักงาน ปปท. เขต ${escapeHtml(d.proposedRegion||'.......')}<br>ที่อยู่ ${blank} ถนน ${blank}<br>อำเภอ ${blank} จังหวัด ${blank} รหัสไปรษณีย์ ${blank}`:'สำนักงาน ป.ป.ท.<br>อาคารซอฟต์แวร์ปาร์ค ถนนแจ้งวัฒนะ<br>อำเภอปากเกร็ด จังหวัดนนทบุรี ๑๑๑๒๐'}</p></div><p class="official-date">(วัน เดือน ปี)</p><div class="official-subject"><strong>เรื่อง</strong><span>การรับเรื่องร้องเรียน</span><strong>เรียน</strong><span>${escapeHtml(c.complainant||'ชื่อ - สกุล ผู้ร้องเรียน')}</span><strong>อ้างถึง</strong><span>หนังสือร้องเรียนของท่าน ฉบับลงวันที่ ${blank}</span></div><div class="official-body"><p>ตามหนังสือที่อ้างถึง ท่านได้ร้องเรียนต่อสำนักงานคณะกรรมการป้องกันและปราบปรามการทุจริตในภาครัฐ (สำนักงาน ป.ป.ท.) กรณี ${escapeHtml(d.documentSubject||c.subject)} นั้น</p><p>บัดนี้ สำนักงาน ป.ป.ท. ได้รับเรื่องร้องเรียนของท่านไว้ดำเนินการแล้ว เป็นเรื่องร้องเรียนที่ ${escapeHtml(registryReceiptNumber(c)||blank)} (รหัสติดตามเรื่องร้องเรียนเลขที่ ${escapeHtml(c.trackingYear||blank)}) และมอบหมายให้สำนักงานป้องกันและปราบปรามการทุจริตในภาครัฐ ${escapeHtml(d.proposedRegion||'เขต ......')} ดำเนินการตรวจสอบข้อเท็จจริง ทั้งนี้ หากท่านประสงค์จะทราบรายละเอียดความคืบหน้าผลการดำเนินการสามารถติดต่อสอบถามข้อมูลไปยัง ปปท. เขต .......... เลขที่ ........ ถนน.......... ตำบล........... อำเภอ......... จังหวัด............ รหัสไปรษณีย์ .................... โทร. ......................... โดยใช้รหัสติดตามเรื่องร้องเรียนที่ได้แจ้งไว้ข้างต้น และท่านสามารถติดตามสถานะเรื่องร้องเรียนของท่านผ่าน QR CODE ด้านท้ายหนังสือฉบับนี้ โดยให้ท่านระบุเลขเรื่องร้องเรียน และเลขติดตามของท่านตามที่กำหนด</p><p>จึงเรียนมาเพื่อทราบ</p><p class="official-thanks">ขอแสดงความนับถือ</p></div><div class="official-signature"><p>(${blank})</p><p>${regional?'ผู้อำนวยการ ปปท. เขต ...........':'ผู้อำนวยการกองบริหารคดี'} ปฏิบัติราชการแทน</p><p>เลขาธิการคณะกรรมการป้องกันและปราบปรามการทุจริตในภาครัฐ</p></div><p class="official-form-code">${id}</p></article></div>`;
    }
    if(id==='58/2-05')return `<div class="document-page-set" id="activePaper"><article class="a4-paper template-memo"><div class="memo-heading"><img class="memo-garuda" src="${GARUDA_IMAGE}" alt="ตราครุฑ"><h2>บันทึกข้อความ</h2></div><div class="memo-metadata"><p><strong>ส่วนราชการ</strong> ศูนย์รับเรื่องร้องเรียน กบค. <strong>โทร.</strong> 1907</p><p><strong>ที่</strong> ${escapeHtml(outgoingAt(state,id)||'ปป 0004.2/')} <strong>วันที่</strong> ${blank}</p></div><div class="official-subject"><strong>เรื่อง</strong><span>การส่งเอกสารเรื่องร้องเรียนเพื่อดำเนินการต่อไป</span><strong>เรียน</strong><span>ผอ. กบค.</span></div><div class="official-body"><p>ตั้งแต่วันที่ ${blank} ถึงวันที่ ${blank}<br>ศรร. ได้รับเรื่องร้องเรียนที่อยู่ในอำนาจหน้าที่ของ ปปท. เขต ${escapeHtml(d.proposedRegion||'.............')} ดังนี้<br>๑. ตามมาตรา ๖๒ (พรป. ปปช.) จำนวน ......................เรื่อง<br>๒. ระบบการตรวจสอบข้อเท็จจริง จำนวน ......................เรื่อง<br>๓. เรื่องเดิมเรื่องร้องเรียน จำนวน ......................เรื่อง</p><p>จึงเห็นควรส่งเอกสารเรื่องร้องเรียน จำนวนทั้งสิ้น .................เรื่อง ไปยัง ปปท. เขต ${escapeHtml(d.proposedRegion||'............')} เพื่อดำเนินการในส่วนที่เกี่ยวข้องต่อไป</p><p>จึงเรียนมาเพื่อโปรดพิจารณา หากเห็นชอบโปรดลงนามตามหนังสือที่เสนอมาพร้อมนี้</p></div><div class="official-signature"><p>(${blank})</p><p>นสส./นยธ</p><p>(${blank})</p><p>นยธ. ชก. รกน. ผอ. ศรร.</p></div><p class="official-form-code">58/2-05</p></article><article class="a4-paper template-memo"><div class="memo-heading"><img class="memo-garuda" src="${GARUDA_IMAGE}" alt="ตราครุฑ"><h2>บันทึกข้อความ</h2></div><div class="memo-metadata"><p><strong>ส่วนราชการ</strong> กองบริหารคดี <strong>โทร.</strong> ๑๙๐๗</p><p><strong>ที่</strong> ปป ๐๐๐๔/ <strong>วันที่</strong> ${blank}</p></div><div class="official-subject"><strong>เรื่อง</strong><span>การส่งเอกสารเรื่องร้องเรียนเพื่อดำเนินการต่อไป</span><strong>เรียน</strong><span>ผอ. ปปท. เขต ${escapeHtml(d.proposedRegion||'.............')}</span></div><div class="official-body"><p>ด้วย กบค. ได้รับเรื่องร้องเรียนที่อยู่ในอำนาจหน้าที่ของ ปปท. เขต ${escapeHtml(d.proposedRegion||'.................')} โดยมีเรื่องร้องเรียน ดังนี้</p><table class="official-table"><thead><tr><th>ลำดับ</th><th>ประเภทเรื่องร้องเรียน</th><th>จำนวน (เรื่อง)</th><th>หมายเหตุ</th></tr></thead><tbody><tr><td>1</td><td>เรื่องร้องเรียนตามมาตรา ๖๒ (พรป. ปปช.)</td><td></td><td>ลงข้อมูลในระบบฯ แล้ว</td></tr><tr><td>2</td><td>เรื่องร้องเรียนจากระบบการตรวจสอบข้อเท็จจริง ป.ป.ท.</td><td></td><td>ลงข้อมูลในระบบฯ แล้ว</td></tr><tr><td>3</td><td>เรื่องเดิมเรื่องร้องเรียน</td><td></td><td></td></tr><tr><td>4</td><td>เรื่องอื่น ๆ</td><td></td><td></td></tr><tr><td colspan="2">รวมทั้งสิ้น</td><td></td><td></td></tr></tbody></table><p>ในการนี้ จึงขอส่งต้นฉบับเอกสารเรื่องร้องเรียนตามกรณีดังกล่าว เพื่อดำเนินการในส่วนที่เกี่ยวข้องต่อไป รายละเอียดตามบัญชีเอกสารเรื่องร้องเรียนที่แนบมาพร้อมนี้</p><p>จึงเรียนมาเพื่อโปรดพิจารณา</p></div><div class="official-signature"><p>(${blank})</p><p>ผอ. กบค.</p></div><p class="official-form-code">58/2-05</p></article><article class="a4-paper template-memo"><h2>บัญชีเอกสารเรื่องร้องเรียนของ ปปท. เขต ${escapeHtml(d.proposedRegion||'........................')}</h2><p>ตามหนังสือ ปป 0004/..................... ลงวันที่ ${blank}</p><p>๑. เรื่องร้องเรียนตามมาตรา ๖๒ (พรป. ปปช.) จำนวน ......................เรื่อง</p><table class="official-table"><thead><tr><th>ลำดับ</th><th>เลขสำนวนเรื่องร้องเรียน</th><th>จำนวน (แผ่น)</th><th>หมายเหตุ</th></tr></thead><tbody><tr><td>1</td><td>${escapeHtml(registryReceiptNumber(c)||'')}</td><td>${packTotal(state)}</td><td></td></tr></tbody></table><p>๒. เรื่องร้องเรียนจากระบบการตรวจสอบข้อเท็จจริง จำนวน ......................เรื่อง</p><p>๓. เรื่องเดิมร้องเรียน จำนวน ......................เรื่อง</p><p>รวมเรื่องร้องเรียนทั้งสิ้น จำนวน .................................................เรื่อง</p><p>ผู้รับผิดชอบ ${escapeHtml(d.assignedOfficer||blank)} ตำแหน่ง ${blank}</p><p class="official-form-code">58/2-05</p></article></div>`;
    return '';
  }
  function documentPaper(state,id,role){
    if(role==='admin')return '';
    if(id===ALL_DOCUMENTS_ID)return combinedDocumentPaper(state,role);
    if([REVIEW_RECORD_ID,LEGACY_REVIEW_RECORD_ID,'form3','pcms'].includes(id))return form3Paper(state,role);
    ...
    if(id==='1-02')return officialPaper(f102Paper(state));
    const legacy={'pakfa':'1-01','n103':'1-03','f104':'1-04','f105':'1-05','f106':'1-06','f107':'1-07','paklap':'back-cover'};
    id=legacy[id]||id;
    if(id==='back-cover')return `<article class="a4-paper template-confidential-cover" id="activePaper"><div class="confidential-cover-frame"><span class="confidential-cover-code">รปภ.๑๐</span><strong class="confidential-cover-label confidential-cover-top">ลับ</strong><div class="confidential-cover-content"><h2>ใบปกเอกสารลับ</h2><p>เอกสารแนบในปกนี้ เป็นเอกสารลับซึ่งจะต้องระมัดระวังและปฏิบัติ</p><p>ตามระเบียบว่าด้วยการรักษาความปลอดภัยแห่งชาติโดยเคร่งครัด</p></div><strong class="confidential-cover-label confidential-cover-bottom">ลับ</strong></div></article>`;
    if(id==='1-01')return officialPaper(packDocumentPaper(state,'pakfa'));
    if(id==='1-03')return officialPaper(packDocumentPaper(state,'n103'));
    if(id==='1-04')return officialPaper(officialOutgoingDocument(state,'1-04'));
    if(id==='1-05')return officialPaper(packDocumentPaper(state,'f105'));
    if(id==='1-06')return officialPaper(packDocumentPaper(state,'f106'));
    if(id==='1-07')return officialPaper(packDocumentPaper(state,'f107'));
    if(id==='1-11')return officialPaper(officialOutgoingDocument(state,'1-11'));
    if(['1-08','1-09','1-10','1-12','1-13','1-14'].includes(id))return officialPaper(corpusDocumentPaper(state,id));
    if(['58/2-03','58/2-04','58/2-05'].includes(id))return corpusDocumentPaper(state,id).replace('เลขสำนวนเรื่องร้องเรียน','เลขรับเรื่องร้องเรียน');
  ...
  function packDocumentPaper(state,id){
    const c=caseView(state),d=state.documentData,caseType=packCaseTypeOf(state),binding=officialBindings(state,canonicalDocumentId(id));
    ...
    if(id==='pakfa')return `<article class="a4-paper template-cover official-blue-cover" id="activePaper">
      ...
      <div class="cover-body">
        <p class="cover-subject"><strong>เรื่องที่</strong> ${escapeHtml(d.caseNumber||'ออกหลัง ผู้อำนวยการกองบริหารคดี ลงนาม')}</p>
    ...
    if(id==='f106')return `<article class="a4-paper template-official-letter" id="activePaper">
      ...
      <div class="official-subject"><strong>เรื่อง</strong><span>การรับเรื่องร้องเรียน</span><strong>เรียน</strong><span>${binding.recipientHtml}</span><strong>อ้างถึง</strong>${binding.complaintReferenceHtml}</div>
      ...
      ${binding.contactHtml}<div class="official-contact"><p>${escapeHtml(d.assignedOfficer?'(นาย/นาง/นางสาว '+d.assignedOfficer+' เจ้าของสำนวน)':'(นาย/นาง/นางสาว .................................. เจ้าของสำนวน)')}</p></div>
    ...
    if(id==='f107')return `<article class="a4-paper template-official-letter official-letter-107" id="activePaper">
      ...
      <div class="official-body"><p>ตามที่อ้างถึง ท่านได้ร้องเรียนไว้ต่อคณะกรรมการป้องกันและปราบปรามการทุจริตแห่งชาติ (คณะกรรมการ ป.ป.ช.) กรณีกล่าวหาร้องเรียนเจ้าหน้าที่ของรัฐ สังกัด ${escapeHtml(c.agency||'...............................................')} มีพฤติการณ์ ${escapeHtml(c.detail||'..............................................................')} นั้น</p>
        ...
        <p class="official-thanks">ขอแสดงความนับถือ</p></div>
      <div class="official-signature">${signer('')}<p>เลขาธิการคณะกรรมการป้องกันและปราบปรามการทุจริตในภาครัฐ</p><p>หรือผู้ที่ได้รับมอบหมาย</p></div>
      ${binding.contactHtml}<div class="official-contact official-contact-107"><p>${escapeHtml(d.assignedOfficer?'(นาย/นาง/นางสาว '+d.assignedOfficer+' เจ้าของสำนวน)':'(นาย/นาง/นางสาว .................................. เจ้าของสำนวน)')}</p></div>
      <p class="official-form-code official-form-code-107">ปปท. 1-07</p>
    </article>`;
    if(id==='pcms')return form3Paper(state);
    if(id==='paklap')return documentPaper(state,'back-cover',role);
  ...
  function renderWalkin(){
    ...
    const regionalIntake=['regional-clerk','regional-officer'].includes(intakeRole);
    ...
    const intakeChannels=regionalIntake?REGIONAL_CHANNELS:['Walk-In','สายด่วน 1206','เบอร์โทร ปปท. เขต 1-9','Email','สปน. 1111','Traffy Fondue','ศปท. ป.ป.ท.','Feedback','การข่าวภายใน ป.ป.ท.','ศบช. / กนป.','เครือข่ายภาคประชาชน','ข้อสั่งการ','เหตุสงสัย','หนังสือจากหน่วยงานอื่น','สื่อสารองค์กร / Facebook','จดหมาย','ม.62 (ป.ป.ช.)',NACC_API_CHANNEL];
    ...
    const anonymousField=document.createElement('div');anonymousField.className='ws-field ws-field-full';anonymousField.innerHTML='<label class="ws-choice"><input type="checkbox" id="wiAnonymous"><span><strong>ขอปกปิดข้อมูลผู้ร้อง</strong><small>ใช้เมื่อให้ข้อมูลตัวตนแก่สำนักงาน แต่ไม่ประสงค์ให้เปิดเผยชื่อ ที่อยู่ โทรศัพท์ อีเมล และข้อมูลระบุตัวตน</small></span></label>';$('#wiEmail',shell).closest('.ws-field').after(anonymousField);
    // --- staff-intake parity: wire networkStatus, agency select, accused list
    const wiTargetAgencySelect=$('#wiTargetAgency',shell);
    if(wiTargetAgencySelect)wiTargetAgencySelect.innerHTML=buildAgencyOptions('เลือกหน่วยงาน');
    const wiNetworkStatusSelect=$('#wiNetworkStatus',shell);
    const wiNetworkCodeInput=$('#wiNetworkVerifyCode',shell);
    const toggleWiNetwork=()=>{
      const isNetwork=Boolean(wiNetworkStatusSelect?.value);
      if(wiNetworkCodeInput){
        wiNetworkCodeInput.disabled=!isNetwork;
        wiNetworkCodeInput.required=isNetwork;
        if(!isNetwork)wiNetworkCodeInput.value='';
      }
    };
    wiNetworkStatusSelect?.addEventListener('change',()=>{toggleWiNetwork();render()});
    toggleWiNetwork();
    // รูปแบบหัวข้อ: auto-generate from first accused (like complaint-form)
    const wiSubjectTemplateSelect=$('#wiSubjectTemplate',shell);
    const wiSubjectInput=$('#wiSubject',shell);
    let wiSubjectAutoFilled=false;
    wiSubjectInput?.addEventListener('input',()=>{wiSubjectAutoFilled=false});
    function wiBuildSubjectFromAccused(){
      const first=shell.querySelector('#wiAccusedList .wi-accused-card');
      if(!first) return '';
      const n=first.querySelector('[data-field="name"]')?.value.trim()||'';
      const a=first.querySelector('[data-field="agency"]')?.value.trim()||'';
      const r=first.querySelector('[data-field="relation"]')?.value.trim()||'';
      let s=n?`ร้องเรียน\${n}`:'ร้องเรียนเจ้าหน้าที่ของรัฐ';
      if(a) s+=` สังกัด\${a}`;
      if(r) s+=` กรณี\${r}`;
      return s;
    }
    function wiApplyTemplate(){
      const v=wiSubjectTemplateSelect?.value||'';
      if(!v) return;
      const first=shell.querySelector('#wiAccusedList .wi-accused-card');
      if(!first){wiSubjectTemplateSelect.value='';return}
      const built=wiBuildSubjectFromAccused();
      if(built && built!=='ร้องเรียนเจ้าหน้าที่ของรัฐ'){wiSubjectInput.value=built;wiSubjectAutoFilled=true;render()}
    }
    wiSubjectTemplateSelect?.addEventListener('change',wiApplyTemplate);
    // auto-suggest when accused changes
    const wiAutoSuggestSubject=()=>{
      if(wiSubjectInput.value && !wiSubjectAutoFilled) return;
      const s=wiBuildSubjectFromAccused();
      if(!s || s==='ร้องเรียนเจ้าหน้าที่ของรัฐ') return;
      wiSubjectInput.value=s;wiSubjectAutoFilled=true;render();
    };
    // accused list (multi) — same pattern as complaint-form addAccused
    let wiAccusedSeq=0;
    const wiAccusedList=$('#wiAccusedList',shell);
    const wiAccusedEmpty=$('#wiAccusedEmpty',shell);
    // render is defined later — use lazy resolver to avoid temporal dead zone
    const wiRerender=()=>{try{render()}catch{}};
    function wiAddAccused(){
      wiAccusedSeq++;
      const card=document.createElement('div');
      card.className='wi-accused-card ws-card';
      card.style.cssText='padding:1rem;margin-bottom:.75rem;position:relative';
      card.innerHTML=`<button type="button" class="ws-button ghost wi-remove-accused" style="position:absolute;top:.5rem;right:.5rem;padding:.2rem .6rem">ลบ</button><div class="ws-field"><strong>ผู้ถูกร้องเรียนที่ ${wiAccusedSeq}</strong></div><div class="ws-grid-2" style="margin-top:.5rem"><div class="ws-field"><label>ชื่อ-นามสกุล / หน่วยงาน</label><input type="text" class="form-control" data-field="name" placeholder="ชื่อผู้ถูกร้องหรือหน่วยงาน"></div><div class="ws-field"><label>ตำแหน่ง</label><input type="text" class="form-control" data-field="position" placeholder="ตำแหน่ง"></div><div class="ws-field"><label>สังกัดหน่วยงาน</label><select class="form-select" data-field="agency">${buildAgencyOptions('เลือกหน่วยงาน')}</select></div><div class="ws-field"><label>ความเกี่ยวข้อง / พฤติการณ์โดยย่อ</label><input type="text" class="form-control" data-field="relation" placeholder="พฤติการณ์โดยย่อ"></div></div>`;
      card.querySelector('.wi-remove-accused').addEventListener('click',()=>{card.remove();if(!wiAccusedList.querySelector('.wi-accused-card'))wiAccusedEmpty?.classList.remove('ws-hidden');wiRerender()});
      wiAccusedList?.appendChild(card);
      wiAccusedEmpty?.classList.add('ws-hidden');
      card.querySelectorAll('input,select').forEach(el=>{el.addEventListener('input',()=>{wiRerender();try{wiAutoSuggestSubject()}catch{}});});
      card.querySelectorAll('select').forEach(el=>el.addEventListener('change',()=>{wiRerender();try{wiAutoSuggestSubject()}catch{}}));
      wiRerender();
    }
    $('#wiAddAccused',shell)?.addEventListener('click',wiAddAccused);
    // seed one empty accused card to match UX expectation (user can fill or leave empty — not required)
    wiAddAccused();
    // keep first card empty by default for demo compatibility; clear seeded values
    if(wiAccusedList?.firstElementChild){
      wiAccusedList.firstElementChild.querySelectorAll('input').forEach(i=>i.value='');
      const sel=wiAccusedList.firstElementChild.querySelector('select[data-field="agency"]');
      if(sel)sel.value='';
    }
    const receiptTab=$('[data-wi-tab="receipt"]',shell);receiptTab.disabled=true;receiptTab.textContent='58/2-02 ใบแจ้งเลขติดตาม';
    bindCitizenIdInput($('#wiCitizen',shell));
    ...
    const complaintPaper=()=>{
      const titlePrefix=val('wiTitlePrefix').trim();
      const firstName=val('wiFirstName').trim();
      const lastName=val('wiLastName').trim();
      const fullName=[titlePrefix,firstName,lastName].filter(Boolean).join(' ')||[firstName,lastName].filter(Boolean).join(' ');
      const accusedList=[...shell.querySelectorAll('#wiAccusedList .wi-accused-card')].map(card=>({
        name:card.querySelector('[data-field=\"name\"]')?.value.trim()||'',
        position:card.querySelector('[data-field=\"position\"]')?.value.trim()||'',
        agency:card.querySelector('[data-field=\"agency\"]')?.value.trim()||'',
        relation:card.querySelector('[data-field=\"relation\"]')?.value.trim()||''
      })).filter(x=>x.name||x.position||x.agency||x.relation);
      const primaryAccused=accusedList[0]||{name:'',position:'',agency:'',relation:''};
      const targetAgency=val('wiTargetAgency').trim();
      const accusedName=primaryAccused.name||targetAgency||'';
      const isAgency=/สำนักงาน|กรม|กระทรวง|เทศบาล|องค์การ|หน่วยงาน|มหาวิทยาลัย|โรงพยาบาล/.test(accusedName);
      const networkStatus=val('wiNetworkStatus').trim();
      const isNetworkMember=Boolean(networkStatus);
      const networkVerifyCode=val('wiNetworkVerifyCode').trim();
      ...
      return `<article class="a4-paper template-petition" id="activePaper">
        ...
        <section class="petition-section"><h3>ข้อมูลผู้ร้องเรียน</h3>
          ...
          <p>คำนำหน้านาม ${field(titlePrefix)} ชื่อ ${field(firstName)} นามสกุล ${field(lastName)} หมายเลขบัตรประชาชน ${field(val('wiCitizen'))}</p>
          <p>ที่อยู่ปัจจุบัน ${field(val('wiAddress'))}</p>
          <p>อำเภอ/เขต ${field(val('wiDistrict'))} จังหวัด ${field(val('wiProvince'))} รหัสไปรษณีย์ ${field(val('wiPostcode'))} &nbsp; ตำบล/แขวง ${field(val('wiSubdistrict'))}</p>
          <p>หมายเลขโทรศัพท์ที่ติดต่อได้ ${field(val('wiPhone'))} EMAIL ${field(val('wiEmail'))}</p>
          <p>ท่านเป็นเครือข่ายของสำนักงาน ป.ป.ท. ${checkMark(!isNetworkMember)} ไม่ได้เป็นเครือข่าย&nbsp;&nbsp;&nbsp; ${checkMark(isNetworkMember)} เป็นเครือข่าย ${field(isNetworkMember?(networkStatus+(networkVerifyCode?' ('+networkVerifyCode+')':'')): '')}</p>
        </section>
        <section class="petition-section"><h3>ข้อมูลบุคคล/หน่วยงานที่ร้องเรียน</h3>
          <p>${checkMark(!isAgency)} ร้องเรียนเจ้าหน้าที่ของรัฐ (กรณีมากกว่า 1 รายให้เพิ่มในรายละเอียดการร้องเรียน/เบาะแส)</p>
          <p>คำนำหน้านาม ${field('')} ชื่อ ${field(isAgency?'':primaryAccused.name)} นามสกุล ${field('')}</p>
          <p>ตำแหน่ง ${field(primaryAccused.position)} สังกัด ${field(isAgency?'':primaryAccused.agency)}</p>
          <p>กรม ${field('')} กระทรวง ${field('')}</p>
          <p>${checkMark(isAgency)} ร้องเรียนหน่วยงาน กรม ${field(isAgency?primaryAccused.agency:'')} กระทรวง/อื่น ๆ ${field(isAgency?targetAgency:'')}</p>
          <p>ชื่อโครงการที่ต้องการร้องเรียน (ถ้ามี) ${field(val('wiProjectName'))}</p>
          ${accusedList.length>1?`<div style="margin-top:.5rem"><strong>ผู้ถูกร้องเพิ่มเติม (${accusedList.length-1} ราย):</strong> ${accusedList.slice(1).map((a,i)=>`<p>${i+2}. ${field(a.name)} — ตำแหน่ง ${field(a.position)} สังกัด ${field(a.agency)} ${a.relation?' — พฤติการณ์: '+field(a.relation):''}</p>`).join('')}</div>`:''}
        ...
        <section class="petition-section petition-followup">
          ...
          <p><strong>ในกรณีเดียวกันมีการร้องเรียนไปที่หน่วยงานอื่นใดหรือไม่ ผลดำเนินการเป็นประการใด</strong> ${field(val('wiOtherAgency'))}</p>
          <p><strong>ได้มอบเอกสารประกอบเรื่องร้องเรียน ดังนี้</strong></p>
          <div class="petition-attachments">${attachments.length?attachments.map((a,index)=>`<p>${index+1}. ${escapeHtml(attachmentName(a))}${escapeHtml(attachmentDetails(a))}</p>${attachmentDesc(a)?`<p class="petition-attachment-desc">${escapeHtml(attachmentDesc(a))}</p>`:''}`).join(''):'<p>............................................................................................................................................................</p><p>............................................................................................................................................................</p><p>............................................................................................................................................................</p>'}</div>
        </section>
    ...
    function wiRepositionDatePopover(){positionPopoverNear($('#wiIncidentDateTrigger',shell),$('#wiThaiCalendarPopover',shell),280,360)}
    function wiOpenDatePicker(){
      ...
      wiRepositionDatePopover();
      document.addEventListener('click',wiOnDocClickCloseCalendar);
      window.addEventListener('scroll',wiRepositionDatePopover,true);
      window.addEventListener('resize',wiRepositionDatePopover);
    }
    function wiCloseDatePicker(){
      ...
      window.removeEventListener('scroll',wiRepositionDatePopover,true);
      window.removeEventListener('resize',wiRepositionDatePopover);
    }
    ...
    function wiRepositionTimePopover(){positionPopoverNear($('#wiIncidentTimeTrigger',shell),$('#wiTimePickerPopover',shell),290,420)}
    function wiOpenTimePicker(){
      ...
      wiRepositionTimePopover();
      document.addEventListener('click',wiOnDocClickCloseTimePicker);
      window.addEventListener('scroll',wiRepositionTimePopover,true);
      window.addEventListener('resize',wiRepositionTimePopover);
      requestAnimationFrame(()=>{wiHourWheelApi.setValue(h);wiMinuteWheelApi.setValue(m);wiRepositionTimePopover()});
    }
    function wiCloseTimePickerPopover(){
      ...
      window.removeEventListener('scroll',wiRepositionTimePopover,true);
      window.removeEventListener('resize',wiRepositionTimePopover);
    }
    ...
    $('#wiDraft').onclick=()=>{const accusedDraft=[...shell.querySelectorAll('#wiAccusedList .wi-accused-card')].map(c=>({name:c.querySelector('[data-field="name"]')?.value||'',agency:c.querySelector('[data-field="agency"]')?.value||''})).filter(x=>x.name||x.agency);localStorage.setItem('ecmis-walkin-draft',JSON.stringify({subject:val('wiSubject'),projectName:val('wiProjectName'),targetAgency:val('wiTargetAgency'),accusedDraft,address:val('wiAddress'),networkStatus:val('wiNetworkStatus'),placeDescription:val('wiPlaceDescription'),otherAgency:val('wiOtherAgency'),complainantTitle:val('wiTitlePrefix'),complainantFirst:val('wiFirstName'),complainantLast:val('wiLastName'),detail:val('wiDetail'),channel:val('wiChannel'),region:regionalIntake?regionalRegion:val('wiIntakeRegion')}));notify('success','บันทึกร่างแล้ว','ข้อมูลยังไม่ถูกลงรับและยังไม่ออกเลขติดตาม')};
    ...
    $('#wiReceive').onclick=async()=>{
      ...
      const firstName=val('wiFirstName').trim();
      const lastName=val('wiLastName').trim();
      const wiComplainantFull=[titlePrefix,firstName,lastName].filter(Boolean).join(' ');
      const personalFields=[wiComplainantFull,val('wiCitizen'),val('wiPhone'),val('wiEmail'),val('wiAddress')].map(v=>String(v||''));
      ...
      // collect accused list
      const accusedCards=[...shell.querySelectorAll('#wiAccusedList .wi-accused-card')].map(card=>({
        name:card.querySelector('[data-field="name"]')?.value.trim()||'',
        position:card.querySelector('[data-field="position"]')?.value.trim()||'',
        agency:card.querySelector('[data-field="agency"]')?.value.trim()||'',
        relation:card.querySelector('[data-field="relation"]')?.value.trim()||''
      ...
      const networkStatus=val('wiNetworkStatus').trim();
      const networkVerifyCode=val('wiNetworkVerifyCode').trim();
      if(networkStatus&&!networkVerifyCode)return notify('warning','ข้อมูลเครือข่ายไม่ครบ','เลือกเครือข่ายแล้วต้องกรอกรหัสยืนยันเครือข่าย ป.ป.ท.');
      const ok=await confirmDo('ยืนยันการลงรับเรื่อง','เมื่อกดลงรับ เรื่องจะถูกบันทึกพร้อมออกเลขติดตามและ PIN ทันที ก่อนส่งให้เจ้าหน้าที่กลั่นกรองเรื่องซ้ำ','ลงรับเรื่อง');
      if(!ok.isConfirmed)return;
      const pair=window.ECMIS?.createTrackingNumber?.();
      if(!pair?.yearSequence||!pair?.pin)return notify('warning','ไม่สามารถออกเลขรับบริการ','ไม่พบฟังก์ชันจัดสรรเลขรับบริการ กรุณาลองใหม่');
      let qr='';
      const caseId=`ECMIS-${pair.yearSequence}`;
      const primaryAgency=accusedCards[0]?.agency||val('wiTargetAgency')||val('wiTargetAgency')||'';
      const primaryAccusedName=accusedCards[0]?.name||'';
      receipt={id:caseId,trackingYear:pair.yearSequence,trackingCode:pair.pin,allocationStatus:pair.allocationStatus||'allocated',received:now(),receivingOfficer:val('wiOfficer'),assignedOfficer:val('wiOfficer'),subject:val('wiSubject'),projectName:val('wiProjectName'),targetAgency:val('wiTargetAgency'),complainant:wiComplainantFull,titlePrefix,firstName,lastName,address:val('wiAddress'),networkStatus,networkVerifyCode,accusedList:accusedCards,placeDescription:val('wiPlaceDescription'),otherAgency:val('wiOtherAgency'),citizenId:val('wiCitizen'),identityLast4,id4:isValidThaiCitizenId(citizenDigits)?citizenDigits.slice(-4):'',phone:phoneDigits,anonymousDetected,agency:primaryAgency,accusedName:primaryAccusedName,channel:selectedChannel,province:val('wiProvince'),district:val('wiDistrict'),subdistrict:val('wiSubdistrict'),region:selectedRegion||'ส่วนกลาง',place:val('wiPlace'),incidentDate:val('wiIncidentDate'),incidentTime:wiIncidentTimeText(),detail:val('wiDetail'),damage:val('wiDamage'),request:val('wiRequest'),attachments:[],qr};
      receipt.registry={};
      const state=initialState(receipt);
      saveState(receipt.id,state);
      $('#wiDocStatus').textContent=`ลงรับแล้ว · เลขรับบริการ ${receipt.trackingYear}`;
      $('#wiDocStatus').classList.add('success');
      receiptTab.disabled=false;
      tab='complaint';
      $$('[data-wi-tab]',shell).forEach(x=>x.classList.toggle('active',x.dataset.wiTab===tab));
      render();
      return notify('success','ลงรับเรื่องแล้ว',`ออกเลขรับบริการ ${receipt.trackingYear} และ PIN แล้ว พร้อมส่งต่อให้ ${ROLE_LABELS[state.workflow.owner]}`);
    };
    render();
    ...
    $('#wiReceive').onclick=async()=>{
      ...
      const requiresTrackingDocument=issuesTrackingImmediately(val('wiChannel'))||['Walk-In','จดหมาย'].includes(val('wiChannel'));
  ...
  function renderStaff(){
    const root=$('#staffApp');if(!root)return;const params=new URLSearchParams(location.search);const isA5Route=params.get('view')==='a5';const authAdapter=globalThis.ECMISIntakeAuth;const aggregateAuth=authAdapter?.mode?.()==='aggregate';if(aggregateAuth&&!authAdapter.guard()){root.innerHTML='';return}const aggregateRole=aggregateAuth?authAdapter.currentRole('a4'):'';if(aggregateAuth&&!aggregateRole){root.innerHTML='<main class="ws-container"><section class="ws-card ws-callout">บัญชีนี้ไม่มีบทบาทในระบบรับเรื่องร้องเรียน</section></main>';return}let selectedRole=aggregateRole||params.get('role')||sessionStorage.getItem('ecmis-a4-role')||'admin';let activeRole=selectedRole;let activeRegion=REGIONAL_REGIONS.includes(params.get('region'))?params.get('region'):(sessionStorage.getItem('ecmis-a4-regional-region')||'เขต 1');if(!REGIONAL_REGIONS.includes(activeRegion))activeRegion='เขต 1';let selectedId=isA5Route?null:params.get('case')||null;let adminQueue=['unassigned','assigned'].includes(params.get('queue'))?params.get('queue'):'unassigned';let filters={};
    const approvalInFlight=new Set();
    async function runApprovalOnce(action,work){
      const key=`${selectedId||'unknown'}:${action}`;
      if(approvalInFlight.has(key)){notify('info','กำลังดำเนินการอนุมัติ','ระบบรับคำสั่งอนุมัติของเรื่องนี้แล้ว');return false}
    ...
    async function runCaseNumberApprovalLocked(caseId,work,{allowSignedAwaitingApproval=false}={}){
      if(!navigator.locks?.request){
        notify('error','ไม่สามารถออกเลขสำนวนอย่างปลอดภัย','เบราว์เซอร์นี้ไม่รองรับการล็อกทะเบียนเลขสำนวน กรุณาใช้เบราว์เซอร์ที่รองรับ Web Locks API');
        return {status:'blocked',state:null};
      }
      return navigator.locks.request('ecmis-a4-case-number-issuance',{mode:'exclusive'},async()=>{
        const freshState=getState(caseId),freshData=freshState.documentData,freshWorkflow=freshState.workflow;
        const alreadyApproved=Boolean(freshData.caseNumber)||(!allowSignedAwaitingApproval&&(Boolean(freshData.approvalStatus||freshData.approvedAt)||['officer-finalize-screening','officer-dispatch','activity5-dispatch','notification-confirmed'].includes(freshWorkflow.stage)));
        if(alreadyApproved)return {status:'already',state:freshState};
        return work(freshState);
      });
    }
    ...
    const updateIntakeLink=()=>root.querySelector('a[href^="staff-intake.html"],a[href="complaint-form.html?mode=walkin"]')?.setAttribute('href',['regional-clerk','regional-officer'].includes(selectedRole)?`staff-intake.html?role=${encodeURIComponent(selectedRole)}&region=${encodeURIComponent(activeRegion)}`:'staff-intake.html');
    updateIntakeLink();
    ...
    const lockRole=aggregateAuth&&authAdapter.showRoleSelector()===false;
    if(roleSelect&&lockRole)roleSelect.disabled=true;
    ...
    const syncUrl=()=>{
      ...
      if(isRegionalRole(selectedRole))next.searchParams.set('region',activeRegion);else next.searchParams.delete('region');
    ...
    const updateRoleControls=()=>{
      const regional=isRegionalRole(selectedRole);
      const suggestionsBtn = document.getElementById('manageSuggestions');
      if (suggestionsBtn) suggestionsBtn.classList.toggle('ws-hidden', selectedRole === 'anonymous');
      const backupsBtn = document.getElementById('manageBackups');
      if (backupsBtn) backupsBtn.classList.toggle('ws-hidden', selectedRole !== 'division');
      ...
      if(channelFilter)channelFilter.innerHTML=`<option value="">ทุกช่องทาง</option>${(regional?REGIONAL_CHANNELS:['Website','Walk-in','สายด่วน 1206','หนังสือราชการ',NACC_API_CHANNEL]).map(channel=>`<option>${channel}</option>`).join('')}`;
      const regionFilter=$('#filterRegion');
      if(regionFilter){regionFilter.disabled=regional;regionFilter.value=regional?activeRegion:''}
      updateIntakeLink();
    };
    const visibilityContext=queue=>({role:selectedRole,activeRegion,adminQueue:queue||adminQueue});
    const canViewState=state=>roleCanSeeState(state,visibilityContext());
    $('#wsRole').value=selectedRole;updateRoleControls();$('#wsRole').onchange=e=>{if(lockRole){e.target.value=selectedRole;return}selectedRole=e.target.value;activeRole=selectedRole;sessionStorage.setItem('ecmis-a4-role',selectedRole);updateRoleControls();delete $('#caseListView').dataset.anonymousReady;if(selectedId&&!canViewState(getState(selectedId)))selectedId=null;syncUrl();if(selectedId)renderDetail();else{$('#caseDetailView').classList.add('ws-hidden');$('#caseListView').classList.remove('ws-hidden')}renderList()};
    ...
    if(searchInput){
      ...
      searchInput.addEventListener('input',()=>{
        ...
        searchTimer=setTimeout(()=>{
          ...
          const caseHits=allCases().map(c=>getState(c.id)).filter(state=>roleCaseSearchText(state,selectedRole).includes(q));
          ...
          let html='';
          if(caseHits.length){html+=`<div class="ws-search-group">คดี (${caseHits.length})</div>`+caseHits.slice(0,6).map(st=>{const c=st.caseData;const tracking=['central-registry-clerk','case-admin-clerk'].includes(selectedRole)?'':` · เลขติดตาม ${escapeHtml(c.trackingYear||'รอออกเลขรับเรื่อง')}`;return`<button type="button" class="ws-search-item" data-search-case="${c.id}"><span>${escapeHtml(c.subject||'ไม่ระบุชื่อเรื่อง')}</span><small>เลขรับเรื่อง ${escapeHtml(caseReferenceForRole(c,selectedRole))}${tracking} · ${escapeHtml(c.channel)}</small></button>`}).join('')}
      ...
      searchResults.addEventListener('click',e=>{
        ...
        if(docBtn){if(!selectedId){selectedId=allCases()[0]?.id||null;if(selectedId){syncUrl();renderDetail()}}const tab=document.querySelector(`[data-doc="${docBtn.dataset.searchDoc}"]`);if(tab)tab.click();closeSearch();searchInput.blur();return}
    ...
    const duplicateObserver=new MutationObserver(()=>{
      enhanceNaccIndicators();
      enhanceNaccDecisionLock();
    ...
    function filterDateRepositionPopover(){positionPopoverNear($('#filterDateTrigger'),$('#filterDateCalendarPopover'),280,380)}
    function filterDateOpenPicker(){
      ...
      filterDateRepositionPopover();
      document.addEventListener('click',filterDateOnDocClick);
      window.addEventListener('scroll',filterDateRepositionPopover,true);
      window.addEventListener('resize',filterDateRepositionPopover);
    }
    function filterDateClosePicker(){
      ...
      window.removeEventListener('scroll',filterDateRepositionPopover,true);
      window.removeEventListener('resize',filterDateRepositionPopover);
    }
    ...
    function mergedCases(){
      ...
      return [...merged.values()].sort((a,b)=>caseSortTime(b)-caseSortTime(a)||String(b.id).localeCompare(String(a.id),"th",{numeric:true}));
    }
    function allCases(){return mergedCases().filter(caseData=>canViewState(getState(caseData.id)))}
    function canRegionalAccess(caseData){return !isRegionalRole(selectedRole)||(caseData?.region===activeRegion&&isRegionalChannel(caseData?.channel))}
    ...
    function naccAlerts(state){return window.ECMISActivity4DocumentRules?.naccIntakeAlerts(state)||[]}
    function enhanceNaccDecisionLock(){
      if(!selectedId)return;
      const state=getState(selectedId);
      if(!isNaccApiCase(state))return;
      const selected=$('input[name="decision"][value="18/1ข"]');
      const grid=selected?.closest('.ws-choice-grid');
      if(!grid||grid.dataset.naccLocked)return;
      grid.dataset.naccLocked='true';
      grid.innerHTML='<div class="nacc-decision-lock"><span class="nacc-source-badge">เรื่องจาก ป.ป.ช.</span><strong>รับไว้ดำเนินการ 18/1 (ข)</strong><small>ระบบกำหนดจากข้อมูล API และล็อกไม่ให้เปลี่ยนมาตรา</small><input type="radio" name="decision" value="18/1ข" checked disabled></div>';
    }
    function enhanceNaccIndicators(){
      $$('[data-case]').forEach(row=>{
        const state=getState(row.dataset.case);
        if(!isNaccApiCase(state)||row.dataset.naccMarked)return;
        row.dataset.naccMarked='true';
        const flagCell=row.lastElementChild;
        if(!flagCell)return;
        const alerts=naccAlerts(state);
        flagCell.insertAdjacentHTML('afterbegin',`<div class="nacc-row-flags"><span class="nacc-source-badge">เรื่องจาก ป.ป.ช.</span>${alerts.length?`<span class="nacc-alert-count">แจ้งเตือน ${alerts.length}</span>`:''}</div>`);
      });
      if(!selectedId)return;
      const state=getState(selectedId);
      if(!isNaccApiCase(state))return;
      const detail=$('#caseDetailView');
      const head=$('.ws-case-head',detail);
      if(head&&!detail.dataset.naccMarked){
        detail.dataset.naccMarked='true';
        const alerts=naccAlerts(state);
        head.insertAdjacentHTML('afterend',`<section class="nacc-intake-notice"><header><span class="nacc-source-badge">เรื่องจาก ป.ป.ช.</span><strong>มาตรา 18/1 (ข) · รับผ่าน API</strong></header>${alerts.length?`<div class="nacc-alert-list">${alerts.map(alert=>`<article class="${alert.severity}"><strong>${escapeHtml(alert.title)}</strong><span>${escapeHtml(alert.action)}</span></article>`).join('')}</div>`:'<p>ยังไม่พบเงื่อนไขที่ต้องแจ้งเตือนเพิ่มเติม</p>'}</section>`);
        if(alerts.length&&window.Swal){
          const key=`ecmis-nacc-alert-${selectedId}`;
          if(sessionStorage.getItem(key)!=='shown'){
            sessionStorage.setItem(key,'shown');
            setTimeout(()=>Swal.fire({icon:'warning',title:'เรื่องจาก ป.ป.ช. ต้องตรวจสอบ',html:alerts.map(alert=>`<p><strong>${escapeHtml(alert.title)}</strong><br>${escapeHtml(alert.action)}</p>`).join(''),confirmButtonText:'รับทราบ',confirmButtonColor:'#082b50'}),0);
          }
        }
      }
    }
    ...
    function outgoingRegisterPanel(state){
      const api=window.ECMISActivity4OutgoingRegisters;
      if(!api||!['officer-dispatch','nacc-dispatch'].includes(state.workflow.stage)||state.workflow.complete)return '';
      const slots=api.resolveOutgoingSlots(state),ledger=api.readLedger(localStorage),selections=state.documentData.outgoingRegisterSelections||{},actorContext={role:activeRole,scope:'central',storage:localStorage};
      if(!slots.length)return '';
      const cards=slots.map(slot=>{
        const doc=OFFICIAL_DOCUMENTS[slot.documentId]||{name:slot.documentId};
        const referenced=slot.mode==='reference'?api.issuanceForSlot(state,slot.referenceSlotId,ledger):null;
        const issuance=slot.mode==='reference'?referenced:api.issuanceForSlot(state,slot.slotId,ledger);
        const title=`ปปท. ${slot.documentId} ${doc.name}`;
        if(slot.mode==='reference')return `<article class="outgoing-register-card reference" data-outgoing-slot="${escapeHtml(slot.slotId)}"><header><div><span class="outgoing-register-eyebrow">เอกสารอ้างอิงเลข</span><strong>${escapeHtml(title)}</strong></div><b class="outgoing-register-status neutral">ใช้เลขร่วม</b></header><div class="outgoing-reference-detail"><span>อ้างอิงเลขจาก ปปท. 1-11</span><output>${escapeHtml(issuance?.number||'รอออกเลขหนังสือหลัก')}</output></div></article>`;
        if(issuance){
          const legacy=issuance.status==='legacy-unverified';
          const register=api.registerCatalog(actorContext)[issuance.registerId];
          const registerLabel=issuance.registerLabel||register?.label||issuance.registerId||'ไม่ระบุทะเบียน';
          const issuedAtDate=new Date(issuance.issuedAt||''),issuedAt=Number.isNaN(issuedAtDate.getTime())?'ไม่ระบุเวลา':new Intl.DateTimeFormat('th-TH',{dateStyle:'medium',timeStyle:'short'}).format(issuedAtDate);
          return `<article class="outgoing-register-card ${legacy?'warning':'issued'}" data-outgoing-slot="${escapeHtml(slot.slotId)}"><header><div><span class="outgoing-register-eyebrow">${legacy?'เลขเดิมที่ยังยืนยันทะเบียนไม่ได้':'ออกเลขแล้ว'}</span><strong>${escapeHtml(title)}</strong></div><b class="outgoing-register-status ${legacy?'attention':'complete'}">${legacy?'ต้องตรวจสอบทะเบียน':'ออกเลขและล็อกแล้ว'}</b></header><div class="ws-readonly outgoing-issued-details"><dl><div><dt>ทะเบียน</dt><dd>${escapeHtml(registerLabel)}</dd></div><div class="outgoing-number-detail"><dt>เลขหนังสือขาออก</dt><dd>${escapeHtml(issuance.number)}</dd></div><div><dt>ผู้ออกเลข</dt><dd>${escapeHtml(issuance.issuedBy||'ไม่ระบุผู้บันทึก')}</dd></div><div><dt>วันเวลาที่ออกเลข</dt><dd>${escapeHtml(issuedAt)}</dd></div></dl></div></article>`;
        }
        if(slot.mode==='blocked-unconfigured'){
          const prefix=REGIONAL_PREFIX_BY_REGION[state.caseData.region]||'ปป ????';
          return `<article class="outgoing-register-card blocked" data-outgoing-slot="${escapeHtml(slot.slotId)}"><header><div><span class="outgoing-register-eyebrow">ทะเบียนเขตยังไม่ได้กำหนด</span><strong>${escapeHtml(title)}</strong></div><b class="outgoing-register-status attention">รอบันทึกเลขทางการ</b></header><p class="outgoing-register-note">ห้ามใช้ prefix ส่วนกลางแทนเลขเขต บันทึกได้เฉพาะเลขทางการที่เขตออกจริง<br><small style="color:#6c757d">ตัวอย่างเขตนี้: <code>${escapeHtml(prefix)}/????</code> — เลขรัน 4 หลัก/ปี พ.ศ. เช่น ${escapeHtml(prefix)}/0001 — <a href="#" onclick="event.preventDefault(); const d=document.querySelector('#regionalPrefixHint details'); if(d) d.open=true; document.getElementById('regionalPrefixHint').scrollIntoView({behavior:'smooth'})">ดูตารางทั้งหมด</a></small></p><div class="outgoing-manual-fields" style="display:grid;grid-template-columns:1fr 1fr;gap:.6rem;align-items:end"><label class="ws-field" style="margin:0"><span>เลขหนังสือที่เขตออกจริง *</span><input data-outgoing-manual-number="${escapeHtml(slot.slotId)}" placeholder="เช่น ${escapeHtml(prefix)}/0001" style="width:100%"></label><label class="ws-field" style="margin:0"><span>ที่มา/หมายเหตุ *</span><input data-outgoing-manual-note="${escapeHtml(slot.slotId)}" placeholder="เช่น สมุดทะเบียนเขต เล่ม 1 หน้า 12" style="width:100%"></label><button type="button" class="ws-button secondary" data-action="record-manual-outgoing" data-slot-id="${escapeHtml(slot.slotId)}" style="grid-column:1 / -1;justify-self:end;padding:.45rem .9rem">บันทึกเลขทางการของเขต</button></div></article>`;
        }
        const registerId=slot.fixedRegisterId||selections[slot.slotId]||'';
        const allowed=api.allowedRegistersForSlot(slot,actorContext);
        const addRegister=['1-03','1-04','1-06'].includes(slot.documentId)?`<button type="button" class="ws-button ghost outgoing-add-register" data-action="add-outgoing-register" data-slot-id="${escapeHtml(slot.slotId)}">เพิ่มทะเบียนใหม่</button>`:'';
        const selector=slot.mode==='fixed'?`<div class="outgoing-fixed-register"><span>ทะเบียน</span><strong>${escapeHtml(api.REGISTER_DEFINITIONS[slot.fixedRegisterId]?.label||slot.fixedRegisterId)}</strong><small>กำหนดจากแบบเอกสาร</small></div>`:`<div class="outgoing-register-picker"><label class="ws-field"><span>ทะเบียนเลขหนังสือขาออก *</span><select data-outgoing-register="${escapeHtml(slot.slotId)}"><option value="">เลือกทะเบียน</option>${allowed.map(register=>`<option value="${escapeHtml(register.id)}" ${register.id===registerId?'selected':''}>${escapeHtml(register.label)} · ${escapeHtml(register.prefix)}</option>`).join('')}</select></label>${addRegister}</div>`;
        return `<article class="outgoing-register-card pending" data-outgoing-slot="${escapeHtml(slot.slotId)}"><header><div><span class="outgoing-register-eyebrow">เลขหนังสือขาออก</span><strong>${escapeHtml(title)}</strong></div><b class="outgoing-register-status pending">รอออกเลข</b></header>${selector}<div class="outgoing-issue-row"><div><span>สถานะเลขหนังสือ</span><output>ยังไม่ได้ออกเลข</output></div><button type="button" class="ws-button primary outgoing-issue-button" data-action="issue-outgoing-number" data-slot-id="${escapeHtml(slot.slotId)}" data-fixed-register="${escapeHtml(slot.fixedRegisterId||'')}">ออกเลขหนังสือ</button></div></article>`;
      }).join('');
      const hintTable=`<div id="regionalPrefixHint" style="display:none;margin:0 1rem 1rem;background:#f8f9fa;border:1px solid #e9ecef;border-radius:8px;padding:.8rem;overflow:auto"><table class="ws-table" style="width:100%;font-size:.85rem;margin:0"><thead><tr><th>เขต</th><th>เลขต้น</th><th>ตัวอย่าง</th></tr></thead><tbody>${Object.entries(REGIONAL_PREFIX_BY_REGION).filter(([k])=>k!=='ส่วนกลาง').map(([region,prefix])=>`<tr><td>${escapeHtml(region)}</td><td><code>${escapeHtml(prefix)}</code></td><td><code>${escapeHtml(prefix)}/????</code></td></tr>`).join('')}<tr><td>ส่วนกลาง (ศรร)</td><td><code>ปป 0004.2</code></td><td><code>ปป 0004.2/????</code></td></tr></tbody></table><small style="color:#6c757d">อ้างอิงเอกสารทะเบียนหนังสือราชการ ป.ป.ท. (ปป 0014-0022)</small></div>`;
      return `<section class="ws-section outgoing-register-section" data-outgoing-register-section><header class="outgoing-register-section-head" style="padding:1rem 1.2rem"><div style="display:flex;justify-content:space-between;align-items:center;gap:1rem"><div><h3 style="margin:0;display:flex;align-items:center;gap:.6rem">เลขหนังสือขาออก <button type="button" onclick="const el=document.getElementById('regionalPrefixHint'); el.style.display=el.style.display==='none'?'block':'none'; if(el.style.display==='block') el.scrollIntoView({behavior:'smooth',block:'nearest'})" style="background:#0b2e59;color:#fff;width:22px;height:22px;border-radius:50%;border:none;display:inline-flex;align-items:center;justify-content:center;font-weight:700;font-size:.85rem;cursor:pointer" title="ดูตารางเลขต้น">?</button></h3><p style="margin:.3rem 0 0;color:#6c757d;font-size:.85rem">เลือกทะเบียน แล้วให้ระบบ Running ตามทะเบียนและปี พ.ศ. เอกสารแต่ละฉบับแยกเลขกัน</p></div></div></header>${hintTable}<div class="outgoing-register-list" style="padding:0 1rem 1rem">${cards}</div></section>`;
    }
    function captureOutgoingSelections(state){
      const selections={...(state.documentData.outgoingRegisterSelections||{})};
      $$('[data-outgoing-register]').forEach(select=>{if(select.value)selections[select.dataset.outgoingRegister]=select.value});
      state.documentData.outgoingRegisterSelections=selections;
      return state;
    }
    function outgoingDispatchValidation(state){
      const api=window.ECMISActivity4OutgoingRegisters;
      return api?api.validateRequiredIssuances(state,api.readLedger(localStorage),{storage:localStorage}):{valid:false,errors:['OUTGOING_RULES_UNAVAILABLE']};
    }
    function duplicatePackagePanel(state){
      const d=state.documentData;
      if(d.approvedOpinion!=='suspected_duplicate'||!d.duplicateMergedAt)return '';
      const target=findOriginalCase(d.approvedLinkedCaseId);
      const sourceFiles=Array.isArray(state.caseData.attachments)?state.caseData.attachments:[];
      const targetFiles=Array.isArray(target?.caseData?.attachments)?target.caseData.attachments:[];
      const fileList=(files,empty)=>files.length?files.map(file=>`<li>${escapeHtml(typeof file==='string'?file:file?.name||'เอกสารแนบ')}</li>`).join(''):`<li>${empty}</li>`;
      const prepared=Boolean(d.duplicatePackagePreparedAt);
      return `<section class="ws-section"><h3>จัดชุดเอกสารเรื่องที่รวม</h3><div class="ws-callout"><strong>เรื่องปัจจุบัน ${escapeHtml(state.caseData.id)}</strong> รวมกับ <strong>${escapeHtml(d.approvedLinkedCaseId)}</strong><br>สถานะ: ${prepared?`ยืนยันจัดชุดครบแล้ว โดย ${escapeHtml(d.duplicatePackagePreparedBy||'เจ้าหน้าที่รับเรื่อง')}`:'รอยืนยันจัดชุดเอกสารทั้งหมดก่อนจัดส่ง'}</div><div class="ws-grid-2"><div><strong>เอกสารเรื่องปัจจุบัน</strong><ul>${fileList(sourceFiles,'ไม่มีรายชื่อไฟล์แนบในข้อมูลเรื่อง')}</ul></div><div><strong>เอกสารเรื่องเดิม</strong><ul>${fileList(targetFiles,'ไม่มีรายชื่อไฟล์แนบในข้อมูลเรื่องเดิม')}</ul></div></div>${prepared?'':`<button type="button" class="ws-button secondary" data-action="confirm-duplicate-package">ยืนยันจัดชุดเอกสารครบแล้ว</button>`}</section>`;
    }
    function captureActivity5Dispatch(state){
      captureOutgoingSelections(state);
      const d=state.documentData,value=id=>$(`#${id}`)?.value?.trim()||'';
      if(d.decision==='not-accept'){
        const method=value('notificationMethod')||d.notificationEvidence?.method||'letter';
        d.notificationEvidence={
          method,
          notifiedAt:value('notificationNotifiedAt')||d.notificationEvidence?.notifiedAt||'',
          approvedTemplateId:value('notificationApprovedTemplateId'),
          outgoingLetterNo:value('notificationOutgoingLetterNo'),
          letterDate:value('notificationLetterDate'),
          recipient:value('notificationRecipient'),
          contact:value('notificationContact'),
          evidenceName:$('#notificationEvidenceFile')?.files?.[0]?.name||d.notificationEvidence?.evidenceName||'',
          note:value('notificationNote')
        };
        return state;
      }
    ...
    function notificationEvidenceReadonly(evidence){
      const methodLabels={letter:'หนังสือ',email:'อีเมล',phone:'โทรศัพท์'};
      const rows=[
        ['วิธีแจ้งผล',methodLabels[evidence.method]||evidence.method],
        ['วันเวลาที่แจ้ง',evidence.notifiedAt],
        ...(evidence.method==='letter'?[['รหัสแบบหนังสือที่อนุมัติ',evidence.approvedTemplateId],['เลขหนังสือขาออก',evidence.outgoingLetterNo],['วันที่หนังสือ',evidence.letterDate],['ผู้รับหนังสือ',evidence.recipient]]:[]),
        ...(evidence.method==='email'?[['อีเมลผู้รับ',evidence.recipient]]:[]),
        ...(evidence.method==='phone'?[['หมายเลขโทรศัพท์',evidence.contact]]:[]),
        ['ชื่อหลักฐาน',evidence.evidenceName],
        ['หมายเหตุ',evidence.note]
      ].filter(([,value])=>String(value||'').trim());
      return `<section class="ws-section notification-evidence"><h3>หลักฐานการแจ้งผลที่ยืนยันแล้ว</h3><div class="ws-readonly"><dl>${rows.map(([label,value])=>`<div><dt>${escapeHtml(label)}</dt><dd>${escapeHtml(value)}</dd></div>`).join('')}</dl></div></section>`;
    }
    function notificationEvidenceEditor(state){
      const evidence=state.documentData.notificationEvidence||{},method=evidence.method||'letter';
      if(state.workflow.stage==='notification-confirmed'&&state.workflow.complete)return notificationEvidenceReadonly(evidence);
      return `<section class="nacc-dispatch-hero"><p>ขั้นตอนหลังผู้มีอำนาจอนุมัติ</p><h2>บันทึกหลักฐานการแจ้งผลไม่รับไว้ดำเนินการ</h2><span>เลือกวิธีแจ้งผลและบันทึกหลักฐานให้ครบก่อนยืนยันปิดเรื่อง</span></section><section class="ws-section notification-evidence"><h3>ข้อมูลการแจ้งผล</h3><div class="ws-grid-2"><div class="ws-field"><label>วิธีแจ้งผล *</label><select id="notificationMethod"><option value="letter" ${method==='letter'?'selected':''}>หนังสือ</option><option value="email" ${method==='email'?'selected':''}>อีเมล</option><option value="phone" ${method==='phone'?'selected':''}>โทรศัพท์</option></select></div><div class="ws-field"><label>วันเวลาที่แจ้ง *</label><input id="notificationNotifiedAt" type="datetime-local" value="${escapeHtml(evidence.notifiedAt||'')}"></div><div class="ws-field notification-method-fields" data-notification-method="letter"><label>รหัสแบบหนังสือที่อนุมัติ *</label><input id="notificationApprovedTemplateId" value="${escapeHtml(evidence.approvedTemplateId||'')}"></div><div class="ws-field notification-method-fields" data-notification-method="letter"><label>เลขหนังสือขาออก *</label><input id="notificationOutgoingLetterNo" value="${escapeHtml(evidence.outgoingLetterNo||'')}"></div><div class="ws-field notification-method-fields" data-notification-method="letter"><label>วันที่หนังสือ *</label><input id="notificationLetterDate" type="date" value="${escapeHtml(evidence.letterDate||'')}"></div><div class="ws-field notification-method-fields" data-notification-method="letter,email"><label id="notificationRecipientLabel">ผู้รับหนังสือ / อีเมลผู้รับ *</label><input id="notificationRecipient" value="${escapeHtml(evidence.recipient||'')}"></div><div class="ws-field notification-method-fields" data-notification-method="phone"><label>หมายเลขโทรศัพท์ *</label><input id="notificationContact" inputmode="tel" value="${escapeHtml(evidence.contact||'')}"></div><div class="ws-field notification-method-fields" data-notification-method="email,phone"><label>ไฟล์หลักฐาน</label><input id="notificationEvidenceFile" type="file" accept=".pdf,.jpg,.jpeg,.png"><small>${escapeHtml(evidence.evidenceName||'ยังไม่ได้แนบไฟล์')}</small></div><div class="ws-field ws-field-full notification-method-fields" data-notification-method="email,phone"><label>หมายเหตุหลักฐาน</label><textarea id="notificationNote">${escapeHtml(evidence.note||'')}</textarea><small>อีเมลและโทรศัพท์ต้องมีไฟล์หลักฐานหรือหมายเหตุอย่างน้อยหนึ่งรายการ</small></div></div></section>`;
    }
    function wireNotificationEvidence(body){
      const method=$('#notificationMethod');
      if(!method)return;
      const refresh=()=>$$('.notification-method-fields',body).forEach(section=>section.classList.toggle('ws-hidden',!section.dataset.notificationMethod.split(',').includes(method.value)));
    ...
    function enhanceActivity5Dispatch(){
      ...
      const packagePending=d.approvedOpinion==='suspected_duplicate'&&!d.duplicatePackagePreparedAt;
      const actions=$('.ws-actions');if(actions)actions.innerHTML=state.workflow.complete?activity5Link(state):`<button class="ws-button secondary" data-action="activity5-dispatch-draft">บันทึกร่างข้อมูลจัดส่ง</button><button class="ws-button primary" data-action="activity5-dispatch-confirm" ${packagePending?'disabled':''}>ยืนยันจัดส่งไปยังเขต</button>`;
    }
    ...
    function enhanceNaccTransferReasons(){
      const box=$('#naccReasonBox');if(!box)return;
      const grid=$$('.ws-choice-grid',box).at(-1);if(!grid||grid.dataset.exactNaccReasons)return;
      const state=getState(selectedId);grid.dataset.exactNaccReasons='true';
      grid.innerHTML=NACC_TRANSFER_REASONS.map(reason=>`<label class="ws-choice"><input type="checkbox" name="naccReason" value="${escapeHtml(reason.id)}" ${state.documentData.reasons.includes(reason.id)?'checked':''}><span><strong>${escapeHtml(reason.label)}</strong></span></label>`).join('');
      grid.addEventListener('change',()=>{const working=capture(state);saveState(selectedId,working);renderDocumentInto($('#paperStage'),documentPaper(working,$('[data-doc].active')?.dataset.doc||REVIEW_RECORD_ID,activeRole))});
      const reviewCallout=$('.ws-callout',box);
      if(state.documentData.needsReasonReview&&reviewCallout&&!$('[data-action="confirm-nacc-reasons"]',reviewCallout)){
        reviewCallout.insertAdjacentHTML('beforeend','<button type="button" class="ws-button primary" data-action="confirm-nacc-reasons">ยืนยันเหตุผลที่เลือกใหม่</button>');
        $('[data-action="confirm-nacc-reasons"]',reviewCallout).onclick=async()=>{
          const working=capture(state);
          if(working.documentData.decision!=='send-nacc'||!working.documentData.reasons.length)return notify('warning','ยังไม่ได้เลือกเหตุผล','เลือกเหตุผลส่งสำนักงาน ป.ป.ช. อย่างน้อย 1 ข้อก่อนยืนยัน');
          const confirmation=await confirmDo('ยืนยันเหตุผล Form 3','เหตุผลเดิมจะเก็บไว้ในประวัติ และใช้เหตุผลปัจจุบันที่เลือกแทน','ยืนยันเหตุผล');
          if(!confirmation.isConfirmed)return;
          const reviewedAt=new Date().toISOString(),reviewedBy=assignedOfficerName(working.documentData);
          const result=window.ECMISActivity4DocumentRules?.confirmForm3ReasonReview(working.documentData,{reviewedAt,reviewedBy});
          if(!result?.valid)return notify('warning','ยืนยันเหตุผลไม่ได้','ตรวจสอบผลพิจารณาและเลือกเหตุผลปัจจุบันอย่างน้อย 1 ข้อ');
          working.documentData=result.documentData;
          working.decisionHistory.push({text:`เจ้าหน้าที่รับเรื่องยืนยันเหตุผล Form 3 ใหม่ โดยคงเหตุผลเดิมไว้ในประวัติ`,time:now()});
          saveState(selectedId,working);notify('success','ยืนยันเหตุผลแล้ว','ระบบเก็บเหตุผลเดิมไว้ในประวัติและใช้เหตุผลปัจจุบันที่ยืนยัน');renderDetail();
        };
      }
    }
    async function manageBackupOfficers(){
      if(activeRole!=='division')return;
      ...
      settings.history.push({primary,previousBackup:previous,newBackup:substitute,actor:'ผู้อำนวยการกองบริหารคดี',time:now()});
    ...
    async function manageSuggestions(){
      if(activeRole==='anonymous'||!window.Swal)return;
      const settings=readSuggestionSettings();
    ...
    function enhanceSmartInputs(){
      ...
      if(currentState)lockSignedDocumentSources(currentState);
    }
    function captureNaccDispatch(state){
      captureOutgoingSelections(state);
      const value=id=>$(`#${id}`)?.value?.trim()||'',d=state.documentData;
      const dispatchValues={naccLetterDate:value('naccLetterDate')||d.naccLetterDate,naccSendMethod:value('naccSendMethod')||d.naccSendMethod||'EMS',naccEms:value('naccEms')||d.naccEms,naccSentDate:value('naccSentDate')||d.naccSentDate,naccBoardNote:value('naccBoardNote')||d.naccBoardNote,naccProofName:$('#naccProof')?.files?.[0]?.name||d.naccProofName,naccNotified:Boolean($('#naccNotifyComplainant')?.checked),boardResolutionAttached:Boolean($('#boardResolutionAttached')?.checked),originalDossierAttached:Boolean($('#originalDossierAttached')?.checked)};
      const dispatchResult=window.ECMISActivity4DocumentRules?.applyDocumentSourceEdits(state,{section:'dispatch',values:dispatchValues});
      const dispatched=dispatchResult?.record||state;
      const officerResult=window.ECMISActivity4DocumentRules?.applyDocumentSourceEdits(dispatched,{section:'officer',values:{naccBoardNo:value('naccBoardNo')||d.naccBoardNo,naccBoardDate:value('naccBoardDate')||d.naccBoardDate}});
      return officerResult?.record||dispatched;
    }
    ...
    function renderList(){
      ...
      $('#dashboardPending').textContent=states.filter(s=>dashboardBucket(s)==='pending').length;
      $('#dashboardReview').textContent=states.filter(s=>dashboardBucket(s)==='review').length;
      $('#dashboardDone').textContent=states.filter(s=>dashboardBucket(s)==='done').length;
      const adminStates=selectedRole==='admin'?mergedCases().map(c=>getState(c.id)):[];
      const unassignedCount=adminStates.filter(state=>roleCanSeeState(state,visibilityContext('unassigned'))).length;
      const assignedCount=adminStates.filter(state=>roleCanSeeState(state,visibilityContext('assigned'))).length;
    ...
    function screeningAnalysisCard(c){
      ...
      const displayId=caseReferenceForRole(findOriginalCase(analysis.caseId)?.caseData,selectedRole);
    ...
    function screeningDecisionForm(state,{withAdditionalInfo=false}={}){
      ...
      return `<div class="screening-options">
        <label class="ws-choice screening-choice"><input type="radio" name="screeningDecision" value="new"><span><strong>ความเห็นเจ้าหน้าที่: เรื่องใหม่</strong><small>ยังไม่ใช่ผลอนุมัติ ต้องเสนอ ผอ.ศรร. และ ผอ.กบค. ตามลำดับ</small></span></label>
        <label class="ws-choice screening-choice"><input type="radio" name="screeningDecision" value="suspected_duplicate"><span><strong>ความเห็นเจ้าหน้าที่: สงสัยว่าเป็นเรื่องซ้ำ</strong><small>ขั้นนี้บันทึกความเห็นเท่านั้น ยังไม่รวมหรือปิดเรื่อง</small></span></label>
      </div>
      ...
      <div id="screeningDuplicateBox" class="ws-hidden">
        <div class="ws-field"><label>เลือกเรื่องเดิม *</label><select id="screeningLinkedCase"><option value="">— เลือกเรื่องเดิม —</option>${candidates.map(cd=>`<option value="${escapeHtml(cd.id)}" ${cd.id===analysis.caseId?'selected':''}>${escapeHtml(caseReferenceForRole(cd,selectedRole))} · ${escapeHtml(cd.complainant)} · ${escapeHtml(cd.subject)}</option>`).join('')}</select></div>
    ...
    function adminScreeningBlock(state){
      ...
      const decidedSummary=decided==='duplicate'
        ?`<div class="screening-summary"><span class="ws-status danger">เรื่องซ้ำ — รวมกับเรื่องเดิม ${escapeHtml(caseDisplayId(original))}</span><p>ตัดสินใจโดย ${escapeHtml(d.screening.decidedBy||'—')} · ${escapeHtml(d.screening.decidedAt||'—')}</p><p>หมายเหตุ: ${escapeHtml(d.screening.note||'—')}</p></div>`
        :decided==='new'
          ...
          :'';
      return `${screeningAnalysisCard(c)}${decidedSummary}<p class="duplicate-policy-note">ธุรการ ศรร. มีหน้าที่ตรวจสอบเบื้องต้นและให้ความเห็นเท่านั้น — การตัดสินใจเรื่องใหม่/เรื่องซ้ำเป็นของเจ้าหน้าที่รับเรื่อง ระบบไม่ตัดสินใจแทน</p><div class="ws-field screening-admin-opinion"><label>ความเห็นธุรการ (ตรวจเบื้องต้น — ส่งต่อให้เจ้าหน้าที่รับเรื่อง)</label><textarea id="adminNote" placeholder="บันทึกข้อสังเกตหรือความเห็นเบื้องต้นเพื่อส่งต่อให้เจ้าหน้าที่รับเรื่องตัดสินใจ">${escapeHtml(d.adminNote)}</textarea></div>${d.mergedDuplicates?.length?`<div class="screening-merged-list"><h4>เรื่องซ้ำที่รวมเข้ามา</h4><ul>${d.mergedDuplicates.map(m=>`<li>${escapeHtml(caseReferenceForRole(findOriginalCase(m.caseId)?.caseData,selectedRole))} · ${escapeHtml(m.at)}${m.note?` — ${escapeHtml(m.note)}`:''}</li>`).join('')}</ul></div>`:''}`;
    }
    function reliefInitialPackFields(documentData){
      const d=documentData||{};
      return `<div id="reliefInitialPackBox" class="${d.decision==='58/2'?'':'ws-hidden'}" style="margin-top:.8rem"><h3>หน่วยงานผู้ดำเนินการตามมาตรา 58/2</h3><div class="ws-choice-grid"><label class="ws-choice"><input type="radio" name="reliefOperationKind" value="central" ${d.reliefOperationKind==='central'?'checked':''}><span><strong>ศูนย์รับเรื่องร้องเรียน (ส่วนกลาง)</strong><small>ใช้แบบ 58/2-03</small></span></label><label class="ws-choice"><input type="radio" name="reliefOperationKind" value="regional" ${d.reliefOperationKind==='regional'?'checked':''}><span><strong>สำนักงาน ป.ป.ท. เขต</strong><small>ใช้แบบ 58/2-04</small></span></label></div><div id="reliefCentralTransferBox" class="${d.reliefOperationKind==='central'?'':'ws-hidden'}" style="margin-top:.6rem"><p>ส่วนกลางต้องส่งต้นฉบับสำนวนต่อให้สำนักงาน ป.ป.ท. เขตหรือไม่ *</p><div class="ws-choice-grid"><label class="ws-choice"><input type="radio" name="centralSendsDossierToRegion" value="yes" ${d.centralSendsDossierToRegion===true?'checked':''}><span><strong>ส่งต่อไปเขต</strong><small>เพิ่มแบบ 58/2-05</small></span></label><label class="ws-choice"><input type="radio" name="centralSendsDossierToRegion" value="no" ${d.centralSendsDossierToRegion===false?'checked':''}><span><strong>ไม่ส่งต่อไปเขต</strong><small>ไม่ใช้แบบ 58/2-05</small></span></label></div></div></div>`;
    }
    function editorFor(state){const d=state.documentData,c=state.caseData,w=state.workflow;if(activeRole==='central-registry-clerk')return `<div class="ws-section"><h3>บันทึกเลขสารบรรณกลาง</h3>${caseReadonly(c)}<div class="ws-field" style="margin-top:.8rem"><label for="centralIncomingRegistry">เลขรับสารบรรณกลาง *</label><input id="centralIncomingRegistry" value="${escapeHtml(c.registry?.office||'')}" autocomplete="off"><small>กรอกเฉพาะเลขที่สารบรรณกลาง ชั้น 14 ออกให้ แล้วส่งต่อธุรการ กบค.</small></div></div>`;if(activeRole==='case-admin-clerk')return `<div class="ws-section"><h3>ลงทะเบียนรับ กบค.</h3>${caseReadonly(c)}<div class="ws-callout" style="margin-top:.7rem">${c.registry?.kbk?`เลขรับ กบค. ${escapeHtml(c.registry.kbk)} บันทึกแล้ว`:'ตรวจสอบข้อมูลและออกเลขรับ กบค. ก่อนส่งต่อให้ธุรการ ศรร.'}</div></div>`;if(activeRole==='regional-clerk')return `<div class="ws-section"><h3>ลงทะเบียนรับประจำเขต</h3>${caseReadonly(c)}<div class="ws-callout" style="margin-top:.7rem">${c.registry?.region?`เลขทะเบียนรับ ${escapeHtml(c.region)} ${escapeHtml(c.registry.region)} บันทึกแล้ว`:`ตรวจสอบข้อมูลและออกเลขทะเบียนรับของ ${escapeHtml(c.region)} ก่อนส่งให้เจ้าหน้าที่เขต`}</div></div>`;if(activeRole==='regional-director')return `<div class="ws-section"><h3>ข้อมูลจากเจ้าหน้าที่เขต</h3>${caseReadonly(c)}<div class="ws-callout" style="margin-top:.7rem">ความเห็นเจ้าหน้าที่: ${escapeHtml(d.officerOpinion||'ไม่ระบุ')}</div><div class="ws-field" style="margin-top:.8rem"><label>ความเห็นและคำสั่ง</label><textarea id="divisionOpinion">${escapeHtml(d.divisionOpinion)}</textarea></div></div><section class="signature-workflow"><header><div><span>การลงนามเอกสารอิเล็กทรอนิกส์</span><h3>ผอ.สำนักงาน ป.ป.ท. เขต</h3></div><b class="${d.divisionSignature?.signed?'signed':''}">${d.divisionSignature?.signed?'ลงนามแล้ว':'รอการลงนาม'}</b></header><div class="signature-workflow-body">${d.divisionSignature?.signed?'<div class="signature-proof signature-proof-signed"></div>':'<button type="button" class="ws-button primary" data-action="regional-director-sign">ลงนามเอกสาร</button>'}</div></section>`;if(isOfficerRole(activeRole)&&!['officer','admin','regional-officer'].includes(w.owner)&&!w.complete)return `<div class="ws-section"><h3>ข้อมูลจากขั้นตอนก่อนหน้า</h3>${caseReadonly(c)}</div><div class="ws-callout" style="margin:.4rem 0 .9rem">ส่งให้ ${escapeHtml(ROLE_LABELS[w.owner]||w.owner)} พิจารณาแล้ว — ข้อมูลถูกล็อค รอผลการพิจารณา</div><div class="ws-section"><h3>ผลการพิจารณาที่ส่งไปแล้ว</h3><div class="ws-readonly"><dl><div><dt>ผลการพิจารณา</dt><dd>${({'18/1ก':'รับไว้ดำเนินการ 18/1 (ก)','18/1ข':'รับไว้ดำเนินการ 18/1 (ข)','18/4':'รับไว้ดำเนินการ 18/4','58/2':'ดำเนินการตาม 58/2','send-nacc':'ส่งสำนักงาน ป.ป.ช.','not-accept':'ไม่รับไว้ดำเนินการ'})[d.decision]||d.decision}</dd></div><div><dt>ชื่อเรื่องสำหรับเอกสาร</dt><dd><span class="doc-editable" data-editable="documentSubject" contenteditable="true" spellcheck="false">${escapeHtml(d.documentSubject||c.subject)}</span></dd></div><div><dt>เขตที่เสนอให้มอบหมาย</dt><dd>${escapeHtml(d.proposedRegion||c.region||'ไม่ระบุ')}</dd></div><div><dt>ความเห็นเจ้าหน้าที่</dt><dd>${escapeHtml(d.officerOpinion||'ไม่ระบุ')}</dd></div>${d.reasons.length?`<div><dt>เหตุผลส่ง ป.ป.ช.</dt><dd>${d.reasons.map(escapeHtml).join(', ')}</dd></div>`:''}${d.naccOtherChecked&&d.naccOtherReason?`<div><dt>อื่น ๆ (ส่ง ป.ป.ช.)</dt><dd>${escapeHtml(d.naccOtherReason)}</dd></div>`:''}${d.notAcceptReason?`<div><dt>เหตุผลไม่รับไว้ดำเนินการ</dt><dd>${escapeHtml(d.notAcceptReason)}</dd></div>`:''}${d.notAcceptOtherChecked&&d.notAcceptOtherReason?`<div><dt>อื่น ๆ (ไม่รับฯ)</dt><dd>${escapeHtml(d.notAcceptOtherReason)}</dd></div>`:''}${d.anonymous?`<div><dt>บัตรสนเท่ห์</dt><dd>ใช่</dd></div>`:''}${d.reviewRoute==='division'?`<div><dt>เส้นทาง</dt><dd>ส่งตรง ผู้อำนวยการกองบริหารคดี${d.absenceReasonType?` (${escapeHtml(d.absenceReasonType)})`:''}</dd></div>`:''}</dl></div></div>`;if(activeRole==='admin'){
      const registry=c.registry||{};
      const registryHistory=(registry.office||registry.kbk||registry.srr)?`<div class="ws-callout" style="margin-bottom:.8rem"><strong>ประวัติเลขรับ</strong><br>${registry.office?`เลขรับสารบรรณกลาง ชั้น 14: ${escapeHtml(registry.office)}<br>`:''}${registry.kbk?`เลขรับ กบค.: ${escapeHtml(registry.kbk)}<br>`:''}${registry.srr?`เลขรับ ศรร.: ${escapeHtml(registry.srr)}`:'รอธุรการ ศรร. ออกเลขรับ'}</div>`:'';
      const duplicate=analyzeDuplicateCase(c);
      const duplicateDisplayId=caseReferenceForRole(findOriginalCase(duplicate.caseId)?.caseData,selectedRole);
    ...
    function actionsFor(state){
      const reset='<button class="ws-button ghost" data-action="reset">ล้างข้อมูลและเริ่มใหม่</button>';
      if(activeRole==='case-admin-clerk')return `${reset}<button class="ws-button primary" data-action="case-admin-register">ออกเลขรับ กบค. และส่งต่อ</button>`;
      if(activeRole==='regional-clerk')return `${reset}<button class="ws-button primary" data-action="regional-register">ออกเลขทะเบียนรับและส่งเจ้าหน้าที่เขต</button>`;
      if(activeRole==='regional-director')return `${reset}<button class="ws-button danger" data-action="regional-director-return">ส่งกลับเจ้าหน้าที่เขตแก้ไข</button><button class="ws-button primary" data-action="regional-director-approve" ${state.documentData.divisionSignature?.signed?'':'disabled'}>อนุมัติและส่งกลับเจ้าหน้าที่เขต</button>`;
      if(activeRole==='admin'){
        const hasAssignedWork=Boolean(state.documentData.assignedOfficer)&&state.assignmentHistory.some(entry=>['assign','reassign'].includes(entry.action)||/\[(มอบหมาย|มอบหมายใหม่)\]|มอบหมายให้/.test(entry.text||''));
        const correctionActive=state.workflow.correction&&!state.workflow.correction.resolvedAt;
        const canRecall=hasAssignedWork&&!correctionActive&&!state.workflow.complete&&!['nacc-dispatch','anonymous-box'].includes(state.workflow.stage);
        return `${reset}${canRecall?'<button class="ws-button danger" data-action="recall-assignment">ดึงงานกลับ</button>':''}<button class="ws-button secondary" data-action="draft">บันทึกร่าง</button><button class="ws-button primary" data-action="assign">${state.workflow.status==='รอมอบหมายใหม่'?'มอบหมายใหม่':'ลงรับและมอบหมาย'}</button>`;
      }
      if(isOfficerRole(activeRole)){
        const officerLocked=!['officer','admin','regional-officer'].includes(state.workflow.owner)&&!state.workflow.complete;
        const duplicateClosed=state.workflow.stage==='duplicate-closed'&&state.workflow.complete;
        const screeningPending=!['new','suspected_duplicate'].includes(state.documentData.screening?.officerOpinion||state.documentData.screening?.decision)&&!duplicateClosed;
        if(state.workflow.stage==='officer-finalize-screening')return '<button class="ws-button primary" data-action="officer-finalize-screening">ยืนยันดำเนินการตามผลอนุมัติ</button>';
        if(duplicateClosed)return `${reset}<button class="ws-button secondary" data-action="print-duplicate-receipt">พิมพ์ใบแจ้ง (เลขเดิม)</button>`;
        const correctionActive=state.workflow.correction&&!state.workflow.correction.resolvedAt;
        const submitLabel=correctionActive?(state.workflow.correction.source==='regional-director'?'แก้ไขและส่ง ผอ.เขต พิจารณาใหม่':'แก้ไขและส่ง ผอ.ศรร. พิจารณาใหม่'):'บันทึกและส่งพิจารณา';
        return officerLocked?'<button class="ws-button secondary" data-action="print">พิมพ์แบบบันทึก</button>':`${reset}<button class="ws-button secondary" data-action="draft">บันทึกแบบบันทึก</button><button class="ws-button secondary" data-action="print">พิมพ์แบบบันทึก</button><button class="ws-button danger" data-action="officer-refuse">ปฏิเสธรับงาน</button>${screeningPending?'':`<button class="ws-button primary" data-action="officer-send">${submitLabel}</button>`}`;
      }
      if(activeRole==='center')return `${reset}<button class="ws-button danger" data-action="center-return">ส่งกลับเจ้าหน้าที่แก้ไข</button><button class="ws-button primary" data-action="center-send">บันทึกความเห็นและส่ง ผอ.กบค.</button>`;
      return `${reset}${activity5Link(state)}<button class="ws-button danger" data-action="division-return">ส่งกลับเจ้าหน้าที่แก้ไข</button><button class="ws-button primary" data-action="approve">อนุมัติและส่งกลับเจ้าหน้าที่</button>`;
    }
    const unguardedActionsFor=actionsFor;
    actionsFor=state=>{
      const roleStage={
        'central-registry-clerk':'central-registry',
        'case-admin-clerk':'case-admin-registry',
        'regional-clerk':'regional-registry',
        'regional-director':'regional-director'
      }[activeRole];
      if(roleStage&&(state.workflow.owner!==activeRole||state.workflow.stage!==roleStage))return '';
      if(activeRole==='central-registry-clerk')return '<button class="ws-button primary" data-action="central-registry-confirm">ยืนยันเลขรับและส่งธุรการ กบค.</button>';
      if(activeRole==='case-admin-clerk'&&!requiresCaseAdminRegistry(state.caseData.channel,state.caseData.region)&&!isCentralRegistryChannel(state.caseData.channel,state.caseData.region))return '';
      return unguardedActionsFor(state);
    };
    function correctionNotice(state){
      const correction=state.workflow.correction;
      if(!correction||correction.resolvedAt)return '';
      const divisionNotice=correction.source==='division'?'<strong>ผอ.กบค. ต้องการความเห็นจาก ผอ.ศรร. เพิ่มเติม</strong><br>':'';
      return `<div class="ws-callout" style="margin-bottom:.8rem">${divisionNotice}ส่งกลับโดย ${escapeHtml(correction.requestedBy||ROLE_LABELS[correction.source]||correction.source)} · ${escapeHtml(correction.requestedAt||'')}<br>เหตุผล: ${escapeHtml(correction.reason||'ไม่ระบุ')}</div>`;
    }
    function documentShopA4(state){
      const docs = Object.entries(OFFICIAL_DOCUMENTS).map(([id,doc])=>({id, name: doc.name, pages: doc.pages, register: doc.register}));
      const existing = new Set(officialDocumentIds(state));
      return `<div class="a5-shop-bulk" style="display:flex;align-items:center;gap:.6rem;padding:.6rem .9rem;background:#f0f4ff;border-bottom:1px solid #d7dee6;position:sticky;top:0;z-index:1">
        <label style="display:flex;align-items:center;gap:.4rem;cursor:pointer"><input type="checkbox" id="a4ShopSelectAll"> เลือกทั้งหมด</label>
        <span style="margin-left:.4rem;font-size:.82rem">เลือก <b id="a4ShopSelectedCount">0</b> รายการ</span>
        <div style="margin-left:auto;display:flex;gap:.4rem">
          <button type="button" class="ws-button secondary" data-a4-shop-bulk-clear style="padding:.22rem .55rem;font-size:.75rem">ล้าง</button>
          <button type="button" class="ws-button primary" data-a4-shop-bulk-add disabled style="padding:.22rem .6rem;font-size:.75rem">เพิ่มที่เลือก</button>
        </div>
      </div><div class="a5-doc-shop" style="display:grid;grid-template-columns:repeat(auto-fill,minmax(210px,1fr));gap:.65rem;padding:.9rem">
        <div class="a5-shop-card" style="border:2px dashed #a9bdd5;border-radius:12px;padding:.75rem .8rem;background:#f0f7ff;display:flex;flex-direction:column;gap:.4rem;min-height:118px;align-items:center;justify-content:center;text-align:center">
          <strong style="font-size:.85rem;color:#12365f">อัปโหลดเอกสาร</strong>
          <small style="color:#6b7a8c">PDF / รูป / ไฟล์ประกอบ</small>
          <button type="button" class="ws-button primary" data-a4-shop-upload style="margin-top:.4rem">เลือกไฟล์</button>
          <input type="file" id="a4ShopFileInput" accept="*/*" style="display:none">
        </div>
        ${docs.map(d=>{
          const ex = existing.has(d.id);
          const checkDisabled = ex ? 'disabled' : '';
          const checkBox = ex ? '<span style="font-size:.71rem;color:#18794e">✓ เพิ่มแล้ว</span>' : `<label style="display:flex;align-items:center;gap:.3rem;font-size:.75rem;cursor:pointer"><input type="checkbox" data-a4-shop-check="${d.id}" ${checkDisabled}> เลือก</label>`;
          return `<div class="a5-shop-card" style="border:1px solid #d7dee6;border-radius:12px;padding:.75rem .8rem;background:#fff;display:flex;flex-direction:column;gap:.35rem;min-height:132px">
            <div style="font-size:.82rem;font-weight:700;color:#12365f">ปปท. ${d.id} ${d.name}</div>
            <small style="color:#6b7a8c">${d.pages} หน้า · ${d.register}</small>
            <div style="display:flex;align-items:center;justify-content:space-between;margin-top:.3rem"><small style="color:${ex?'#18794e':'#9a6700'}">${ex?'✓ มีในชุด':'○ ยังไม่เพิ่ม'}</small>${checkBox}</div>
            <button type="button" class="ws-button ${ex?'secondary':'primary'}" data-a4-shop-open="${d.id}" style="padding:.22rem .55rem;font-size:.75rem;margin-top:.2rem">${ex?'เปิดดู':'ใช้เอกสาร'}</button>
          </div>`;
        }).join('')}
      </div>`;
    }
    function operationalWorkspace(state){const w=state.workflow;const editor=`<section class="ws-card ws-editor ${activeRole==='admin'?'ws-admin-workspace':''}"><header class="ws-editor-head"><div><p class="ws-kicker">งานของ ${ROLE_LABELS[activeRole]}</p><h2>${w.complete?'ตรวจสอบข้อมูลที่ดำเนินการแล้ว':'ดำเนินการเรื่องร้องเรียน'}</h2></div><span class="ws-status">${w.status}</span></header><div class="ws-editor-body">${correctionNotice(state)}${editorFor(state)}${activeRole==='admin'?'':`<div class="ws-section"><h3>ประวัติการตัดสินใจ</h3><ul class="ws-history">${state.decisionHistory.length?state.decisionHistory.map(x=>`<li>${escapeHtml(x.text)}<time>${x.time}</time></li>`).join(''):'<li>ยังไม่มีประวัติการตัดสินใจ</li>'}</ul></div>`}</div></section>`;if(['central-registry-clerk','case-admin-clerk'].includes(activeRole))return `<div class="ws-admin-layout">${editor}</div>`;if(activeRole==='admin')return `<div class="ws-admin-layout">${editor}</div>`;return `<div class="document-workspace">${editor}<aside class="ws-doc-pane"><div class="ws-doc-toolbar"><div class="ws-doc-tabs">${docTabs(state,activeRole)}</div><button type="button" class="ws-button secondary" data-a4-doc-shop-toggle style="background:#eef4ff;border-color:#c9d9e8">📄 เลือกแบบพิมพ์</button><label class="ws-doc-jump" title="ข้ามไปยังเอกสาร"><span aria-hidden="true">ไปที่</span><select id="docJump" aria-label="เลือกเอกสาร"><option value="">เอกสาร…</option>${docSelect(state,activeRole)}</select></label><button class="ws-button secondary" data-action="print">พิมพ์/PDF</button><button type="button" class="ws-button secondary ws-doc-pane-toggle" title="ย่อแผงเอกสาร" aria-label="ย่อแผงเอกสาร" aria-expanded="true" style="padding:.42rem .62rem;line-height:1">»</button></div><button type="button" class="ws-doc-pane-rail" title="ขยายแผงเอกสาร" aria-label="ขยายแผงเอกสาร">«<span>เอกสาร</span></button><div class="doc-edit-bar" aria-label="แถบจัดรูปแบบ"><select class="doc-edit-font" data-format="fontName" title="ฟอนต์"><option value="">ฟอนต์</option><option>Sarabun</option><option>TH Sarabun New</option><option>Tahoma</option><option>Times New Roman</option><option>Angsana New</option></select><select class="doc-edit-size" data-format="fontSize" title="ขนาดตัวอักษร"><option value="">ขนาด</option><option value="3">16</option><option value="4">18</option><option value="5">20</option><option value="6">24</option><option value="7">32</option></select><span class="doc-edit-bar-sep"></span><button type="button" data-format="bold" title="ตัวหนา"><b>B</b></button><button type="button" data-format="italic" title="ตัวเอียง"><i>I</i></button><button type="button" data-format="underline" title="ขีดเส้นใต้"><u>U</u></button><span class="doc-edit-bar-sep"></span><label class="doc-edit-color" title="สีตัวอักษร"><input type="color" data-format="foreColor" value="#17232e"></label><span class="doc-edit-bar-sep"></span><button type="button" data-format="justifyLeft" title="ชิดซ้าย">ซ้าย</button><button type="button" data-format="justifyCenter" title="กึ่งกลาง">กลาง</button><button type="button" data-format="justifyRight" title="ชิดขวา">ขวา</button><span class="doc-edit-bar-sep"></span><button type="button" data-format="insertUnorderedList" title="รายการหัวข้อ">•≡</button><button type="button" data-format="insertOrderedList" title="รายการลำดับเลข">1≡</button><span class="doc-edit-bar-sep"></span><button type="button" data-format="outdent" title="ลดย่อหน้า">⤺</button><button type="button" data-format="indent" title="เพิ่มย่อหน้า">⤻</button><span class="doc-edit-bar-sep"></span><button type="button" data-format="undo" title="เลิกทำ">↶</button><button type="button" data-format="redo" title="ทำซ้ำ">↷</button><span class="doc-edit-bar-sep"></span><span class="doc-edit-bar-tip">หน้าไหลอัตโนมัติ</span></div><button type="button" class="doc-edit-fab" data-action="toggle-edit" title="เปิด/ปิดโหมดแก้ไขเอกสาร"><span class="doc-edit-fab-icon" aria-hidden="true">✏️</span><span class="doc-edit-fab-label">แก้ไขเอกสาร</span></button><div id="a4DocShop" class="ws-hidden" style="max-height:62vh;overflow:auto;border-top:1px solid #e4e9f0;background:#fbfcfe"></div><div class="ws-paper-stage" id="paperStage">${documentPaper(state,firstDocumentId(state,activeRole),activeRole)}</div></aside></div>`}
    function renderDetail(){const state=getState(selectedId);if(!canViewState(state)){
      const c=state?.caseData||{}, w=state?.workflow||{}, d=state?.documentData||{};
      const reason = `เคสนี้อยู่ระหว่างขั้นตอนของ ${ROLE_LABELS[w.owner]||w.owner||'—'} (${w.stage||'—'}) • ภูมิภาค ${c.region||'—'} • ช่องทาง ${c.channel||'—'}`;
      $('#caseListView').classList.add('ws-hidden');
      const view=$('#caseDetailView'); view.classList.remove('ws-hidden');
      const trackingMeta=['central-registry-clerk','case-admin-clerk'].includes(selectedRole)?'':`<span>เลขติดตาม ${escapeHtml(c.trackingYear||'รอออกเลขรับเรื่อง')}</span>`;
      const issuedCaseNumber=d.caseNumber?caseIssuanceCard(state):'';
      view.innerHTML=`<button class="ws-button ghost" id="backList" style="margin-bottom:.8rem">กลับรายการเรื่องร้องเรียน</button><div class="ws-callout" style="margin:.6rem 0;background:#fff8e1;border-color:#ffe082;display:flex;justify-content:space-between;align-items:center"><span>ดูได้อย่างเดียว — ${escapeHtml(reason)}</span><span style="background:#fff3cd;border:1px solid #ffe69c;padding:4px 8px;border-radius:6px;font-weight:700;font-size:.8rem">READONLY</span></div><section class="ws-card ws-case-head"><div><p class="ws-kicker">${escapeHtml(c.channel||'—')} · ${escapeHtml(c.region||'—')}</p><h1>เลขรับเรื่อง ${escapeHtml(caseReferenceForRole(c,selectedRole))}</h1><div class="ws-case-meta"><span>${escapeHtml(c.subject||'—')}</span><span>รับเรื่อง ${c.received||'—'}</span>${trackingMeta}</div></div><div><span class="ws-status">${w.status||'—'}</span><p style="margin:.4rem 0 0;font-size:.8rem;color:#687789">ผู้รับผิดชอบ: ${ROLE_LABELS[w.owner]||w.owner||'—'}</p></div></section>${issuedCaseNumber}<nav class="ws-card ws-stagebar a5" aria-label="สถานะและลำดับการดำเนินงาน">${stagebar(state)}</nav><div style="opacity:0.9;pointer-events:none">${operationalWorkspace(state)}</div><div class="ws-actions" style="justify-content:space-between"><span style="background:#f1f3f5;border:1px solid #dee2e6;padding:6px 10px;border-radius:6px;font-weight:600;color:#495057">READONLY — แก้ไขไม่ได้</span><button class="ws-button ghost" id="backList2">กลับรายการ</button></div>`;
      $('#backList').onclick=()=>{view.classList.add('ws-hidden');$('#caseListView').classList.remove('ws-hidden');selectedId=null;syncUrl();renderList()};
      const bl2=document.getElementById('backList2'); if(bl2) bl2.onclick=()=> document.getElementById('backList').click();
      setTimeout(()=>{ const paper=$('#paperStage'); if(paper) { try{ paper.innerHTML=documentPaper(state, firstDocumentId(state,selectedRole), selectedRole); scheduleDocumentPagination(paper);}catch(e){} } }, 50);
      return;
    }const c=state.caseData,w=state.workflow,d=state.documentData;const trackingMeta=['central-registry-clerk','case-admin-clerk'].includes(selectedRole)?'':`<span>เลขติดตาม ${escapeHtml(c.trackingYear||'รอออกเลขรับเรื่อง')}</span>`;const issuedCaseNumber=d.caseNumber?caseIssuanceCard(state):'';$('#caseListView').classList.add('ws-hidden');const view=$('#caseDetailView');view.classList.remove('ws-hidden');view.innerHTML=`<button class="ws-button ghost" id="backList" style="margin-bottom:.8rem">กลับรายการเรื่องร้องเรียน</button><section class="ws-card ws-case-head"><div><p class="ws-kicker">${escapeHtml(c.channel)} · ${escapeHtml(c.region)}</p><h1>เลขรับเรื่อง ${escapeHtml(caseReferenceForRole(c,selectedRole))}</h1><div class="ws-case-meta"><span>${escapeHtml(c.subject)}</span><span>รับเรื่อง ${c.received}</span>${trackingMeta}</div></div><div><span class="ws-status ${w.complete?'success':''}">${w.status}</span><p style="margin:.4rem 0 0;font-size:.8rem;color:#687789">ผู้รับผิดชอบ: ${ROLE_LABELS[w.owner]||w.owner||'—'}</p></div></section>${issuedCaseNumber}<nav class="ws-card ws-stagebar a5" aria-label="สถานะและลำดับการดำเนินงาน">${stagebar(state)}</nav>${operationalWorkspace(state)}<div class="ws-actions">${actionsFor(state)}</div>`;$('#backList').onclick=()=>{view.classList.add('ws-hidden');$('#caseListView').classList.remove('ws-hidden');selectedId=null;syncUrl();renderList()};wireDetail(state);enhanceSmartInputs();scheduleDocumentPagination($('#paperStage'))}
    function capture(state){
      const original=JSON.parse(JSON.stringify(state)),working=JSON.parse(JSON.stringify(state)),d=working.documentData;
      const value=id=>$('#'+id)?.value?.trim()||'';
      if(activeRole==='admin'){d.assignedOfficer=value('adminAssignee');d.backupOfficer=value('adminBackup');if($('#adminNote'))d.adminNote=value('adminNote')}
      if(isOfficerRole(activeRole)){
        d.documentSubject=value('documentSubject')||working.caseData.subject||'';d.decision=$('input[name="decision"]:checked')?.value||d.decision;d.reasons=$$('input[name="naccReason"]:checked').map(x=>x.value);d.notAcceptReason=value('notAcceptReason');d.anonymous=$('#anonymous')?.checked||false;d.proposedRegion=value('proposedRegion')||d.proposedRegion;d.officerOpinion=value('officerOpinion');d.reviewRoute=$('input[name="route"]:checked')?.value||'center';d.absenceReasonType=value('absenceReasonType');d.absenceNote=value('absenceNote');d.actingOfficer=value('actingOfficer')||d.actingOfficer;d.actingOrder=value('actingOrder')||d.actingOrder;
        const wc=ensureWorkingCopy(working);WORKING_COPY_FIELDS.forEach(([key])=>{const el=$('#wc'+key[0].toUpperCase()+key.slice(1));if(el)wc[key]=el.value.trim()});
      }
      if(activeRole==='center'){d.centerDecision=$('input[name="centerDecision"]:checked')?.value||'';d.centerOpinion=value('centerOpinion')}
      if(activeRole==='division'||activeRole==='acting'||activeRole==='regional-director'){d.divisionOpinion=value('divisionOpinion');d.actingOrder=value('actingOrder')||d.actingOrder}
      if(activeRole==='admin'||isOfficerRole(activeRole)){const sc=d.screening||(d.screening={});const slc=$('#screeningLinkedCase');if(slc&&d.screening?.decision!=='duplicate')sc.linkedCaseId=slc.value;const sn=$('#screeningNote');if(sn&&d.screening?.decision!=='duplicate')sc.note=sn.value.trim();const sai=$('#screeningAdditionalInfo');if(sai&&d.screening?.decision!=='duplicate')sc.additionalInfo=sai.value.trim()}
      const section=isOfficerRole(activeRole)?'officer':activeRole==='center'?'center':['division','acting','regional-director'].includes(activeRole)?'division':'';
      if(!section)return working;
      const result=window.ECMISActivity4DocumentRules?.applyDocumentSourceEdits(original,{section,values:d});
      return result?.record||original;
    }
  
  const captureDecisionBase=capture;
  capture=state=>{
    let captured=captureDecisionBase(state),d=captured.documentData;
    if(isNaccApiCase(captured)){
      captured=window.ECMISActivity4DocumentRules.normalizeNaccApiIntake(captured);
      d=captured.documentData;
    }
    const officialValues={};
    ['naccIncomingLetterNo','naccIncomingLetterDate','naccBlackTrackingNo','naccRecipientName','naccProvince','naccBoardNo','naccBoardDate','naccBoardAgenda','naccBoardAttachmentPages','naccDossierAttachmentPages','naccReportYear','accusedName','accusedPositionAffiliation','complaintLetterDate','recipientName','contactOfficeName','contactOfficeAddress','contactOfficePhone','contactOfficeFax','ownerPosition'].forEach(field=>{const input=$('#'+field);if(input)officialValues[field]=input.value.trim()});
    const transferKind=$('input[name="naccTransferKind"]:checked')?.value;
    if(['forward','return'].includes(transferKind))officialValues.naccTransferKind=transferKind;
    const boardResolutionAttached=$('#boardResolutionAttached');
    const originalDossierAttached=$('#originalDossierAttached');
    if(boardResolutionAttached)officialValues.boardResolutionAttached=boardResolutionAttached.checked;
    if(originalDossierAttached)officialValues.originalDossierAttached=originalDossierAttached.checked;
    const reliefOperationKind=$('input[name="reliefOperationKind"]:checked')?.value;
    if(['central','regional'].includes(reliefOperationKind))officialValues.reliefOperationKind=reliefOperationKind;
    const reliefDossierChoice=$('input[name="centralSendsDossierToRegion"]:checked')?.value;
    if(reliefOperationKind==='central'&&['yes','no'].includes(reliefDossierChoice))officialValues.centralSendsDossierToRegion=reliefDossierChoice==='yes';
    if(reliefOperationKind==='regional')officialValues.centralSendsDossierToRegion=null;
    const officialResult=window.ECMISActivity4DocumentRules?.applyDocumentSourceEdits(captured,{section:'officer',values:officialValues});
    captured=officialResult?.record||captured;d=captured.documentData;
    const selectedDecision=$('input[name="decision"]:checked')?.value;
    if(selectedDecision){
      const validation=window.ECMISActivity4DocumentRules?.validateDecisionRecord({...captured,documentData:{...d,decision:selectedDecision}});
      const awaitingNaccTransferKind=selectedDecision==='send-nacc'&&validation?.errors?.every(error=>['invalid-nacc-transfer-kind','form3-reason-review-required'].includes(error));
      const awaitingReliefChoice=selectedDecision==='58/2'&&validation?.errors?.every(error=>['invalid-relief-operation-kind','missing-central-region-dossier-choice'].includes(error));
      if(validation?!validation.valid&&!awaitingNaccTransferKind&&!awaitingReliefChoice:!['18/1ก','18/1ข','18/4','58/2','send-nacc','not-accept'].includes(selectedDecision))throw new Error('Unsupported Form 3 decision');
    }
    return captured;
  };
  ...
  function wireDetail(state){const refreshPaper=()=>{const paper=$('#paperStage');if(!paper)return;const activeDoc=$('[data-doc].active')?.dataset.doc||firstDocumentId(state,activeRole);paper.innerHTML=documentPaper(capture(state),activeDoc,activeRole);lockSignedPaper(state,paper)};const applyDecision=()=>{const updated=capture(state);updated.documentData.documentPack={...(updated.documentData.documentPack||{}),caseType:packCaseTypeOf(updated)};saveState(selectedId,updated);renderDetail()};bindCitizenIdInput($('#wcCitizenId'));$$('input,textarea,select',$('#caseDetailView')).forEach(x=>{if(x.id==='proposedRegion')return;x.addEventListener('input',refreshPaper)});
      // live save officerOpinion + badge update กันงงว่าพิมพ์แล้วทำไมยังรอความเห็น
      const opinionEl = document.getElementById('officerOpinion');
      if (opinionEl && !opinionEl.dataset.liveSaved) {
        opinionEl.dataset.liveSaved='1';
        let t=0;
        opinionEl.addEventListener('input', ()=>{
          clearTimeout(t);
          t=setTimeout(()=>{
            const v=opinionEl.value.trim();
            const st=getState(selectedId);
            if (st.documentData.officerOpinion !== v) {
              st.documentData.officerOpinion=v;
              saveState(selectedId, st);
              const badge=document.querySelector('.signature-workflow b');
              const hint=document.querySelector('.signature-workflow .signature-pending small');
              if (badge) badge.textContent = v ? 'รอการลงนาม' : 'รอความเห็น';
              if (hint) hint.textContent = v ? 'แบบบันทึกการพิจารณาและกลั่นกรองเรื่องร้องเรียน/เบาะแส' : 'ต้องกรอกความเห็นเจ้าหน้าที่ก่อนลงนาม';
            }
          }, 350);
        });
      }
      $$('input[name="naccReason"]').forEach(input=>input.addEventListener('change',applyDecision));
      const naccReasonBox=$('#naccReasonBox');if(naccReasonBox&&!$('#reliefInitialPackBox'))naccReasonBox.insertAdjacentHTML('beforebegin',reliefInitialPackFields(state.documentData));
      $$('input[name="reliefOperationKind"],input[name="centralSendsDossierToRegion"]').forEach(input=>input.addEventListener('change',applyDecision));
      const proposedRegionEl=$('#proposedRegion');if(proposedRegionEl&&!proposedRegionEl.dataset.regionBound){proposedRegionEl.dataset.regionBound='true';proposedRegionEl.addEventListener('change',()=>{const scrollY=window.scrollY;refreshPaper();requestAnimationFrame(()=>window.scrollTo(0,scrollY))})}$$('input[name="decision"]').forEach(x=>x.onchange=applyDecision);$$('input[name="screeningDecision"]').forEach(x=>x.onchange=()=>{$('#screeningDuplicateBox')?.classList.toggle('ws-hidden',x.value!=='suspected_duplicate')});$$('.wc-tab').forEach(button=>button.addEventListener('click',()=>{const target=button.dataset.wcTab;$$('.wc-tab').forEach(b=>{const on=b.dataset.wcTab===target;b.classList.toggle('active',on);b.setAttribute('aria-selected',on?'true':'false')});$$('.wc-panel').forEach(p=>p.classList.toggle('ws-hidden',p.dataset.wcPanel!==target))}));$('#anonymous')?.addEventListener('change',applyDecision);$$('input[name="route"]').forEach(x=>x.onchange=()=>$('#absenceBox').classList.toggle('ws-hidden',x.value!=='division'));$$('[data-doc]').forEach(b=>b.onclick=()=>{$$('[data-doc]').forEach(x=>{const selected=x===b;x.classList.toggle('active',selected);x.setAttribute('aria-selected',String(selected))});const paper=$('#paperStage');paper.innerHTML=documentPaper(capture(state),b.dataset.doc,activeRole);lockSignedPaper(state,paper)});$$('[data-action]').forEach(b=>b.onclick=()=>handleAction(b.dataset.action,state));$$('.pack-open').forEach(b=>b.onclick=()=>{const tab=$$('[data-doc]').find(t=>t.dataset.doc===b.dataset.docOpen);if(tab)tab.onclick()});
      // A4 shop — flexible upload + shopping
      const a4Toggle = document.querySelector('[data-a4-doc-shop-toggle]');
      if (a4Toggle && !a4Toggle.dataset.bound) {
        a4Toggle.dataset.bound='1';
        a4Toggle.onclick = () => {
          const shop=document.getElementById('a4DocShop');
          const paper=document.getElementById('paperStage');
          if(!shop||!paper) return;
          const hidden=shop.classList.contains('ws-hidden');
          if(hidden){
            shop.innerHTML=documentShopA4(getState(selectedId) || state);
            shop.classList.remove('ws-hidden');
            paper.parentElement.classList.add('ws-hidden');
            a4Toggle.textContent='✕ ปิด';
          }else{
            shop.classList.add('ws-hidden');
            paper.parentElement.classList.remove('ws-hidden');
            a4Toggle.textContent='📄 เลือกแบบพิมพ์';
          }
        };
      }
      document.querySelectorAll('[data-a4-shop-open]').forEach(btn=>{
        if(btn.dataset.bound) return; btn.dataset.bound='1';
        btn.onclick=()=>{
          const id=btn.dataset.a4ShopOpen;
          const shop=document.getElementById('a4DocShop');
          if(shop) shop.classList.add('ws-hidden');
          const paperPar=document.getElementById('paperStage')?.parentElement;
          if(paperPar) paperPar.classList.remove('ws-hidden');
          const a4Toggle2=document.querySelector('[data-a4-doc-shop-toggle]');
          if(a4Toggle2) a4Toggle2.textContent='📄 เลือกแบบพิมพ์';
          const tab=document.querySelector(`[data-doc="${id}"]`);
          if(tab) tab.click();
          else {
            const st=getState(selectedId) || state;
            document.getElementById('paperStage').innerHTML=documentPaper(st, id, activeRole);
          }
        };
      });
      const a4UploadBtn=document.querySelector('[data-a4-shop-upload]');
      if(a4UploadBtn && !a4UploadBtn.dataset.bound){
        a4UploadBtn.dataset.bound='1';
        a4UploadBtn.onclick=()=> document.getElementById('a4ShopFileInput')?.click();
      }
      const a4FileInput=document.getElementById('a4ShopFileInput');
      if(a4FileInput && !a4FileInput.dataset.bound){
        a4FileInput.dataset.bound='1';
        a4FileInput.onchange=()=>{
          const file=a4FileInput.files?.[0]; if(!file) return;
          const reader=new FileReader();
          reader.onload=()=>{
            try{
              const st=getState(selectedId) || state;
              st.caseData.attachments=attachmentsOf(st.caseData);
              st.caseData.attachments.push({name:file.name,type:file.type||'ไฟล์',size:file.size,desc:'อัปโหลดจากช็อป',dataUrl:String(reader.result).slice(0,200000)});
              saveState(selectedId, st);
              const shop=document.getElementById('a4DocShop');
              if(shop && !shop.classList.contains('ws-hidden')) shop.innerHTML=documentShopA4(getState(selectedId)||st);
              notify('success','อัปโหลดแล้ว',file.name);
      ...
      const packCard=$('.pack-card');
      if(packCard&&state.documentData.naccTransferKind==='return'&&!$('.pack-evidence',packCard)){
        packCard.querySelector('.pack-work-record,.pack-count-card')?.insertAdjacentHTML('beforebegin',`<div class="pack-evidence"><h4>หลักฐานประกอบการส่งคืน</h4><label class="ws-choice"><input type="checkbox" id="boardResolutionAttached" ${state.documentData.boardResolutionAttached===true?'checked':''}><span><strong>สำเนามติคณะกรรมการ ป.ป.ท.</strong><small>ยืนยันว่าแนบหลักฐานแล้ว</small></span></label><label class="ws-choice"><input type="checkbox" id="originalDossierAttached" ${state.documentData.originalDossierAttached===true?'checked':''}><span><strong>สำนวนและเอกสารต้นฉบับจากสำนักงาน ป.ป.ช. ครบชุด</strong><small>ยืนยันว่าแนบหลักฐานแล้ว</small></span></label></div>`);
        $$('#boardResolutionAttached,#originalDossierAttached',packCard).forEach(input=>input.addEventListener('change',()=>{const working=capture(state);saveState(selectedId,working)}));
      }
...
opinionPaper.addEventListener('blur',event=>{
      ...
      if(signedDocumentLocked(state)){
        lockSignedPaper(state,$('#paperStage'));
        const editButton=$('[data-action="toggle-edit"]');if(editButton)editButton.disabled=true;
      }
      lockSignedDocumentSources(state);
    ...
    capture=state=>{
      let captured=captureBase(state),d=captured.documentData;
      if(isOfficerRole(activeRole)){
        const values={naccOtherChecked:$('#naccOtherChecked')?.checked||false,naccOtherReason:$('#naccOtherReason')?.value?.trim()||''};
        const packPages={};
        $$('[data-pack-page]').forEach(input=>{packPages[input.dataset.packPage]=Number(input.value)||0});
        values.documentPack={...(d.documentPack||{}),sourceVersion:DOCUMENT_SOURCE_VERSION,caseType:packCaseTypeOf(captured),pages:packPages};
        const result=window.ECMISActivity4DocumentRules?.applyDocumentSourceEdits(captured,{section:'officer',values});
        captured=result?.record||captured;d=captured.documentData;
      }
      return captured;
    };
    async function handleAction(action,state){
      if(action==='toggle-edit'){if(signedDocumentLocked(state))return notify('warning','เอกสารลงนามแล้ว','เอกสาร Form 3 และเอกสารประกอบฉบับนี้ถูกล็อก กรุณาใช้คำสั่งส่งกลับแก้ไขเพื่อสร้างฉบับใหม่');const paper=$('#paperStage');if(!paper)return;const isOn=paper.classList.toggle('doc-edit-mode');const pane=paper.closest('.ws-doc-pane');pane?.querySelector('.doc-edit-fab')?.classList.toggle('active',isOn);pane?.querySelector('.doc-edit-bar')?.classList.toggle('show',isOn);applyEditableAll(paper,isOn);return}if(action==='print'){await scheduleDocumentPagination($('#paperStage'));window.print();return}
      if(action==='reset'){
        if(state.documentVersions?.some(version=>['signed','superseded','legacy-unverified'].includes(version?.status))||state.documentData.dispatchConfirmedAt)return notify('warning','ไม่สามารถลบเอกสารได้','เรื่องนี้มีเอกสารลงนามหรือจัดส่งแล้ว ระบบเก็บหลักฐานแบบไม่ลบและไม่เขียนทับ');
      ...
      if(action==='central-registry-confirm'){
        if(w.owner!=='central-registry-clerk'||w.stage!=='central-registry'||!isCentralRegistryChannel(state.caseData.channel,state.caseData.region))return notify('warning','ไม่สามารถบันทึกเลขรับ','เรื่องไม่ได้อยู่ในคิวสารบรรณกลางตามช่องทางและพื้นที่ที่กำหนด');
        const incomingNumber=String($('#centralIncomingRegistry')?.value||'').trim();
        if(!incomingNumber)return notify('warning','ยังไม่ได้กรอกเลขรับ','กรอกเลขรับสารบรรณกลางก่อนยืนยัน');
        state.caseData.registry=state.caseData.registry||{};
        state.caseData.registry.office=incomingNumber;
        state.caseData.registry.officeAt=state.caseData.registry.officeAt||new Date().toISOString();
        w.owner='case-admin-clerk';w.stage='case-admin-registry';w.status='รอธุรการ กบค. ออกเลขรับ กบค.';
        add(`ธุรการสารบรรณกลาง ชั้น 14 บันทึกเลขรับ ${incomingNumber} และส่งต่อธุรการ กบค.`);
        saveState(selectedId,state);notify('success','บันทึกเลขรับแล้ว','ส่งต่อให้ธุรการ กบค.');renderDetail();renderList();return;
      }
      if(action==='case-admin-register'){
        if(w.owner!=='case-admin-clerk'||w.stage!=='case-admin-registry'||(!requiresCaseAdminRegistry(state.caseData.channel,state.caseData.region)&&!isCentralRegistryChannel(state.caseData.channel,state.caseData.region)))return notify('warning','ไม่สามารถออกเลขรับ กบค.','เรื่องไม่ได้อยู่ในคิวรับ กบค. ตามช่องทางและพื้นที่ที่กำหนด');
        state.caseData.registry=state.caseData.registry||{};
        if(!state.caseData.registry.kbk){state.caseData.registry.kbk=nextRegistryNumber('kbk');state.caseData.registry.kbkAt=new Date().toISOString()}
        w.owner='admin';w.stage='admin-registry';w.status='รอธุรการ ศรร. ออกเลขรับ ศรร. และมอบหมาย';
        add(`ธุรการ กบค. ออกเลขรับ กบค. ${state.caseData.registry.kbk} และส่งต่อธุรการ ศรร.`);
        saveState(selectedId,state);notify('success','บันทึกเลขรับ กบค. แล้ว',`เลขรับ กบค. ${state.caseData.registry.kbk} · ส่งต่อให้ธุรการ ศรร. ออกเลขรับ ศรร.`);renderDetail();renderList();return;
      }
      if(action==='regional-register'){
        if(w.owner!=='regional-clerk'||w.stage!=='regional-registry'||!isRegionalChannel(state.caseData.channel)||state.caseData.region!==activeRegion)return notify('warning','ไม่สามารถออกเลขทะเบียนรับ','เรื่องไม่ได้อยู่ในคิวรับของเขตที่เลือก');
        state.caseData.registry=state.caseData.registry||{};
        if(!state.caseData.registry.region){state.caseData.registry.region=nextRegistryNumber(`region-${state.caseData.region}`);state.caseData.registry.regionAt=new Date().toISOString()}
        w.owner='regional-officer';w.stage='regional-officer';w.status='รอเจ้าหน้าที่เขตกลั่นกรองและพิจารณา';
        add(`ธุรการประจำเขตออกเลขทะเบียนรับ ${state.caseData.region} ${state.caseData.registry.region} และส่งให้เจ้าหน้าที่เขต`);
        saveState(selectedId,state);notify('success','บันทึกเลขทะเบียนรับแล้ว','ส่งต่อให้เจ้าหน้าที่เขตดำเนินการ');renderDetail();renderList();return;
      }
      if(action==='regional-director-sign'){
        if(w.owner!=='regional-director'||w.stage!=='regional-director')return notify('warning','ไม่สามารถลงนาม','เรื่องไม่ได้อยู่ในขั้นพิจารณาของ ผอ.สำนักงาน ป.ป.ท. เขต');
        if(!d.divisionOpinion)return notify('warning','ยังไม่มีคำสั่ง','กรอกความเห็นและคำสั่งก่อนลงนาม');
        const signer='ผอ.สำนักงาน ป.ป.ท. เขต';
        const confirmed=await confirmSignature(state,signer,signer);
        if(!confirmed.isConfirmed)return;
        const regionalSignature=signatureRecord(signer,signer);
        if(!recordSignedDocumentVersion(state,'division',regionalSignature))return notify('error','บันทึกฉบับลงนามไม่สำเร็จ','ระบบไม่เขียนทับหลักฐานเดิม กรุณาตรวจสอบสถานะเอกสาร');
        add(`${signer} ลงนามอิเล็กทรอนิกส์แล้ว`);
        saveState(selectedId,state);notify('success','ลงนามเรียบร้อย','พร้อมอนุมัติหรือส่งกลับเจ้าหน้าที่เขต');renderDetail();return;
      }
      if(action==='regional-director-return'){
        if(w.owner!=='regional-director'||w.stage!=='regional-director')return notify('warning','ไม่สามารถส่งกลับ','เรื่องไม่ได้อยู่ในขั้นพิจารณาของ ผอ.สำนักงาน ป.ป.ท. เขต');
        if(!d.divisionOpinion)return notify('warning','ต้องระบุเหตุผล','กรอกคำสั่งหรือประเด็นที่ต้องแก้ไข');
        if(!createCorrectionVersion(state,'regional-director','ผอ.สำนักงาน ป.ป.ท. เขต',d.divisionOpinion))return notify('error','สร้างฉบับแก้ไขไม่สำเร็จ','ไม่พบฉบับลงนามที่ใช้งานอยู่ ระบบจึงไม่เปลี่ยนแปลงเอกสาร');
        const transition=transitionCase(state,'regional-director-return',{correctionApplied:true,reason:d.divisionOpinion},'regional-director','ผอ.สำนักงาน ป.ป.ท. เขต');
        if(!transition.ok)return transitionBlocked(transition,'ไม่สามารถส่งกลับ');
        saveState(selectedId,state);notify('success','ส่งกลับแล้ว','เรื่องกลับไปยังเจ้าหน้าที่เขตตามพื้นที่รับผิดชอบ');renderDetail();renderList();return;
      }
      if(action==='regional-director-approve'){
        await runApprovalOnce(action,async()=>{
          if(w.owner!=='regional-director'||w.stage!=='regional-director')return notify('warning','ไม่สามารถอนุมัติ','เรื่องไม่ได้อยู่ในขั้นพิจารณาของ ผอ.สำนักงาน ป.ป.ท. เขต');
          if(!d.divisionOpinion)return notify('warning','ยังไม่มีคำสั่ง','กรอกความเห็นและคำสั่งก่อนอนุมัติ');
          if(!d.divisionSignature?.signed)return notify('warning','ยังไม่ได้ลงนาม','ผอ.สำนักงาน ป.ป.ท. เขต ต้องลงนามก่อนอนุมัติ');
          const caseNumberEligible=isCaseNumberEligible(d);
          const caseId=selectedId;
          const approveFresh=async approvalState=>{
            const approvalData=approvalState.documentData,approvalWorkflow=approvalState.workflow;
            const freshCaseNumberEligible=isCaseNumberEligible(approvalData);
            const transition=transitionCase(approvalState,'regional-director-approve',{},'regional-director',approvalData.divisionSignature.signerName);
            if(!transition.ok){transitionBlocked(transition,'ไม่สามารถอนุมัติ');return {status:'failed',state:approvalState}}
            if(freshCaseNumberEligible&&!approvalData.caseNumber){
              issueCaseNumber(approvalState);
              approvalData.publicStatus='สำนักงานได้รับเรื่องของท่านไว้แล้ว และจะดำเนินการเรื่องของท่านต่อไป';
            }
            approvalWorkflow.status=approvalData.decision==='send-nacc'?'รอเจ้าหน้าที่รับเรื่องประจำเขตจัดส่งสำนักงาน ป.ป.ช.':'อนุมัติแล้ว รอเจ้าหน้าที่รับเรื่องประจำเขตบันทึกข้อมูลการจัดส่ง';
            approvalState.decisionHistory.push({text:'ผอ.สำนักงาน ป.ป.ท. เขต อนุมัติแล้ว ส่งงานกลับเจ้าหน้าที่รับเรื่องประจำเขตเพื่อบันทึกข้อมูลจัดส่ง',time:now()});
            saveState(caseId,approvalState);
            return {status:'approved',state:approvalState};
          };
          const result=caseNumberEligible&&!d.caseNumber
            ?await runCaseNumberApprovalLocked(caseId,approveFresh)
            :await approveFresh(state);
          if(!result||['blocked','failed'].includes(result.status))return;
          const approvedState=getState(caseId);
          const persistedCaseNumberEligible=isCaseNumberEligible(approvedState.documentData);
          const showedCaseNumber=persistedCaseNumberEligible&&approvedState.documentData.caseNumber?await showCaseNumberIssuedSuccess(approvedState,{regional:true}):false;
          selectedRole='regional-officer';activeRole=selectedRole;activeRegion=approvedState.caseData.region;sessionStorage.setItem('ecmis-a4-role',selectedRole);sessionStorage.setItem('ecmis-a4-regional-region',activeRegion);$('#wsRole').value=selectedRole;updateRoleControls();syncUrl();
          if(!showedCaseNumber)notify(result.status==='already'?'info':'success',result.status==='already'?'อนุมัติเรื่องนี้แล้ว':'อนุมัติแล้ว',result.status==='already'?'ระบบใช้ผลอนุมัติที่บันทึกไว้แล้ว':'ส่งงานกลับเจ้าหน้าที่รับเรื่องประจำเขตเพื่อบันทึกข้อมูลจัดส่ง');renderDetail();renderList();
        });
        return;
      }
      if(action==='officer-sign'){
        try{
        // fallback: อ่านค่าจาก DOM โดยตรงกัน capture() ตก (id ชน, debounce)
        const liveScreeningVal = String(document.querySelector('input[name="screeningDecision"]:checked')?.value || document.getElementById('screeningLinkedCase')?.value || '').trim();
        const liveOpinionVal = String(document.getElementById('officerOpinion')?.value || '').trim();
        if (liveOpinionVal && !String(d.officerOpinion||'').trim()) { d.officerOpinion = liveOpinionVal; state.documentData.officerOpinion = liveOpinionVal; }
        const screeningVal = String(d.screening?.officerOpinion||d.screening?.decision||'').trim();
        // ถ้ายังไม่มี screening แต่ DOM เลือกไว้แล้วให้ใช้ DOM
        let effectiveScreening = screeningVal;
        if (!['new','suspected_duplicate'].includes(effectiveScreening) && ['new','suspected_duplicate'].includes(liveScreeningVal)) effectiveScreening = liveScreeningVal;
        const screeningPending=!['new','suspected_duplicate'].includes(effectiveScreening);
        if(screeningPending)return notify('warning','ยังไม่ได้ยืนยันกลั่นกรองเรื่องซ้ำ','เลือก “เรื่องใหม่” หรือ “สงสัยว่าเป็นเรื่องซ้ำ” และกดยืนยันการกลั่นกรองก่อนลงนาม');
        if(d.decision==='58/2'){
          const reliefValidation=window.ECMISActivity4DocumentRules?.validateDecisionRecord(state);
          if(!reliefValidation?.valid)return notify('warning','ข้อมูลชุดเอกสาร 58/2 ยังไม่ครบ','เลือกหน่วยงานผู้ดำเนินการ และระบุว่าส่วนกลางส่งต้นฉบับสำนวนไปเขตหรือไม่ก่อนลงนาม');
        }
        if(!String(d.officerOpinion||'').trim() && !liveOpinionVal)return notify('warning','ยังไม่มีความเห็นเจ้าหน้าที่','กรอกข้อเท็จจริงและความเห็นก่อนลงนาม');
        ...
        if(!confirmed.isConfirmed)return;
        const officerSignature=signatureRecord('เจ้าหน้าที่รับเรื่อง',officerName);
        if(!recordSignedDocumentVersion(state,'officer',officerSignature))return notify('error','บันทึกฉบับลงนามไม่สำเร็จ','ระบบไม่เขียนทับหลักฐานเดิม กรุณาตรวจสอบสถานะเอกสาร');
        const transitionRole=selectedRole==='regional-officer'?'regional-officer':'officer';
        const transition=transitionCase(state,'officer-sign',{},transitionRole,officerName);
        if(!transition.ok)return transitionBlocked(transition,'ไม่สามารถบันทึกสถานะลงนาม');
        ...
        }catch(e){ console.error(e); return notify('error','ลงนามไม่สำเร็จ','ระบบขัดข้อง: '+(e?.message||e)); }
      }
      if(action==='center-sign'){
        ...
        if(!confirmed.isConfirmed)return;
        const centerSignature=signatureRecord('ผู้อำนวยการศูนย์รับเรื่องร้องเรียน','ผู้อำนวยการศูนย์รับเรื่องร้องเรียน');
        if(!recordSignedDocumentVersion(state,'center',centerSignature))return notify('error','บันทึกฉบับลงนามไม่สำเร็จ','ระบบไม่เขียนทับหลักฐานเดิม กรุณาตรวจสอบสถานะเอกสาร');
      ...
      if(action==='division-sign'){
        ...
        if(!confirmed.isConfirmed)return;
        const divisionSignature=signatureRecord(activeRole==='acting'?'ผู้รักษาราชการแทน':'ผู้อำนวยการกองบริหารคดี',signer);
        if(!recordSignedDocumentVersion(state,'division',divisionSignature))return notify('error','บันทึกฉบับลงนามไม่สำเร็จ','ระบบไม่เขียนทับหลักฐานเดิม กรุณาตรวจสอบสถานะเอกสาร');
        add(`${signer} ลงนามอิเล็กทรอนิกส์แล้ว`);
        const caseId=selectedId;
        saveState(caseId,state);
        if(!isCaseNumberEligible(state.documentData)){
          notify('success','ลงนามเรียบร้อย','ระบบบันทึกหลักฐานการลงนามแล้ว พร้อมดำเนินการอนุมัติ');renderDetail();return;
        }
        const issueFresh=async signedState=>{
          const signedData=signedState.documentData;
          if(!signedData.divisionSignature?.signed||!isCaseNumberEligible(signedData))return {status:'failed',state:signedState};
          if(signedData.caseNumber)return {status:'already',state:signedState};
          issueCaseNumber(signedState);
          signedData.publicStatus='สำนักงานได้รับเรื่องของท่านไว้แล้ว และจะดำเนินการเรื่องของท่านต่อไป';
          signedState.decisionHistory.push({text:`ระบบออกเลขสำนวน ${signedData.caseNumber} หลัง ${signedData.divisionSignature.signerName||'ผู้อำนวยการกองบริหารคดี'} ลงนาม`,time:now()});
          saveState(caseId,signedState);
          return {status:'issued',state:signedState};
        };
        const result=await runCaseNumberApprovalLocked(caseId,issueFresh,{allowSignedAwaitingApproval:true});
        if(!result||['blocked','failed'].includes(result.status)){renderDetail();return}
        const persistedState=getState(caseId);
        if(persistedState.documentData.caseNumber)await showCaseNumberIssuedSuccess(persistedState);
        renderDetail();renderList();return;
        }catch(e){
          console.error(e);
          const storageFull=e?.name==='QuotaExceededError';
          return notify('error','ลงนามไม่สำเร็จ',storageFull?'พื้นที่จัดเก็บของ Mock เต็ม ระบบยังไม่บันทึกลายเซ็น กรุณาล้างข้อมูลทดสอบที่ไม่ใช้แล้ว':'ระบบขัดข้อง: '+(e?.message||e));
        }
      }
      if(action==='draft'){if(isOfficerRole(activeRole)&&!['officer','admin','regional-officer'].includes(w.owner)&&!w.complete)return notify('warning','ข้อมูลถูกล็อค','เรื่องนี้อยู่ระหว่างการพิจารณาของผู้มีอำนาจ');
      ...
      if(action==='screening-confirm'){
        ...
        if(screeningChoice==='suspected_duplicate'){
          const linkedCaseId=$('#screeningLinkedCase')?.value||'';
          if(!linkedCaseId)return notify('warning','ยังไม่ได้เลือกเรื่องเดิม','เลือกเรื่องเดิมที่สงสัยว่าซ้ำก่อนบันทึกความเห็น');
          ...
          const confirmed=await confirmDo('บันทึกความเห็นว่าสงสัยเป็นเรื่องซ้ำ',`ระบบจะบันทึกความเห็นและเสนอ ผอ.ศรร. กับ ผอ.กบค. ตามลำดับ โดยยังไม่รวมหรือปิดเรื่อง ${caseDisplayId(original)}`,'บันทึกความเห็น');
          if(!confirmed.isConfirmed)return;
          d.screening={decision:'suspected_duplicate',officerOpinion:'suspected_duplicate',decidedAt:decidedIso,decidedBy:decider,linkedCaseId,linkedTracking,note:screeningNote,additionalInfo,reportedToSupervisor:d.screening?.reportedToSupervisor||null};
          w.status='รอเจ้าหน้าที่ลงนามและเสนอความเห็นตามสายอนุมัติ';w.complete=false;
          add(`บันทึกความเห็นเจ้าหน้าที่: สงสัยว่าเป็นเรื่องซ้ำกับ ${caseDisplayId(original)} — ยังไม่รวมหรือปิดเรื่อง`);
          saveState(selectedId,state);
          notify('success','บันทึกความเห็นแล้ว','ยังไม่มีการรวมหรือปิดเรื่อง ต้องเสนอตามสายอนุมัติก่อน');
          renderDetail();renderList();return;
        }
        const confirmed=await confirmDo('ยืนยันเรื่องใหม่','ยืนยันว่าเป็นเรื่องใหม่ และดำเนินการตามขั้นตอนถัดไป','ยืนยันเรื่องใหม่');
        if(!confirmed.isConfirmed)return;
        d.screening={decision:'new',officerOpinion:'new',decidedAt:decidedIso,decidedBy:decider,linkedCaseId:'',linkedTracking:'',note:String($('#screeningNote')?.value||'').trim(),additionalInfo:'',reportedToSupervisor:d.screening?.reportedToSupervisor||null};
        const nextRoute=w.stage==='pre-registry-screening'?routeAfterNewScreening(state.caseData.channel,state.caseData.region):null;
        if(nextRoute)Object.assign(w,nextRoute);
        else w.status='รอเจ้าหน้าที่พิจารณา';
        add('ตัดสินใจกลั่นกรอง: เรื่องใหม่ — ดำเนินการตามขั้นตอนถัดไป');
        saveState(selectedId,state);
        notify('success','ยืนยันเรื่องใหม่แล้ว',nextRoute?`ส่งต่อให้ ${ROLE_LABELS[nextRoute.owner]} ดำเนินการลงรับแล้ว`:'ดำเนินการตามขั้นตอนถัดไป');
        renderDetail();renderList();return;
      }
      if(action==='screening-undo'){
        ...
        d.screening={decision:'',officerOpinion:'',decidedAt:'',decidedBy:'',linkedCaseId:'',linkedTracking:'',note:'',additionalInfo:'',reportedToSupervisor:d.screening?.reportedToSupervisor||null};
      ...
      if(action==='officer-finalize-screening'){
        const approvedOpinion=d.approvedOpinion;
        const linkedCaseId=d.approvedLinkedCaseId;
        const originalState=approvedOpinion==='suspected_duplicate'?findOriginalCase(linkedCaseId):null;
        if(approvedOpinion==='suspected_duplicate'&&!originalState?.caseData)return notify('warning','ไม่พบเรื่องเดิม','ไม่สามารถรวมเรื่องตามผลอนุมัติได้');
        const legacyApprovedCaseMissingNumber=approvedOpinion==='new'&&['18/1ก','18/1ข','18/4'].includes(d.decision)&&!d.caseNumber&&d.divisionSignature?.signed;
        if(legacyApprovedCaseMissingNumber){
          issueCaseNumber(state);
          d.publicStatus='สำนักงานได้รับเรื่องของท่านไว้แล้ว และจะดำเนินการเรื่องของท่านต่อไป';
          add(`ระบบแก้ไขข้อมูลเรื่องที่อนุมัติไว้ก่อนปรับกระบวนงาน: บันทึกเลขสำนวน ${d.caseNumber} ตามหลักฐานการลงนามของ ${d.divisionSignature.signerName||'ผอ.กบค.'}`);
        }
        const transition=transitionCase(state,'officer-finalize-screening',{linkedCaseRecord:originalState},selectedRole==='regional-officer'?'regional-officer':'officer',d.assignedOfficer||ROLE_LABELS[selectedRole]);
        if(!transition.ok)return transitionBlocked(transition,'ไม่สามารถดำเนินการตามผลอนุมัติ');
        if(transition.code==='ALREADY_CONFIRMED'){notify('info','ดำเนินการแล้ว','ระบบไม่รวมหรือเปลี่ยนสถานะซ้ำ');return}
        if(approvedOpinion==='suspected_duplicate'){
          const effect=transition.effects.find(item=>item.type==='UPSERT_LINKED_CASE');
          if(!effect?.record)return notify('error','รวมเรื่องไม่สำเร็จ','ระบบไม่ได้รับข้อมูลเรื่องเป้าหมายสำหรับบันทึกแบบอะตอมมิก');
          const store=readStore();
          store[selectedId]=state;
          store[effect.caseId]=effect.record;
          writeStore(store);
        }else add('เจ้าหน้าที่รับเรื่องยืนยันดำเนินการต่อในฐานะเรื่องใหม่ตามผลอนุมัติ ผอ.กบค.');
        if(approvedOpinion!=='suspected_duplicate')saveState(selectedId,state);
        notify('success','ดำเนินการตามผลอนุมัติแล้ว',w.status);renderDetail();renderList();return;
      }
      if(action==='assign'){
        ...
        if(!ok.isConfirmed)return;
        state.caseData.registry=state.caseData.registry||{};
        if(!state.caseData.registry.srr){state.caseData.registry.srr=nextRegistryNumber('srr');state.caseData.registry.srrAt=new Date().toISOString()}
      ...
      if(action==='officer-send'){const canSubmit=selectedRole==='regional-officer'?w.owner==='regional-officer'&&w.stage==='regional-officer'&&canRegionalAccess(state.caseData):w.owner==='officer'&&w.stage==='officer';if(!canSubmit)return notify('warning','ไม่สามารถส่งพิจารณา','เรื่องไม่ได้อยู่ในขั้นเจ้าหน้าที่ก่อนเสนอผู้มีอำนาจ');
        if(!['18/1ก','18/1ข','18/4','58/2','send-nacc','not-accept'].includes(d.decision))return notify('warning','ผลพิจารณาเดิมใช้ต่อไม่ได้','เลือกผลพิจารณาจาก 6 ตัวเลือกปัจจุบันก่อนส่งพิจารณา');
        if(d.decision==='58/2'&&!window.ECMISActivity4DocumentRules?.validateDecisionRecord(state)?.valid)return notify('warning','ข้อมูลชุดเอกสาร 58/2 ยังไม่ครบ','เลือกหน่วยงานผู้ดำเนินการ และระบุว่าส่วนกลางส่งต้นฉบับสำนวนไปเขตหรือไม่ก่อนส่งพิจารณา');
        if(d.decision==='send-nacc'&&d.needsReasonReview)return notify('warning','ต้องยืนยันเหตุผลใน Form 3','เลือกและยืนยันเหตุผลส่งสำนักงาน ป.ป.ช. ที่ถูกต้องอีกครั้งก่อนส่งพิจารณา');
        if(!['new','suspected_duplicate'].includes(d.screening?.officerOpinion||d.screening?.decision))return notify('warning','ยังไม่ได้บันทึกความเห็นกลั่นกรอง','ต้องบันทึกความเห็นว่าเป็นเรื่องใหม่หรือสงสัยว่าเป็นเรื่องซ้ำก่อนส่งพิจารณา');
        ...
        if(d.decision==='send-nacc'&&!['forward','return'].includes(d.naccTransferKind))return notify('warning','ยังไม่ได้เลือกลักษณะการส่ง','เลือกส่งเรื่องที่รับเองหรือส่งคืนเรื่องตามมาตรา 62');
        ...
        const route=$('input[name="route"]:checked')?.value||d.reviewRoute||'center';
        if(route==='division'&&!d.absenceReasonType)return notify('warning','ยังไม่ได้เลือกประเภทเหตุผล','เลือกประเภทเหตุผลที่ต้องเสนอข้าม ผู้อำนวยการศูนย์รับเรื่องร้องเรียน');
        ...
        const previewPages=await officerPreviewPages(state);
        const reviewTarget=route==='division'?(d.actingOfficer||'ผู้อำนวยการกองบริหารคดี'):'ผู้อำนวยการศูนย์รับเรื่องร้องเรียน';
        const reviewResult=window.Swal?await Swal.fire({title:`ตรวจสอบชุดเอกสารก่อนส่ง ${reviewTarget}`,html:officerSendReviewHtml(previewPages,reviewTarget),customClass:{popup:'ecmis-document-preview-dialog',htmlContainer:'ecmis-document-preview-content',confirmButton:'ecmis-preview-confirm',cancelButton:'ecmis-preview-cancel'},showCancelButton:true,confirmButtonText:`ยืนยันส่งต่อ ${reviewTarget}`,cancelButtonText:'กลับไปแก้ไข',confirmButtonColor:'#082b50',cancelButtonColor:'#687789',allowOutsideClick:false,didOpen:()=>{
          let current=0;
          let zoom=Number.parseFloat(getComputedStyle($('.officer-document-preview')).getPropertyValue('--officer-preview-zoom'))||.62;
          const visited=new Set();
          const viewport=$('#officerPreviewViewport'),title=$('#officerPreviewTitle'),counter=$('#officerPreviewCounter'),prev=$('#officerPreviewPrev'),next=$('#officerPreviewNext'),zoomOut=$('#officerPreviewZoomOut'),zoomIn=$('#officerPreviewZoomIn'),zoomValue=$('#officerPreviewZoomValue'),sequenceCheck=$('#officerReviewSequenceCheck'),sequenceText=$('#officerReviewSequenceText'),guidance=$('#officerReviewGuidance'),confirmButton=Swal.getConfirmButton(),actions=Swal.getActions(),dialogTitle=Swal.getTitle();
          $('#officerReviewTitleSlot').append(dialogTitle);
          $('#officerReviewActions').append(actions);
          const updateZoom=()=>{
            $('.officer-document-preview').style.setProperty('--officer-preview-zoom',zoom);
            zoomValue.value=`${Math.round(zoom*100)}%`;
            zoomOut.disabled=zoom<=.42;
          ...
          const renderPreview=()=>{
            ...
            $$('[data-officer-preview-index]').forEach((button,index)=>{
              button.disabled=index>Math.max(...visited);
              button.classList.toggle('current',index===current);
              button.classList.toggle('checked',visited.has(index)&&index!==current);
              button.setAttribute('aria-current',index===current?'page':'false');
              button.querySelector('.officer-review-document-state').textContent=visited.has(index)&&index!==current?'✓':String(index+1);
            });
            sequenceText.textContent=`ตรวจแล้ว ${visited.size} จาก ${previewPages.length} หน้า`;
            const complete=current>=previewPages.length-1;
            sequenceCheck.classList.toggle('complete',complete);
            sequenceCheck.querySelector('span').textContent=complete?'✓':'•';
            guidance.textContent=complete?'ตรวจถึงหน้าสุดท้ายแล้ว สามารถส่งต่อหรือกลับไปแก้ไขได้':'ปุ่มส่งต่อจะพร้อมใช้งานเมื่อตรวจถึงหน้าสุดท้าย';
          ...
          $$('[data-officer-preview-index]').forEach((button,index)=>button.addEventListener('click',()=>{current=index;renderPreview()}));
          zoomOut.addEventListener('click',()=>{zoom=Math.max(.42,Number((zoom-.1).toFixed(2)));updateZoom()});
          zoomIn.addEventListener('click',()=>{zoom=Math.min(1,Number((zoom+.1).toFixed(2)));updateZoom()});
          updateZoom();
          renderPreview();
        } }):{isConfirmed:confirm('ตรวจสอบชุดเอกสารครบถ้วนแล้วหรือไม่\nคลิกตกลงเพื่อส่งต่อ ผู้อำนวยการศูนย์รับเรื่องร้องเรียน')};
        previewPages.forEach(page=>{if(page.objectUrl)URL.revokeObjectURL(page.objectUrl)});
        if(!reviewResult.isConfirmed)return;
        const transitionRole=selectedRole==='regional-officer'?'regional-officer':'officer';
        const activeCorrectionSource=w.correction&&!w.correction.resolvedAt?w.correction.source:'';
        const transition=transitionCase(state,'officer-send',{},transitionRole,d.assignedOfficer||ROLE_LABELS[selectedRole]);
        if(!transition.ok)return transitionBlocked(transition,'ไม่สามารถส่งพิจารณา');
        w.status=w.owner==='regional-director'?'รอ ผอ.สำนักงาน ป.ป.ท. เขต พิจารณา':w.owner==='center'?'รอ ผู้อำนวยการศูนย์รับเรื่องร้องเรียน ให้ความเห็น':`รอ ${ROLE_LABELS[w.owner]||w.owner} ลงนามอนุมัติ`;
        if(!activeCorrectionSource)add(w.owner==='regional-director'?'เจ้าหน้าที่เขตเสนอความเห็นและส่ง ผอ.สำนักงาน ป.ป.ท. เขต พิจารณา':route==='division'?`เจ้าหน้าที่รับเรื่องเสนอข้าม ผอ.ศรร. ตามเหตุผล ${d.absenceReasonType}: ${d.absenceNote}`:'เจ้าหน้าที่รับเรื่องเสนอความเห็นกลั่นกรองและส่ง ผู้อำนวยการศูนย์รับเรื่องร้องเรียน');
      }
      if(action==='center-return'){
        if(!d.centerOpinion)return notify('warning','ต้องระบุเหตุผล','กรอกประเด็นที่ต้องแก้ไขก่อนส่งกลับ');
        if(!createCorrectionVersion(state,'center','ผู้อำนวยการศูนย์รับเรื่องร้องเรียน',d.centerOpinion))return notify('error','สร้างฉบับแก้ไขไม่สำเร็จ','ไม่พบฉบับลงนามที่ใช้งานอยู่ ระบบจึงไม่เปลี่ยนแปลงเอกสาร');
        const transition=transitionCase(state,'center-return',{correctionApplied:true,reason:d.centerOpinion},'center','ผู้อำนวยการศูนย์รับเรื่องร้องเรียน');
        if(!transition.ok)return transitionBlocked(transition,'ไม่สามารถส่งกลับ');
        d.centerDecision='disagree';
      }
      if(action==='center-send'){
        ...
        const transition=transitionCase(state,'center-send',{},'center',d.centerSignature.signerName||'ผู้อำนวยการศูนย์รับเรื่องร้องเรียน');
        if(!transition.ok)return transitionBlocked(transition,'ไม่สามารถส่งพิจารณา');
        w.status='รอ ผู้อำนวยการกองบริหารคดี';add(`ผู้อำนวยการศูนย์รับเรื่องร้องเรียน เห็นชอบให้มอบหมาย ${d.proposedRegion||'เขตที่เสนอ'} และส่ง ผู้อำนวยการกองบริหารคดี`);
      }
      ...
      if(action==='division-return'){
        if(!d.divisionOpinion)return notify('warning','ต้องระบุเหตุผล','กรอกคำสั่งหรือประเด็นที่ต้องแก้ไข');
        const correctionActor=activeRole==='acting'?(d.actingOfficer||'ผู้รักษาราชการแทน'):'ผู้อำนวยการกองบริหารคดี';
        if(!createCorrectionVersion(state,'division',correctionActor,d.divisionOpinion))return notify('error','สร้างฉบับแก้ไขไม่สำเร็จ','ไม่พบฉบับลงนามที่ใช้งานอยู่ ระบบจึงไม่เปลี่ยนแปลงเอกสาร');
        const transition=transitionCase(state,'division-return',{correctionApplied:true,reason:d.divisionOpinion},activeRole,correctionActor);
        if(!transition.ok)return transitionBlocked(transition,'ไม่สามารถส่งกลับ');
      }
      ...
      if(action==='approve'){
        await runApprovalOnce(action,async()=>{
          const caseId=selectedId,approverRole=activeRole;
          const approvalPreview=getState(caseId),previewData=approvalPreview.documentData;
          if(!previewData.divisionOpinion)return notify('warning','ยังไม่มีคำสั่ง','กรอกความเห็นและคำสั่งก่อนอนุมัติ');
          if(!previewData.divisionSignature?.signed)return notify('warning','ยังไม่ได้ลงนาม','ผู้อำนวยการกองบริหารคดี หรือผู้รักษาราชการแทนต้องลงนามก่อนอนุมัติ');
          if(isCaseNumberEligible(previewData)&&!previewData.caseNumber)return notify('warning','ยังไม่มีเลขสำนวน','เรื่องใหม่ที่รับไว้ดำเนินการต้องมีเลขสำนวนจากขั้นตอนลงนามก่อนอนุมัติ');
          const confirmed=await confirmDo('ยืนยันอนุมัติและส่งกลับเจ้าหน้าที่','ระบบจะบันทึกผลอนุมัติและส่งงานกลับเจ้าหน้าที่รับเรื่อง โดยใช้เลขสำนวนที่ออกไว้แล้ว','อนุมัติและส่งกลับ');
          if(!confirmed.isConfirmed)return;
          const approvalState=getState(caseId),approvalData=approvalState.documentData,approvalWorkflow=approvalState.workflow;
          if(isCaseNumberEligible(approvalData)&&!approvalData.caseNumber)return notify('warning','ยังไม่มีเลขสำนวน','สถานะเรื่องเปลี่ยนระหว่างยืนยัน กรุณาลงนามเพื่อออกเลขสำนวนก่อนอนุมัติ');
          const approvalActor=approvalData.divisionSignature?.signerName||previewData.divisionSignature.signerName;
          const transition=transitionCase(approvalState,'approve',{},approverRole,approvalData.divisionSignature?.signerName||approvalActor);
          if(!transition.ok)return transitionBlocked(transition,'ไม่สามารถอนุมัติ');
          approvalWorkflow.status='ผู้อำนวยการกองบริหารคดี อนุมัติแล้ว — รอเจ้าหน้าที่รับเรื่องดำเนินการตามผลอนุมัติ';
          approvalState.decisionHistory.push({text:`${approvalActor} อนุมัติความเห็นกลั่นกรอง และส่งกลับเจ้าหน้าที่รับเรื่องเพื่อดำเนินการขั้นสุดท้าย`,time:now()});
          approvalState.documentVersions.push({version:approvalState.documentVersions.length+1,actor:ROLE_LABELS[approverRole],time:now()});
          saveState(caseId,approvalState);
          const approvedState=getState(caseId),approvedWorkflow=approvedState.workflow;
          notify('success','อนุมัติเรียบร้อยแล้ว',approvedWorkflow.stage==='officer-finalize-screening'?'ระบบส่งงานกลับเจ้าหน้าที่รับเรื่องเพื่อดำเนินการตามผลอนุมัติ':'ระบบส่งงานกลับเจ้าหน้าที่รับเรื่องเพื่อบันทึกข้อมูลการจัดส่ง');
          selectedRole=approvedWorkflow.owner;activeRole=selectedRole;sessionStorage.setItem('ecmis-a4-role',selectedRole);
          if(selectedRole==='regional-officer'&&REGIONAL_REGIONS.includes(approvedState.caseData.region)){activeRegion=approvedState.caseData.region;sessionStorage.setItem('ecmis-a4-regional-region',activeRegion)}
          $('#wsRole').value=selectedRole;updateRoleControls();syncUrl();renderDetail();renderList();
        });
        return;
      }
    ...
    root.addEventListener('click',async event=>{
      ...
      const issueOutgoingButton=event.target.closest('[data-action="issue-outgoing-number"]');
      if(issueOutgoingButton){
        event.preventDefault();event.stopImmediatePropagation();
        const api=window.ECMISActivity4OutgoingRegisters;
        if(!api){notify('error','ออกเลขไม่ได้','ไม่พบโมดูลทะเบียนเลขหนังสือขาออก');return}
        let state=getState(selectedId);
        if(!['officer-dispatch','nacc-dispatch'].includes(state.workflow.stage)||state.workflow.complete){notify('warning','ออกเลขไม่ได้','ออกเลขได้หลังอนุมัติและงานกลับมายังเจ้าหน้าที่รับเรื่องเท่านั้น');return}
        const slotId=issueOutgoingButton.dataset.slotId;
        const slot=api.resolveOutgoingSlots(state).find(item=>item.slotId===slotId);
        if(!slot){notify('warning','ไม่พบรายการเอกสาร','โหลดเรื่องใหม่แล้วลองอีกครั้ง');return}
        const selector=$(`[data-outgoing-register="${slotId}"]`);
        const registerId=issueOutgoingButton.dataset.fixedRegister||selector?.value||'';
        const actor={role:selectedRole==='regional-officer'?'regional-officer':'officer',id:state.documentData.assignedOfficer||selectedRole,name:state.documentData.assignedOfficer||ROLE_LABELS[selectedRole]};
        const actorContext={role:actor.role,scope:actor.role==='regional-officer'?'regional':'central',storage:localStorage};
        const selection=api.validateRegisterSelection(slot,registerId,actorContext);
        if(!selection.valid){notify('warning','ยังเลือกทะเบียนไม่ครบ','เลือกทะเบียนเลขหนังสือขาออกก่อนกดออกเลข');return}
        const register=api.registerCatalog(actorContext)[registerId],previewYear=new Date().getFullYear()+543,doc=OFFICIAL_DOCUMENTS[slot.documentId];
        const confirmation=await confirmDo('ยืนยันออกเลขหนังสือ',`${register.label} · ${register.prefix}/[Running] · ปี ${previewYear} · ปปท. ${slot.documentId} ${doc?.name||''}\nเลขที่ออกแล้วแก้ไขหรือคืนเข้าทะเบียนไม่ได้`,'ยืนยันออกเลข');
        if(!confirmation.isConfirmed)return;
        const issuedAt=new Date(),issuedAtISO=issuedAt.toISOString(),buddhistYear=issuedAt.getFullYear()+543;
        const result=await api.issueLocalOutgoingNumber({storage:localStorage,caseId:state.caseData.id,slotId:slot.slotId,documentId:slot.documentId,purpose:slot.purpose,registerId,buddhistYear,now:issuedAtISO,actor,actorContext,workflowStage:state.workflow.stage,selectionSource:slot.mode==='fixed'?'fixed':'user'});
        if(!result.ok){notify('error','ออกเลขไม่สำเร็จ',`ระบบไม่บันทึกเลข (${result.errorCode||'UNKNOWN'})`);return}
        state.documentData.outgoingRegisterSelections={...(state.documentData.outgoingRegisterSelections||{}),[slot.slotId]:registerId};
        state=api.rehydrateOutgoingProjection(state,api.readLedger(localStorage));
        if(!result.replayed)state.decisionHistory.push({text:`[ออกเลขหนังสือขาออก] ปปท. ${slot.documentId} · ${register.label} · ${result.issuance.number} · โดย ${actor.name}`,time:new Intl.DateTimeFormat('th-TH',{dateStyle:'medium',timeStyle:'short'}).format(issuedAt)});
        saveState(selectedId,state);
        notify(result.replayed?'info':'success',result.replayed?'เอกสารนี้มีเลขแล้ว':'ออกเลขหนังสือเรียบร้อยแล้ว',result.issuance.number);
        renderDetail();renderList();return;
      }
      const addOutgoingRegisterButton=event.target.closest('[data-action="add-outgoing-register"]');
      if(addOutgoingRegisterButton){
        event.preventDefault();event.stopImmediatePropagation();
        const api=window.ECMISActivity4OutgoingRegisters,slotId=addOutgoingRegisterButton.dataset.slotId;
        const documentId=slotId.split(':')[0];
        if(!api||!['1-03','1-04','1-06'].includes(documentId))return;
        let label='',prefix='',confirmed=false;
        if(window.Swal){
          const result=await Swal.fire({title:'เพิ่มทะเบียนเลขหนังสือขาออก',html:'<div class="outgoing-register-dialog"><label>ชื่อทะเบียน<input id="outgoingRegisterLabel" class="swal2-input" autocomplete="off"></label><label>คำนำหน้าเลขหนังสือ<input id="outgoingRegisterPrefix" class="swal2-input" autocomplete="off" placeholder="เช่น ปป 0004.3"></label></div>',showCancelButton:true,confirmButtonText:'เพิ่มทะเบียน',cancelButtonText:'ยกเลิก',confirmButtonColor:'#082b50',focusConfirm:false,preConfirm:()=>{const nextLabel=document.getElementById('outgoingRegisterLabel')?.value.trim()||'',nextPrefix=document.getElementById('outgoingRegisterPrefix')?.value.trim()||'';if(!nextLabel||!nextPrefix){Swal.showValidationMessage('กรอกชื่อทะเบียนและคำนำหน้าเลขหนังสือให้ครบ');return false}return {label:nextLabel,prefix:nextPrefix}}});
          confirmed=result.isConfirmed;label=result.value?.label||'';prefix=result.value?.prefix||'';
        }else{
          label=prompt('ชื่อทะเบียนเลขหนังสือขาออก')?.trim()||'';
          if(label)prefix=prompt('คำนำหน้าเลขหนังสือ')?.trim()||'';
          confirmed=Boolean(label&&prefix);
        }
        if(!confirmed)return;
        const result=api.createCustomRegister({storage:localStorage,documentId,label,prefix});
        if(!result.ok){notify('warning','เพิ่มทะเบียนไม่ได้',result.errorCode==='CATALOG_DUPLICATE'?'ชื่อทะเบียนหรือคำนำหน้าซ้ำกับทะเบียนที่มีอยู่':result.errorCode==='CATALOG_FIELDS_REQUIRED'?'กรอกชื่อทะเบียนและคำนำหน้าให้ครบ':'ไม่สามารถบันทึกทะเบียนใหม่ได้');return}
        const state=getState(selectedId);
        state.documentData.outgoingRegisterSelections={...(state.documentData.outgoingRegisterSelections||{}),[slotId]:result.register.id};
        saveState(selectedId,state);
        notify('success','เพิ่มทะเบียนใหม่แล้ว',`ระบบเลือกทะเบียนใหม่นี้ให้เอกสาร ปปท. ${documentId} แล้ว`);
        renderDetail();renderList();return;
      }
      const manualOutgoingButton=event.target.closest('[data-action="record-manual-outgoing"]');
      if(manualOutgoingButton){
        event.preventDefault();event.stopImmediatePropagation();
        const api=window.ECMISActivity4OutgoingRegisters,state=getState(selectedId),slotId=manualOutgoingButton.dataset.slotId;
        const number=$(`[data-outgoing-manual-number="${slotId}"]`)?.value?.trim()||'';
        const sourceNote=$(`[data-outgoing-manual-note="${slotId}"]`)?.value?.trim()||'';
        const actor={role:'regional-officer',id:state.documentData.assignedOfficer||'regional-officer',name:state.documentData.assignedOfficer||ROLE_LABELS['regional-officer']};
        const confirmation=await confirmDo('ยืนยันเลขหนังสือที่เขตออกจริง',`${number||'ยังไม่ระบุเลข'}\nระบบจะบันทึกเป็นเลขจากทะเบียนเขต ไม่เพิ่ม Running ของส่วนกลาง`,'ยืนยันบันทึก');
        if(!confirmation.isConfirmed)return;
        const result=api.recordManualAuthoritativeNumber({storage:localStorage,caseId:state.caseData.id,slotId,documentId:slotId.split(':')[0],purpose:'main',number,sourceNote,actor,buddhistYear:new Date().getFullYear()+543});
        if(!result.ok){notify('warning','บันทึกเลขไม่ได้',result.errorCode==='OUTGOING_NUMBER_REQUIRED'?'กรอกเลขหนังสือและที่มาของทะเบียนเขตให้ครบ':`ระบบไม่บันทึกเลข (${result.errorCode})`);return}
        const projected=api.rehydrateOutgoingProjection(state,result.ledger);
        if(!result.replayed)projected.decisionHistory.push({text:`[บันทึกเลขหนังสือเขต] ${result.issuance.number} · ${sourceNote} · โดย ${actor.name}`,time:now()});
        saveState(selectedId,projected);notify('success','บันทึกเลขหนังสือเขตแล้ว',result.issuance.number);renderDetail();renderList();return;
      }
      const activity5DispatchButton=event.target.closest('[data-action="confirm-duplicate-package"],[data-action="activity5-dispatch-draft"],[data-action="activity5-dispatch-confirm"]');
      if(activity5DispatchButton){
        ...
        if(action==='confirm-duplicate-package'){
          const dispatchRole=selectedRole==='regional-officer'?'regional-officer':'officer';
          const transition=transitionCase(state,'confirm-duplicate-package',{},dispatchRole,d.assignedOfficer||ROLE_LABELS[dispatchRole]);
          if(transition.code==='ALREADY_CONFIRMED'){notify('info','ยืนยันไว้แล้ว','ระบบไม่สร้างประวัติการจัดชุดเอกสารซ้ำ');return}
          if(!transition.ok)return transitionBlocked(transition,'ยืนยันจัดชุดเอกสารไม่สำเร็จ');
          saveState(selectedId,state);
          notify('success','ยืนยันจัดชุดเอกสารครบแล้ว','ดำเนินการบันทึกข้อมูลและยืนยันจัดส่งไปยังเขตได้');
          renderDetail();renderList();return;
        }
        if(action==='activity5-dispatch-draft'){
          ...
          notify('success',d.decision==='not-accept'?'บันทึกร่างหลักฐานแล้ว':'บันทึกร่างข้อมูลจัดส่งแล้ว',d.decision==='not-accept'?'ข้อมูลยังไม่ได้รับการยืนยันแจ้งผลและยังไม่ปิดเรื่อง':'ข้อมูลยังไม่ได้รับการยืนยันจัดส่งไปยังเขตผู้รับผิดชอบ');return;
        }
        if(d.decision==='not-accept'){
          const evidence=d.notificationEvidence||{};
          const validation=window.ECMISActivity4DocumentRules?.validateNotificationEvidence(evidence);
          if(!validation?.valid){
            const labels={
              'invalid-notification-method':'เลือกวิธีแจ้งผลเป็นหนังสือ อีเมล หรือโทรศัพท์',
              'missing-notifiedAt':'ระบุวันเวลาที่แจ้งผล',
              'missing-approved-template':'ระบุรหัสแบบหนังสือที่อนุมัติ',
              'missing-outgoing-letter-no':'ระบุเลขหนังสือขาออก',
              'missing-letter-date':'ระบุวันที่หนังสือ',
              'missing-recipient':'ระบุผู้รับหนังสือ',
              'invalid-email-recipient':'ระบุอีเมลผู้รับให้ถูกต้อง',
              'missing-email-evidence':'แนบไฟล์หลักฐานหรือระบุหมายเหตุการส่งอีเมล',
              'invalid-phone-contact':'ระบุหมายเลขโทรศัพท์อย่างน้อย 5 หลัก',
              'missing-phone-evidence':'แนบไฟล์หลักฐานหรือระบุหมายเหตุการโทรศัพท์'
            };
            const detail=(validation?.errors||[]).map(error=>labels[error]||error).join(' · ');
            notify('warning','หลักฐานการแจ้งผลยังไม่ครบ',detail||'ตรวจสอบข้อมูลหลักฐานการแจ้งผล');
            return;
          }
          const confirmation=await confirmDo('ยืนยันการแจ้งผลและปิดเรื่อง','เมื่อยืนยันแล้ว หลักฐานจะเปิดอ่านอย่างเดียวและเรื่องจะสิ้นสุดกระบวนงาน','ยืนยันแจ้งผลและปิดเรื่อง');
          if(!confirmation.isConfirmed)return;
          const dispatchRole=selectedRole==='regional-officer'?'regional-officer':'officer';
          const transition=transitionCase(state,'confirm-dispatch',{notificationEvidence:evidence},dispatchRole,d.assignedOfficer||ROLE_LABELS[dispatchRole]);
          if(transition.code==='ALREADY_CONFIRMED'){notify('info','ยืนยันไว้แล้ว','ระบบไม่สร้างประวัติการแจ้งผลซ้ำ');return}
          if(!transition.ok)return transitionBlocked(transition,'ยืนยันการแจ้งผลไม่สำเร็จ');
          const time=now(),methodLabel={letter:'หนังสือ',email:'อีเมล',phone:'โทรศัพท์'}[evidence.method]||evidence.method;
          state.workflow.status='แจ้งผลไม่รับไว้ดำเนินการแล้ว';
          state.decisionHistory.push({text:`เจ้าหน้าที่รับเรื่องยืนยันแจ้งผลผู้ร้องทาง${methodLabel}และปิดเรื่อง`,time});
          state.documentVersions.push({version:state.documentVersions.length+1,actor:d.assignedOfficer||ROLE_LABELS[dispatchRole],time});
          saveState(selectedId,state);
          notify('success','ยืนยันการแจ้งผลเรียบร้อยแล้ว','บันทึกหลักฐานและปิดกระบวนงานกิจกรรมที่ 4 แล้ว');renderDetail();renderList();
          return;
        }
        const outgoingValidation=outgoingDispatchValidation(state);
        if(!outgoingValidation.valid){notify('warning','เลขหนังสือขาออกยังไม่ครบ','ออกเลขให้เอกสารที่กำหนดครบทุกฉบับก่อนยืนยันจัดส่ง');return}
        ...
        if(!confirmation.isConfirmed)return;
        const dispatchRole=selectedRole==='regional-officer'?'regional-officer':'officer';
        const transition=transitionCase(state,'confirm-dispatch',{},dispatchRole,d.assignedOfficer||ROLE_LABELS[dispatchRole]);
        if(transition.code==='ALREADY_CONFIRMED'){notify('info','ยืนยันไว้แล้ว','ระบบไม่สร้างรายการจัดส่งหรือส่งต่อซ้ำ');return}
        if(!transition.ok)return transitionBlocked(transition,'ยืนยันการจัดส่งไม่สำเร็จ');
        const time=now(),dispatchedAt=d.dispatchConfirmedAt;
        const closesActivity4=d.decision==='58/2';
        state.workflow.status=closesActivity4?'จัดส่งตามมาตรา 58/2 แล้ว':`จัดส่งเรื่องไปยัง ${d.dispatchDestinationUnit} แล้ว`;
        ...
        const createA5=transition.effects.some(effect=>effect.type==='CREATE_A5_PENDING');
        const handoffResult=createA5?window.ECMISActivity5Handoff?.create(localStorage,state,dispatchedAt,dispatchRole)||null:{eligible:true};
        ...
        saveState(selectedId,state);
        globalThis.ECMISIntakeAggregate?.publishCase?.(state);
        notify('success','ยืนยันการจัดส่งเรียบร้อยแล้ว',closesActivity4?'บันทึกหลักฐานและปิดกระบวนงานกิจกรรมที่ 4 แล้ว':`จัดส่งเรื่องไปยัง ${d.dispatchDestinationUnit} แล้ว และส่งข้อมูลเข้าสู่กระบวนงานลำดับถัดไปเรียบร้อยแล้ว`);renderDetail();renderList();return;
      }
      ...
      if(approveNaccButton&&getState(selectedId).documentData.decision==='send-nacc'){
        ...
        await runApprovalOnce('approve',async()=>{
          ...
          const transition=transitionCase(state,'approve',{},activeRole,approvalActor);
          if(!transition.ok)return transitionBlocked(transition,'ไม่สามารถอนุมัติส่ง ป.ป.ช.');
      ...
      if(naccActionButton){
        ...
        const outgoingValidation=outgoingDispatchValidation(state);
        if(!outgoingValidation.valid){notify('warning','เลขหนังสือขาออกยังไม่ครบ','ออกเลขให้เอกสารที่กำหนดครบทุกฉบับก่อนยืนยันส่งสำนักงาน ป.ป.ช.');return}
        if(!d.naccLetterNo||!d.naccLetterDate||!d.naccSentDate){notify('warning','ข้อมูลการจัดส่งยังไม่ครบ','กรอกเลขหนังสือ วันที่หนังสือ และวันที่ส่งให้ครบ');return}
        if(d.naccTransferKind==='return'&&(!d.naccBoardNo||!d.naccBoardDate)){notify('warning','ข้อมูลมติยังไม่ครบ','การส่งคืนเรื่องตามมาตรา 62 ต้องกรอกเลขที่และวันที่มติคณะกรรมการ ป.ป.ท.');return}
        if(d.naccTransferKind==='return'&&(!d.boardResolutionAttached||!d.originalDossierAttached)){notify('warning','หลักฐานส่งคืนยังไม่ครบ','ยืนยันแนบสำเนามติคณะกรรมการ ป.ป.ท. และสำนวนต้นฉบับจากสำนักงาน ป.ป.ช. ครบชุด');return}
        ...
        if(!confirmation.isConfirmed)return;
        const dispatchRole=selectedRole==='regional-officer'?'regional-officer':'officer';
        const transition=transitionCase(state,'nacc-dispatch-confirm',{},dispatchRole,d.assignedOfficer||ROLE_LABELS[dispatchRole]);
        if(transition.code==='ALREADY_CONFIRMED'){notify('info','ยืนยันไว้แล้ว','ระบบไม่สร้างรายการจัดส่งซ้ำ');return}
        if(!transition.ok)return transitionBlocked(transition,'ยืนยันส่งสำนักงาน ป.ป.ช. ไม่สำเร็จ');
        const time=now();
        state.workflow.status='ส่งสำนักงาน ป.ป.ช. แล้ว';
        state.decisionHistory.push({text:`[จัดส่ง ป.ป.ช.] เลขหนังสือ ${d.naccLetterNo} · ${d.naccSendMethod}${d.naccEms?` ${d.naccEms}`:''}${d.naccTransferKind==='return'?` · มติ ${d.naccBoardNo}`:' · รายงานคณะกรรมการภายหลังตามรอบเดือน'} · แจ้งผู้ร้องแล้ว`,time});
      ...
      if(assignButton){
        ...
        const transition=transitionCase(state,'assign',{assignedOfficer:d.assignedOfficer,backupOfficer:d.backupOfficer||''},'admin','ธุรการ ศรร.');
        if(!transition.ok)return transitionBlocked(transition,'ไม่สามารถมอบหมาย');
        if(previousOfficer&&previousOfficer!==d.assignedOfficer)d.officerSignature={signed:false,signerName:'',signerRole:'เจ้าหน้าที่รับเรื่อง',signedAt:'',signatureId:''};
        Object.assign(state.assignmentHistory.at(-1),{text});
      ...
      const transition=transitionCase(state,'recall-assignment',{reason},'admin','ธุรการ ศรร.');
      if(!transition.ok)return transitionBlocked(transition,'ไม่สามารถดึงงานกลับ');
      Object.assign(state.assignmentHistory.at(-1),{text:`[ดึงกลับ] ดึงงานกลับจาก ${previousOfficer}${previousBackupOfficer?` · ผู้ปฏิบัติงานแทน ${previousBackupOfficer}`:''} · เหตุผล: ${reason} · ดำเนินการโดยธุรการ ศรร.`});
      state.decisionHistory.push({text:`ธุรการ ศรร. ดึงงานกลับจาก ${previousOfficer} เพื่อมอบหมายใหม่`,time:now()});
      state.documentVersions.push({version:state.documentVersions.length+1,actor:'ธุรการ ศรร.',time:now()});
      adminQueue='unassigned';
      syncUrl();
      saveState(selectedId,state);
    ...
    if(!isA5Route)syncUrl();renderList();if(selectedId)renderDetail();
  }
  window.ECMISActivity4Routing=Object.freeze({initialRoute,postScreeningRoute,issuesTrackingImmediately,requiresCaseAdminRegistry,isRegionalChannel,intakeWorkflow,routeAfterNewScreening,caseReferenceForRole,roleCaseSearchText,roleCanSeeState,dashboardBucket});
  // A4 shop delegated — stable for dynamically created cards + flexible upload
  document.addEventListener('click', e=>{
    const a4Toggle = e.target.closest('[data-a4-doc-shop-toggle]');
    if(a4Toggle){
      e.preventDefault();
      const shop=document.getElementById('a4DocShop');
      const paper=document.getElementById('paperStage');
      ...
      const hidden=shop.classList.contains('ws-hidden');
      if(hidden){
        const st=getState(selectedId) || readStore()[Object.keys(readStore())[0]];
        if(st) shop.innerHTML=documentShopA4(st);
        shop.classList.remove('ws-hidden');
        paper.parentElement.classList.add('ws-hidden');
        a4Toggle.textContent='✕ ปิด';
      }else{
    ...
    const a4Open=e.target.closest('[data-a4-shop-open]');
    if(a4Open){
      e.preventDefault();
      const id=a4Open.dataset.a4ShopOpen;
      const shop=document.getElementById('a4DocShop');
      if(shop) shop.classList.add('ws-hidden');
      const paperPar=document.getElementById('paperStage')?.parentElement;
      if(paperPar) paperPar.classList.remove('ws-hidden');
      const t2=document.querySelector('[data-a4-doc-shop-toggle]');
      if(t2) t2.textContent='📄 เลือกแบบพิมพ์';
      const st=getState(selectedId);
      if(!st) return;
      const tab=document.querySelector(`[data-doc="${id}"]`);
      if(tab) tab.click();
      else document.getElementById('paperStage').innerHTML=documentPaper(st, id, activeRole);
      return;
    }
    const a4BulkAdd=e.target.closest('[data-a4-shop-bulk-add]');
    if(a4BulkAdd){
      const checks=[...document.querySelectorAll('#a4DocShop [data-a4-shop-check]:checked')];
      if(!checks.length) return notify('warning','ยังไม่ได้เลือก','ติ๊กเอกสารที่ต้องการก่อน');
      let added=0;
      for(const chk of checks){
        const id=chk.dataset.a4ShopCheck;
        const st=getState(selectedId);
        if(!st) continue;
        if(officialDocumentIds(st).includes(id)) continue;
        // For A4, adding means ensuring document is in official list (it already is via officialDocumentIds logic), just need to ensure it's visible
        // For demo, mark as added by ensuring it appears in shop as exists (officialDocumentIds will include it)
        added++;
      }
      const shop=document.getElementById('a4DocShop');
      if(shop && !shop.classList.contains('ws-hidden')){
        const st2=getState(selectedId);
        if(st2) shop.innerHTML=documentShopA4(st2);
      }
      notify('success','เพิ่มแล้ว',`เลือก ${added} เอกสาร เข้าชุดแล้ว`);
      return;
    }
    const a4BulkClear=e.target.closest('[data-a4-shop-bulk-clear]');
    if(a4BulkClear){
      document.querySelectorAll('#a4DocShop [data-a4-shop-check]').forEach(cb=> cb.checked=false);
      const all=document.getElementById('a4ShopSelectAll');
      if(all) all.checked=false;
      const cnt=document.getElementById('a4ShopSelectedCount');
      if(cnt) cnt.textContent='0';
      const addBtn=document.querySelector('[data-a4-shop-bulk-add]');
      if(addBtn) addBtn.disabled=true;
      return;
    }
    const a4Upload=e.target.closest('[data-a4-shop-upload]');
    if(a4Upload){
      e.preventDefault();
      document.getElementById('a4ShopFileInput')?.click();
      return;
    }
  });
  document.addEventListener('change', e=>{
    const a4Check=e.target.closest('[data-a4-shop-check]');
    if(a4Check){
      const cnt=document.querySelectorAll('#a4DocShop [data-a4-shop-check]:checked').length;
      const el=document.getElementById('a4ShopSelectedCount');
      if(el) el.textContent=String(cnt);
      const addBtn=document.querySelector('[data-a4-shop-bulk-add]');
      if(addBtn) addBtn.disabled=cnt===0;
      const all=document.getElementById('a4ShopSelectAll');
      const total=document.querySelectorAll('#a4DocShop [data-a4-shop-check]:not([disabled])').length;
      if(all) all.checked=total>0 && cnt===total;
      return;
    }
    const a4SelectAll=e.target.closest('#a4ShopSelectAll');
    if(a4SelectAll){
      const checked=a4SelectAll.checked;
      document.querySelectorAll('#a4DocShop [data-a4-shop-check]:not([disabled])').forEach(cb=> cb.checked=checked);
      const cnt=document.querySelectorAll('#a4DocShop [data-a4-shop-check]:checked').length;
      const el=document.getElementById('a4ShopSelectedCount');
      if(el) el.textContent=String(cnt);
      const addBtn=document.querySelector('[data-a4-shop-bulk-add]');
    ...
    const input=e.target.closest('#a4ShopFileInput');
    ...
    reader.onload=()=>{
      try{
        ...
        st.caseData.attachments=attachmentsOf(st.caseData);
        st.caseData.attachments.push({name:file.name,type:file.type||'ไฟล์',size:file.size,desc:'อัปโหลดจากช็อป',dataUrl:String(reader.result).slice(0,200000)});
        saveState(selectedId || st.caseData.id, st);
        const shop=document.getElementById('a4DocShop');
        if(shop && !shop.classList.contains('ws-hidden')) shop.innerHTML=documentShopA4(getState(selectedId)||st);
        notify('success','อัปโหลดแล้ว',file.name);
      }catch(err){ notify('warning','อัปโหลดไม่สำเร็จ',String(err)); }
      input.value='';
    };
    reader.readAsDataURL(file);
  });
