(() => {
  ...
  function currentA5Role() {
    ...
    if (fromUrl && A5_ROLE_LABELS[fromUrl]) return fromUrl;
  ...
  function menuFor(role) {
    const list = [
      { label: 'ภาพรวมรับเรื่องร้องเรียน', icon: 'dashboard', href: roleUrl('dashboard-g4.html', role) },
      { label: 'รายการเรื่องร้องเรียน', icon: 'inbox', href: roleUrl('staff-workflow.html', role) }];
    if (role === 'admin') {
      list.push(
    ...
    // จัดการคำแนะนำ: ทุกคนยกเว้น anonymous แก้ได้
    if (role !== 'anonymous') {
      list.push({ label: 'จัดการคำแนะนำ', icon: 'settings', command: 'manageSuggestions' });
    }
    // จัดการผู้ปฏิบัติงานแทน: เฉพาะ ผอ.กบค. (division) เท่านั้น
    if (role === 'division') {
      list.push({ label: 'จัดการผู้ปฏิบัติงานแทน', icon: 'settings', command: 'manageBackups' });
    }
    return list;
  }
  ...
  function a5MenuFor(role) {
    return [
      { label: 'ภาพรวมกระบวนการไต่สวน', icon: 'dashboard', href: roleUrl('dashboard-g5.html', role) },
      ...A5_MENU_ITEMS.map(([q, label]) => ({
      label,
  ...
  function render() {
    ...
    const isDashboardG5 = file === 'dashboard-g5.html';
    const isDashboardG4 = file === 'dashboard-g4.html';
    const isA5 = params.get('view') === 'a5' || isDashboardG5;
