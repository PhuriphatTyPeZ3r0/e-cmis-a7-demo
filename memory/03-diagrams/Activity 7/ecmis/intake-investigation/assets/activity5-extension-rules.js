(function initializeActivity5ExtensionRules(root) {
  ...
  function policy(reportType, formId, baseDays, normalRoundLimit, roundAuthorityTiers, requiredDocumentCodes, roundAuthorityAlternatives = {}) {
    return deepFreeze({
      reportType,
      formId,
      baseDays,
  ...
  const EXTENSION_POLICIES = deepFreeze({
    ...
    }, [], { 2: [AUTHORITY_TIERS.SECRETARY_GENERAL_PERSONAL] }),
    [EXTENSION_TYPES.FULL_INQUIRY]: policy("644", "FORM_3", 270, 4, {
      ...
      3: [AUTHORITY_TIERS.SECRETARY_GENERAL_PERSONAL],
      4: [AUTHORITY_TIERS.SECRETARY_GENERAL_PERSONAL]
    })
  });
  ...
  function evaluateNormalRound(extensionType, roundNo) {
    ...
    const authorityTier = extensionPolicy.roundAuthorityTiers[roundNo];
    return envelope(true, "NORMAL_EXTENSION_ROUND", {
      mode: "NORMAL",
      roundNo,
      authorityTier,
      authorityTiers: [authorityTier, ...(extensionPolicy.roundAuthorityAlternatives?.[roundNo] || [])],
      normalRoundLimit: extensionPolicy.normalRoundLimit
    });
  }
  ...
  function addCivilYears(value, years) {
    const text = isoDate(value);
    if (!text || !Number.isInteger(years)) return "";
  ...
  function getDeadlineReminderSchedule(extensionType, periodStartedAt) {
    const extensionPolicy = getExtensionPolicy(extensionType);
    const startedAt = isoDate(periodStartedAt);
    if (!extensionPolicy) return envelope(false, "INVALID_EXTENSION_TYPE", null, [{ field: "extensionType" }]);
    if (!startedAt) return envelope(false, "DEADLINE_BASIS_MISSING", null, [{ field: "periodStartedAt" }]);
    const reminderDays = extensionType === EXTENSION_TYPES.PRELIMINARY_INQUIRY ? [15, 30, 45] : [];
    return envelope(true, "DEADLINE_REMINDERS_SCHEDULED", {
      extensionType,
      periodStartedAt: startedAt,
      reminders: reminderDays.map(elapsedDay => ({ elapsedDay, dueAt: addCivilDays(startedAt, elapsedDay) }))
    });
  }

  function evaluateFullInquiryAge(input = {}) {
    const receivedFirstAt = isoDate(input.receivedFirstAt);
    const asOf = isoDate(input.asOf);
    if (!receivedFirstAt) return envelope(false, "RECEIVED_DATE_MISSING", null, [{ field: "receivedFirstAt" }]);
    if (!asOf) return envelope(false, "AS_OF_DATE_INVALID", null, [{ field: "asOf" }]);
    const foreignRelated = input.foreignRelated === true;
    const twoYearReportAt = addCivilYears(receivedFirstAt, 2);
    const ceilingYears = foreignRelated ? 5 : 3;
    const ceilingAt = addCivilYears(receivedFirstAt, ceilingYears);
    const status = asOf >= ceilingAt
      ? "CEILING_REACHED"
      : asOf >= twoYearReportAt
        ? "CASE_REPORT_REQUIRED"
        : "WITHIN_CASE_AGE";
    return envelope(true, "FULL_INQUIRY_AGE_EVALUATED", {
      receivedFirstAt,
      asOf,
      foreignRelated,
      twoYearReportAt,
      ...
      ceilingAt,
      status,
      automaticExtensionAllowed: false
    });
  }
  ...
  const api = deepFreeze({
    ...
    getDeadlineReminderSchedule,
    evaluateFullInquiryAge,
