...
-- 4. RLS: qualifier lookup is read-only reference data (anon_select only,
--    matching tbl_res_offense_basis/tbl_law_pacc_article); the junction
--    needs insert too since users attach tags when building an agenda item
--    (matching tbl_res_calendar_item_case's existing anon_insert+select).
...
CREATE POLICY anon_insert ON public.tbl_res_calendar_item_qualifier FOR INSERT TO anon WITH CHECK (true);
CREATE POLICY anon_delete ON public.tbl_res_calendar_item_qualifier FOR DELETE TO anon USING (true);
