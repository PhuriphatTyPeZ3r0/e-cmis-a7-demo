...
/* --------------------------------------------------------------------------
   ACROBAT VIEWER ENGINE (Adobe Acrobat Style Document Preview)
   Centralized Global Document Viewer for E-CMIS
   -------------------------------------------------------------------------- */

function renderAcrobatDocumentViewer(containerEl, pages, pageClass, runningHeaderHtml) {
  if (!containerEl) return;
  const pageCount = pages.length;

  let viewerRoot = containerEl.querySelector('.acrobat-viewer-root');
  let isInitialBuild = false;

  if (!viewerRoot) {
    isInitialBuild = true;
    containerEl.innerHTML = `
      <div class="acrobat-viewer-root">
        <!-- Acrobat Dark Toolbar -->
        <div class="acrobat-toolbar">
          <div class="acrobat-tb-group">
            <button type="button" class="acrobat-btn" id="acrobatBtnThumbnails" title="แถบแสดงภาพย่อ (Thumbnails)">
              <i class="fa-solid fa-table-cells-large"></i>
            </button>
            <button type="button" class="acrobat-btn" id="acrobatBtnSearch" title="ค้นหาในเอกสาร (Find)">
              <i class="fa-solid fa-magnifying-glass"></i>
            </button>
          </div>

          <div class="acrobat-tb-group">
            <button type="button" class="acrobat-btn" id="acrobatBtnPrev" title="หน้าก่อนหน้า">
              <i class="fa-solid fa-chevron-up"></i>
            </button>
            <div class="acrobat-page-input-wrap">
              <input type="number" class="acrobat-page-input" id="acrobatPageInput" value="1" min="1" max="${pageCount}">
              <span class="acrobat-page-total" id="acrobatPageTotal">/ ${pageCount}</span>
            </div>
            <button type="button" class="acrobat-btn" id="acrobatBtnNext" title="หน้าถัดไป">
              <i class="fa-solid fa-chevron-down"></i>
            </button>

            <div class="acrobat-divider"></div>

            <button type="button" class="acrobat-btn" id="acrobatBtnZoomOut" title="ซูมออก (-10%)">
              <i class="fa-solid fa-minus"></i>
            </button>
            <select class="acrobat-zoom-select" id="acrobatZoomSelect" title="ระดับการซูม">
              <option value="fit-width">พอดีความกว้าง (Fit Width)</option>
              <option value="fit-page">พอดีหน้ากระดาษ (Fit Page)</option>
            ...
            <button type="button" class="acrobat-btn" id="acrobatBtnZoomIn" title="ซูมเข้า (+10%)">
              <i class="fa-solid fa-plus"></i>
            </button>
          </div>

          <div class="acrobat-tb-group">
            <button type="button" class="acrobat-btn" id="acrobatBtnRotate" title="หมุนตามเข็มนาฬิกา (90°)">
              <i class="fa-solid fa-rotate-right"></i>
            </button>
            <button type="button" class="acrobat-btn" id="acrobatBtnPrint" title="พิมพ์เอกสาร (Print)">
              <i class="fa-solid fa-print"></i>
            </button>
            <button type="button" class="acrobat-btn" id="acrobatBtnFullscreen" title="ขยายเต็มหน้าจอ (Fullscreen)">
              <i class="fa-solid fa-expand"></i>
            </button>
          </div>
        </div>

        <!-- In-document Search Bar (Collapsible) -->
        <div class="acrobat-search-bar d-none" id="acrobatSearchBar">
          <div class="acrobat-search-input-wrap">
            <i class="fa-solid fa-magnifying-glass acrobat-search-icon"></i>
            <input type="text" class="acrobat-search-input" id="acrobatSearchInput" placeholder="ค้นหาข้อความในเอกสาร...">
            <span class="acrobat-search-count" id="acrobatSearchCount">0/0</span>
          </div>
          <button type="button" class="acrobat-btn acrobat-btn-sm" id="acrobatBtnSearchPrev" title="ก่อนหน้า">
            <i class="fa-solid fa-chevron-up"></i>
          </button>
          <button type="button" class="acrobat-btn acrobat-btn-sm" id="acrobatBtnSearchNext" title="ถัดไป">
            <i class="fa-solid fa-chevron-down"></i>
          </button>
          <button type="button" class="acrobat-btn acrobat-btn-sm" id="acrobatBtnSearchClose" title="ปิดค้นหา">
            <i class="fa-solid fa-xmark"></i>
          </button>
        </div>

        <!-- Main Viewport (Thumbnails Drawer + Dark Canvas) -->
        <div class="acrobat-main-viewport">
          <aside class="acrobat-thumbnails-drawer d-none" id="acrobatThumbnailsDrawer">
            <div class="acrobat-thumbnails-head">
              <span><i class="fa-solid fa-file-lines me-1"></i>หน้าเอกสาร</span>
            </div>
            <div class="acrobat-thumbnails-list" id="acrobatThumbnailsList"></div>
          </aside>

          <div class="acrobat-canvas" id="acrobatCanvas">
            <div class="acrobat-pages-container" id="acrobatPagesContainer"></div>
          </div>
        </div>
      </div>
    `;
    viewerRoot = containerEl.querySelector('.acrobat-viewer-root');
  }

  // Preserve or initialize state
  const state = containerEl._acrobatState || {
    zoom: 'fit-width',
    customScale: 1.0,
    rotation: 0,
    currentPage: 1,
  ...
  containerEl._acrobatState = state;

  const pagesContainer = viewerRoot.querySelector('#acrobatPagesContainer');
  const thumbnailsList = viewerRoot.querySelector('#acrobatThumbnailsList');
  const thumbnailsDrawer = viewerRoot.querySelector('#acrobatThumbnailsDrawer');
  const searchBar = viewerRoot.querySelector('#acrobatSearchBar');
  const canvas = viewerRoot.querySelector('#acrobatCanvas');
  const pageInput = viewerRoot.querySelector('#acrobatPageInput');
  const pageTotal = viewerRoot.querySelector('#acrobatPageTotal');
  const zoomSelect = viewerRoot.querySelector('#acrobatZoomSelect');
  const btnThumbnails = viewerRoot.querySelector('#acrobatBtnThumbnails');

  // Update total pages indicator and input limit
  if (pageTotal) pageTotal.textContent = `/ ${pageCount}`;
  if (pageInput) {
    pageInput.max = pageCount;
    pageInput.value = Math.min(state.currentPage || 1, pageCount);
  }

  // 1. Render main A4 pages
  ...
  // 2. Render Page Thumbnails
  function extractSnippet(html) {
    const tmp = document.createElement('div');
    tmp.innerHTML = html;
    const text = tmp.innerText || tmp.textContent || '';
    return text.replace(/\\s+/g, ' ').substring(0, 140);
  }

  thumbnailsList.innerHTML = pages.map((p, i) => {
    const pageNo = i + 1;
    const snippet = extractSnippet(p.blocks.join(''));
    const isActive = pageNo === (state.currentPage || 1) ? ' active' : '';
    return `
      <div class="acrobat-thumb-item${isActive}" data-thumb-page="${pageNo}">
        <div class="acrobat-thumb-card">
          <div style="font-weight:700; font-size:7px; margin-bottom:3px; color:#1a4b77; border-bottom:1px solid #ddd; padding-bottom:1px;">หน้า ${pageNo}</div>
          <div style="opacity:0.65; overflow:hidden; height:120px; font-size:4.5px; line-height:1.2;">${snippet}</div>
        </div>
        <span class="acrobat-thumb-label">${pageNo}</span>
      </div>
    `;
  }).join('');

  // 3. Thumbnails click handler
  thumbnailsList.querySelectorAll('.acrobat-thumb-item').forEach(item => {
    item.addEventListener('click', () => {
      const pNo = parseInt(item.dataset.thumbPage, 10);
      scrollToPage(pNo);
    });
  });

  // 4. Scroll to Page helper
  function scrollToPage(pNo) {
    const targetPage = pagesContainer.querySelector(`[data-page-no="${pNo}"]`);
    if (targetPage) {
  ...
  function updatePageUI(pNo) {
    if (pageInput) pageInput.value = pNo;
    thumbnailsList.querySelectorAll('.acrobat-thumb-item').forEach(item => {
      const isCur = parseInt(item.dataset.thumbPage, 10) === pNo;
      item.classList.toggle('active', isCur);
    });
  }

  // 5. Scroll Spy
  if (!canvas._scrollSpyAttached) {
    canvas._scrollSpyAttached = true;
    let scrollTimer = null;
    canvas.addEventListener('scroll', () => {
      if (scrollTimer) cancelAnimationFrame(scrollTimer);
      scrollTimer = requestAnimationFrame(() => {
        const pageEls = pagesContainer.querySelectorAll('.a4-paper');
        ...
        pageEls.forEach(el => {
          ...
          const dist = Math.abs(rect.top - (canvasRect.top + 40));
          if (dist < minDistance) {
            minDistance = dist;
            closestPage = parseInt(el.dataset.pageNo, 10) || 1;
          }
        });

        if (closestPage !== state.currentPage) {
          state.currentPage = closestPage;
          updatePageUI(closestPage);
        }
      });
    }, { passive: true });
  }

  // 6. Zoom calculation & application
  function applyZoom(zoomMode) {
    state.zoom = zoomMode;
    let scale = 1.0;

    if (zoomMode === 'fit-width') {
      const availableWidth = canvas.clientWidth - 48;
      scale = Math.min(1.5, Math.max(0.35, availableWidth / 794));
    } else if (zoomMode === 'fit-page') {
      const availableHeight = canvas.clientHeight - 48;
      scale = Math.min(1.3, Math.max(0.35, availableHeight / 1123));
    } else {
      scale = parseFloat(zoomMode) || 1.0;
    }

    state.customScale = scale;
    pagesContainer.style.transform = `scale(${scale}) rotate(${state.rotation || 0}deg)`;
    pagesContainer.style.transformOrigin = 'top center';

    // Adjust container margin/size so scrollbars reflect zoomed dimensions
    const baseHeight = (1123 * pageCount) + (20 * (pageCount - 1));
    const scaledHeight = baseHeight * scale;
    const heightDiff = scaledHeight - baseHeight;
    pagesContainer.style.marginBottom = `${heightDiff}px`;

    const baseWidth = 794;
    const scaledWidth = baseWidth * scale;
    const widthDiff = scaledWidth - baseWidth;
    pagesContainer.style.marginRight = `${Math.max(0, widthDiff / 2)}px`;
    pagesContainer.style.marginLeft = `${Math.max(0, widthDiff / 2)}px`;

    if (zoomSelect) {
      if (['fit-width', 'fit-page', '0.5', '0.75', '1.0', '1.25', '1.5', '2.0'].includes(zoomMode)) {
        zoomSelect.value = zoomMode;
      }
    }
  }

  // 7. Bind toolbar buttons only once
  if (isInitialBuild) {
    // Thumbnails Toggle
    btnThumbnails.addEventListener('click', () => {
      state.showThumbnails = !state.showThumbnails;
      thumbnailsDrawer.classList.toggle('d-none', !state.showThumbnails);
      btnThumbnails.classList.toggle('active', state.showThumbnails);
      if (state.zoom === 'fit-width' || state.zoom === 'fit-page') {
        setTimeout(() => applyZoom(state.zoom), 220);
      }
    });

    // Search Toggle
    const btnSearch = viewerRoot.querySelector('#acrobatBtnSearch');
    const searchInput = viewerRoot.querySelector('#acrobatSearchInput');
    const searchCount = viewerRoot.querySelector('#acrobatSearchCount');
    const btnSearchClose = viewerRoot.querySelector('#acrobatBtnSearchClose');
    const btnSearchPrev = viewerRoot.querySelector('#acrobatBtnSearchPrev');
    const btnSearchNext = viewerRoot.querySelector('#acrobatBtnSearchNext');

    function toggleSearch(show) {
      state.showSearch = show !== undefined ? show : !state.showSearch;
      searchBar.classList.toggle('d-none', !state.showSearch);
      btnSearch.classList.toggle('active', state.showSearch);
      if (state.showSearch) {
        searchInput.focus();
        searchInput.select();
      } else {
    ...
    function clearSearchHighlights() {
      pagesContainer.querySelectorAll('mark.acrobat-highlight').forEach(mark => {
    ...
    function performSearch(query) {
      ...
      function highlightTextNodes(node) {
        if (node.nodeType === Node.TEXT_NODE) {
          ...
          if (match && node.parentNode && node.parentNode.nodeName !== 'MARK' && node.parentNode.nodeName !== 'SCRIPT') {
            const span = document.createElement('span');
            span.innerHTML = node.textContent.replace(regex, '<mark class="acrobat-highlight">$1</mark>');
      ...
      highlightTextNodes(pagesContainer);
      const marks = Array.from(pagesContainer.querySelectorAll('mark.acrobat-highlight'));
      state.searchMatches = marks;
      state.currentMatchIndex = marks.length > 0 ? 0 : -1;
    ...
    function updateMatchHighlight() {
      state.searchMatches.forEach((m, idx) => {
        m.classList.toggle('current', idx === state.currentMatchIndex);
      });
      if (state.searchMatches.length > 0 && state.currentMatchIndex >= 0) {
        searchCount.textContent = `${state.currentMatchIndex + 1}/${state.searchMatches.length}`;
        const curMark = state.searchMatches[state.currentMatchIndex];
        curMark.scrollIntoView({ behavior: 'smooth', block: 'center' });
      } else {
    ...
    // Page Stepper & Input
    const btnPrev = viewerRoot.querySelector('#acrobatBtnPrev');
    const btnNext = viewerRoot.querySelector('#acrobatBtnNext');
    ...
    btnNext.addEventListener('click', () => {
      const cur = state.currentPage || 1;
      if (cur < pageCount) scrollToPage(cur + 1);
    });
    ...
    const btnZoomOut = viewerRoot.querySelector('#acrobatBtnZoomOut');
    const btnZoomIn = viewerRoot.querySelector('#acrobatBtnZoomIn');

    btnZoomOut.addEventListener('click', () => {
      const nextScale = Math.max(0.3, Math.round((state.customScale - 0.1) * 10) / 10);
      applyZoom(nextScale.toString());
    });

    btnZoomIn.addEventListener('click', () => {
      const nextScale = Math.min(2.5, Math.round((state.customScale + 0.1) * 10) / 10);
      applyZoom(nextScale.toString());
    });

    // Rotate
    const btnRotate = viewerRoot.querySelector('#acrobatBtnRotate');
    ...
    // Print
    const btnPrint = viewerRoot.querySelector('#acrobatBtnPrint');
    btnPrint.addEventListener('click', () => window.print());

    // Fullscreen Toggle
    const btnFullscreen = viewerRoot.querySelector('#acrobatBtnFullscreen');
    btnFullscreen.addEventListener('click', () => {
      const isFull = viewerRoot.classList.toggle('acrobat-fullscreen');
      btnFullscreen.innerHTML = isFull ? '<i class="fa-solid fa-compress"></i>' : '<i class="fa-solid fa-expand"></i>';
      btnFullscreen.title = isFull ? 'ออกจากมุมมองเต็มจอ (ESC)' : 'ขยายเต็มหน้าจอ (Fullscreen)';
      setTimeout(() => applyZoom(state.zoom), 100);
    });

    document.addEventListener('keydown', (e) => {
      if (e.key === 'Escape' && viewerRoot.classList.contains('acrobat-fullscreen')) {
        viewerRoot.classList.remove('acrobat-fullscreen');
        btnFullscreen.innerHTML = '<i class="fa-solid fa-expand"></i>';
        setTimeout(() => applyZoom(state.zoom), 100);
      }
    });

    // ResizeObserver for auto Fit Width/Page on panel resize
  ...
  thumbnailsDrawer.classList.toggle('d-none', !state.showThumbnails);
  btnThumbnails.classList.toggle('active', !!state.showThumbnails);
  applyZoom(state.zoom || 'fit-width');
}

/* ---------------------------------------------------------- PAGINATION PROBE & PAGINATE */
function paginateDoc(containerEl, opts){
  ...
  /* probe ใช้วัดความสูงเนื้อหาจริงที่ความกว้างหน้ากระดาษจริง (210mm) */
  ...
  // Render via Centralized Adobe Acrobat Style Document Viewer
  renderAcrobatDocumentViewer(containerEl, pages, pageClass, runningHeaderHtml);
}
