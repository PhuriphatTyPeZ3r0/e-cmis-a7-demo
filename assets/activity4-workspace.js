(() => {
  ...
  function renderStaff(){
    ...
    function capture(state){const d=state.documentData;const value=id=>$('#'+id)?.value?.trim()||'';if(activeRole==='admin'){d.assignedOfficer=value('adminAssignee');d.backupOfficer=value('adminBackup');d.adminNote=value('adminNote')}if(activeRole==='officer'){d.decision=$('input[name="decision"]:checked')?.value||d.decision;d.reasons=$$('input[name="naccReason"]:checked').map(x=>x.value);d.notAcceptReason=value('notAcceptReason');d.anonymous=$('#anonymous')?.checked||false;d.officerOpinion=value('officerOpinion');d.absenceNote=value('absenceNote')}if(activeRole==='center'){d.centerDecision=$('input[name="centerDecision"]:checked')?.value||'';d.centerOpinion=value('centerOpinion');d.centerAdditionalDetail=value('centerAdditionalDetail')}if(activeRole==='division'||activeRole==='acting'){d.divisionOpinion=value('divisionOpinion');d.divisionAdditionalDetail=value('divisionAdditionalDetail')}return state}
    ...
    async function handleAction(action,state){if(action==='print'){window.print();return}if(action==='reset'){const ok=await confirmDo('เริ่มข้อมูลเรื่องนี้ใหม่','ลบเฉพาะข้อมูลกระบวนงานของเรื่องนี้ในเบราว์เซอร์ ข้อมูลเรื่องอื่นไม่เปลี่ยน','เริ่มใหม่');if(!ok.isConfirmed)return;const store=readStore();delete store[selectedId];writeStore(store);notify('success','เริ่มข้อมูลเรื่องนี้ใหม่แล้ว','ประวัติและร่างของเรื่องนี้ถูกล้างออก');renderDetail();renderList();return}state=capture(state);const d=state.documentData,w=state.workflow;const add=text=>state.decisionHistory.push({text,time:now()});let handoffResult=null;if(action==='draft'){state.documentVersions.push({version:state.documentVersions.length+1,actor:ROLE_LABELS[activeRole],time:now()});saveState(selectedId,state);notify('success','บันทึกร่างแล้ว','ข้อมูลถูกเก็บในเรื่องนี้และจะแสดงให้ผู้ดำเนินการคนถัดไปเห็น');renderDetail();return}if(action==='assign'){if(!d.assignedOfficer)return notify('warning','ยังไม่ได้เลือกผู้รับผิดชอบ','เลือกเจ้าหน้าที่รับผิดชอบหลักก่อนลงรับและมอบหมาย');const ok=await confirmDo('ยืนยันการลงรับและมอบหมาย',`ส่งงานให้ ${d.assignedOfficer}`,'ลงรับและมอบหมาย');if(!ok.isConfirmed)return;const previous=state.assignmentHistory.at(-1)?.text||'';state.assignmentHistory.push({text:`${previous?'เปลี่ยนผู้รับผิดชอบเป็น':'มอบหมายให้'} ${d.assignedOfficer} โดยธุรการ ศรร.`,time:now()});w.owner='officer';w.stage='officer';w.status='รอเจ้าหน้าที่พิจารณา';add(`ธุรการ ศรร. ลงรับและมอบหมายให้ ${d.assignedOfficer}`)}if(action==='officer-send'){if(!d.officerOpinion)return notify('warning','ยังไม่มีความเห็นเจ้าหน้าที่','กรอกข้อเท็จจริงและความเห็นก่อนส่งพิจารณา');if(d.decision==='send-nacc'&&!d.reasons.length)return notify('warning','ยังไม่ได้เลือกเหตุผล','เลือกเหตุผลส่งสำนักงาน ป.ป.ช. อย่างน้อย 1 ข้อ');const route=$('input[name="route"]:checked')?.value;if(route==='division'&&!d.absenceNote)return notify('warning','ต้องระบุเหตุผลการข้าม ผอ.ศรร.','กรอกหมายเหตุการลา/ไม่อยู่ก่อนส่ง ผอ.กบค.');w.owner=route==='division'?'division':'center';w.stage=w.owner;w.status=route==='division'?'รอ ผอ.กบค.':'รอ ผอ.ศรร.';add(`เจ้าหน้าที่รับเรื่องจัดทำ Form 3 และส่ง ${ROLE_LABELS[w.owner]}`)}if(action==='center-return'){if(!d.centerOpinion)return notify('warning','ต้องระบุเหตุผล','กรอกประเด็นที่ต้องแก้ไขก่อนส่งกลับ');w.owner='officer';w.stage='officer';w.status='ส่งกลับเจ้าหน้าที่แก้ไข';d.centerDecision='disagree';add('ผอ.ศรร. ส่งกลับเจ้าหน้าที่รับเรื่องแก้ไข')}if(action==='center-send'){if(!d.centerOpinion)return notify('warning','ยังไม่มีความเห็น ผอ.ศรร.','กรอกความเห็นก่อนส่ง ผอ.กบค.');w.owner='division';w.stage='division';w.status='รอ ผอ.กบค.';add('ผอ.ศรร. ให้ความเห็นและส่ง ผอ.กบค.')}if(action==='division-center'){w.owner='center';w.stage='center';w.status='รอ ผอ.ศรร. ให้ความเห็นเพิ่มเติม';add('ผอ.กบค. ขอความเห็น ผอ.ศรร. เพิ่มเติม')}if(action==='division-return'){if(!d.divisionOpinion)return notify('warning','ต้องระบุเหตุผล','กรอกคำสั่งหรือประเด็นที่ต้องแก้ไข');w.owner='officer';w.stage='officer';w.status='ส่งกลับเจ้าหน้าที่แก้ไข';add('ผอ.กบค. ส่งกลับเจ้าหน้าที่รับเรื่องแก้ไข')}if(action==='approve'){if(!d.divisionOpinion)return notify('warning','ยังไม่มีคำสั่ง','กรอกความเห็นและคำสั่งก่อนอนุมัติ');if(activeRole==='acting'){d.actingOrder=$('#actingOrder')?.value?.trim()||d.actingOrder||'';if(!d.actingOrder)return notify('warning','ยังไม่มีคำสั่งแต่งตั้ง','กรอกเลขที่คำสั่งและวันที่แต่งตั้งผู้รักษาราชการแทนก่อนอนุมัติ')}w.complete=true;w.status=d.anonymous?'ส่งต่อกิจกรรมที่ 7':'ดำเนินการเสร็จสิ้น';add(`${ROLE_LABELS[activeRole]} อนุมัติผลการพิจารณา`);handoffResult=window.ECMISActivity5Handoff?.create(localStorage,state,new Date().toISOString(),activeRole)||null;if(handoffResult?.eligible)w.status='ส่งต่อ Activity 5 แล้ว'}if(['center-return','center-send'].includes(action)&&d.centerAdditionalDetail)add('ผอ.ศรร. เพิ่มรายละเอียดประกอบการพิจารณา โดยไม่แก้ไขข้อมูลคำร้องเดิม');if(['division-center','division-return','approve'].includes(action)&&d.divisionAdditionalDetail)add(`${ROLE_LABELS[activeRole]} เพิ่มรายละเอียดประกอบการพิจารณา โดยไม่แก้ไขข้อมูลคำร้องเดิม`);state.documentVersions.push({version:state.documentVersions.length+1,actor:ROLE_LABELS[activeRole],time:now()});saveState(selectedId,state);const handoffText=handoffResult?.eligible?(handoffResult.created?' สร้างข้อมูลส่งต่อ Activity 5 แล้ว':' พบข้อมูลส่งต่อ Activity 5 เดิมและไม่สร้างซ้ำ'):'';notify('success','บันทึกและส่งงานแล้ว',`สถานะปัจจุบัน: ${w.status}${handoffText}`);renderDetail();renderList()}
    root.addEventListener('click',async event=>{
      ...
      if(approveNaccButton&&getState(selectedId).documentData.decision==='send-nacc'){
        ...
        state.decisionHistory.push({text:`${ROLE_LABELS[activeRole]} อนุมัติให้ส่งสำนักงาน ป.ป.ช. และส่งงานกลับเจ้าหน้าที่รับเรื่องเพื่อจัดส่ง`,time});
